...
bool ItemEntity::Init(std::shared_ptr<entt::registry> const& registry)
{
...
uint64_t ItemEntity::ProcessMessage(MESSAGE& message)
{
    ...
    {
        DeleteParticle();
        auto const         mdlEID  = GetModelEIDFromCharacterEID(message.EntityID());
        std::string const& locName = message.String();
    ...
    case 5: {
        DeleteParticle();
        auto const         mdlID          = GetModelEIDFromCharacterEID(message.EntityID());
        auto const         mdlToTieID     = GetModelEIDFromCharacterEID(message.EntityID());
        std::string const& sLocName       = message.String();
        std::string const& sStartEvntName = message.String();
        std::string const& sEndEvntName   = message.String();
...
void ItemEntity::SetModelToPosition(CMatrix const& mtx) const
{
    if (m_pModel) {
        core.Send_Message(
            m_eidModel,
            "lffffffffffff",
            MSG_MODEL_SET_POSITION,
            m_mtxpos.Pos().x,
            m_mtxpos.Pos().y,
            m_mtxpos.Pos().z,
            m_mtxpos.Vx().x,
...
void ItemEntity::SetTechnique(char const* pcTechnique) const
{
    if (m_pModel) {
        auto* pRootNod = m_pModel->GetNode(0);
        if (pRootNod) { pRootNod->SetTechnique(pcTechnique); }
    }
}

bool ItemEntity::TieToLocator(entid_t mdlEID, char const* pcLocName)
{
...
void ItemEntity::SetEventListener(
    entid_t mdlEID, entid_t mdlToTieEID, char const* pcLocName, char const* pcStartEvent, char const* pcEndEvent)
{
    auto* pMdl = static_cast<MODEL*>(core.GetEntityPointer(mdlEID));
    if (!pMdl) return;
...
void ItemEntity::EventListener::Event(Animation* animation, int32_t playerIndex, char const* eventName)
{
        ...
        if (pcParticleName && pcParticleName[0]) {
            ...
            if (eidParticle) {
                ...
                m_pParticle     = (VPARTICLE_SYSTEM*)core.Send_Message(
                    eidParticle, "lsffffffl", PS_CREATE_RIC, pcParticleName, vPos.x, vPos.y, vPos.z, 0.0f, 1.0f, 0.0f, 0);
                return true;
            }
        }
    }
