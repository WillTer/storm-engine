...
{
    m_pMaterial->GetImgRender()->TranslateBasePosToRealPos(
        static_cast<float>(nLeft), static_cast<float>(nTop), m_BasePos.left, m_BasePos.top);
    m_pMaterial->GetImgRender()->TranslateBasePosToRealPos(
        static_cast<float>(nRight), static_cast<float>(nBottom), m_BasePos.right, m_BasePos.bottom);
    m_pMaterial->UpdateFlagOn();
}
...
void BIImage::CutClock(float fBegin, float fEnd, float fFactor)
{
    ...
    fp.y = 0.5f;
    m_aRelPos.push_back(fp);
    auto const fEndAng = fBegin + (fEnd - fBegin) * fFactor;
