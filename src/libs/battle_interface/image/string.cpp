...
{
    m_pImgRender->TranslateBasePosToRealPos(static_cast<float>(nLeft), static_cast<float>(nTop), m_BasePos.left, m_BasePos.top);
    m_pImgRender->TranslateBasePosToRealPos(static_cast<float>(nRight), static_cast<float>(nBottom), m_BasePos.right, m_BasePos.bottom);
    m_bUpdateString = true;
}

void BIString::SetString(char const* pcStr)
{
    m_sText         = pcStr;
    m_bUpdateString = true;
}
