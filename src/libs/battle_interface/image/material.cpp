...
{
    if (m_bMakeBufferUpdate) RemakeBuffers();
...
void BIImageMaterial::DeleteImage(BIImage const* pImg)
{
    if (m_bDeleteEverything) return;
...
void BIImageMaterial::SetTexture(char const* pcTextureName)
{
...
bool BIImageMaterial::GetOutputRangeByPriority(int32_t nBegPrior, int32_t nEndPrior, size_t& nStartIndex, size_t& nTriangleQuantity)
{
    ...
    for (n = 0; n < m_apImage.size(); n++) {
        if (m_apImage[n]->GetPrioritet() >= nBegPrior) break;
        nStartIndex += m_apImage[n]->GetTriangleQuantity() * 3;
    }

    nTriangleQuantity = 0;
    for (; n < m_apImage.size(); n++) {
        if (m_apImage[n]->GetPrioritet() > nEndPrior) break;
...
void BIImageMaterial::RecalculatePriorityRange()
{
    ...
    auto const oldMin = m_nMinPriority;
    auto const oldMax = m_nMaxPriority;
    m_nMinPriority = m_nMaxPriority = m_apImage[0]->GetPrioritet();
    for (int32_t n = 1; n < m_apImage.size(); n++) {
        auto const p = m_apImage[n]->GetPrioritet();
        if (p < m_nMinPriority)
            m_nMinPriority = p;
        else if (p > m_nMaxPriority)
            m_nMaxPriority = p;
    }
    if (oldMin != m_nMinPriority || oldMax != m_nMaxPriority) m_pImageRender->MaterialSorting();
}

void BIImageMaterial::InsertImageToList(BIImage* pImg)
{
    Assert(pImg);
    auto const nPrior = pImg->GetPrioritet();
    int32_t    n;
    for (n = 0; n < m_apImage.size(); n++)
        if (m_apImage[n]->GetPrioritet() > nPrior) break;
