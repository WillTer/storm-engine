...
void MousePointer::InitMouseCursors()
{
    ...
    size_t const q = pACursors->GetAttributesNum();
    for (int32_t n = 0; n < q; n++) {
        auto* pA = pACursors->GetAttributeClass(n);
        if (pA) {
            int32_t const i = pA->GetAttributeAsDword("index", -1);
    ...
    m_pIcon          = m_pManager->GetImageRender()->CreateImage(
        BIType_square,
        m_aCursors[BI_CURSOR_COMMON].texture.c_str(),
        0xFF808080,
        m_aCursors[BI_CURSOR_COMMON].uv,
        GetCurrentCursorIconPos(),
...
void MousePointer::MoveCursor()
{
    ...
    if (m_mousepos.x > static_cast<float>(m_cursorzone.right)) m_mousepos.x = static_cast<float>(m_cursorzone.right);
    if (m_mousepos.y < static_cast<float>(m_cursorzone.top)) m_mousepos.y = static_cast<float>(m_cursorzone.top);
    if (m_mousepos.y > static_cast<float>(m_cursorzone.bottom)) m_mousepos.y = static_cast<float>(m_cursorzone.bottom);
}

void MousePointer::SetCurrentCursor()
{
    int32_t const nNewCursor = BI_CURSOR_COMMON;
    ...
    if (nNewCursor != m_nCurrentCursor) {
        if (!m_pIcon || m_aCursors[m_nCurrentCursor].texture != m_aCursors[nNewCursor].texture) {
            ...
            m_pIcon          = m_pManager->GetImageRender()->CreateImage(
                BIType_square,
                m_aCursors[nNewCursor].texture.c_str(),
                0xFF808080,
                m_aCursors[nNewCursor].uv,
                GetCurrentCursorIconPos(),
                BI_MOUSECURSOR_ICON_ORDER);
            return;
        }
    ...
    auto const pos = GetCurrentCursorIconPos();
