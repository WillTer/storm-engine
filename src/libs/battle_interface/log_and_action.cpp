...
bool ILogAndActions::Init(std::shared_ptr<storm::ServiceLocator> const& service_locator)
{
...
void ILogAndActions::Execute(uint32_t delta_time)
{
    ...
    auto const delta = delta_time * m_fShiftSpeed;
    auto       top   = 0.f;
...
uint64_t ILogAndActions::ProcessMessage(MESSAGE& message)
{
    switch (message.Long()) {
    case LOG_ADD_STRING: {
        auto const         stringImmortal = message.Long() != 0;
        ...
        if (stringImmortal) {
            ...
            if (last == nullptr)
                ...
                if (param[0] != 0) {
                    ...
                    if ((last->str = new char[len]) == nullptr) { throw std::runtime_error("allocate memory error"); }
    ...
    case LOG_AND_ACTIONS_INIT: {
        m_sOldActionName[0] = '0';
        auto const bfc      = message.Long();
        auto const bls      = message.Long();
        Create(bfc != 0, bls != 0);
    } break;
    ...
    case LI_SET_VISIBLE: m_bDontShowAll = (message.Long() == 0); break;
...
void ILogAndActions::SetString(char const* str, bool immortal)
{
    ...
    if (last != nullptr && last->str != nullptr && storm::iEquals(last->str, str)) return;

    // create a new line descriptor
    auto* newDescr = new STRING_DESCR;
    if (newDescr == nullptr) { throw std::runtime_error("Allocate memory error"); }
    // it will be the last on the list
    newDescr->next = nullptr;
    // add the specified string to it
    auto const len = strlen(str) + 1;
    if ((newDescr->str = new char[len]) == nullptr) { throw std::runtime_error("Allocate memory error"); }
    ...
    // otherwise add it to the end of the list
        ...
        if (newDescr->offset + m_nStringOffset > m_nWindowHeight) {
            int32_t const offsetDelta = static_cast<int32_t>(newDescr->offset) + m_nStringOffset - m_nWindowHeight;
...
void ILogAndActions::SetAction(char const* actionName)
{
    ATTRIBUTES* pA;
