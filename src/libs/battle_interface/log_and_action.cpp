    ...
    if (m_bShowLogStrings)
    {
        ...
        while (ptr != nullptr)
        {
            ...
            if (ptr->alpha <= 255.f)
                ...
                rs->ExtPrint(m_fontID, m_dwColor + (static_cast<int32_t>(ptr->alpha) << 24), 0, nAlign, true,
                             m_fFontScale, 0, 0, strX, m_nWindowUp + static_cast<int32_t>(ptr->offset), "%s", ptr->str);
            else
