    ...
    if (pA) {
        ...
        if (pcTmp)
            sscanf(
                pcTmp,
                "%f,%f,%f,%f",
                &m_rSignStateLeftUV.left,
                &m_rSignStateLeftUV.top,
                &m_rSignStateLeftUV.right,
                &m_rSignStateLeftUV.bottom);
        pcTmp = pA->GetAttribute("shiphpoffset");
        if (pcTmp) sscanf(pcTmp, "%f,%f", &m_pntSignStateLeftOffset.x, &m_pntSignStateLeftOffset.y);
        pcTmp = pA->GetAttribute("shiphpiconsize");
        if (pcTmp) sscanf(pcTmp, "%f,%f", &m_pntSignStateLeftIconSize.x, &m_pntSignStateLeftIconSize.y);
        pcTmp = pA->GetAttribute("shipspuv");
        if (pcTmp)
            sscanf(
                pcTmp,
                "%f,%f,%f,%f",
                &m_rSignStateRightUV.left,
                &m_rSignStateRightUV.top,
                &m_rSignStateRightUV.right,
                &m_rSignStateRightUV.bottom);
        pcTmp = pA->GetAttribute("shipspoffset");
        if (pcTmp) sscanf(pcTmp, "%f,%f", &m_pntSignStateRightOffset.x, &m_pntSignStateRightOffset.y);
        ...
        if (pcTmp) sscanf(pcTmp, "%f,%f", &m_pntSignStarOffset.x, &m_pntSignStarOffset.y);
        ...
        if (pcTmp) sscanf(pcTmp, "%f,%f", &m_pntSignFaceOffset.x, &m_pntSignFaceOffset.y);
        pcTmp = pA->GetAttribute("shipiconsize");
        if (pcTmp) sscanf(pcTmp, "%f,%f", &m_pntSignFaceIconSize.x, &m_pntSignFaceIconSize.y);

        for (n = 0; n < MAX_SIGN_QUANTITY; n++) {
            sprintf_s(param, sizeof(param), "iconoffset%d", n + 1);
            pcTmp = pA->GetAttribute(param);
            if (pcTmp) sscanf(pcTmp, "%f,%f", &m_Sign[n].pntPos.x, &m_Sign[n].pntPos.y);
        }
    }
...
{
    auto const n = CalculateSignQuantity();
    UpdateBuffers(n);
}
...
{
    auto const nBackSquareQ      = nQ;
    auto const nSignStateSquareQ = nQ * 2;
    auto const nSignStarSquareQ  = nQ;
    auto const nSignFaceSquareQ  = nQ;
    ...
    if (pV) {
        ...
        for (n = 0; n < m_nSignFaceSquareQ; n++) {
            vn += WriteSquareToVBuff(
                &pV[vn], m_Sign[n].rFaceUV, m_dwSignFaceColor, m_Sign[n].pntPos + m_pntSignFaceOffset, m_pntSignFaceIconSize);
        }

        for (n = 0; n < m_nSignQ; n++)
            vn += WriteSquareToVBuff(&pV[vn], m_rBackUV, m_dwBackColor, m_Sign[n].pntPos + m_pntBackOffset, m_pntBackIconSize);
        ...
        for (n = 0; n < m_nSignQ; n++) {
            vn += WriteSquareToVBuffWithProgress(
                ...
                m_pntSignStateLeftIconSize,
                1.f - m_Sign[n].fLeftState,
                0.f,
                0.f,
                0.f);
            vn += WriteSquareToVBuffWithProgress(
                ...
                m_rSignStateRightUV,
                m_dwSignStateColor,
                m_Sign[n].pntPos + m_pntSignStateRightOffset,
                m_pntSignStateRightIconSize,
                1.f - m_Sign[n].fRightState,
                0.f,
                0.f,
                0.f);
        }
        ...
        for (n = 0; n < m_nSignQ; n++)
            vn += WriteSquareToVBuffWithProgress(
                ...
                m_rSignStarUV,
                m_dwSignStarColor,
                m_Sign[n].pntPos + m_pntSignStarOffset,
                m_pntSignStarIconSize,
                0.f,
                0.f,
                0.f,
