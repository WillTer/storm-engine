...
void BICommandList::Update(int32_t nTopLine, int32_t nCharacterIndex, int32_t nCommandMode)
{
    ...
    if (nTopLine == m_LeftTopPoint.y && nCharacterIndex == m_nCurrentCommandCharacterIndex && m_nCurrentCommandMode == nCommandMode)
    ...
    if (nOldSelIndex > 0 && nOldSelIndex < m_aUsedCommand.size()) m_nSelectedCommandIndex = nOldSelIndex;

    UpdateShowIcon();
}

size_t BICommandList::AddTexture(char const* pcTextureName, uint32_t nCols, uint32_t nRows)
{
...
int32_t BICommandList::ExecuteConfirm()
{
    ...
    if (m_aUsedCommand[m_nSelectedCommandIndex].nCooldownPictureIndex >= 0 && m_aUsedCommand[m_nSelectedCommandIndex].fCooldownFactor < 1.f)
        return -1;
    ...
    case 0:
        ...
        break;
    default: Update(m_LeftTopPoint.y, m_nCurrentCommandCharacterIndex, endCode);
    }
    return endCode;
}
...
{
    if (m_pARoot) { m_pARoot->SetAttributeUseDword("ComState", (bActive ? 1 : 0)); }
...
void BICommandList::Init()
{
    ...
    if (pAList) {
        ...
        if (attr = pAList->GetAttribute("UDArrow_Texture")) m_sUpDownArrowTexture = attr;
        ...
        BIUtils::ReadPosFromAttr(
            pAList, "UDArrow_Size", m_pntUpDownArrowSize.x, m_pntUpDownArrowSize.y, m_pntUpDownArrowSize.x, m_pntUpDownArrowSize.y);
        BIUtils::ReadPosFromAttr(
            pAList, "UDArrow_Offset_Up", m_pntUpArrowOffset.x, m_pntUpArrowOffset.y, m_pntUpArrowOffset.x, m_pntUpArrowOffset.y);
        BIUtils::ReadPosFromAttr(
            pAList, "UDArrow_Offset_Down", m_pntDownArrowOffset.x, m_pntDownArrowOffset.y, m_pntDownArrowOffset.x, m_pntDownArrowOffset.y);

        if (attr = pAList->GetAttribute("LRArrow_Texture")) m_sLeftRightArrowTexture = attr;
        ...
        BIUtils::ReadPosFromAttr(
            pAList,
            "LRArrow_Size",
            m_pntLeftRightArrowSize.x,
            m_pntLeftRightArrowSize.y,
            m_pntLeftRightArrowSize.x,
            m_pntLeftRightArrowSize.y);
        BIUtils::ReadPosFromAttr(
            pAList, "LRArrow_Offset_Left", m_pntLeftArrowOffset.x, m_pntLeftArrowOffset.y, m_pntLeftArrowOffset.x, m_pntLeftArrowOffset.y);
        BIUtils::ReadPosFromAttr(
            pAList,
            "LRArrow_Offset_Right",
            m_pntRightArrowOffset.x,
            m_pntRightArrowOffset.y,
            m_pntRightArrowOffset.x,
            m_pntRightArrowOffset.y);

        // set values for the menu activity icon
        if (attr = pAList->GetAttribute("ActiveIcon_Texture")) m_sActiveIconTexture = attr;
        BIUtils::ReadPosFromAttr(
            pAList,
            "ActiveIcon_Offset",
            m_pntActiveIconOffset.x,
            m_pntActiveIconOffset.y,
            m_pntActiveIconOffset.x,
            m_pntActiveIconOffset.y);
        BIUtils::ReadPosFromAttr(
            pAList, "ActiveIcon_Size", m_pntActiveIconSize.x, m_pntActiveIconSize.y, m_pntActiveIconSize.x, m_pntActiveIconSize.y);
        ...
        if (attr = pAList->GetAttribute("ActiveIcon_Note")) m_sActiveIconNote = attr;
    }
    ...
    if (pATextures) {
        ...
        for (int n = 0; n < q; n++) {
            ...
            if (pA) {
                TextureDescr td = {
                    pA->GetAttribute("name") ? pA->GetAttribute("name") : std::string(),
                    pA->GetAttributeAsDword("xsize", 1),
                    pA->GetAttributeAsDword("ysize", 1)};

                if (td.nCols < 1) td.nCols = 1;
                if (td.nRows < 1) td.nRows = 1;
...
int32_t BICommandList::AddToIconList(
    int32_t     nTextureNum,
    ...
    char const* pcNoteName)
{
    ...
    if (pcCommandName) uc.sCommandName = pcCommandName;
    if (pcLocName) uc.sLocName = pcLocName;
    if (pcNoteName) uc.sNote = pcNoteName;
...
{
    if (m_aUsedCommand.empty()) return;
...
int32_t BICommandList::ClockIconAdd(int32_t nForePictureNum, int32_t nBackPictureNum, int32_t nTextureNum, RECT& rpos, float fFactor)
{
    ...
    auto* pImg = m_pImgRender->CreateImage(
        BIType_clocksquare, m_aTexture[nTextureNum].sFileName.c_str(), 0xFF808080, GetPictureUV(nTextureNum, nForePictureNum, uv), rpos);
...
void BICommandList::AdditiveIconAdd(float fX, float fY, std::vector<UsedCommand::AdditiveIcon>& aList)
{
    ...
    for (int32_t n = 0; n < aList.size(); n++) {
        ...
        if (IconAdd(aList[n].nPic, aList[n].nTex, rCur) > 0) { fY = static_cast<float>(rCur.bottom); }
    }
}
...
FRECT& BICommandList::GetPictureUV(int32_t nTextureNum, int32_t nPictureNum, FRECT& uv)
{
    if (nTextureNum < 0 || nTextureNum >= m_aTexture.size() || nPictureNum < 0
        ...
        auto const fDU = 1.f / m_aTexture[nTextureNum].nCols;
        auto const fDV = 1.f / m_aTexture[nTextureNum].nRows;
        auto const ny  = nPictureNum / m_aTexture[nTextureNum].nCols;
        auto const nx  = nPictureNum - ny * m_aTexture[nTextureNum].nCols;
        uv.left        = nx * fDU;
    ...
    for (auto n = m_nStartUsedCommandIndex; n < m_aUsedCommand.size() && i < m_nIconShowMaxQuantity; n++) {
        ...
        if (n == m_nSelectedCommandIndex) {
            if (m_aUsedCommand[n].nCooldownPictureIndex < 0)
                ...
                i += ClockIconAdd(
                    m_aUsedCommand[n].nSelPictureIndex,
                    m_aUsedCommand[n].nCooldownPictureIndex,
                    m_aUsedCommand[n].nTextureIndex,
                    rPos,
                    m_aUsedCommand[n].fCooldownFactor);
            ...
            if (m_aUsedCommand[n].nCooldownPictureIndex < 0)
                ...
                i += ClockIconAdd(
                    m_aUsedCommand[n].nNormPictureIndex,
                    m_aUsedCommand[n].nCooldownPictureIndex,
                    m_aUsedCommand[n].nTextureIndex,
                    rPos,
                    m_aUsedCommand[n].fCooldownFactor);
...
void BICommandList::SetNote(char const* pcNote, int32_t nX, int32_t nY)
{
    m_NoteText  = pcNote;
...
ATTRIBUTES* BICommandList::GetCurrentCommandAttribute() const
{
    ...
    size_t const q = pAR->GetAttributesNum();
    for (int32_t n = 0; n < q; n++) {
        auto* pA = pAR->GetAttributeClass(n);
        if (!pA) continue;
        char const* pcCommName = pA->GetAttribute("event");
