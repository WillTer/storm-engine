...
void MESSAGE_ICONS::Update(uint32_t deltaTime)
{
    ...
    if (m_bShowMsgIcon) {
        ...
        if (m_bBlindDirectUp)
            blindColor = BIUtils::GetIntervalColor(m_dwHighBlindColor, m_dwLowBlindColor, m_fCurBlindTime / m_fBlindTimeUp);
        else
            blindColor = BIUtils::GetIntervalColor(m_dwLowBlindColor, m_dwHighBlindColor, m_fCurBlindTime / m_fBlindTimeDown);
        ...
        for (i = 0; i < MESSAGE_ICONS_COLUMN_QUANTITY; i++) {
            ...
            for (j = 0; j < m_pMsgColumns[i].rowQ; j++) {
                ...
                if (m_pMsgColumns[i].pRow[j].bDoBlend) {
                    if ((m_pMsgColumns[i].pRow[j].curTime -= deltaTime * .001f) <= 0.f) {
                        ...
                        if (j < m_pMsgColumns[i].rowQ)
                            std::memcpy(
                                &m_pMsgColumns[i].pRow[j],
                                &m_pMsgColumns[i].pRow[j + 1],
                                sizeof(COLUMN_DESCRIBER::ROW_DESCR) * (m_pMsgColumns[i].rowQ - j));
                        j--;
                        continue;
                    }
                    m_pMsgColumns[i].pRow[j].color = BIUtils::GetIntervalColor(
                        storm::Color {0, 128, 128, 128}.to_hex(),
                        storm::Color {255, 128, 128, 128}.to_hex(),
                ...
                if (m_pMsgColumns[i].pRow[j].bottom < fBottonLimit) {
                    ...
                    if (m_pMsgColumns[i].pRow[j].bottom > fBottonLimit) m_pMsgColumns[i].pRow[j].bottom = fBottonLimit;
                }
            }
                ...
                for (n = 0; n < q; n++) {
                    ...
                    for (k = 0; k < m_pMsgColumns[i].rowQ; k++)
                        if (picNum == m_pMsgColumns[i].pRow[k].pic) {
                            ...
                            m_pMsgColumns[i].pRow[k].color    = storm::Color {255, 128, 128, 128}.to_hex();
                            break;
                        }
            ...
            for (j = 0; j < m_pMsgColumns[i].rowQ; j++) {
                ...
                SetRectangleTexture(
                    &pVBuf[m_nMsgIconQnt * 4],
                    GetTexRectFromPosition(frectTmp, m_pMsgColumns[i].pRow[j].pic, m_nHorzTextureSize, m_nVertTextureSize));
                for (n = 0; n < 4; n++)
...
void MESSAGE_ICONS::Draw() const
{
    ...
    if (m_bShowMsgIcon && m_nMsgIconQnt > 0 && m_idMsgIconsTexture >= 0) {
        ...
        // rs->DrawBuffer(m_vMsgIconBufID, sizeof(BI_COLOR_VERTEX), m_iMsgIconBufID, 0, m_nMsgIconQnt * 4, 0, m_nMsgIconQnt * 2,
        // "battle_msg");
    }
}

void MESSAGE_ICONS::StartData(ATTRIBUTES* pAData[MESSAGE_ICONS_COLUMN_QUANTITY], int32_t pLeft[MESSAGE_ICONS_COLUMN_QUANTITY])
{
    int i, j;
...
bool MESSAGE_ICONS::InitData(entid_t host_eid, /*VDX9RENDER*/ void* _rs, ATTRIBUTES* pARoot)
{
    m_idHost = host_eid;
    // rs       = _rs;
    ...
    char const* stmp = pARoot->GetAttribute("texture");
    ...
    //     BI_COLOR_VERTEX_FORMAT, m_nMsgIconRowQnt * MESSAGE_ICONS_COLUMN_QUANTITY * 4 * sizeof(BI_COLOR_VERTEX), D3DUSAGE_WRITEONLY);
    ...
    for (i = 0; i < MESSAGE_ICONS_COLUMN_QUANTITY; i++) {
        ...
        if (m_pMsgColumns[i].pRow == nullptr) { throw std::runtime_error("allocate memory error"); }
    }
    ...
    if (pVBuf != nullptr) {
        for (i = 0; i < m_nMsgIconRowQnt * MESSAGE_ICONS_COLUMN_QUANTITY * 4; i++) {
            pVBuf[i].w     = 0.5f;
            pVBuf[i].pos.z = 1.f;
            pVBuf[i].col   = storm::Color {255, 128, 128, 128}.to_hex();
        }
        // rs->UnLockVertexBuffer(m_vMsgIconBufID);
    }
