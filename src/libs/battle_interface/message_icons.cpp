            ...
            for (j = 0; j < m_pMsgColumns[i].rowQ; j++) {
                ...
                if (m_pMsgColumns[i].pRow[j].bDoBlend) {
                    ...
                    m_pMsgColumns[i].pRow[j].color = BIUtils::GetIntervalColor(
                        ARGB(0, 128, 128, 128), ARGB(255, 128, 128, 128), m_pMsgColumns[i].pRow[j].curTime / m_fBlendTime);
                }
                // move icons down
...
void MESSAGE_ICONS::Draw() const
{
    ...
    if (m_bShowMsgIcon && m_nMsgIconQnt > 0 && m_idMsgIconsTexture >= 0) {
        ...
        rs->DrawBuffer(m_vMsgIconBufID, sizeof(BI_COLOR_VERTEX), m_iMsgIconBufID, 0, m_nMsgIconQnt * 4, 0, m_nMsgIconQnt * 2, "battle_msg");
    }
}
