...
#include <libs/math/math3d/sphere.h>
    ...
    int32_t        nPrior)
{
    ...
    pImage = static_cast<BIImage*>(pImgRender->CreateImage(BIType_square, sTextureName.c_str(), dwColor, rUV, rPos, nPrior));
    Assert(pImage);
}
...
void ISpyglass::ImageParam::ChangeIcon(BIImageRender* pImgRender, char const* pcTextureName, storm::FRect& frUV)
{
    ...
    if (sTextureName == pcTextureName) {
        ...
        auto const nPrior = pImage ? pImage->GetPrioritet() : ImagePrioritet_DefaultValue;
        STORM_DELETE(pImage);
...
void ISpyglass::Execute(uint32_t delta_time)
{
    ...
    if (m_bIsOn) {
        ...
        if (m_nInfoCharacterIndex >= 0 && m_fInfoKeepDelay > 0.f) {
            ...
            if (m_fInfoKeepDelay <= 0.f) bFindNewTarget = true;
        }
        if (bFindNewTarget) {
...
uint64_t ISpyglass::ProcessMessage(MESSAGE& message)
{
    ...
    {
        std::string const& shipname    = message.String();
        std::string const& shiptype    = message.String();
        ...
        std::string const& captainname    = message.String();
        std::string const& facetexture    = message.String();
...
void ISpyglass::FindNewTargetShip()
{
    ...
    if (pFort) {
        ...
        if (fTrace <= 1.f && fTrace < fFindedDistance) {
            ...
            if (pA) nFindedCharIndex = pA->GetAttributeAsDword("index", -1);
        }
    }
    ...
    if (nFindedCharIndex == -1) {
        if (m_nInfoCharacterIndex != -1 && m_fInfoKeepDelay > 0.f) {
            ...
            for (SHIP_DESCRIBE_LIST::SHIP_DESCR* pSD = g_ShipList.GetShipRoot(); pSD; pSD = pSD->next) {
                if (pMainSD == pSD) continue;
                Sphere      sph;
                auto const& vec = pSD->pShip->GetPos();
                sph.x           = vec.x;
                sph.y           = vec.y;
                sph.z           = vec.z;
                sph.r           = 40.f;
...
void ISpyglass::UpdateCamera()
{
    ...
    if (!m_Camera.bIsActive) {
        ...
        if (m_Camera.fCurActivateTime >= m_Camera.fActivateTime) {
            ...
            if (m_Camera.bIsGrow) m_Camera.bIsActive = true;  // now working fully
    ...
    int32_t     nShipClass)
{
    ...
    if (nCurCannons >= 0 && nMaxCannons >= 0) {
        ...
        if (m_Cannon.pImage) m_Cannon.pImage->CutSide(0.f, 0.f, 0.f, 0.f);  // show
    } else {
        m_txtCannons.sText = "";
        if (m_Cannon.pImage) m_Cannon.pImage->CutSide(0.f, 1.f, 0.f, 0.f);  // hide
    }
    ...
    if (nSailState >= 0 && nSailState < m_aSailUV.size()) {
        ...
        if (m_Sail.pImage) m_Sail.pImage->CutSide(0.f, 1.f, 0.f, 0.f);
    }
    ...
    if (nFencing >= 0) {
        ...
        if (m_ImgCaptainFencing.pImage) m_ImgCaptainFencing.pImage->CutSide(0.f, 1.f, 0.f, 0.f);
    }
