...
bool IBoardingStatus::Init(std::shared_ptr<entt::registry> const& registry)
{
...
{
    switch (message.Long()) {
    case LOG_INIT_CHARCTER_HP: Create(); break;
    case LOG_SET_CHARCTER_HP: {
        auto const myHP    = message.Float();
        auto const enemyHP = message.Float();
