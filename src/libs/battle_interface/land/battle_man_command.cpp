...
BIManCommandList::BIManCommandList(entid_t eid, ATTRIBUTES* pA, /*VDX9RENDER*/ void* rs) : BICommandList(eid, pA, rs)
{
    Init();
}
...
void BIManCommandList::FillIcons()
{
    ...
    if (m_nCurrentCommandMode & BI_COMMODE_USER_ICONS) nIconsQuantity += UserIconsAdding();

    if (m_nCurrentCommandMode & BI_COMMODE_ABILITY_ICONS) nIconsQuantity += AbilityAdding();
...
int32_t BIManCommandList::UserIconsAdding()
{
    ...
    for (int32_t i = 0; i < attrQuant; i++) {
        ...
        retVal += AddToIconList(
            textureNum, pictureNum, selPictureNum, -1, -1, eventName, i + 1, pA->GetAttribute("name"), pA->GetAttribute("note"));
    }
...
int32_t BIManCommandList::AbilityAdding()
{
    ...
    for (int32_t i = 0; i < attrQuant; i++) {
        ...
        char const* eventName = pA->GetAttribute("event");
        retVal += AddToIconList(textureNum, pictureNum, selPictureNum, -1, -1, eventName, -1, nullptr, pA->GetAttribute("note"));
    }
