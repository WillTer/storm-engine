...
void TMP_LONG_STACK::Push(int32_t data)
{
    if (curidx >= datsize) {
        ...
        if (ldat == nullptr) { throw std::runtime_error("allocate memory error"); }
        if (pold != nullptr) {
...
void SetNLongData(VDATA* pvd, int n, ...)
{
    ...
    for (auto i = 0; i < n; i++) {
        ...
        if (!pIVal) break;
        auto const nDefVal = va_arg(args, int32_t);
        if (pvd)
            pvd->Get(*pIVal, i);
        else
            *pIVal = nDefVal;
...
void SHIP_DESCRIBE_LIST::Add(
    int32_t     mainChrIndex,
    int32_t     chIdx,
    ATTRIBUTES* pChAttr,
    ATTRIBUTES* pShipAttr,
    bool        bMyShip,
    bool        bTransferableShip,
    int32_t     relation,
    uint32_t    dwShipColor)
{
    ...
    if (pr == nullptr) { throw std::runtime_error("Can`t allocate memory"); }
...
SHIP_DESCRIBE_LIST::SHIP_DESCR* SHIP_DESCRIBE_LIST::FindShip(int32_t idxCharacter) const
{
    for (auto* ptmp = root; ptmp != nullptr; ptmp = ptmp->next)
        if (ptmp->characterIndex == idxCharacter) return ptmp;
    return nullptr;
}
