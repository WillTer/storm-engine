...
void BIShipCommandList::FillIcons()
{
    ...
    auto    fill           = [this, &nIconsQuantity](std::function<bool(int32_t)> const& check) {
        ...
        if (check(BI_COMMODE_LAND_SELECT)) nIconsQuantity += LandAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0);

        if (check(BI_COMMODE_COMMAND_SELECT)) nIconsQuantity += CommandAdding();

        if (check(BI_COMMODE_CANNON_CHARGE)) nIconsQuantity += ChargeAdding();

        if (check(BI_COMMODE_USER_ICONS)) nIconsQuantity += UserIconsAdding();

        if (check(BI_COMMODE_ABILITY_ICONS)) nIconsQuantity += AbilityAdding();
    ...
    if (auto* data = core.Event(BI_EVENT_GET_CMD_LIST_ORDER_PRIORITY, "l", i)) {
        ...
        // no-event fallback
        fill([this](uint32_t const current_cmd) { return (m_nCurrentCommandMode & current_cmd) != 0; });
    }
...
void BIShipCommandList::Init()
{
    ...
    if (pA) { m_nIconShowMaxQuantity = pA->GetAttributeAsDword("CommandMaxIconQuantity", m_nIconShowMaxQuantity); }
}
...
int32_t BIShipCommandList::ShipAdding(bool allLabel, bool bMyShip, bool bEnemy, bool bNeutral, bool bFriend)
{
    ...
    if (bMyShip && main_sd != selShip) {
        n = AddToIconList(
            main_sd->textureNum,
            main_sd->pictureNum,
            main_sd->selectPictureNum,
            -1,
            main_sd->characterIndex,
            nullptr,
            -1,
            nullptr,
            main_sd->pAttr->GetAttribute("name"));
        if (n > 0) {
            retVal += n;
...
int32_t BIShipCommandList::FortAdding(bool allLabel, bool bFriend, bool bNeutral, bool bEnemy)
{
    ...
    auto const selectedCharacter = m_nCurrentCommandCharacterIndex;
    ...
    do {
        if (bFriend && pL->relation == BI_RELATION_FRIEND || bNeutral && pL->relation == BI_RELATION_NEUTRAL
            ...
            retVal += AddToIconList(
                pL->texIdx, pL->picIdx, pL->selPicIdx, -1, pL->characterIndex, nullptr, -1, pL->pA->GetAttribute("name"), pL->pchr_note);
        }
...
int32_t BIShipCommandList::LandAdding(bool allLabel)
{
    ...
    do {
        ...
        char const* pLocName = nullptr;
        if (pL->pA != nullptr) pLocName = pL->pA->GetAttribute("name");
        retVal += AddToIconList(pL->texIdx, pL->picIdx, pL->selPicIdx, -1, pL->characterIndex, nullptr, -1, pLocName, pL->pchr_note);
    } while ((pL = g_IslandDescr.GetNext()) != nullptr);
    return retVal;
}
...
int32_t BIShipCommandList::CommandAdding()
{
    ...
    for (int32_t i = 0; i < attrQuant; i++) {
        ...
        int32_t const cooldownPictureNum = pA->GetAttributeAsDword("cooldownPicNum", -1);
        int32_t const texNum             = pA->GetAttributeAsDword("texNum", m_nCommandTextureNum);
        char const*   eventName          = pA->GetAttribute("event");
        retVal +=
            AddToIconList(texNum, pictureNum, selPictureNum, cooldownPictureNum, -1, eventName, -1, nullptr, pA->GetAttribute("note"));
    }
...
int32_t BIShipCommandList::ChargeAdding()
{
    ...
    for (auto i = 0; i < lIdx; i++) {
        ...
        int32_t const nNormalPicIndex = pA ? pA->GetAttributeAsDword("picNum", -1) : -1;
        int32_t const nSelectPicIndex = pA ? pA->GetAttributeAsDword("selPicNum", -1) : -1;
...
int32_t BIShipCommandList::UserIconsAdding()
{
    ...
    for (int32_t i = 0; i < attrQuant; i++) {
        ...
        if (pA == nullptr) continue;                              // no such attribute
        if (pA->GetAttributeAsDword("enable", 0) == 0) continue;  // command not available
        int32_t const pictureNum    = pA->GetAttributeAsDword("pic", 0);
        int32_t const selPictureNum = pA->GetAttributeAsDword("selpic", 0);
        int32_t const textureNum    = pA->GetAttributeAsDword("tex", -1);
        retVal += AddToIconList(
            textureNum, pictureNum, selPictureNum, -1, -1, nullptr, i + 1, pA->GetAttribute("name"), pA->GetAttribute("note"));
    }
...
int32_t BIShipCommandList::AbilityAdding()
{
    ...
    for (int32_t i = 0; i < attrQuant; i++) {
        ...
        int32_t const pictureNum    = pA->GetAttributeAsDword("picNum", 0);
        int32_t const selPictureNum = pA->GetAttributeAsDword("selPicNum", 0);
        int32_t const textureNum    = pA->GetAttributeAsDword("texNum", -1);
        // retVal += AddToIconList( textureNum, pictureNum, selPictureNum, -1, -1, pA->GetThisName(), i, null,
        // pA->GetAttribute("note") );
        int32_t const cooldownPictureNum = pA->GetAttributeAsDword("cooldownPicNum", -1);
        char const*   eventName          = pA->GetAttribute("event");
        retVal +=
            AddToIconList(textureNum, pictureNum, selPictureNum, cooldownPictureNum, -1, eventName, -1, nullptr, pA->GetAttribute("note"));
    }
...
int32_t BIShipCommandList::TownAdding(bool allLabel, bool bDiseased, bool bNotDiseased, bool bEnemy, bool bNeutral, bool bFriend)
{
    ...
    do {
        if (pL->locatorType != ISLAND_LOCATOR_TOWN) continue;  // check by type - must be city
