...
void BATTLE_NAVIGATOR::CalculateTextureRect(storm::FRect& texRect, int32_t num, int32_t hq, int32_t vq)
{
    auto const yNum = num / hq;
    auto const xNum = num - yNum * hq;
...
void BATTLE_NAVIGATOR::Draw() const
{
    ...
    // if (m_idIslandTexture >= 0) rs->TextureSet(0, m_idIslandTexture);
    // if (m_pIslandTexture != nullptr) rs->SetTexture(0, m_pIslandTexture);
    // if (m_bYesIsland)
    ...
    //     rs->DrawPrimitive(
    //         D3DPT_TRIANGLEFAN, m_idFireZoneVBuf, sizeof(BI_NOTEXTURE_VERTEX), 0, FIRERANGE_QUANTITY - 2, "battle_only_tfactor");
    // }
    //
    ...
    // rs->ExtPrint(
    //     m_speedFont, 0xFFFFFFFF, 0, PR_ALIGN_CENTER, true, m_fFontScale, 0, 0, m_xWindSpeed, m_ySpeedShow, "%.1f", m_fWindStrength);
    // rs->ExtPrint(m_speedFont, 0xFFFFFFFF, 0, PR_ALIGN_CENTER, true, m_fFontScale, 0, 0, m_xShipSpeed, m_ySpeedShow, "%.1f",
    // m_fShipSpeed);
    //
    // // show current charge
    // if (m_curCharge >= 0) {
...
void BATTLE_NAVIGATOR::Update()
{
    ...
    BI_ONETEXTURE_VERTEX* pV = nullptr;
    // auto* pV = static_cast<BI_ONETEXTURE_VERTEX*>(rs->LockVertexBuffer(m_idEmptyVBuf));
    if (pV != nullptr) {
        SetRectangleVertexPos(
            &pV[4],
            static_cast<float>(m_XNavigator),
            static_cast<float>(m_YNavigator),
            static_cast<float>(m_NavigationWidth),
            static_cast<float>(m_NavigationWidth),
            -m_fAngle);
        SetRectangleVertexPos(
            &pV[8],
            static_cast<float>(m_XNavigator),
            static_cast<float>(m_YNavigator),
            static_cast<float>(m_windWidth),
            static_cast<float>(m_windHeight),
    ...
    if (pV != nullptr) {
        ...
        if (!FloatCompare(m_fCurAnglRightCharge, m_fCurAnglRightDamage)) {
            SetRectangleSegVertexPos(
                ...
                static_cast<float>(m_NavigationWidth),
                static_cast<float>(m_NavigationWidth),
                m_fCurAnglRightCharge,
                m_fBegAnglRightCharge);
            m_nvCannonCharge +=
                SetRectangleSegVertexTex(&pV[m_nvCannonCharge], .5f, .5f, 1.f, 1.f, m_fCurAnglRightCharge, m_fBegAnglRightCharge);
        }
        if (!FloatCompare(m_fCurAnglForwardCharge, m_fCurAnglForwardDamage)) {
            ...
            m_nvCannonCharge +=
                SetRectangleSegVertexTex(&pV[m_nvCannonCharge], .5f, .5f, 1.f, 1.f, m_fBegAnglForwardCharge, m_fCurAnglForwardCharge);
        }
        if (!FloatCompare(m_fCurAnglBackCharge, m_fCurAnglBackDamage)) {
            SetRectangleSegVertexPos(
                ...
                static_cast<float>(m_NavigationWidth),
                static_cast<float>(m_NavigationWidth),
                m_fCurAnglBackCharge,
                m_fBegAnglBackCharge);
            m_nvCannonCharge +=
                SetRectangleSegVertexTex(&pV[m_nvCannonCharge], .5f, .5f, 1.f, 1.f, m_fCurAnglBackCharge, m_fBegAnglBackCharge);
        }
        ...
        if (FloatCompare(m_fCurAnglLeftCharge, m_fCurAnglLeftDamage)) {
            SetRectangleSegVertexPos(
                ...
                m_fCurAnglLeftCharge);
            m_nvCannonReady =
                SetRectangleSegVertexTex(&pV[m_nvCannonCharge], .5f, .5f, 1.f, 1.f, m_fBegAnglLeftCharge, m_fCurAnglLeftCharge);
        }
        if (FloatCompare(m_fCurAnglRightCharge, m_fCurAnglRightDamage)) {
            SetRectangleSegVertexPos(
                &pV[m_nvCannonCharge + m_nvCannonReady],
                static_cast<float>(m_XNavigator),
                static_cast<float>(m_YNavigator),
                static_cast<float>(m_NavigationWidth),
                static_cast<float>(m_NavigationWidth),
                m_fCurAnglRightCharge,
                m_fBegAnglRightCharge);
            m_nvCannonReady += SetRectangleSegVertexTex(
                &pV[m_nvCannonCharge + m_nvCannonReady], .5f, .5f, 1.f, 1.f, m_fCurAnglRightCharge, m_fBegAnglRightCharge);
        }
        if (FloatCompare(m_fCurAnglForwardCharge, m_fCurAnglForwardDamage)) {
            SetRectangleSegVertexPos(
                &pV[m_nvCannonCharge + m_nvCannonReady],
                ...
                m_fCurAnglForwardCharge);
            m_nvCannonReady += SetRectangleSegVertexTex(
                &pV[m_nvCannonCharge + m_nvCannonReady], .5f, .5f, 1.f, 1.f, m_fBegAnglForwardCharge, m_fCurAnglForwardCharge);
        }
        if (FloatCompare(m_fCurAnglBackCharge, m_fCurAnglBackDamage)) {
            SetRectangleSegVertexPos(
                &pV[m_nvCannonCharge + m_nvCannonReady],
                ...
                m_fBegAnglBackCharge);
            m_nvCannonReady += SetRectangleSegVertexTex(
                &pV[m_nvCannonCharge + m_nvCannonReady], .5f, .5f, 1.f, 1.f, m_fCurAnglBackCharge, m_fBegAnglBackCharge);
        }
        ...
        if (!FloatCompare(m_fCurAnglLeftDamage, m_fEndAnglLeftCharge)) {
            SetRectangleSegVertexPos(
                ...
                static_cast<float>(m_NavigationWidth),
                static_cast<float>(m_NavigationWidth),
                m_fCurAnglLeftDamage,
                m_fEndAnglLeftCharge);
            m_nvCannonDamage = SetRectangleSegVertexTex(
                &pV[m_nvCannonCharge + m_nvCannonReady], .5f, .5f, 1.f, 1.f, m_fCurAnglLeftDamage, m_fEndAnglLeftCharge);
        }
        if (!FloatCompare(m_fCurAnglRightDamage, m_fEndAnglRightCharge)) {
            SetRectangleSegVertexPos(
                &pV[m_nvCannonCharge + m_nvCannonReady + m_nvCannonDamage],
                static_cast<float>(m_XNavigator),
                static_cast<float>(m_YNavigator),
                static_cast<float>(m_NavigationWidth),
                static_cast<float>(m_NavigationWidth),
                m_fEndAnglRightCharge,
                m_fCurAnglRightDamage);
            m_nvCannonDamage += SetRectangleSegVertexTex(
                &pV[m_nvCannonCharge + m_nvCannonReady + m_nvCannonDamage],
                .5f,
                .5f,
        ...
        if (!FloatCompare(m_fCurAnglBackDamage, m_fEndAnglBackCharge)) {
            SetRectangleSegVertexPos(
                ...
                m_fCurAnglBackDamage);
            m_nvCannonDamage += SetRectangleSegVertexTex(
                &pV[m_nvCannonCharge + m_nvCannonReady + m_nvCannonDamage], .5f, .5f, 1.f, 1.f, m_fEndAnglBackCharge, m_fCurAnglBackDamage);
        }
...
{
    BI_NOTEXTURE_VERTEX* pv = nullptr;
...
void BATTLE_NAVIGATOR::FillOneSideFireRange(BI_NOTEXTURE_VERTEX* pv, ATTRIBUTES* pShip, ATTRIBUTES* pChar, char const* pstr) const
{
    ...
    auto const dAng   = fSizeAng / static_cast<float>((BI_ONESIDE_SIZE - 1));
...
void BATTLE_NAVIGATOR::Init(/*VDX9RENDER*/ void* RenderService, Entity* pOwnerEI)
{
    int         i;
    char const* tmpstr;
    ...
    m_dwChargeCannon  = storm::Color {255, 255, 0, 0}.to_hex();
    m_dwReadyCannon   = storm::Color {255, 0, 255, 0}.to_hex();
    m_dwDamagedCannon = storm::Color {255, 64, 64, 64}.to_hex();
    ...
    // color of the sea
    m_dwSeaColor = BIUtils::GetIntFromAttr(pARoot, "argbSeaColor", storm::Color {255, 255, 255, 255}.to_hex());
    // color of the cannon fire zone
    m_dwFireZoneColor = BIUtils::GetIntFromAttr(pARoot, "argbFireZoneColor", storm::Color {255, 255, 255, 255}.to_hex());
    // enemy ship color
    m_dwEnemyShipColor = BIUtils::GetIntFromAttr(pARoot, "argbEnemyShipColor", storm::Color {255, 255, 255, 255}.to_hex());
    // color of your ship
    m_dwFrendShipColor = BIUtils::GetIntFromAttr(pARoot, "argbFrendShipColor", storm::Color {255, 255, 255, 255}.to_hex());
    // neutral ship color
    m_dwNeutralShipColor = BIUtils::GetIntFromAttr(pARoot, "argbNeutralShipColor", storm::Color {255, 255, 255, 255}.to_hex());
    // color of a sinking ship
    m_dwDeadShipColor = BIUtils::GetIntFromAttr(pARoot, "argbDeadShipColor", storm::Color {255, 255, 255, 255}.to_hex());
    // background gradient color
    m_dwBackGradColor1 = BIUtils::GetIntFromAttr(pARoot, "argbBackMaxColor", storm::Color {255, 0, 0, 128}.to_hex());
    m_dwBackGradColor2 = BIUtils::GetIntFromAttr(pARoot, "argbBackMinColor", storm::Color {55, 0, 0, 128}.to_hex());
    ...
    m_fBegAnglForwardCharge = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "forwardChargeBegAngle", 0)) / 180.f * PI;
    m_fEndAnglForwardCharge = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "forwardChargeEndAngle", 0)) / 180.f * PI;

    m_fBegAnglBackCharge = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "backwardChargeBegAngle", 0)) / 180.f * PI;
    m_fEndAnglBackCharge = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "backwardChargeEndAngle", 0)) / 180.f * PI;

    // get speed angles
    m_fBegAnglShipSpeed = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "shipSpeedBegAngle", 0)) / 180.f * PI;
    m_fCurAnglShipSpeed = m_fEndAnglShipSpeed = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "shipSpeedEndAngle", 0)) / 180.f * PI;
    m_fBegAnglWindSpeed                       = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "windSpeedBegAngle", 0)) / 180.f * PI;
    m_fCurAnglWindSpeed = m_fEndAnglWindSpeed = static_cast<float>(BIUtils::GetIntFromAttr(pARoot, "windSpeedEndAngle", 0)) / 180.f * PI;
    ...
    m_WindPos.x = 160;
    m_WindPos.y = 160;
    if ((tmpstr = BIUtils::GetStringFromAttr(pARoot, "windPos", nullptr)) != nullptr) sscanf(tmpstr, "%d,%d", &m_WindPos.x, &m_WindPos.y);
    ...
    // D3DUSAGE_WRITEONLY); m_idFireZoneVBuf = rs->CreateVertexBuffer(BI_NOTEX_VERTEX_FORMAT, FIRERANGE_QUANTITY *
    // sizeof(BI_NOTEXTURE_VERTEX), D3DUSAGE_WRITEONLY); m_idShipsVBuf =
    //     rs->CreateVertexBuffer(BI_COLORONLY_VERTEX_FORMAT, MAX_ENEMY_SHIP_QUANTITY * 3 * sizeof(BI_COLORONLY_VERTEX),
    //     D3DUSAGE_WRITEONLY);
    // m_idGradBackVBuf  = rs->CreateVertexBuffer(BI_COLORONLY_VERTEX_FORMAT, 3 * sizeof(BI_COLORONLY_VERTEX), D3DUSAGE_WRITEONLY);
    // m_idCurChargeVBuf = rs->CreateVertexBuffer(BI_ONETEX_VERTEX_FORMAT, 4 * 4 * sizeof(BI_ONETEXTURE_VERTEX), D3DUSAGE_WRITEONLY);
    if (m_idEmptyVBuf == -1 || m_idCourseVBuf == -1 || m_idCannonVBuf == -1 || m_idSpeedVBuf == -1 || m_idMapVBuf == -1
        || m_idFireZoneVBuf == -1 || m_idShipsVBuf == -1 || m_idGradBackVBuf == -1 || m_idCurChargeVBuf == -1) {
    ...
    // auto* pv = static_cast<BI_NOTEXTURE_VERTEX*>(rs->LockVertexBuffer(m_idFireZoneVBuf));
    if (pv != nullptr) {
        auto const vCenter = CVECTOR(static_cast<float>(m_XNavigator), static_cast<float>(m_YNavigator), 1.f);
    ...
    // ships on the map
    BI_COLORONLY_VERTEX* pcv = nullptr;
    ...
    if (pV != nullptr) {
        ...
        SetRectangleVertexPos(
            &pV[0],
            static_cast<float>(m_ChargePos.x),
            static_cast<float>(m_ChargePos.y),
            static_cast<float>(m_ChargeSize.x),
            static_cast<float>(m_ChargeSize.y));
        SetRectangleVertexPos(
            &pV[4],
            static_cast<float>(m_WindPos.x),
            static_cast<float>(m_WindPos.y),
            static_cast<float>(m_WindSize.x),
            static_cast<float>(m_WindSize.y));
        SetRectangleVertexPos(
            &pV[8],
            static_cast<float>(m_SailPos.x),
            static_cast<float>(m_SailPos.y),
            static_cast<float>(m_SailSize.x),
            static_cast<float>(m_SailSize.y));
        SetRectangleVertexPos(
            &pV[12],
            static_cast<float>(m_PowderPos.x),
            static_cast<float>(m_PowderPos.y),
            static_cast<float>(m_PowderSize.x),
            static_cast<float>(m_PowderSize.y));
        // rs->UnLockVertexBuffer(m_idCurChargeVBuf);
    }
}

int32_t BATTLE_NAVIGATOR::SetRectangleVertexPos(BI_ONETEXTURE_VERTEX* v, float x, float y, float width, float height, float angle) const
{
...
int32_t BATTLE_NAVIGATOR::SetRectangleVertexTex(BI_ONETEXTURE_VERTEX* v, float x, float y, float width, float height, float angle)
{
    if (v == nullptr) return 0;
...
void BATTLE_NAVIGATOR::SetMainCharacterData()
{
    ...
    if (g_bExternMapCenter) {
        ...
        auto const cPos = psd->pShip->GetPos();
        m_fXPos         = cPos.x;
        m_fYPos         = cPos.z;
    }
    if (g_bMapScaling) {
    ...
    // get ship y angle
    auto const cAng = psd->pShip->GetAng();
    m_fAngle        = cAng.y;
...
{
    BI_COLORONLY_VERTEX* pv = nullptr;
    ...
    auto        idx            = 0;
    auto const  fSqrMapRad     = m_fMapRadius * m_fMapRadius;
    ...
    for (auto* psd = g_ShipList.GetShipRoot(); psd != nullptr; psd = psd->next) {
        ...
        auto const tmp = fX * cosf(m_fAngle) - fY * sinf(m_fAngle);
        ...
        if (psd->dwShipColor == 0) {
            switch (psd->relation) {
            case BI_RELATION_ENEMY: dwColor = m_dwEnemyShipColor; break;
            case BI_RELATION_FRIEND: dwColor = m_dwFrendShipColor; break;
            case BI_RELATION_NEUTRAL: dwColor = m_dwNeutralShipColor; break;
            }
        } else
            dwColor = psd->dwShipColor;
        if (psd->isDead) dwColor = m_dwDeadShipColor;
        // determine the angle of the ship
        auto const fAngle = psd->pShip->GetAng().y - m_fAngle;
        // fill buffer
...
{
    switch (side) {
    case 0: return h / 2 * tanf(angl);
    case 1: return w / 2;
    case 2: return -h / 2 * tanf(angl);
    case 3: return -w / 2;
    }
    return 0.f;
}
...
float BATTLE_NAVIGATOR::CalculateCrossY(int side, float w, float h, float angl)
{
    switch (side) {
    case 0: return -h / 2;
    case 1: return w / 2 * tanf(angl - PI / 2);
    case 2: return h / 2;
    case 3: return -w / 2 * tanf(angl - PI / 2);
    }
    return 0.f;
}

int32_t BATTLE_NAVIGATOR::SetRectangleSegVertexPos(
    BI_ONETEXTURE_VERTEX* v, float x, float y, float width, float height, float begAngle, float endAngle)
{
    if (v == nullptr) return 0;
    ...
    auto const alpha = atan2f(width / 2, height / 2);

    auto const begseg = begAngle < alpha ? 0
        : begAngle < (PI - alpha)        ? 1
        : begAngle < (PI + alpha)        ? 2
        : begAngle < (2 * PI - alpha)    ? 3
                                         : 0;
    auto const endseg = endAngle < alpha ? 0
        : endAngle < (PI - alpha)        ? 1
        : endAngle < (PI + alpha)        ? 2
...
int32_t BATTLE_NAVIGATOR::SetRectangleSegVertexTex(
    BI_ONETEXTURE_VERTEX* v, float x, float y, float width, float height, float begAngle, float endAngle)
{
    if (v == nullptr) return 0;
...
void BATTLE_NAVIGATOR::SetIsland()
{
    ...
    if (pM != nullptr) {
        ...
        //     if (rs->CreateTexture(
        //             MAP_TEXTURE_WIDTH, MAP_TEXTURE_HEIGHT, 1, D3DUSAGE_RENDERTARGET, D3DFMT_R5G6B5, D3DPOOL_DEFAULT, &m_pIslandTexture)
        //         == D3D_OK) {
        //         // SIMPLE OUT
