...
bool SHIPPOINTER::Init(std::shared_ptr<entt::registry> const& registry)
{
...
uint64_t SHIPPOINTER::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_SP_CHANGESHIP: {
        auto const chrIdx = message.Long();
...
void SHIPPOINTER::UpdateShipPointer() const
{
    ...
    if (pv != nullptr) {
        ...
        auto const    cvhorz = k * .5f * !((campos - ourPos) ^ CVECTOR(0.f, 1.f, 0.f));
