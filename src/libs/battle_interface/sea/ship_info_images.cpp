...
void ShipInfoImages::UpdateShipList()
{
    ...
    for (pSD = g_ShipList.GetShipRoot(); pSD; pSD = pSD->next)
        if (IsEnableShowShipInfo(pSD)) m_nShipQ++;
    if (m_nShipQ <= 0) return;
    ...
    n = 0;
    for (pSD = g_ShipList.GetShipRoot(); pSD; pSD = pSD->next) {
        if (!IsEnableShowShipInfo(pSD)) continue;
...
void ShipInfoImages::UpdateShipData(int32_t nShipNum, SHIP_DESCRIBE_LIST::SHIP_DESCR* pSD)
{
    ...
    WriteSquareVertex(&m_pVBuffRelation[nShipNum * 4], shippos, m_vRelationOffset, m_fpRelationSize, GetUVForRelation(pSD->relation), 1.f);

    // write progress back square
    WriteSquareVertex(&m_pVBuffBackProgress[nShipNum * 4], shippos, m_vProgressBackOffset, m_fpProgressBackSize, m_uvProgressBack, 1.f);

    // write progress squares
    //---------------------------------
    // write hull
    WriteSquareVertex(&m_pVBuffProgress[nShipNum * 3 * 4], shippos, m_vHullOffset, m_fpProgressSize, m_uvHull, GetProgressHull(pSD));
    // write sail
    WriteSquareVertex(&m_pVBuffProgress[nShipNum * 3 * 4 + 4], shippos, m_vSailOffset, m_fpProgressSize, m_uvSail, GetProgressSail(pSD));
    // write crew
    WriteSquareVertex(&m_pVBuffProgress[nShipNum * 3 * 4 + 8], shippos, m_vCrewOffset, m_fpProgressSize, m_uvCrew, GetProgressCrew(pSD));
}
...
const FRECT& ShipInfoImages::GetUVForRelation(int32_t nRelation) const
{
    ...
    case 0: return m_uvRelation[1]; break;  // enemy
    }
