...
BIShipIcon::BIShipIcon(entid_t BIEntityID, /*VDX9RENDER*/ void* pRS)
{
    // Assert(pRS);
...
void BIShipIcon::Draw()
{
    ...
    if (m_nVBufID != -1 && m_nIBufID != -1) {
        ...
        //         m_nIBufID,
        //         nStartV,
        //         m_nShipStateSquareQ * 4,
        //         nStartI,
        //         m_nShipStateSquareQ * 2,
        //         "battle_colorRectangle");
        // }
        ...
        //         m_nIBufID,
        //         nStartV,
        //         m_nShipClassSquareQ * 4,
        //         nStartI,
        //         m_nShipClassSquareQ * 2,
    ...
    for (int32_t n = 0; n < m_nShipQuantity; n++) {
        if (m_Ship[n].pASailorQuantity) {
            ...
            //     static_cast<int32_t>(m_Ship[n].pntPos.y) + m_SailorFontOffset.y,
            //     "%d",
        ...
        if (!m_Ship[n].sShipName.empty()) {
            ...
            //     static_cast<int32_t>(m_Ship[n].pntPos.y) + m_ShipNameFontOffset.y,
            //     "%s",
...
void BIShipIcon::Init(ATTRIBUTES* pRoot, ATTRIBUTES* pA)
{
    ...
    m_pCommandList = new BIShipCommandList(m_idHostEntity, pRoot, /*m_pRS*/ nullptr);
    ...
    m_nShipStateTextureID = -1;
    m_dwShipStateColor    = storm::Color {255, 128, 128, 128}.to_hex();
    ...
    if (pA) {
        ...
        if (pcTmp) sscanf(pcTmp, "%ld,%ld", &m_SailorFontOffset.x, &m_SailorFontOffset.y);
        ...
        if (pcTmp) sscanf(pcTmp, "%ld,%ld", &m_ShipNameFontOffset.x, &m_ShipNameFontOffset.y);
        ...
        if (pcTmp) {
            do {
                m_aClassProgress.push_back(BIUtils::GetFromStr_Float((char const*&)pcTmp, 0.f));
            } while (pcTmp[0]);
        }
...
size_t BIShipIcon::AddTexture(char const* pcTextureName, int32_t nCols, int32_t nRows) const
{
...
{
    auto const n = CalculateShipQuantity();
    UpdateBuffers(n);
}
...
int32_t BIShipIcon::CalculateShipQuantity()
{
    ...
    for (pSD = g_ShipList.GetShipRoot(); pSD; pSD = pSD->next) {
        if (m_Ship[0].nCharacterIndex == pSD->characterIndex) continue;  // the main character is already on the list
        if (pSD->isMyShip) {
...
{
    auto const    nBackSquareQ      = nShipQ;
    auto const    nShipStateSquareQ = nShipQ * 2;
    int32_t const nShipClassSquareQ = nShipQ;
    int32_t const nShipSquareQ      = nShipQ;

    int32_t const nMaxSquareQ = BIUtils::GetMaxFromFourLong(nBackSquareQ, nShipStateSquareQ, nShipClassSquareQ, nShipSquareQ);
    ...
    if (pV) {
        ...
        for (n = 0; n < m_nShipQuantity; n++) {
            vn += WriteSquareToVBuffWithProgress(
                ...
                m_rShipHPUV,
                m_dwShipStateColor,
                m_Ship[n].pntPos + m_pntShipHPOffset,
                m_pntShipHPIconSize,
                GetProgressShipHP(n),
                0.f,
                0.f,
            ...
            vn += WriteSquareToVBuffWithProgress(
                ...
                m_rShipSPUV,
                m_dwShipStateColor,
                m_Ship[n].pntPos + m_pntShipSPOffset,
                m_pntShipSPIconSize,
                GetProgressShipSP(n),
                0.f,
                0.f,
        ...
        for (n = 0; n < m_nShipQuantity; n++)
            vn += WriteSquareToVBuffWithProgress(
                ...
                m_rShipClassUV,
                m_dwShipClassColor,
                m_Ship[n].pntPos + m_pntShipClassOffset,
                m_pntShipClassIconSize,
                0.f,
                0.f,
                0.f,
                1.f - GetProgressShipClass(n));
...
int32_t BIShipIcon::WriteSquareToVBuff(
    BI_COLOR_VERTEX* pv, storm::FRect const& uv, uint32_t color, storm::FPoint const& center, storm::FPoint const& size)
{
    if (!pv) return 0;

    auto const  fLeft   = static_cast<float>(center.x - size.x / 2);
    auto const  fTop    = static_cast<float>(center.y - size.y / 2);
    float const fRight  = fLeft + size.x;
    float const fBottom = fTop + size.y;
...
int32_t BIShipIcon::WriteSquareToVBuffWithProgress(
    BI_COLOR_VERTEX*     pv,
    ...
    float                fClampRight)
{
    ...
    float const fLeftUV   = uv.left + (uv.right - uv.left) * fClampLeft;
    float const fRightUV  = uv.right - (uv.right - uv.left) * fClampRight;
    float const fTopUV    = uv.top + (uv.bottom - uv.top) * fClampUp;
    float const fBottomUV = uv.bottom - (uv.bottom - uv.top) * fClampDown;
...
float BIShipIcon::GetProgressShipClass(int32_t nShipNum)
{
    ...
    float const f = m_aClassProgress[m_Ship[nShipNum].nShipClass];
    return f;
}

void BIShipIcon::GetShipUVFromPictureIndex(int32_t nPicIndex, storm::FRect& rUV)
{
    ...
    float const ny = m_dwShipNCols ? std::floor(static_cast<float>(nPicIndex) / m_dwShipNCols)
                                   : std::floor(static_cast<float>(nPicIndex) / pictureCount);
    float const nx = m_dwShipNRows ? static_cast<float>(nPicIndex) - ny * m_dwShipNRows : static_cast<float>(nPicIndex) - ny * pictureCount;
