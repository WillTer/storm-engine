    ...
    if (m_bYesShowAll) {
        if (m_bVisible) {
            ...
            if (m_bShowBattleNavigator) {
                ...
                m_LinesInfo.Draw();
                if (m_bShowBattleBorder) m_BattleBorder.Draw();
            }
        }
    } else {
        m_bYesShowAll = true;
...
uint64_t BATTLE_INTERFACE::ProcessMessage(MESSAGE& message)
{
    ...
    {
        auto const     chIdx             = message.Long();
        auto* const    pChAttr           = message.AttributePointer();
        auto* const    pShipAttr         = message.AttributePointer();
        auto const     bMyShip           = (message.Long() != 0L);
        auto const     relation          = message.Long();
        uint32_t const dwShipColor       = message.ParamValid() ? message.Long() : 0;
        bool const     bTransferableShip = message.ParamValid() ? (message.Long() != 0) : bMyShip;
        g_ShipList.Add(
            AttributesPointer ? AttributesPointer->GetAttributeAsDword("MainChrIndex", -1) : -1,
            chIdx,
            pChAttr,
            pShipAttr,
            bMyShip,
...
void BATTLE_INTERFACE::CheckSeaState()
{
    ...
    if (g_IslandDescr.GetFirstLocator() != nullptr) bSailTo = true;
