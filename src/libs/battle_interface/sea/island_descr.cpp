...
void ISLAND_DESCRIBER::SetIsland(ATTRIBUTES* pAIsland)
{
    ...
    if ((m_pLocators = new LOCATOR_DESCR[m_nLocators]) == nullptr) { throw std::runtime_error("Can`t allocate memory"); }
    // go through all locators
    ATTRIBUTES* pATmp;
    for (auto i = 0; i < m_nLocators; i++) {
        m_pLocators[i].pchr_note = nullptr;
        if ((pATmp = m_pLocators[i].pA = pA->GetAttributeClass(i)) == nullptr) continue;
...
ISLAND_DESCRIBER::LOCATOR_DESCR* ISLAND_DESCRIBER::GetFirstLocator()
{
    ...
    m_lastFindIdx = 0;
    if (m_nLocators == 0 && m_pLocators == nullptr) return nullptr;
...
ISLAND_DESCRIBER::LOCATOR_DESCR* ISLAND_DESCRIBER::FindLocator(LOCATOR_DESCR* p, size_t nMax)
{
    ...
    switch (m_nFindType) {
    case ISLAND_FIND_LOCATOR: m_lastFindIdx++; return &p[0];
    ...
    default:
        for (i = 0; i < nMax; i++)
            if (p[i].locatorType == ISLAND_LOCATOR_FORT) switch (m_nFindType) {
                case ISLAND_FIND_FORT: m_lastFindIdx += i + 1; return &p[i];
                ...
                case ISLAND_FIND_ENEMYFORT:
                    ...
                    break;
                case ISLAND_FIND_BROKENFORT: m_lastFindIdx += i + 1; return &p[i];
                }
        break;
    }
    // found nothing
...
ISLAND_DESCRIBER::LOCATOR_DESCR* ISLAND_DESCRIBER::FindLocatorByName(char* name) const
{
    ...
    for (auto i = 0; i < m_nLocators; i++) {
        ...
        char const* curName = m_pLocators[i].pA->GetAttribute("name");
        if (curName != nullptr && storm::iEquals(name, curName)) return &m_pLocators[i];
    }
    return nullptr;
}
...
void ISLAND_DESCRIBER::Refresh() const
{
    ...
    for (auto i = 0; i < m_nLocators; i++) {
        if (m_pLocators[i].locatorType == ISLAND_LOCATOR_FORT) {
            m_pLocators[i].relation = GetVDATALong(core->Event(BI_EVENT_GET_FORT_RELATION, "a", m_pLocators[i].pA), BI_RELATION_NEUTRAL);
        }
    }
}
