...
int32_t PCS_CONTROLS::CreateControl(char const* control_name)
{
    ...
    for (n = 0; n < nControlsNum; n++) {
        if (storm::iEquals(control_name, pUserControls[n].name)) return n;
    }
    n = nControlsNum;
    nControlsNum++;
    pUserControls.resize(nControlsNum);
    auto const len        = strlen(control_name) + 1;
...
int32_t PCS_CONTROLS::AddControlTreeNode(int32_t nParent, char const* pcBaseControl, char const* pcOutControl, float fTimeOut)
{
    ...
    if (ntree >= 0 && pcOutControl) {
        auto const nc = CreateControl((char*)pcOutControl);
        if (nc >= 0) {
...
bool PCS_CONTROLS::GetControlState(char const* control_name, CONTROL_STATE& _state_struct)
{
    ...
    int32_t    n;
    auto const bControlFound = false;
...
bool PCS_CONTROLS::GetControlState(int32_t control_code, CONTROL_STATE& _state_struct)
{
    ...
    if (ControlsTab[system_code].update_frame == nFrameCounter) {
        ...
        if (system_code == CE_MOUSE_X_AXIS || system_code == CE_MOUSE_Y_AXIS) {
            ...
            if (IsKeyPressed(system_code)) {
                ...
                if (ControlsTab[system_code].state.state == CST_INACTIVE || ControlsTab[system_code].state.state == CST_INACTIVATED) {
    ...
    if ((pUserControls[control_code].flags & USE_AXIS_AS_BUTTON) && pUserControls[control_code].state != FORCE_DWORD) {
        if (_state_struct.lValue > 0) {
            if (pUserControls[control_code].state == CST_INACTIVE || pUserControls[control_code].state == CST_INACTIVATED)
    ...
    if (_state_struct.state != CST_INACTIVE)
        if (!(system_code == CE_MOUSE_X_AXIS || system_code == CE_MOUSE_Y_AXIS)) nLastControlTime = 0;
    return true;
}
...
void PCS_CONTROLS::Update(uint32_t DeltaTime)
{
    ...
    ControlsTab[system_code].state.fValue = static_cast<float>(ControlsTab[system_code].state.lValue) * fMouseSensivityX;
    ControlsTab[system_code].update_frame = nFrameCounter;

    system_code                           = CE_MOUSE_Y_AXIS;
    ControlsTab[system_code].state.lValue = nMouseDy;
    ControlsTab[system_code].state.fValue = static_cast<float>(ControlsTab[system_code].state.lValue) * fMouseSensivityY;
    ControlsTab[system_code].update_frame = nFrameCounter;
...
bool PCS_CONTROLS::SetControlState(char const* control_name, CONTROL_STATE& _state_struct)
{
    ...
    if (control_name == nullptr) return false;
    for (n = 0; n < nControlsNum; n++) {
        if (storm::iEquals(control_name, pUserControls[n].name)) return SetControlState(n, _state_struct);
    }
    return false;
}
...
bool PCS_CONTROLS::IsKeyPressed(int vk)
{
    ...
    if (vk == VK_LBUTTON)
        ...
        if (!input_->KeyboardKeyState(VK_MENU)) pressed = input_->KeyboardKeyState(vk);
    } else if (vk == VK_SHIFT) {
        pressed = input_->KeyboardKeyState(VK_LSHIFT) || input_->KeyboardKeyState(VK_RSHIFT);
    } else if (vk == VK_RETURN) {
        pressed = input_->KeyboardSDLKeyState(SDL_SCANCODE_RETURN) || input_->KeyboardSDLKeyState(SDL_SCANCODE_KP_ENTER);
    } else if (vk == VK_UP) {
...
void PCS_CONTROLS::HandleEvent(InputEvent const& evt)
{
    if (evt.type == InputEvent::KeyboardKeyDown) {
        ...
        auto const& text = std::get<std::string>(evt.data);
