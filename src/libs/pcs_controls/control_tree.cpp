...
ControlTree::ControlChild::ControlChild() : bWaitReleaseControl(false)
{
    index           = -1;
    sControlName    = "";
    bActive         = false;
...
void ControlTree::ControlChild::Process(float fDeltaTime, ControlTree* pControlTree)
{
    ...
    if (bActive) {
        if (fCurTime <= fTimeOut) { fCurTime += fDeltaTime; }
        /*else
        {
    ...
    // if the control has not been activated, then check it for activation
    {
        ...
        if (cs.state == CST_INACTIVATED || cs.state == CST_INACTIVE) return;
    ...
    if (bWaitReleaseControl && !bChildActive) {
        if (cs.state == CST_INACTIVE || cs.state == CST_INACTIVATED) bActive = false;
    }
}

bool ControlTree::ControlChild::ControlInAction(ControlTree* pControlTree, char const* pcControlName, int32_t _nLayer)
{
    ...
    if (bActive) {
        for (int32_t n = 0; n < aChild.size(); n++) {
            if (aChild[n].ControlInAction(pControlTree, pcControlName, _nLayer)) bWaitReleaseControl = true;
        }
    }
...
void ControlTree::Process()
{
    ...
    for (n = 0; n < m_aOutControlList.size(); n++) {
        if (m_aOutControlList[n].state == CST_INACTIVATED) m_aOutControlList[n].state = CST_INACTIVE;
        if (m_aOutControlList[n].state == CST_ACTIVATED) m_aOutControlList[n].state = CST_ACTIVE;
    }
...
int32_t ControlTree::AddControlChild(int32_t nParentIdx, char const* pcControlName, char const* pcOutControlName, float fTimeOut)
{
...
CONTROL_STATE_TYPE ControlTree::CheckControlActivated(char const* pcControlName)
{
    if (!pcControlName) return CST_INACTIVE;
    for (int32_t n = 0; n < m_aOutControlList.size(); n++)
        if (m_aOutControlList[n].sControlName == pcControlName) return m_aOutControlList[n].state;
    return CST_INACTIVE;
}
...
bool ControlTree::AddOutControl(char const* pcOutControlName, bool isActive)
{
    ...
    int32_t n;
    for (n = 0; n < m_aOutControlList.size(); n++)
        if (m_aOutControlList[n].sControlName == pcOutControlName) break;
    ...
    // control pressed
    auto const cs_prev = m_aOutControlList[n].state;
    if (isActive) {
...
void ControlTree::ControlInAction(char const* pcControlName, int32_t nLayer)
{
    m_RootControl.ControlInAction(this, pcControlName, nLayer);
}

bool ControlTree::ExcludeControlFromActive(char const* pcControlName)
{
    for (int32_t n = 0; n < m_aOutControlList.size(); n++) {
