...
{
    return (
        pointType == PT_TYPE_MAST_1 || pointType == PT_TYPE_MAST_2 || pointType == PT_TYPE_MAST_3 || pointType == PT_TYPE_MAST_4
...
{
    return (
        pointType == PT_TYPE_CANNON_L || pointType == PT_TYPE_CANNON_R || pointType == PT_TYPE_CANNON_F || pointType == PT_TYPE_CANNON_B);
};
...
Path SailorsPoints::getPath(int src, int dst, int l)
{
    ...
    for (auto i = 0; i < points.count; i++) {
        ...
        if (mPath.min == -1 || x.min < mPath.min) mPath = x;
    }
        ...
        for (auto i = 0; i < points.count; i++)
            ...
            for (auto _l = 0; _l < links.count; _l++)
                if (  //! links.link[_l].disabled &&
                    ((links.link[_l].first == m && links.link[_l].next == i) || (links.link[_l].first == i && links.link[_l].next == m))) {
                    matrix[i][m] = Dest(
                        CVECTOR(points.point[i].x, points.point[i].y, points.point[i].z),
...
int SailorsPoints::WriteToFile(std::string fileName)
{
    ...
    for (auto i = 0; i < points.count; i++) {
        ...
        sprintf_s(
            buffer, sizeof(buffer), "%f,%f,%f,%d", points.point[i].x, points.point[i].y, points.point[i].z, points.point[i].pointType);
