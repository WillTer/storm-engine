...
ShipMan::ShipMan(ShipMan &&other) noexcept
    : modelID{other.modelID}, pos{std::move(other.pos)}, ang{std::move(other.ang)}, ptTo{std::move(other.ptTo)},
      angTo{std::move(other.angTo)}, dir{std::move(other.dir)}, spos{std::move(other.spos)}, dieTime{other.dieTime},
      inWater{other.inWater}, jumpSpeedX{other.jumpSpeedX}, jumpSpeedY{other.jumpSpeedY}, path{std::move(other.path)},
      mode{other.mode}, lastMode{other.lastMode}, newWayPoint{other.newWayPoint}, lastWayPoint{other.lastWayPoint},
      targetWayPoint{other.targetWayPoint}, lastTargetPoint{other.lastTargetPoint}, moveTo{other.moveTo},
      manSpeed{other.manSpeed}, rotSpeed{other.rotSpeed}
{
...
bool ShipMan::MoveToPosition(uint32_t &dltTime, SailorsPoints &sailorsPoints, ShipState &shipState)
{
    if (const auto model = static_cast<MODEL *>(core.GetEntityPointer(modelID)))
    {
        ...
        const auto dFuture = SQR(pos.x + manSpeed * dir.x - ptTo.x) + SQR(pos.y + manSpeed * dir.y - ptTo.y) +
        ...
        for (auto &man2 : shipMan)
        {
                ...
                man2.path.currentPointPosition >= 0 && man2.path.currentPointPosition < man2.path.length)
            {
                ...
                if (d < 1.0f)
                {
                    ...
                    {
                        man1.spos.x +=
                            0.2f * (+man1.dir.z * (1 - d) - man1.spos.x) / 15.0f * static_cast<float>(dltTime) / 20.0f;
                        man1.spos.z +=
                            0.2f * (-man1.dir.x * (1 - d) - man1.spos.z) / 15.0f * static_cast<float>(dltTime) / 20.0f;
                    }
                    else
                    {
                        if (&man1 < &man2)
...
uint64_t Sailors::ProcessMessage(MESSAGE &message)
{
    ...
    case AI_MESSAGE_CANNON_RELOAD: {
        ...
        const auto walk = std::ranges::find_if(shipWalk, [shipID](const auto &walk) { return walk.shipID == shipID; });
    ...
    case MSG_PEOPLES_ON_SHIP_MASTFALL: {
        if (auto *const attrs = message.AttributePointer())
        {
            const auto walk = std::ranges::find_if(
                shipWalk, [attrs](const auto &walk) { return walk.ship->GetACharacter() == attrs; });
            if (walk != std::end(shipWalk))
            {
...
uint32_t Sailors::AttributeChanged(ATTRIBUTES *attr)
{
    ...
    if (*attr == "IsOnDeck")
    {
        ...
        {
            const auto walk = std::ranges::find_if(
                shipWalk, [](const auto &walk) { return walk.ship->GetACharacter()->GetAttribute("MainCharacter"); });
            if (walk != std::end(shipWalk))
            {
                walk->bHide = true;
            }
