...
ShipMan::ShipMan(ShipMan&& other) noexcept
...
bool ShipMan::RotateToAngle(uint32_t& dltTime, SailorsPoints& sailorsPoints)
{
    if (fabs(angTo.y - ang.y)) {
        ...
        if (fabs(angTo.y - ang.y) < rotSpeed * dltTime / 100.0f) ang = angTo;
...
int ShipMan::FindRandomPoint(SailorsPoints& sailorsPoints, ShipState& shipState)
{
    ...
    for (auto m = 0; m < sailorsPoints.points.count; m++) {
        for (auto i = 0; i < sailorsPoints.points.count; i++) {
            int const ran = rand() % sailorsPoints.points.count;
...
int ShipMan::FindRandomPointWithoutType(SailorsPoints const& sailorsPoints) const
// Find any simple point
{
    for (size_t i = 0; i != sailorsPoints.points.count; ++i) {
        auto const idx = rand() % sailorsPoints.points.count;
        if (sailorsPoints.points.point[idx].pointType == PT_TYPE_NORMAL) { return idx; }
    }
...
void ShipMan::FindNextPoint(SailorsPoints& sailorsPoints, ShipState& shipState)
{
    ...
    if (moveTo != MOVE_TO_CANNON) {
        auto const cannon = GetNearestEmptyCannon(sailorsPoints);
        if (cannon >= 0) {
            path.length = 0;
            path.min    = -1;
    ...
    if (!path.length || path.currentPointPosition == path.length - 1 || path.min == -1) {
        ...
        if (path.min != -1) {
            ...
            ApplyTargetPoint(
                CVECTOR(
                    sailorsPoints.points.point[newWayPoint].x,
        ...
        if ((shipState.mode == SHIP_WAR || moveTo == MOVE_TO_CANNON) && mode == MAN_WALK) mode = MAN_RUN;
...
void ShipMan::ApplyTargetPoint(CVECTOR pt, bool randomWalk)
{
    ...
    if (mode != MAN_CLIMB_UP) { angTo.y = Vector2Angle(dir); }
}
...
bool ShipMan::MoveToPosition(uint32_t& dltTime, SailorsPoints& sailorsPoints, ShipState& shipState)
{
    if (auto const model = static_cast<MODEL*>(core->GetEntityPointer(modelID))) {
        ...
        auto const dFuture =
            SQR(pos.x + manSpeed * dir.x - ptTo.x) + SQR(pos.y + manSpeed * dir.y - ptTo.y) + SQR(pos.z + manSpeed * dir.z - ptTo.z);
...
int ShipMan::GetNearestEmptyCannon(SailorsPoints& sailorsPoints) const
{
    ...
    for (auto i = 0; i < sailorsPoints.points.count; i++)
        if (sailorsPoints.points.point[i].IsCannon() && sailorsPoints.points.point[i].buisy == false
            ...
            dest = Dest(CVECTOR(sailorsPoints.points.point[i].x, sailorsPoints.points.point[i].y, sailorsPoints.points.point[i].z), pos);
...
void ShipMan::NewAction(SailorsPoints& sailorsPoints, ShipState& shipState, uint32_t& dltTime)
{
    ...
    if (mode != MAN_CLIMB_UP && path.currentPointPosition == path.length - 1 || mode == MAN_STAY) {
        ...
        case SHIP_WAR:
        case SHIP_STORM: mode = MAN_RUN; break;
        }
    }
    ...
    if (sailorsPoints.points.point[newWayPoint].IsMast()) {
        ...
        if (lastTargetPoint >= 0 && lastTargetPoint < sailorsPoints.points.count) sailorsPoints.points.point[lastTargetPoint].buisy = false;
    }
}
...
void ShipMan::UpdatePos(uint32_t& dltTime, SailorsPoints& sailorsPoints, ShipState& shipState)
{
    ...
    case MAN_SWIM: Swim(dltTime, sailorsPoints, shipState); break;
    case MAN_JUMP: Jump(dltTime, sailorsPoints, shipState); break;
    }
...
bool ShipWalk::Init(entid_t _shipID, int editorMode, char const* shipType, std::vector<std::string>&& shipManModels)
{
    bHide  = false;
    shipID = _shipID;
    ...
    if (!editorMode) {
        ...
        if (sailorsPoints.points.count <= 0 || sailorsPoints.links.count <= 0) return false;

        // Find broken masts
        auto*       attr      = ship->GetACharacter();
        auto const* mastsAttr = attr->FindAClass(attr, "Ship.Masts");
        ...
        int const iNumMasts = mastsAttr->GetAttributesNum();
        ...
        if (!shipManModels.empty()) { shipManModels_ = std::move(shipManModels); }

        // people count
        // ATTRIBUTES *att = ship->GetACharacter();
        auto const* paShip      = ship->GetAShip();
        auto        peopleCount = 5;
        if (paShip) { peopleCount = paShip->GetAttributeAsDword("lowpolycrew", peopleCount); }
...
{
    for (auto i = 0; i < sailorsPoints.points.count; i++) {
        if (sailorsPoints.points.point[i].pointType == PT_TYPE_MAST_1 && iMastIndex == 1) sailorsPoints.points.point[i].disabled = true;

        if (sailorsPoints.points.point[i].pointType == PT_TYPE_MAST_2 && iMastIndex == 2) sailorsPoints.points.point[i].disabled = true;

        if (sailorsPoints.points.point[i].pointType == PT_TYPE_MAST_3 && iMastIndex == 3) sailorsPoints.points.point[i].disabled = true;

        if (sailorsPoints.points.point[i].pointType == PT_TYPE_MAST_4 && iMastIndex == 4) sailorsPoints.points.point[i].disabled = true;

        if (sailorsPoints.points.point[i].pointType == PT_TYPE_MAST_5 && iMastIndex == 5) sailorsPoints.points.point[i].disabled = true;
    }

    for (auto& man: shipMan) {
        if (sailorsPoints.points.point[man.newWayPoint].disabled || sailorsPoints.points.point[man.lastWayPoint].disabled) {
            man.mode       = MAN_JUMP;
    ...
    for (auto& man: shipMan) {
        if (10 * rand() / RAND_MAX < 3 && man.mode != MAN_JUMP && man.mode != MAN_SWIM) {
            ...
            if (sqrt((pos.x - v.x) * (pos.x - v.x) + (pos.z - v.z) * (pos.z - v.z)) < 1) {
                ...
                man.mode       = MAN_JUMP;
                man.jumpSpeedY = -(JUMP_SPEED_Y_EXPLOSION + rand() * JUMP_SPEED_Y_EXPLOSION / static_cast<float>(RAND_MAX));
...
void ShipWalk::CheckPosition(uint32_t const& dltTime)
{
    for (auto& man: shipMan) {
    ...
    for (auto& man1: shipMan) {
        for (auto& man2: shipMan) {
            if (&man1 != &man2 && Dest(man1.pos, man2.pos, 1) && (man1.mode == MAN_WALK || man1.mode == MAN_RUN)
                && man1.path.currentPointPosition >= 0 && man1.path.currentPointPosition < man1.path.length
                && man2.path.currentPointPosition >= 0 && man2.path.currentPointPosition < man2.path.length) {
                float const d = Dest(man1.pos, man2.pos);

                if (d < 1.0f) {
                    // if go in different directions
                    if (man1.path.point[man1.path.currentPointPosition] != man2.path.point[man2.path.currentPointPosition]) {
                        man1.spos.x += 0.2f * (+man1.dir.z * (1 - d) - man1.spos.x) / 15.0f * static_cast<float>(dltTime) / 20.0f;
                        man1.spos.z += 0.2f * (-man1.dir.x * (1 - d) - man1.spos.z) / 15.0f * static_cast<float>(dltTime) / 20.0f;
                    } else {
                        if (&man1 < &man2) {
                            man1.spos.x += 0.2f * (+man1.dir.z * (1 - d) - man1.spos.x) / 15.0f * static_cast<float>(dltTime) / 20.0f;
                            man1.spos.z += 0.2f * (-man1.dir.x * (1 - d) - man1.spos.z) / 15.0f * static_cast<float>(dltTime) / 20.0f;
    ...
    for (auto walk = std::begin(shipWalk); walk != std::end(shipWalk); ++walk) {
        ...
        if (!walk->shipState.dead) {
            ...
            if (walk->ship && !walk->shipState.dead && !editorMode) {
                ...
                auto const* shipModeAttr = shipAttr->FindAClass(shipAttr, "ship.POS.mode");
                if (shipModeAttr) { walk->shipState.mode = shipModeAttr->GetAttributeAsDword(); }
            }
        }
    }
...
{
    auto const msgId = message.Long();
    switch (msgId) {
    // Add people to the ship
    case AI_MESSAGE_ADD_SHIP: {
        auto const               shipID = message.EntityID();
        std::string const&       c      = message.String();
        ...
        if (message.GetFormat() == "lise") {
            ...
            if (pvd != nullptr) {
                auto const nq = pvd->GetElementsNum();
    ...
    case AI_MESSAGE_CANNON_RELOAD: {
        ...
        auto const walk = std::ranges::find_if(shipWalk, [shipID](auto const& walk) { return walk.shipID == shipID; });
    ...
    case MSG_PEOPLES_ON_SHIP_MASTFALL: {
        if (auto* const attrs = message.AttributePointer()) {
            auto const walk = std::ranges::find_if(shipWalk, [attrs](auto const& walk) { return walk.ship->GetACharacter() == attrs; });
            if (walk != std::end(shipWalk)) { walk->SetMastBroken(message.Long()); }
        }
        break;
    }
    ...
    case MSG_PEOPLES_ON_SHIP_HULLHIT: {
        if (auto* const attrs = message.AttributePointer()) {
            auto const walk = std::ranges::find_if(shipWalk, [attrs](auto const& walk) { return walk.ship->GetACharacter() == attrs; });
            if (walk != std::end(shipWalk)) {
                auto const x = message.Float();
                auto const y = message.Float();
                auto const z = message.Float();
...
uint32_t Sailors::AttributeChanged(ATTRIBUTES* attr)
{
    ...
    if (*attr == "IsOnDeck") {
        if (attr->GetAttributeAsDword() != 0) {
            auto const walk =
                std::ranges::find_if(shipWalk, [](auto const& walk) { return walk.ship->GetACharacter()->GetAttribute("MainCharacter"); });
            if (walk != std::end(shipWalk)) { walk->bHide = true; }
        } else {
