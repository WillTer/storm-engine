...
bool ShipTracks::ShipTrack::Update(SHIP_BASE* pShip)
{
    ...
    std::string const sTex1 = pATrack1->GetAttribute("Texture");
    ...
    std::string const sTex2 = pATrack2->GetAttribute("Texture");
...
{
    auto const dwNewSize = (dwSize / (100) + 1) * (100);
...
{
    auto const dwNewSize = (dwSize / (20) + 1) * (20);
...
void ShipTracks::ShipTrack::Execute(float fDeltaTime)
{
    ...
    auto const vCurPos = pShip->GetPos();
    auto const vCurAng = pShip->GetAng();

    auto const vBoxSize = pShip->GetBoxsize();
    ...
    if (fCurrentDistance > fTrackDistance) {
        auto const fSpeed = Min(1.0f, pShip->GetCurrentSpeed() / 20.0f);
        for (int32_t i = 0; i < static_cast<int32_t>(fCurrentDistance / fTrackDistance); i++) {
            auto const fDistance = (i + 1) * fTrackDistance;
