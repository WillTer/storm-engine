    ...
    while (true) {
        ...
        if (!pNode) {
            if (iNumMasts) {
                ...
                if (!pAQMasts) pAQMasts = GetACharacter()->CreateSubAClass(GetACharacter(), "Ship.MastsQty");
                pAQMasts->SetAttributeUseDword(nullptr, iNumMasts);
            }
            break;
        }
        auto const* const cNodeName = pNode->GetName();
        ...
        if (storm::iEquals(cNodeName, MAST_IDENTIFY, std::size(MAST_IDENTIFY) - 1)) {
            ...
            sscanf(static_cast<char const*>(&cNodeName[std::size(MAST_IDENTIFY) - 1]), "%d", &iNum);
...
bool SHIP::BuildHulls()
{
    ...
    while (true) {
        ...
        if (storm::iEquals(cNodeName, HULL_IDENTIFY, std::size(HULL_IDENTIFY) - 1)) {
            ...
            sscanf((char const*)&cNodeName[std::size(HULL_IDENTIFY) - 1], "%d", &iNum);
