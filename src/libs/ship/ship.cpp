...
void SHIP::InitSailState()
{
    ...
    if (pAShipStopped && pAShipStopped->GetAttributeAsDword()) {
        ...
        if (pASpeedZ && pASpeedZ->GetAttributeAsFloat() > 0.0f) { fNewSailState = 1.0f; }
    }
...
float SHIP::GetRotationAngle(float* pfTime)
{
    ...
    auto const fK      = SHIP_TEST_Y_TIME * State.fMassInertia * State.vInertiaBrake.y;
...
float SHIP::GetBrakingDistance(float* pfTime)
{
    ...
    auto const fK      = SHIP_TEST_Z_TIME * State.fMassInertia * State.vInertiaBrake.z;
...
void SHIP::CheckShip2Strand(float fDeltaTime)
{
    ...
    for (int32_t i = 0; i < MAX_KEEL_POINTS; i++) {
        float      fRes;
        auto const vTmp = pModel->mtx * vKeelContour[i];
...
void SHIP::Execute(uint32_t DeltaTime)
{
    ...
    mRoot = CMatrix(
        vAng.x + vCurDeadDir.x,
        vAng.y,
        vAng.z + vCurDeadDir.z,
        vPos.x + fXOffset,
        vPos.y - State.fShipImmersion - SP.fWaterLine,
        vPos.z + fZOffset);
    SetMatrix(mRoot);
            ...
            if (!pMasts[i].bBroken) {
                ...
                if (fShipRes <= 1.0f) {
                    ...
                    pV = core.Event(
                        ...
                        GetACharacter(),
                        pACollideCharacter);
                    if (pV != nullptr) { pM->fDamage = Clamp(pV->GetFloat()); }
                }
...
void SHIP::HullFall(hull_t* pM)
{
    if (pM && pM->pNode && pM->fDamage >= 1.0f) {
        ...
        if (!pAHulls) pAHulls = GetACharacter()->CreateSubAClass(GetACharacter(), "Ship.Hulls");
        Assert(pAHulls);
        ...
        for (int32_t i = 0; i < iNumMasts; i++) {
            ...
            if (pMast && pMast->pNode && !pMasts[i].bBroken) {
                ...
                if (bOk) {
                    ...
                    if (!pAMasts) pAMasts = GetACharacter()->CreateSubAClass(GetACharacter(), "Ship.Masts");
                    Assert(pAMasts);
                    pAMasts->SetAttributeUseFloat(str, 1.0f);
                    pMast->bBroken = true;
                    pMast->fDamage = 1.0f;
...
uint64_t SHIP::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_SHIP_CREATE:
        ...
        break;
    case MSG_SHIP_SET_SAIL_STATE: SetSailState(message.Float()); break;
    ...
    case MSG_SHIP_ACTIVATE_FIRE_PLACE: {
        auto const         dwFPIndex = static_cast<uint32_t>(message.Long());
        std::string const& str       = message.String();
        std::string const& str1      = message.String();
        std::string const& str2      = message.String();
        // int32_t iSoundID = message.Long();
        auto const fRunTime            = message.Float();
        auto const iBallCharacterIndex = message.Long();
    ...
    case MSG_SHIP_SET_CUSTOM_FLAG: {
        entid_t const flag_entity = message.EntityID();
        if (flag_entity != invalid_entity) { flagEntity_ = flag_entity; }
        break;
    }
    ...
    case MSG_SHIP_DO_FAKE_FIRE: {
        std::string const& cBort     = message.String();
...
void SHIP::FakeFire(char const* sBort, float fRandTime)
{
    GEOS::LABEL label;
    GEOS::INFO  info;
    NODE*       pNode;
...
bool SHIP::Mount(ATTRIBUTES* _pAShip)
{
    ...
    if (!pName) {
        core.Trace(
            ...
            static_cast<char const*>(GetACharacter()->GetAttribute("name")),
            static_cast<char const*>(GetACharacter()->GetAttribute("lastname")),
            static_cast<char const*>(GetACharacter()->GetAttribute("id")));
    ...
    auto const fCapacity = static_cast<float>(SP.iCapacity);
    auto       fLength   = static_cast<float>(30);

    float const maxw        = 10000.0f;
    ...
    bool const bLights = (pALights) ? pALights->GetAttributeAsDword() != 0 : false;
    bool const bFlares = (pAFlares) ? pAFlares->GetAttributeAsDword() != 0 : false;
    ...
    if (pSea) State.vPos.y = pSea->WaveXZ(State.vPos.x + fXOffset, State.vPos.z + fZOffset);
...
void SHIP::ScanShipForFirePlaces()
{
    ...
    std::string const sFirePlace  = "fireplace";
    std::string const sFirePlaces = "fireplaces";
    uint32_t          dwIdx       = 0;
    while (pNode = pModel->GetNode(dwIdx)) {
        pNode->geo->GetInfo(info);
...
float SHIP::Cannon_Trace(int32_t iBallOwner, const CVECTOR& vSrc, const CVECTOR& vDst)
{
    ...
    int32_t const iOurIndex = GetIndex(GetACharacter());
    if (iBallOwner == iOurIndex) return 2.0f;

    for (int32_t i = 0; i < iNumMasts; i++)
        if (!pMasts[i].bBroken) {
            mast_t*     pM   = &pMasts[i];
            float const fRes = pM->pNode->Trace(vSrc, vDst);
    ...
    if (fRes <= 1.0f) {
        ...
        for (uint32_t i = 0; i < aFirePlaces.size(); i++)
            if (!aFirePlaces[i].isActive()) {
                float const fDistance = aFirePlaces[i].GetDistance(vTemp);
...
void SHIP::Save(CSaveLoad* pSL)
{
    ...
    pSL->SaveLong(uniIDX);
    pSL->SaveDword(bUse);
    pSL->SaveBuffer((char const*)&ShipPoints[0][0], sizeof(ShipPoints));
    pSL->SaveVector(vSpeedAccel);
    pSL->SaveBuffer((char const*)&SP, sizeof(SP));
    ...
    pSL->SaveBuffer((char const*)&vKeelContour[0], sizeof(vKeelContour));
    ...
    pSL->SaveBuffer((char const*)&Strength[0], sizeof(Strength));
...
void SHIP::Load(CSaveLoad* pSL)
{
    ...
    std::string const sTmp = pSL->LoadString();
    ...
    for (i = 0; i < dwNum; i++) {
        ...
        if (aFirePlaces[i].isActive())
            core.Event(
                ...
                aFirePlaces[i].GetBallCharacterIndex(),
                i,
                aFirePlaces[i].GetRunTime());
    }
