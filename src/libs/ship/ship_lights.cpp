...
float ShipLights::GetAttributeAsFloat(ATTRIBUTES* pA, char const* pName, float fDefault)
{
...
void ShipLights::AddDynamicLights(VAI_OBJBASE* pObject, const CVECTOR& vPos)
{
    ...
    for (uint32_t i = 0; i < aLights.size(); i++)
        if (aLights[i].pObject == pObject && aLights[i].bDynamicLight) dwNum++;
    if (dwNum >= 4) return;
...
bool ShipLights::SetLabel(ShipLight* pL, MODEL* pModel, char const* pStr)
{
...
void ShipLights::AddFlare(VAI_OBJBASE* pObject, bool bLight, MODEL* pModel, const GEOS::LABEL& label)
{
    ...
    if (str[1] == 'm' || str[1] == 'r') {
        ...
        // rey found
        if (str[1] == 'm') {
            if (str[3] >= 'a' && str[3] <= 'z') { sprintf_s(str2, "rey_%c%d", str[3], iMastIndex); }
        } else {
            if (str[5] >= 'a' && str[5] <= 'z') { sprintf_s(str2, "rey_%c%d", str[5], iMastIndex); }
        }
...
void ShipLights::AddLights(VAI_OBJBASE* pObject, MODEL* pModel, bool bLights, bool bFlares)
{
    ...
    std::string const sLightType = "default";
    ...
    std::string const sFlares = "flares";
    std::string const sLights = "lights";
    uint32_t          dwIdx   = 0;
    while (pNode = pModel->GetNode(dwIdx)) {
        ...
        for (uint32_t i = 0; i < static_cast<uint32_t>(info.nlabels); i++) {
            ...
            if (sLights == label.group_name) { AddFlare(pObject, true, pModel, label); }
        }
        dwIdx++;
    }
}
...
void ShipLights::SetLights(VAI_OBJBASE* pObject)
{
    ...
    for (i = 0; i < aLights.size(); i++) {
        if (aLights[i].bOff || aLights[i].bLightOff) continue;
...
{
    float const fDeltaTime = static_cast<float>(dwDeltaTime) * 0.001f;
    ...
    for (uint32_t i = 0; i < aLights.size(); i++) {
        ...
        if (L.bOff || L.bLightOff) continue;
        ...
        if (!L.bDynamicLight) {
            ...
            for (uint32_t j = 0; j < 2; j++) {
                ...
                float const fIns = o.fOldValue + (o.fNewValue - o.fOldValue) * o.fK;
...
void ShipLights::Realize(uint32_t dwDeltaTime)
{
    ...
    for (uint32_t i = 0; i < aLights.size(); i++) {
        ...
        if (bReflection) {
            float const fDistance = sqrtf(~(L.vCurPos - vCamPos));
...
void ShipLights::ResetLights(VAI_OBJBASE* pObject, bool bLight)
{
    for (auto& light: aLights) {
        if (!light.bOff) { light.bLightOff = !bLight; }
    }
}
...
{
    for (uint32_t i = 0; i < aLights.size(); i++)
        if (aLights[i].pObject == pObject) aLights[i].bDead = true;
}
