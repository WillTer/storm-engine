...
bool InfoHandler::Init(std::shared_ptr<entt::registry> const& registry)
{
...
void InfoHandler::Realize(uint32_t delta_time) const
{
    ...
    m_rs->DrawPrimitiveUP(
        D3DPT_TRIANGLELIST, D3DFVF_XYZRHW | D3DFVF_TEX1 | D3DFVF_TEXTUREFORMAT2, 2, drawbuf_base, sizeof(drawbuf_base[0]), "Fader");
}
...
bool InfoHandler::DoPreOut()
{
    ...
    char const*    picTexureFile     = AttributesPointer->GetAttribute("picfilename");
    char const*    picBackTexureFile = AttributesPointer->GetAttribute("picbackfilename");
    uint32_t const TMP_VERTEX_FORMAT = (D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1 | D3DFVF_TEXTUREFORMAT2);
    struct TMP_VERTEX {
        CVECTOR  pos;
        float    w;
        uint32_t col;
    ...
    if (m_rs->GetRenderTarget(&pRenderTarget) != D3D_OK) { return false; }
    ...
    if (inStrStart) {
        auto const ntmp = m_rs->StringWidth(inStrStart, 0, fScale);
        nOutWidth       = static_cast<int>(sqrtf(4.f * ntmp * nOutOffset) + .9f);
        // record width is approximately four times the height
        if (nOutWidth > static_cast<int>(desc.Width)) nOutWidth = desc.Width;
        nInsideRectWidth = nOutWidth + nBorderWidth * 2;
        if (nInsideRectWidth > static_cast<int>(desc.Width)) nInsideRectWidth = desc.Width;

        nRowQ = 0;
        for (auto const* ps = inStrStart; ps != nullptr && *ps;) {
            ps = GetCutString(ps, nOutWidth, fScale);
            nRowQ++;
        }
        if (nRowQ * nOutOffset > static_cast<int>(desc.Height)) nRowQ = desc.Height / nOutOffset;
        nInsideRectHeight = nRowQ * nOutOffset + nBorderWidth * 2;
        if (nInsideRectHeight > static_cast<int>(desc.Height)) nInsideRectHeight = desc.Height;
    }
    ...
    if (isOK) {
        ...
        if (inStrStart) {
            ...
            // show strings
            auto      ntmp = 0;
            int const topY = (desc.Height - nRowQ * nOutOffset) / 2;
            for (auto* ps = inStrStart; ps != nullptr && *ps;) {
                auto* const oldps = ps;
                ps                = GetCutString(ps, nOutWidth, fScale);
                if (!ps || ps == oldps) break;
                StringToBufer(outStr, sizeof(outStr), oldps, ps - oldps);
                m_rs->ExtPrint(0, 0xFFFFFFFF, 0, PR_ALIGN_CENTER, true, fScale, 0, 0, desc.Width / 2, topY + ntmp * nOutOffset, outStr);
                ntmp++;
                if (ntmp >= nRowQ) break;
            }
        }
...
char const* InfoHandler::GetCutString(char const* pstr, int nOutWidth, float fScale) const
{
    ...
    for (ps = pstr; ps && *ps; ps++) {
        ...
        if (*ps == 32) {
            ...
            param[n]    = 0;
            int const j = m_rs->StringWidth(param, 0, fScale);
            if (j < nOutWidth) {
                oldps = ps;
                continue;
            }
    ...
    param[nt]    = 0;
    int const jt = m_rs->StringWidth(param, 0, fScale);
    if (jt < nOutWidth) return ps;
    if (oldps) return oldps;

    return ps;
}

void InfoHandler::StringToBufer(char* outStr, int sizeBuf, char const* inStr, int copySize) const
{
