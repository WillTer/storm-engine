...
char* XI_ParseStr(char* inStr, char* buf, size_t bufSize, char devChar = ',')
{
    ...
    for (curStr = inStr; *curStr != 0; curStr++) {
        ...
        if (curSize < static_cast<int>(bufSize) - 1) buf[curSize++] = *curStr;
    }
    buf[curSize] = 0;
...
bool XINTERFACE::Init(std::shared_ptr<entt::registry> const& registry)
{
...
uint64_t XINTERFACE::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_INTERFACE_SETHELPSTRING: {
        std::string const& param = message.String();
        if (m_pContHelp != nullptr) static_cast<CXI_CONTEXTHELP*>(m_pContHelp)->SetTempHelp(param.c_str());
    } break;
    case MSG_INTERFACE_SET_CURRENT_NODE: {
        auto* pNewCurNode = (CINODE*)message.Pointer();
        if (pNewCurNode != nullptr) SetCurNode(pNewCurNode);
    } break;
    ...
    case MSG_INTERFACE_SET_NODE_USING: {
        ...
        if (pTmpNod != nullptr) pTmpNod->SetUsing(nUsingCode != 0);
    } break;

    case MSG_INTERFACE_SCROLL_CHANGE: {
        std::string const& sNodeName = message.String();
        ...
        if (pScrollNode != nullptr) {
            if (pScrollNode->m_nNodeType == NODETYPE_SCROLLIMAGE) static_cast<CXI_SCROLLIMAGE*>(pScrollNode)->ChangeScroll(nItemNum);
            if (pScrollNode->m_nNodeType == NODETYPE_VIMGSCROLL) static_cast<CXI_VIMAGESCROLL*>(pScrollNode)->ChangeScroll(nItemNum);
        }
    } break;
    ...
    case MSG_INTERFACE_SET_EVENT: {
        std::string const& param    = message.String();
        std::string const& nodeName = message.String();
        int32_t            nCommand = message.Long();
        //
        auto* pEvent = new EVENT_Entity;
        if (pEvent == nullptr) throw std::runtime_error("allocate memory error");
        pEvent->next = m_pEvents;
        m_pEvents    = pEvent;
        //
        auto len           = param.size() + 1;
        pEvent->sEventName = new char[len];
        if (pEvent->sEventName == nullptr) throw std::runtime_error("allocate memory error");
        memcpy(pEvent->sEventName, param.c_str(), len);
        //
        len               = nodeName.size() + 1;
        pEvent->sNodeName = new char[len];
        if (pEvent->sNodeName == nullptr) throw std::runtime_error("allocate memory error");
    ...
    case MSG_INTERFACE_CREATE_STRING: {
        ...
        for (l = 0; l < m_nStringQuantity; l++) {
            if (m_stringes[l].sStringName != nullptr && storm::iEquals(m_stringes[l].sStringName, param)) break;
        }
    ...
    case MSG_INTERFACE_DEL_SCROLLIMAGE: {
        ...
        CINODE* pNod = m_pNodes->FindNode(param.c_str());
        if (pNod != nullptr) {
            if (pNod->m_nNodeType == NODETYPE_SCROLLIMAGE) static_cast<CXI_SCROLLIMAGE*>(pNod)->DeleteImage(imgNum);
            if (pNod->m_nNodeType == NODETYPE_VIMGSCROLL) static_cast<CXI_VIMAGESCROLL*>(pNod)->DeleteImage(imgNum);
        }
    } break;
    ...
    case MSG_INTERFACE_REFRESH_SCROLL:  // "ls"
    {
        ...
        CINODE* pNod = m_pNodes->FindNode(param.c_str());
        if (pNod != nullptr) {
            if (pNod->m_nNodeType == NODETYPE_SCROLLIMAGE) static_cast<CXI_SCROLLIMAGE*>(pNod)->RefreshScroll();
            if (pNod->m_nNodeType == NODETYPE_VIMGSCROLL) static_cast<CXI_VIMAGESCROLL*>(pNod)->RefreshScroll();
        }
    } break;
    ...
    case MSG_INTERFACE_SET_TITLE_STRINGS: {
        ...
        if (m_pNodes == nullptr) break;
        CINODE* pNod = m_pNodes->FindNode(param.c_str());
        if (pNod != nullptr && pNod->m_nNodeType == NODETYPE_QTITLE) static_cast<CXI_QUESTTITLE*>(pNod)->SetNewTopQuest(pA, tn);
    } break;
    case MSG_INTERFACE_CHECK_QRECORD: {
        ATTRIBUTES* pA = message.AttributePointer();
        if (pA == nullptr) break;
        std::string const& param = message.String();
        char const*        pText = pA->GetAttribute("Text");
        if (pText == nullptr) break;
        char        subText[256];
        char const* pCur = pText;
        while (true) {
            subText[0] = 0;
            sscanf(pCur, "%[^,]", subText);
            int subSize = strlen(subText);
            if (storm::iEquals(subText, param)) return 1;
            pCur += subSize;
            if (*pCur == ',') pCur++;
    ...
    case MSG_INTERFACE_INIT_QTEXT_SHOW: {
        ...
        if (m_pNodes == nullptr) break;
        CINODE* pNod = m_pNodes->FindNode(param.c_str());
        if (pNod != nullptr && pNod->m_nNodeType == NODETYPE_QTEXTS) static_cast<CXI_QUESTTEXTS*>(pNod)->StartQuestShow(pA, qn);
    } break;
    case MSG_INTERFACE_SET_SCROLLER: {
        ...
        if (m_pNodes == nullptr) break;
        CINODE* pNod = m_pNodes->FindNode(param.c_str());
        if (pNod != nullptr && pNod->m_nNodeType == NODETYPE_SCROLLER) static_cast<CXI_SCROLLER*>(pNod)->SetRollerPos(spos);
    } break;
    ...
    case MSG_INTERFACE_NEW_SAVE_FILE_NAME: {
        std::string const& param = message.String();
        if (NewSaveFileName(param.c_str())) return 0;
        return 1;
    } break;
    ...
    case MSG_INTERFACE_CHECK_AGAIN: Assert(0); break;

    case MSG_INTERFACE_IS_READY_LOAD: return LoadIsExist(); break;

    case MSG_INTERFACE_SET_TOOLTIP: {
        char const* pcHeader         = message.StringPointer();
        char const* pcText1          = message.StringPointer();
        int32_t     dwTextColor1     = message.Long();
        char const* pcText2          = message.StringPointer();
        int32_t     dwTextColor2     = message.Long();
        char const* pcText3          = message.StringPointer();
        int32_t     dwTextColor3     = message.Long();
        char const* pcText4          = message.StringPointer();
        int32_t     dwTextColor4     = message.Long();
        char const* pcPicTextureName = message.StringPointer();
        char const* pcPicGroupName   = message.StringPointer();
        char const* pcPicImageName   = message.StringPointer();
    ...
    if (ini->GetSectionName(platform, sizeof(platform) - 1)) {
        ...
        do {
            if (std::string(platform).starts_with("PC_SCREEN_")) {
                ...
                // +- 3%
                if (iniRatio * 0.97 <= windowRatio && windowRatio <= iniRatio * 1.03) sectionFound = true;
            }
...
void XINTERFACE::LoadDialog(char const* sFileName)
{
    ...
    if (ini->ReadString(section, findName, skey, sizeof(skey) - 1, ""))
        while (true) {
            ...
            if (param[0]) SFLB_CreateNode(ownerIni.get(), ini.get(), param, nodeName, priority);
    ...
    if (m_pContHelp != nullptr) {
        ...
        int32_t const nListSize = static_cast<CXI_CONTEXTHELP*>(m_pContHelp)->m_helpQuantity;
    ...
    if (ini->ReadString(section, "DefaultHelp", param, sizeof(param) - 1, "")) {
        ...
        if (m_strDefHelpTextureFile) memcpy(m_strDefHelpTextureFile, param, len);
    }
    ...
    if (ini->ReadString(section, "DefaultHelpUV", param, sizeof(param) - 1, "")) {
        CINODE::GetDataStr(
            param,
            "ffff",
            &m_frectDefHelpTextureUV.left,
            &m_frectDefHelpTextureUV.top,
...
void XINTERFACE::SFLB_CreateNode(INIFILE* pOwnerIni, INIFILE* pUserIni, char const* sNodeType, char const* sNodeName, int32_t priority)
{
    ...
    char param[1024];
    if (!pOwnerIni->ReadString(sNodeType, "class", param, sizeof(param) - 1, "")) sprintf_s(param, "%s", sNodeType);
    ...
    if (pNewNod) {
        ...
        auto const len      = strlen(sNodeName) + 1;
        ...
        if (usedini && usedini->ReadString(pNewNod->m_nodeName, "command", param, sizeof(param) - 1, "")) do {
                ...
                sscanf(param, "%[^,]", stmp);
                // search command
                int const nComNum = FindCommand(stmp);
                ...
                while (nFirstChar < strlen(param)) {
                    ...
                    if (!strncmp(stmp, "com:", 4)) {
                        ...
                        if (pHead == nullptr) throw std::runtime_error("allocate memory error");
...
void XINTERFACE::DeleteNode(char const* pcNodeName)
{
    ...
    for (pNod = m_pNodes; pNod; pNod = pNod->m_next) {
        if (pNod->m_nodeName && storm::iEquals(pNod->m_nodeName, pcNodeName)) break;
        pPrevNod = pNod;
    }
    // found?
    if (pNod) {
...
void XINTERFACE::SetTooltip(
    ...
    char const* pcText2,
    uint32_t    dwTextColor2,
    char const* pcText3,
    uint32_t    dwTextColor3,
    char const* pcText4,
    uint32_t    dwTextColor4,
    char const* pcPicTextureName,
    char const* pcPicGroupName,
    char const* pcPicImageName,
    int32_t     nPicWidth,
    int32_t     nPicHeight)
{
    CINODE* pTmpNod;
    pTmpNod               = FindNode("tooltip_frame", nullptr);
    CXI_BORDER* pNodFrame = pTmpNod ? pTmpNod->m_nNodeType == NODETYPE_BORDER ? static_cast<CXI_BORDER*>(pTmpNod) : nullptr : nullptr;
    pTmpNod               = FindNode("tooltip_titlerect", nullptr);
    CXI_RECTANGLE* pNodTitleRect =
        pTmpNod ? pTmpNod->m_nNodeType == NODETYPE_RECTANGLE ? static_cast<CXI_RECTANGLE*>(pTmpNod) : nullptr : nullptr;
    pTmpNod                = FindNode("tooltip_picture", nullptr);
    CXI_PICTURE* pNodPic   = pTmpNod ? pTmpNod->m_nNodeType == NODETYPE_PICTURE ? static_cast<CXI_PICTURE*>(pTmpNod) : nullptr : nullptr;
    ...
    CXI_STRCOLLECTION* pNodTitle =
        pTmpNod ? pTmpNod->m_nNodeType == NODETYPE_STRINGCOLLECTION ? static_cast<CXI_STRCOLLECTION*>(pTmpNod) : nullptr : nullptr;
    ...
    if (pcPicTextureName && pcPicTextureName[0]) pNodPic->SetNewPicture(false, (char*)pcPicTextureName);
...
void XINTERFACE::DoControl()
{
    ...
    if (m_bMouseClick || m_bDblMouseClick) {
        ...
        if (oldKeyState.dwKeyCode != curKS.dwKeyCode) {
            ...
            if (curKS.enterButton) wActCode = ACTION_ACTIVATE;
...
void XINTERFACE::MouseMove()
{
    ...
    if (csv.lValue != 0 || csh.lValue != 0) {
        ...
        while (pNod != nullptr) {
            if (pNod->m_bUse) {
                if (!m_pCurToolTipNode && pNod->CheckByToolTip(fXMousePos + m_lXMouse, fYMousePos + m_lYMouse)) m_pCurToolTipNode = pNod;
                if (!pNod->m_bLockedNode) {
                    if (pNod->m_bSelected) pNod->MouseThis(fXMousePos + m_lXMouse, fYMousePos + m_lYMouse);
        ...
        if (m_pCurToolTipNode) m_pCurToolTipNode->CheckByToolTip(fXMousePos + m_lXMouse, fYMousePos + m_lYMouse);
    }
    ...
    if (fXMousePos < m_fpMouseOutZoneOffset.x) fOutX += fXMousePos - m_fpMouseOutZoneOffset.x;
    if (fXMousePos > dwScreenWidth - m_fpMouseOutZoneOffset.x) fOutX += fXMousePos - dwScreenWidth + m_fpMouseOutZoneOffset.x;

    if (fYMousePos < m_fpMouseOutZoneOffset.y) fOutY += fYMousePos - m_fpMouseOutZoneOffset.y;
    if (fYMousePos > dwScreenHeight - m_fpMouseOutZoneOffset.y) fOutY += fYMousePos - dwScreenHeight + m_fpMouseOutZoneOffset.y;
...
void XINTERFACE::MouseDeClick()
{
    ...
    CINODE* pNewNod     = nullptr;
    if (m_idButton == MOUSE_LBUTTON) pNewNod = m_pMouseNode->DoAction(ACTION_MOUSECLICK, bBreakPress, true);
    ...
    while (searchNod != nullptr) {
        if (searchNod->m_bUse && !searchNod->m_bLockedNode) {
            if (searchNod->m_bSelected && searchNod->m_bClickable && searchNod->CheckCommandUsed(ACTION_MOUSECLICK)) {
                if (searchNod->IsClick(m_idButton, xPos, yPos)) findNod = searchNod;
            }
...
void XINTERFACE::ShowPrevTexture()
{
    ...
    if (dwBlendColor > 0xFFL) dwBlendColor = 0xFFL;
    uint32_t const dwColor = 0xFFFFFF | (dwBlendColor << 24L);

    float const fWaveAmplitude = m_fWaveAmplitude * sinf(3.14f * m_nBlendColor / m_nBlendStepMax);
...
bool XINTERFACE::SetCurNode(CINODE* pNod)
{
    ...
    if (m_pContHelp != nullptr) static_cast<CXI_CONTEXTHELP*>(m_pContHelp)->ChangeNode(pNod);
...
uint32_t XINTERFACE::AttributeChanged(ATTRIBUTES* patr)
{
    if (patr != nullptr && patr->GetParent() != nullptr && patr->GetParent()->GetParent() != nullptr) {
        ...
        if (sParentName == nullptr || !storm::iEquals(sParentName, "pictures")) return 0;
        char const* sImageName = patr->GetParent()->GetThisName();
        if (sImageName == nullptr) return 0;
        // find this picture
        IMAGE_Entity* pImList = m_imgLists;
        while (pImList != nullptr) {
            if (pImList->sImageName != nullptr && storm::iEquals(pImList->sImageName, sImageName)) break;
...
bool XINTERFACE::SFLB_GetSaveFileData(std::filesystem::path const& saveName, int32_t bufSize, char* buf)
{
    ...
    if (strSize > 0) memcpy(buf, stringData, strSize);
...
char* XINTERFACE::SaveFileFind(int32_t saveNum, char* buffer, size_t bufSize, int32_t& fileSize)
{
    ...
    if (pSFD->save_file_name) {
        ...
        if (q >= static_cast<int>(bufSize)) q = bufSize - 1;
...
void XINTERFACE::ReleaseDinamicPic(char const* sPicName)
{
    ...
    for (findImg = m_imgLists; findImg != nullptr; findImg = findImg->next) {
        if (findImg->sImageName != nullptr && storm::iEquals(findImg->sImageName, sPicName)) break;
        prevImg = findImg;
    }
...
{
    size_t prevLen = 0;
    if (prevStr != nullptr) prevLen = strlen(prevStr) + std::size(".") - 1;

    int const q = pAttr->GetAttributesNum();
    for (int k = 0; k < q; k++) {
        ATTRIBUTES* pA = pAttr->GetAttributeClass(k);
        if (pA == nullptr) continue;

        char const* attrVal  = pA->GetThisAttr();
        char const* attrName = pA->GetThisName();
...
bool CONTROLS_CONTAINER::Init(std::shared_ptr<entt::registry> const& registry)
{
    Entity::Init(registry);
    ...
    while (pCont) {
        ...
        while (pDescr) {
            if (pDescr->controlName) {
                ...
                case CST_ACTIVE: cs.state = CST_ACTIVE; break;
                }
                ...
                int32_t const lVal = insideCS.lValue;
...
bool CONTROLS_CONTAINER::CreateConteinerList(ATTRIBUTES* pA)
{
    ...
    int const q = pA->GetAttributesNum();
    for (int i = 0; i < q; i++) {
        ATTRIBUTES* pAttr = pA->GetAttributeClass(i);
        if (!pAttr) continue;
        char const* containerName = pAttr->GetThisName();
        if (!containerName) continue;
        AddContainer(containerName);
        SetContainerLimitVal(containerName, static_cast<float>(atof(pAttr->GetThisAttr())));

        int const cntSize = pAttr->GetAttributesNum();
        for (int n = 0; n < cntSize; n++) {
            AddControlsToContainer(containerName, pAttr->GetAttributeName(n), static_cast<float>(atof(pAttr->GetAttribute(n))));
        }
    }
...
void CONTROLS_CONTAINER::AddContainer(char const* container)
{
    ...
    if (!pContainers) throw std::runtime_error("allocate memory error");
    pContainers->fMaxVal   = 1.f;
    pContainers->pControls = nullptr;
    pContainers->next      = pCont;

    auto const len          = strlen(container) + 1;
    pContainers->resultName = new char[len];
    if (!pContainers->resultName) throw std::runtime_error("allocate memory error");
...
void CONTROLS_CONTAINER::AddControlsToContainer(char const* container, char const* controlName, float fValLimit)
{
    ...
    if (!pCont->pControls) throw std::runtime_error("allocate memory error");

    pCont->pControls->fValLimit = fValLimit;
    pCont->pControls->next      = pCtrl;

    auto const len                = strlen(controlName) + 1;
    pCont->pControls->controlName = new char[len];
    if (!pCont->pControls->controlName) throw std::runtime_error("allocate memory error");
...
CONTROLS_CONTAINER::CONTEINER_DESCR* CONTROLS_CONTAINER::FindContainer(char const* sContainer)
{
    ...
    CONTEINER_DESCR* pCont = pContainers;
    while (pCont)
        if (pCont->resultName != nullptr && storm::iEquals(pCont->resultName, sContainer)) return pCont;
    return nullptr;
}
...
CONTROLS_CONTAINER::CONTEINER_DESCR::CONTROL_DESCR* CONTROLS_CONTAINER::CONTEINER_DESCR::FindControl(char const* cntrlName)
{
    ...
    while (pCtrl) {
        if (pCtrl->controlName && storm::iEquals(pCtrl->controlName, cntrlName)) return pCtrl;
        pCtrl = pCtrl->next;
    }
