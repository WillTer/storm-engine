            ...
            for (auto i = 0; i < m_nStringQuantity; i++)
                if (m_stringes[i].bUsed)
                {
                    pRenderService->ExtPrint(
                        m_stringes[i].fontNum, m_stringes[i].dwColor, 0, m_stringes[i].eAlignment, true,
                        m_stringes[i].fScale, dwScreenWidth, dwScreenHeight, m_stringes[i].x, m_stringes[i].y, "%s",
                        static_cast<const char *>(tmpAttr->GetAttribute(m_stringes[i].sStringName)));
                }
    }
...
void XINTERFACE::MouseClick(bool bFirstClick)
{
    if (!bFirstClick && m_nPressDelay > 0)
        return;
    CINODE *clickNod = GetClickNode(m_pNodes, static_cast<int32_t>(fXMousePos) + m_lXMouse,
                                    static_cast<int32_t>(fYMousePos) + m_lYMouse);
    if (!clickNod)
    {
        if (bFirstClick)
            m_pMouseNode = nullptr;
...
int32_t XINTERFACE::PrintIntoWindow(int32_t wl, int32_t wr, int32_t idFont, uint32_t dwFCol, uint32_t dwBCol,
                                    int32_t align, bool shadow, float scale, int32_t sxs, int32_t sys, int32_t left,
                                    int32_t top, const char *str, int nWidthForScaleCorrecting, int nSplit)
{
    if (!str)
        return 0;
