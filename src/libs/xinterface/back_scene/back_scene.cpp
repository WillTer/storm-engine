...
void InterfaceBackScene::LightParam::UpdateParams(float fTime)
{
    ...
    while (fColorTimer > fColorPeriod + fAddPeriod) {
        if (fColorTimer > fAddPeriodMax * 10.0f) { fColorTimer = fAddPeriodMax * 10.0f; }
    ...
    auto const fPer = fColorPeriod + fAddPeriod;
...
void InterfaceBackScene::MenuDescr::Set(
    CMatrix*    pMtx,
...
InterfaceBackScene::~InterfaceBackScene()
{
    ...
    for (auto const& light: m_aLights)
        delete light;
    for (auto const& model: m_apAniModel)
        delete model;
...
void InterfaceBackScene::Execute(uint32_t Delta_Time)
{
    ...
    if (cs.state == CST_ACTIVATED) { ChoosePrevMenu(); }
    core->Controls->GetControlState("IDown", cs);
    if (cs.state == CST_ACTIVATED) { ChooseNextMenu(); }
    core->Controls->GetControlState("ILClick", cs);
    if (cs.state == CST_ACTIVATED) { ExecuteMenu(CheckMousePos(m_pntOldMouse.x, m_pntOldMouse.y)); }
    core->Controls->GetControlState("IAction", cs);
    if (cs.state == CST_ACTIVATED) { ExecuteMenu(m_nSelectMenuIndex); }

    if (core->Controls->GetDebugAsyncKeyState(VK_CONTROL) < 0) {
        CMatrix mtx;
        mtx.BuildMatrix(m_vCamAng);
        auto const vz = mtx * CVECTOR(0.f, 0.f, 1.f);
        auto const vx = mtx * CVECTOR(1.f, 0.f, 0.f);
        ...
        auto const fSideSpeed = 0.5f * fForwardSpeed;
        ...
        auto const fRotateSpeed = 0.001f * Delta_Time;
...
{
    auto const nMsgCode = message.Long();
    switch (nMsgCode) {
    case 0: {
        // load model
    ...
    {
        auto const  nStartIdx = message.Long();
    ...
    {
        auto const nControlCode = message.Long();
        if (nControlCode & 1)
            ChooseNextMenu();
        else
            ChoosePrevMenu();
    ...
    {
        auto const num  = message.Long();  // menu number
        auto const flag = message.Long();  // selectable state
...
void InterfaceBackScene::LoadModel(char const* pcModelName)
{
    // delete all
    if (m_pModel) {
        core->EraseEntity(m_eiModel);
...
bool InterfaceBackScene::FindLocator(char const* pcLocName, CMatrix* pMtx, CVECTOR* pPos, float* pYAng) const
{
...
void InterfaceBackScene::SetLocatorPosition(MODEL* pModel, char const* pcLocName, CVECTOR& pos, NODE*& pNodPtr)
{
    pos = 0.f;
    if (pModel && pcLocName) {
        for (int32_t n = 0; n < 100; n++) {
...
void InterfaceBackScene::CreateMenuList(int32_t nStartIndex, ATTRIBUTES* pAMenu)
{
    ...
    ATTRIBUTES*   pA;
    CMatrix       mtx;
    int32_t const q = pAMenu->GetAttributesNum();
    for (int32_t n = 0; n < q; n++) {
        pA = pAMenu->GetAttributeClass(n);
        if (!pA) continue;
        if (!FindLocator(pA->GetAttribute("locname"), &mtx, nullptr, nullptr)) {
            core->Trace("Warning! Interface Back scene: Can`t find locator %s", static_cast<char const*>(pA->GetAttribute("locname")));
        }
        ...
        pMD->Set(
            &mtx,
            pA->GetAttribute("sel"),
            pA->GetAttribute("norm"),
            pA->GetAttribute("event"),
            pA->GetAttribute("path"),
            pA->GetAttribute("technique"));
...
void InterfaceBackScene::ChooseNextMenu()
{
    ...
    for (n = m_nSelectMenuIndex + 1; n < m_aMenuDescr.size(); n++)
        if (m_aMenuDescr[n]->bSelectable) break;
...
int32_t InterfaceBackScene::CheckMousePos(float fX, float fY)
{
    ...
    for (int32_t n = 0; n < m_aMenuDescr.size(); n++)
        if (m_aMenuDescr[n]->bSelectable && m_aMenuDescr[n]->pActive && m_aMenuDescr[n]->pActive->Trace(vStart, vEnd) <= 1.f) return n;
    return -1;
}
...
void InterfaceBackScene::InitAniModel(ATTRIBUTES* pAParam)
{
    ...
    char const* pcMdlName       = pAParam->GetAttribute("model");
    char const* pcAniName       = pAParam->GetAttribute("animation");
    char const* pcAniActionName = pAParam->GetAttribute("aniaction");
    if (!pcMdlName) {
...
void InterfaceBackScene::InitStaticModel(ATTRIBUTES* pAParam)
{
    ...
    char const* pcMdlName       = pAParam->GetAttribute("model");
    char const* pcTechniqueName = pAParam->GetAttribute("technique");
    if (!pcMdlName) {
    ...
    if (pObj->pModel) {
        pObj->pModel->mtx = mtx;
        if (pcTechniqueName) { pObj->pModel->GetNode(0)->SetTechnique(pcTechniqueName); }
    }
...
void InterfaceBackScene::ProcessedFlys(float dltTime)
{
    ...
    float const   day = dltTime * 1.4f;
    float const   da  = dltTime * 5.6f;
    // calculate
...
void InterfaceBackScene::DrawParticles(void* prts, int32_t num, int32_t size, int32_t texture, char const* tech, bool isEx, int32_t numU)
{
    ...
    for (int32_t i = 0; i < num; i++) {
        ...
        float const size  = parts->size * 0.5f;
        float const sn    = sinf(parts->angle);
        float const cs    = cosf(parts->angle);
