...
void InterfaceBackScene::CreateMenuList(int32_t nStartIndex, ATTRIBUTES *pAMenu)
{
    ...
    for (int32_t n = 0; n < q; n++)
    {
        ...
        {
            core.Trace("Warning! Interface Back scene: Can`t find locator %s",
                       static_cast<const char *>(pA->GetAttribute("locname")));
        }
...
void InterfaceBackScene::DrawParticles(void *prts, int32_t num, int32_t size, int32_t texture, const char *tech,
                                       bool isEx, int32_t numU)
{
    CMatrix camMtx;
    m_pRS->GetTransform(D3DTS_VIEW, camMtx);
