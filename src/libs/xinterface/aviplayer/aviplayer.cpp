...
CAviPlayer::CAviPlayer() : dstPnt(), lockRect(), v {}
{
    m_bContinue  = true;
    m_bShowVideo = true;
...
void CAviPlayer::Execute(uint32_t delta_time)
{
    if (m_bContinue == false) {
        ...
        //         if (pAMStream != nullptr) pAMStream->SetState(STREAMSTATE_STOP);
        // #endif
        CleanupInterfaces();
...
void CAviPlayer::Realize(uint32_t delta_time)
{
    ...
    //     auto const copySize = ddsd.lPitch < d3dlkRect.Pitch ? ddsd.lPitch : d3dlkRect.Pitch;
    ...
    //     CleanupInterfaces();
    //     if (!GetInterfaces() || !PlayMedia(filename.c_str())) { m_bContinue = false; }
    // } else {
    //     m_bContinue = false;
    // }
#else
...
bool CAviPlayer::PlayMedia(char const* fileName)
{
#ifdef _WIN32  // FIX_LINUX ddraw.h and amstream.h
    // auto          hr = S_OK;
    // DDSURFACEDESC ddsd;
    ...
    // pd3dsurf->Release();
    //
    // hr = rs->CreateTexture(AVI_GetTextureSize(srcWidth), AVI_GetTextureSize(srcHeight), 1, 0, d3d9surf_desc.Format, D3DPOOL_MANAGED,
    // &pTex); if (FAILED(hr)) {
...
bool CAviPlayer::GetInterfaces()
{
    ...
    // HRESULT hr = S_OK;
    // // Initialize COM
    // if (FAILED(hr = CoInitialize(NULL))) return false;
    ...
    //     return false;
    // }
    // hr = CoCreateInstance(CLSID_AMMultiMediaStream, nullptr, CLSCTX_INPROC_SERVER, IID_IAMMultiMediaStream, (void**)&pAMStream);
    // if (FAILED(hr)) {
