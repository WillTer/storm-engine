...
public:
    ...
    void    DeleteNode(char const* pcNodeName);
    CINODE* NewNode(char const* pcNodType);
    ...
    int32_t PrintIntoWindow(
        ...
        int         nSplit                   = 0) override;

    CINODE* FindNode(char const* sNodeName, CINODE* findRoot) override
    {
        if (!findRoot) findRoot = m_pNodes;
        if (findRoot) return findRoot->FindNode(sNodeName);
        return nullptr;
    }

    void                ShowWindow(char const* pcWindowName, bool bShow) override;
    void                DisableWindow(char const* pcWindowName, bool bDisable) override;
    void                AddNodeToWindow(char const* pcNodeName, char const* pcWindowName) override;
    virtual CXI_WINDOW* FindWindow(char const* pcWindowName);
    virtual bool        IsWindowActive(char const* pcWindowName);

    void RegistryExitKey(char const* pcKeyName) override;
...
protected:
    ...
    void LoadIni();
    void LoadDialog(char const* sFileName);
    // control function
    ...
    // Options functions
    void SaveOptionsFile(char const* fileName, ATTRIBUTES* pAttr);
    void LoadOptionsFile(std::string_view filename, ATTRIBUTES* pAttr);
    //
    int LoadIsExist();
    //
    void PrecreateDirForFile(char const* pcFullFileName);
...
class CONTROLS_CONTAINER: public Entity
{
    struct CONTEINER_DESCR {
        ...
        CONTROL_DESCR* FindControl(char const* cntrlName);
    }* pContainers;
...
protected:
    ...
    void AddContainer(char const* container);
    void SetContainerLimitVal(char const* container, float fLimitVal);
    void AddControlsToContainer(char const* container, char const* controlName, float fValLimit);

    CONTEINER_DESCR* FindContainer(char const* sContainer);
};
