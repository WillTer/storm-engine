...
bool HELPCHOOSER::RunChooser(char const* ChooserGroup)
{
    ...
    m_nRectQ = 0;
    if (ini->ReadString(ChooserGroup, "rect", param, sizeof(param) - 1, "")) do {
            m_nRectQ++;
    ...
    for (i = 0; i < m_nRectQ; i++) {
        ...
        if (j > 0) {
            ...
            if (m_psRectName[i] == nullptr) { throw std::runtime_error("Allocate memory error"); }
...
int32_t HELPCHOOSER::GetRectangleRight() const
{
    ...
    auto       top    = m_pRectList[m_nCurRect].top;
    auto const right  = m_pRectList[m_nCurRect].right;
    auto       bottom = m_pRectList[m_nCurRect].bottom;
...
int32_t HELPCHOOSER::GetRectangleUp() const
{
    if (m_nCurRect < 0 || m_nCurRect >= m_nRectQ || m_pRectList == nullptr) return 0;
    auto       left   = m_pRectList[m_nCurRect].left;
    auto const top    = m_pRectList[m_nCurRect].top;
    auto       right  = m_pRectList[m_nCurRect].right;
    auto       bottom = m_pRectList[m_nCurRect].bottom;
...
int32_t HELPCHOOSER::GetRectangleDown() const
{
    ...
    auto       top    = m_pRectList[m_nCurRect].top;
    auto       right  = m_pRectList[m_nCurRect].right;
    auto const bottom = m_pRectList[m_nCurRect].bottom;
...
int32_t HELPCHOOSER::GetRectangleFromPos(float x, float y) const
{
    ...
    for (int32_t i = 0; i < m_nRectQ; i++) {
        if (m_pRectList[i].left <= x && m_pRectList[i].right >= x && m_pRectList[i].top <= y && m_pRectList[i].bottom >= y) return i;
    }
    return -1;
}
