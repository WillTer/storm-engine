...
void CXI_CHANGER::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int      i;
    char     param[255];
    FXYPOINT fPnt;
    ...
    m_nPlaceQuantity = 0;
    if (ini1->ReadString(name1, "place", param, sizeof(param) - 1, "")) do {
            m_nPlaceQuantity++;
    ...
    for (i = 0; i < m_nPlaceQuantity; i++) {
        ...
        if (bRelativeRect) GetRelativeRect(m_pPlace[i]);
...
bool CXI_CHANGER::IsClick(int buttonID, int32_t xPos, int32_t yPos)
{
    if (m_bClickable && m_bUse) {
        for (auto i = 0; i < m_nPlaceQuantity; i++)
            if (xPos > m_pPlace[i].left && xPos < m_pPlace[i].right && yPos > m_pPlace[i].top && yPos < m_pPlace[i].bottom) {
                m_nClickPlaceNum = i;
