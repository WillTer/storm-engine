...
struct ButtonDescribe {
    ButtonDescribe() : bChoose(false), bDisable(false)
    {
        pImg    = nullptr;
        bSetPos = false;
    }
...
protected:
    ...
    void AddButton(char const* pcText, bool bDisable, bool bSelect);
    void ChangeText(int32_t nButtonNum, char const* pcText);
