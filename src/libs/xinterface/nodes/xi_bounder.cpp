...
void CXI_BOUNDER::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int         i;
    char        param[256];
    char        param2[256];
    ...
    if (m_nHorzLineQuantity * fLineWidth < fBoxWidth) m_nHorzLineQuantity++;
    auto fHorzLineWidth = fBoxWidth / m_nHorzLineQuantity;

    m_nVertLineQuantity = static_cast<int>(fBoxHeight / fLineWidth);
    if (m_nVertLineQuantity * fLineWidth < fBoxHeight) m_nVertLineQuantity++;
...
void CXI_BOUNDER::ChangePosition(XYRECT& rNewPos)
{
    ...
    // if (pVert == nullptr) throw std::runtime_error("can not create the index&vertex buffers");

    int32_t i;

    m_rect = rNewPos;

    auto const fBoxWidth      = static_cast<float>(m_rect.right - m_rect.left) - m_fAngleWidth * 2.f;
    auto const fBoxHeight     = static_cast<float>(m_rect.bottom - m_rect.top) - m_fAngleHeight * 2.f;
    auto const fHorzLineWidth = fBoxWidth / m_nHorzLineQuantity;

    auto const fVertLineWidth = fBoxHeight / m_nVertLineQuantity;
