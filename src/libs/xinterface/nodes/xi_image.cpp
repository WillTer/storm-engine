...
void CXI_IMAGE::LoadFromBase(char const* sListName, char const* sPictureName, bool bGetSizeFromSource)
{
    Unload();

    if (sListName) {
        auto const len      = strlen(sListName) + 1;
...
void CXI_IMAGE::LoadAccordingToString(char const* pcImageParam)
{
    ...
    for (auto* pcParam = (char*)pcImageParam; pcParam && pcParam[0];) {
        ...
        auto const nTokenCode = CXI_UTILS::StringGetTokenCode(tokenID);
        switch (nTokenCode) {
        ...
        case InterfaceToken_picture_cut_uv:
            m_bThisIsColorRectangle = false;
            m_nLeftTopCutUV.x       = CXI_UTILS::StringGetFloat((char const*&)pcParam);
            m_nLeftTopCutUV.y       = CXI_UTILS::StringGetFloat((char const*&)pcParam);
            UpdateTexture();
            break;
...
void CXI_IMAGE::Draw(int32_t nX, int32_t nY, ImagePointType alignment)
{
    ...
    if (m_pntLeftTop.x != nLeft || m_pntLeftTop.y != nTop) SetPosition(nLeft, nTop);
...
void CXI_IMAGE::SetSize(int32_t nWidth, int32_t nHeight)
{
    ...
    UpdatePosition();
    if (m_nPictureNum == -1 && m_pcPictureListName) UpdateTexture();
}
...
{
    auto const bDoTextureUpdate = (m_pntSize.x != pos.right - pos.left) || (m_pntSize.y != pos.bottom - pos.top);
    if (m_pntLeftTop.x != pos.left || m_pntLeftTop.y != pos.top || bDoTextureUpdate) {
        ...
        UpdatePosition();
        if (bDoTextureUpdate && m_nPictureNum == -1 && m_pcPictureListName) UpdateTexture();
    }
}
...
void CXI_IMAGE::SetDifferentPosition(int32_t nLeft, int32_t nTop, int32_t nWidth, int32_t nHeight, ImagePointType ptype)
{
    ...
    case IPType_Center:
        ...
        nY -= nHeight / 2;
        break;
    case IPType_RightTop: nX -= nWidth; break;
    }

    bool const bDoTextureUpdate = (nWidth != m_pntSize.x || nHeight != m_pntSize.y);
...
{
    if (nX < m_pntLeftTop.x || nY < m_pntLeftTop.y || nX > m_pntLeftTop.x + m_pntSize.x || nY > m_pntLeftTop.y + m_pntSize.y) return false;
    return true;
}
