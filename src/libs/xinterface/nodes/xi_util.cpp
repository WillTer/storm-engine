...
char const* CXI_UTILS::StringGetTokenID(char*& pcString, char* pcBuffer, int32_t nBufferSize)
{
    ...
    for (auto cCur = pcString[0]; cCur && n < q; pcString++, cCur = pcString[0]) {
        ...
        if (cCur < 0x20) continue;  // skip unused characters
        pcBuffer[n++] = cCur;
    }
...
char const* CXI_UTILS::StringGetTokenString(char*& pcString, char* pcBuffer, int32_t nBufferSize)
{
    if (!pcString || !pcBuffer || nBufferSize <= 0) return nullptr;
    pcBuffer[0] = 0;
...
int32_t CXI_UTILS::StringGetTokenCode(char const* pcTokenID)
{
    if (storm::iEquals(pcTokenID, "color")) return InterfaceToken_color;
    if (storm::iEquals(pcTokenID, "file")) return InterfaceToken_file;
    if (storm::iEquals(pcTokenID, "piclist")) return InterfaceToken_picture_list;
    if (storm::iEquals(pcTokenID, "picname")) return InterfaceToken_picture_name;
    if (storm::iEquals(pcTokenID, "piccutuv")) return InterfaceToken_picture_cut_uv;
    if (storm::iEquals(pcTokenID, "size")) return InterfaceToken_size;
    if (storm::iEquals(pcTokenID, "rectUV")) return InterfaceToken_rectUV;
    if (storm::iEquals(pcTokenID, "pos")) return InterfaceToken_pos;
    if (storm::iEquals(pcTokenID, "text")) return InterfaceToken_text;
    if (storm::iEquals(pcTokenID, "width")) return InterfaceToken_width;
    if (storm::iEquals(pcTokenID, "class")) return InterfaceToken_class;
...
void CXI_UTILS::StringTwoLong(char const* pcString, int32_t& nLong1, int32_t& nLong2)
{
    nLong1 = StringGetInt(pcString);
    nLong2 = StringGetInt(pcString);
}

int32_t CXI_UTILS::StringGetInt(char const*& pcString)
{
    ...
    for (; pcString[0]; pcString++) {
        ...
        if (pcString[0] >= '0' && pcString[0] <= '9') { nRetVal = nRetVal * 10 + pcString[0] - '0'; }
    }
    return nRetVal;
}

float CXI_UTILS::StringGetFloat(char const*& pcString)
{
...
void CXI_UTILS::StringFourFloat(char const* pcString, float& f1, float& f2, float& f3, float& f4)
{
...
void CXI_UTILS::StringFillStringArray(char const* pcString, std::vector<std::string>& asStringsArray)
{
    char  tmpstr[256];
...
int32_t
CXI_UTILS::SplitStringByWidth(char const* pcText, int32_t nFontID, float fFontScale, int32_t nWidth, std::vector<std::string>& asOutStr)
{
    ...
    char    param[1024];
    while (true) {
        int32_t const nToken = GetCurrentTokenIntoString(&pcSrcStr[nSrc]);
...
float CXI_UTILS::GetByStrNumFromAttribute_Float(ATTRIBUTES* pA, char const* pStr, int32_t num, float fDefValue)
{
...
void CXI_UTILS::WriteSquareToVertexBuffer(
    XI_ONETEX_VERTEX* pv, uint32_t color, FXYRECT& uv, int32_t left, int32_t top, int32_t right, int32_t bottom)
{
    pv[0].color = color;
