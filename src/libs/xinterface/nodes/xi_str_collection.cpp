...
void CXI_STRCOLLECTION::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int  i;
    char param[256];
    ...
    // get string quantity
    m_nStr = 0;
    if (ini && ini->ReadString(name, "string", param, sizeof(param) - 1, "")) do {
            m_nStr++;
    ...
    if (m_nStr) {
        ...
        for (i = 0; i < m_nStr; i++) {
            ...
            sscanf(
                param,
                "%[^,],font:%[^,],pos:{%d,%d},fc:{%d,%d,%d,%d},bc:{%d,%d,%d,%d},scale:%f,state:{%[^}]}",
                strName,
                fontName,
                &m_pStrDescr[i].scrPos.x,
                &m_pStrDescr[i].scrPos.y,
...
{
    auto const nX = rNewPos.left - m_rect.left;
    auto const nY = rNewPos.top - m_rect.top;
    if (nX != 0 || nY != 0) {
...
void CXI_STRCOLLECTION::SaveParametersToIni()
{
    ...
    char param[2048];
    for (int32_t n = 0; n < m_nStr; n++) {
        if (m_pStrDescr[n].strID) continue;  // dynamically added string
        ...
        if (m_pStrDescr[n].strStr)
            sprintf_s(
                ...
                m_pStrDescr[n].scrPos.x,
                m_pStrDescr[n].scrPos.y,  // pos
                ALPHA(m_pStrDescr[n].foreColor),
                RED(m_pStrDescr[n].foreColor),
                GREEN(m_pStrDescr[n].foreColor),
                BLUE(m_pStrDescr[n].foreColor),
                ALPHA(m_pStrDescr[n].backColor),
                RED(m_pStrDescr[n].backColor),
                GREEN(m_pStrDescr[n].backColor),
                BLUE(m_pStrDescr[n].backColor),
...
uint32_t CXI_STRCOLLECTION::MessageProc(int32_t msgcode, MESSAGE& message)
{
    ...
    {
        // string ID & data
        std::string const& paramID  = message.String();
        std::string const& paramStr = message.String();
    ...
    {
        auto const nDst = message.Long() - 1;
        auto const nSrc = message.Long() - 1;
    ...
    {
        int32_t const  nStr   = message.Long() - 1;
        uint32_t const nColor = message.Long();
...
void CXI_STRCOLLECTION::ChangeString(int32_t num, char const* sValue) const
{
    if (num >= 0 && num < m_nStr) {
...
{
    if (num >= 0 && num < m_nStr) m_pStrDescr[num].foreColor = dwColor;
}

void CXI_STRCOLLECTION::ChangeStringPos(int32_t num, XYPOINT& pntNewPos) const
{
    if (num >= 0 && num < m_nStr) m_pStrDescr[num].scrPos = pntNewPos;
}

CXI_STRCOLLECTION::STRINGDESCR* CXI_STRCOLLECTION::CreateNewDinamicString(char const* strID, char const* strStr)
{
    if (strID == nullptr || strID[0] == 0) return nullptr;
    int i;
    for (i = 0; i < m_nStr; i++) {
        if (m_pStrDescr[i].strID != nullptr && storm::iEquals(m_pStrDescr[i].strID, strID)) break;
    }
    if (i < m_nStr) {
        ...
        if (m_pStrDescr[i].strStr == nullptr) { throw std::runtime_error("allocate memory error"); }
    ...
    m_nStr++;
    m_pStrDescr = new STRINGDESCR[m_nStr];
    if (m_pStrDescr == nullptr) { throw std::runtime_error("allocate memory error"); }
    if (pOld && i) memcpy(m_pStrDescr, pOld, sizeof(STRINGDESCR) * i);
    delete pOld;
    m_pStrDescr[i]          = {};
    m_pStrDescr[i].nFontNum = -1;
    auto const len1         = strlen(strID) + 1;
    auto const len2         = strlen(strStr) + 1;
    m_pStrDescr[i].strID    = new char[len1];
    m_pStrDescr[i].strStr   = new char[len2];
    if (m_pStrDescr[i].strID == nullptr || m_pStrDescr[i].strStr == nullptr) { throw std::runtime_error("allocate memory error"); }
