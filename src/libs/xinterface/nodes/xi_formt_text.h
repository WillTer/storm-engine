...
public:
    ...
    void  SetFormatedText(char const* str);
...
protected:
    bool    GetLineNext(int fontNum, char const*& pInStr, char* buf, int bufSize) const;
    void    GetOneLine(int fontNum, char const* pStr, char* buf, int bufSize) const;
    ...
    int32_t AddFormatedText(char const* str);
    ...
    void    ReplaceString(int32_t nGrpNum, char const* pSrcStr);
    void    InsertStringBefore(STRING_DESCRIBER* pNextDescr, char const* pSrcStr, int32_t nGrpNum, uint32_t dwColor);
