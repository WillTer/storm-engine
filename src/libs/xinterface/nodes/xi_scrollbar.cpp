...
void CXI_SCROLLBAR::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char     param[255];
    FXYPOINT fPnt;
    ...
    if (m_nSideWidth >= 0) m_nBarWidth = -1;
    if (m_nBarWidth < 0 && m_nSideWidth < 0) m_nSideWidth = (m_rect.bottom - m_rect.top);
...
int CXI_SCROLLBAR::CommandExecute(int wActCode)
{
    if (m_bUse) {
        ...
        case ACTION_MOUSECLICK:
            if (m_clickType == CLICK_TYPE_LEFT) m_bRightPress = false;
            if (m_clickType == CLICK_TYPE_RIGHT) m_bRightPress = true;
            ...
            if (m_nPressedDelay > 0) {
                if (m_bRightPress) return ACTION_RIGHTSTEP;
...
bool CXI_SCROLLBAR::IsClick(int buttonID, int32_t xPos, int32_t yPos)
{
    if (m_bClickable && m_bUse)
        if (xPos >= m_rect.left && xPos <= m_rect.right && yPos >= m_rect.top && yPos <= m_rect.bottom) {
            ...
            if (sideWidth < 0) sideWidth = (m_rect.right - m_rect.left - m_nBarWidth) / 2;
...
void CXI_SCROLLBAR::ChangeValue(bool bGrowing, bool bMultiply)
{
    ...
    if (bMultiply) fStep *= m_fSpeedMultiplay;
    if (bGrowing)
        fVal += fStep;
    else
        fVal -= fStep;
    if (fVal < m_fMinValue) fVal = m_fMinValue;
    if (fVal > m_fMaxValue) fVal = m_fMaxValue;
    if (fVal != m_fCurValue) {
        m_fCurValue = fVal;
        WriteDataToAttribute();
    }
