...
{
    if (m_rect.right - m_rect.left != rNewPos.right - rNewPos.left) rNewPos.right += rNewPos.left + m_rect.right - m_rect.left;
    if (m_rect.bottom - m_rect.top != rNewPos.bottom - rNewPos.top) rNewPos.bottom += rNewPos.top + m_rect.bottom - m_rect.top;
    if (rNewPos.top == m_rect.top && rNewPos.left == m_rect.left) return;  // nothing to change - the same position
    auto const nXAdd = rNewPos.left - m_rect.left;
    auto const nYAdd = rNewPos.top - m_rect.top;
    m_rect           = rNewPos;
...
void CXI_WINDOW::SaveParametersToIni()
{
    ...
    for (int32_t n = 0; n < m_aNodeNameList.size(); n++) {
        auto* pNod = ptrOwner->FindNode(m_aNodeNameList[n].c_str(), nullptr);
        if (pNod) pNod->SaveParametersToIni();
    }
}
...
void CXI_WINDOW::SetShow(bool bShow)
{
    ...
    for (int32_t n = 0; n < m_aNodeNameList.size(); n++) {
        ...
        if (pNod) {
            pNod->m_bUse = bShow;
            if (pNod->m_nNodeType == NODETYPE_WINDOW) static_cast<CXI_WINDOW*>(pNod)->SetShow(bShow);
        }
    }
}
...
void CXI_WINDOW::AddNode(char const* pcNodeName)
{
...
void CXI_WINDOW::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char param[1024];
