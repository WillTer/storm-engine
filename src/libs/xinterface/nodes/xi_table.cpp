...
void XI_TableCellDescribe::Draw(float fLeft, float fTop)
{
    ...
    for (n = 0; n < m_aStrings.size(); n++)
    {
        ...
        CXI_UTILS::PrintTextIntoWindow(
            ...
            static_cast<int32_t>(fLeft + m_aStrings[n].offset.x), static_cast<int32_t>(fTop + fY),
            m_aStrings[n].str.c_str(), static_cast<int32_t>(fLeft), static_cast<int32_t>(fTop),
...
void CXI_TABLE::WriteSquare(XI_ONETEX_VERTEX *pV, int32_t nImgID, uint32_t dwCol, int32_t nX, int32_t nY, int32_t nW,
                            int32_t nH) const
{
    if (!pV)
        return;
