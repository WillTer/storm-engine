...
XI_TableLineDescribe::XI_TableLineDescribe(CXI_TABLE* pTable) : m_dwSpecColor(0)
{
    m_pTable        = pTable;
    m_nRowIndex     = -1;
...
void XI_TableLineDescribe::DrawSpecColor(float fTop) const
{
    if (m_bUseSpecColor) {
        ...
        auto const fBottom                                = fTop + static_cast<float>(m_pTable->m_anRowsHeights[m_nRowIndex]);
        auto const fLeft                                  = static_cast<float>(m_pTable->m_rect.left);
        auto const fRight                                 = static_cast<float>(m_pTable->m_rect.right);
        v[0].pos.x                                        = fLeft;
        v[0].pos.y                                        = fTop;
        v[1].pos.x                                        = fLeft;
        v[1].pos.y                                        = fBottom;
...
void XI_TableCellDescribe::Draw(float fLeft, float fTop)
{
    ...
    if (m_aImage.size() > 0) {
        auto const nX = static_cast<int32_t>(fLeft) + m_nLeftLineWidth;
        auto const nY = static_cast<int32_t>(fTop) + m_nTopLineHeight;
        for (n = 0; n < m_aImage.size(); n++)
            if (m_aImage[n].pImage) m_aImage[n].pImage->Draw(nX + m_aImage[n].offset.x, nY + m_aImage[n].offset.y, IPType_LeftTop);
    }
...
void XI_TableCellDescribe::SetData(int32_t nColIndex, ATTRIBUTES* pAttr, bool bHeader)
{
    ...
    if (pA) {
        if (m_aImage.empty()) m_aImage.push_back(ImgDescribe {});
        ...
        while (true) {
            ...
            if (!pA) break;
            if (static_cast<int32_t>(m_aImage.size()) <= nIconQuantity) m_aImage.push_back(ImgDescribe {});
            LoadImageParam(&m_aImage[nIconQuantity], pA);
            nIconQuantity++;
        }
    }
    ...
    auto nWidth =
        m_pTable->m_anColsWidth[nColIndex] - 2 * m_pTable->m_pntSpaceSize.x - m_nLeftLineWidth - static_cast<int32_t>(m_TextOffset.x);
...
void XI_TableCellDescribe::LoadImageParam(ImgDescribe* pImg, ATTRIBUTES* pA) const
{
    ...
    if (nW > 0 && nH > 0) pImg->pImage->SetSize(nW, nH);
...
void CXI_TABLE::Draw(bool bSelected, uint32_t Delta_Time)
{
    ...
    if (m_bBackPresent) { m_BackImg.Draw(); }
...
int CXI_TABLE::CommandExecute(int wActCode)
{
    if (m_bUse) {
        ...
        case ACTION_ACTIVATE:
            if (m_nSelectIndex >= 0) core->Event("TableActivate", "sl", m_nodeName, m_nSelectIndex);
            break;
        ...
        case ACTION_SPEEDRIGHT:
            if (m_bDoColsSelect && m_nSelectIndex != -1 && m_nSelectColIndex < static_cast<int32_t>(m_anColsWidth.size()) - 1) {
                m_nSelectColIndex++;
                SelectCol(m_nSelectColIndex);
            }
            break;
        ...
        {
            auto const n = GetLineByPoint(ptrOwner->GetMousePoint());
            if (n >= 0 && n <= m_nLineQuantity - m_nTopIndex - (m_pHeader ? 0 : 1)) {
                auto const nCol = GetColByX(static_cast<int32_t>(ptrOwner->GetMousePoint().x));
                if (m_bDoColsSelect)
                    SelectRow(n, nCol);
                else
                    SelectRow(n);
...
void CXI_TABLE::ChangePosition(XYRECT& rNewPos)
{
    ...
    if (m_EditData.bAllEditable) {
        ...
        if (n > 0 && nWAdd > 0) m_anColsWidth[n] += nWAdd;
        for (; n >= 0 && nWAdd < 0; n--) {
        ...
        if (n > 0 && nHAdd > 0) m_anRowsHeights[n] += nHAdd;
        for (; n >= 0 && nHAdd < 0; n--) {
...
{
    if (m_nLineQuantity <= 1) return;  // the only one
    int32_t const n = static_cast<int32_t>(fRelativeScrollPos * (m_nLineQuantity - 1));
    if (n != m_nSelectIndex) {
...
void CXI_TABLE::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    SetGlowCursor(false);
    ...
    // select image
    if (ReadIniString(ini1, name1, ini2, name2, "selectimage", param, sizeof(param), "")) { m_SelectImg.LoadAccordingToString(param); }
    ...
    if (m_nRowQuantity > nUsedQ && m_rect.bottom - m_rect.top > nCommonWidth) {
        int32_t const nH = (m_rect.bottom - m_rect.top - nCommonWidth) / (m_nRowQuantity - nUsedQ);
        for (n = 0; n < m_anRowsHeights.size(); n++)
            if (m_anRowsHeights[n] == 0) m_anRowsHeights[n] = nH;
    }
    ...
    if (m_nColQuantity > nUsedQ && m_rect.right - m_rect.left > nCommonWidth) {
        int32_t const nW = (m_rect.right - m_rect.left - m_nBorderWidth - nCommonWidth) / (m_nColQuantity - nUsedQ);
        for (n = 0; n < m_anColsWidth.size(); n++)
            if (m_anColsWidth[n] == 0) m_anColsWidth[n] = nW;
    }
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "scroller", param, sizeof(param), "")) m_sScrollerName = param;
    m_nSelectIndex = 0;
...
void CXI_TABLE::UpdateBorders()
{
    ...
    if (m_nHLineHeight > 0 && m_nRowQuantity > 2)  // horizontal lines
        q += m_nRowQuantity - 2;
    if (m_bHLineIsBreakable) q *= m_nColQuantity;  // the break line is split by the number of columns
...
void CXI_TABLE::UpdateTableCells()
{
    ...
    UpdateLineQuantity();
    if (m_bVariableLineHeight) UpdateBorders();
...
{
    if (pnt.x < m_rect.left || pnt.x > m_rect.right || pnt.y < m_rect.top || pnt.y > m_rect.bottom) return -1;
...
void CXI_TABLE::SelectRow(int32_t nRowNum)
{
    ...
    int32_t const n = m_nTopIndex + nRowNum - (m_pHeader ? 1 : 0);
    SelectLine(n);
}
...
void CXI_TABLE::SelectLine(int32_t nLineNum)
{
    ...
    UpdateSelectImage();
    UpdateScroller();
    if (!m_bFirstFrame) core->Event("TableSelectChange", "sll", m_nodeName, m_nSelectIndex + 1, m_nSelectColIndex + 1);
}
...
{
    int32_t const nRow = m_nSelectIndex - m_nTopIndex + (m_pHeader ? 1 : 0);
    if (m_nSelectIndex < 0 || nRow < 0 || nRow >= m_anRowsHeights.size()) {
        m_SelectImg.DisableDraw(true);
    } else {
        int32_t const nCol = m_nSelectColIndex;
...
{
    if (nRow < 0 || nRow >= m_anRowsHeights.size()) return m_rect.top;  // error situation
    int32_t nTop = m_rect.top;
...
{
    if (nCol < 0 || nCol >= m_anColsWidth.size()) return m_rect.left;  // error situation
...
{
    if (nSelIndex < 0) return;  // error situation
...
void CXI_TABLE::UpdateLineQuantity()
{
    ...
    if (!pARoot->GetAttributeClass("tr1")) return;  // no one
...
void CXI_TABLE::RecalculateLineHeights()
{
    ...
    // header accounting
    if (m_pHeader) nY += m_anRowsHeights[n++];
