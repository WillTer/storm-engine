        ...
        if (m_glows[i].action != GLOW_ACTION_NONE) {
            ...
            if (m_glows[i].action == GLOW_ACTION_COLORUP)
                m_glows[i].rect.dwColor =
                    ColorInterpolate(m_dwMaxColor, m_dwMinColor, static_cast<float>(m_glows[i].curTime) / m_glows[i].allTime);
            if (m_glows[i].action == GLOW_ACTION_BLEND)
                m_glows[i].rect.dwColor =
                    ColorInterpolate(m_dwMaxColor, m_dwMinColor, 1.f - static_cast<float>(m_glows[i].curTime) / m_glows[i].allTime);
            // show this rectangle
            ...
            if ((m_glows[i].curTime -= Delta_Time) < 0) {
                ...
                case GLOW_ACTION_SHOW:
                    ...
                    m_glows[i].curTime = m_glows[i].allTime =
                        static_cast<int32_t>(m_minGlowTime + static_cast<float>(m_maxGlowTime - m_minGlowTime) * rand() / RAND_MAX);
                    break;
                case GLOW_ACTION_BLEND: m_glows[i].action = GLOW_ACTION_NONE; break;
                }
            }
        } else {
...
void CXI_GLOWER::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int     i;
    char    param[255];
    int32_t x, y;
    ...
    // get texture
    m_texID = -1;
    if (ReadIniString(ini1, name1, ini2, name2, "texture", param, sizeof(param), "")) m_texID = m_rs->TextureCreate(param);

    // calculate glow blinks quantity
    m_nQuantity = 0;
    if (ini1->ReadString(name1, "pos", param, sizeof(param) - 1, "")) do {
            m_nQuantity++;
        } while (ini1->ReadStringNext(name1, "pos", param, sizeof(param) - 1));
    if (m_nQuantity > MAX_USED_RECTANGLE) m_nQuantity = MAX_USED_RECTANGLE;
