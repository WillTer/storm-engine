...
int CXI_SCROLLER::CommandExecute(int wActCode)
{
    if (m_bUse) {
        ...
        case ACTION_MOUSECLICK:
            if (m_upButtonPos.left <= m_curMousePos.x && m_upButtonPos.right >= m_curMousePos.x && m_upButtonPos.top <= m_curMousePos.y
                && m_downButtonPos.bottom >= m_curMousePos.y) {
...
void CXI_SCROLLER::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char param[2048];
...
void CXI_SCROLLER::MakeOwnedControl()
{
    for (int32_t n = 0; n < static_cast<int32_t>(m_asOwnedNodes.size()); n++) {
        ...
        case NODETYPE_TABLE: static_cast<CXI_TABLE*>(pNode)->ScrollerChanged(m_fPos); break;
...
float CXI_SCROLLER::GetOwnedStep()
{
    ...
    if (!pNode) return 0.f;
    switch (pNode->m_nNodeType) {
    case NODETYPE_FORMATEDTEXTS: return static_cast<CXI_FORMATEDTEXT*>(pNode)->GetLineStep(); break;
    case NODETYPE_TABLE: return static_cast<CXI_TABLE*>(pNode)->GetLineStep(); break;
...
{
    auto const pntMouse = ptrOwner->GetMousePoint();
    if (pntMouse.y != m_curMousePos.y) {
        m_curMousePos = pntMouse;

        auto fY = pntMouse.y;
        if (fY < m_rollerPlace.top + m_rollerHeight * .5f) fY = m_rollerPlace.top + m_rollerHeight * .5f;
        if (fY > m_rollerPlace.bottom - m_rollerHeight * .5f) fY = m_rollerPlace.bottom - m_rollerHeight * .5f;
