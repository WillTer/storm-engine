...
void CXI_CONTEXTHELP::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int  i;
    char param[256];
    ...
    m_helpQuantity = 0;
    if (ini1->ReadString(name1, "helpstr", param, sizeof(param) - 1, "")) do {
            m_helpQuantity++;
    ...
    if (m_helpQuantity > 0) {
        if ((m_pHelpList = new HELPEntity[m_helpQuantity]) == nullptr) throw std::runtime_error("allocate memory error");
        ...
        for (i = 0; i < m_helpQuantity; i++) {
            ...
            if (nodeName[0] != 0) {
                auto const len = strlen(nodeName) + 1;
                if ((m_pHelpList[i].nodeName = new char[len]) == nullptr) throw std::runtime_error("allocate memory error");
...
void CXI_CONTEXTHELP::SetTempHelp(char const* pStr)
{
...
char* CXI_CONTEXTHELP::GetCurrentHelpString(uint32_t deltaTime)
{
    ...
    if (m_nCurDelayCounter > 0) {
        if (m_sTempString != nullptr) return m_sTempString;
        if (m_idTempString != -1) return pStringService->GetString(m_idTempString);
    }

    if (m_curHelp != nullptr) return pStringService->GetString(m_curHelp->idHelpString);
