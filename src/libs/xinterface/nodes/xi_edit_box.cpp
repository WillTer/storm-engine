...
int CXI_EDITBOX::CommandExecute(int wActCode)
{
    if (m_bUse) {
        ...
        case ACTION_MOUSECLICK: {
            ...
            char param[256];
            param[0]           = 0;
            char const* tmpstr = pA->GetAttribute("strdata");
...
bool CXI_EDITBOX::IsClick(int buttonID, int32_t xPos, int32_t yPos)
{
    ...
    auto const y = static_cast<int>((yPos - m_nTopOffset) / m_fVAdd);
    if (y >= m_nVert) return false;
    auto const x = static_cast<int>((xPos - m_rect.left - m_nLeftOffset) / m_fHAdd);
...
void CXI_EDITBOX::MouseThis(float fX, float fY)
{
    ...
    auto const y = static_cast<int>((fY - m_nTopOffset) / m_fVAdd);
    if (y >= m_nVert) return;
    auto const x = static_cast<int>((fX - m_rect.left - m_nLeftOffset) / m_fHAdd);
    if (x >= m_nHorz) return;
