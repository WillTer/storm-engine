        ...
        for (n = 0; n < m_nSlotsQnt; n++) {
            ...
            while (pScroll != nullptr) {
                ...
                if (pos.top < m_rect.top) {
                    auto const texCorrect = (m_rect.top - pos.top) / (pos.bottom - pos.top);
                    rectTex.top += texCorrect * (rectTex.bottom - rectTex.top);
                    pos.top = static_cast<float>(m_rect.top);
                }
                if (pos.bottom > m_rect.bottom) {
                    auto const texCorrect = (m_rect.bottom - pos.bottom) / (pos.bottom - pos.top);
                        ...
                        if (pScroll->pCenter.y + m_pStrParam[l].m_nStrY > m_rect.top && !pScroll->bCurNotUse) {
                            if (m_Image[pScroll->imageNum].strNum[l] != -1)
                                ptrOwner->PrintIntoWindow(
                                    m_rect.left - m_leftTextLimit,
                                    m_rect.right + m_rightTextLimit,
                                    m_pStrParam[l].m_nFont,
                                    m_pStrParam[l].m_dwForeColor,
                                    m_pStrParam[l].m_dwBackColor,
                                    m_pStrParam[l].m_nAlign,
                                    true,
                                    m_pStrParam[l].m_fScale,
                                    m_screenSize.x,
                                    m_screenSize.y,
                                    static_cast<int32_t>(pScroll->pCenter.x + m_pStrParam[l].m_nStrX),
...
void CXI_VIMAGESCROLL::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int         i, n, k;
    char        param[256];
    char        param1[256];
    ...
    if (m_nSlotsQnt > 0) {
        ...
        if (!m_dwCurColor || !m_dwNormalColor || !m_dwSelectColor || !m_pPicOffset) { throw std::runtime_error("allocate memory error"); }
    }
    ...
    if (pAttribute != nullptr) {
        ...
        if (m_nCurImage >= m_nListSize || m_nCurImage < 0) m_nCurImage = 0;
        ...
        if (m_nSlotsQnt > 0) {
            ...
            if (!m_idBadTexture || !m_idBadPic) { throw std::runtime_error("allocate memory error"); }
        }
        for (n = 0; n < m_nSlotsQnt; n++) {
            char const* sBadPict;
        ...
        for (i = 0; i < m_nListSize; i++) {
            ...
            if (m_nSlotsQnt > 0) {
                ...
                if (!m_Image[i].bUseSpecTechnique || !m_Image[i].img || !m_Image[i].ptex || !m_Image[i].saveName || !m_Image[i].tex) {
            ...
            if (pListEntity != nullptr) {
                ...
                for (k = 0; k < m_nStringQuantity; k++) {
                    ...
                    if (sStringName != nullptr && sStringName[0] == '#') {
                        auto const len        = strlen(sStringName);
                        m_Image[i].strSelf[k] = new char[len];
                        if (m_Image[i].strSelf[k] == nullptr) throw std::runtime_error("allocate memory error");
                ...
                for (n = 0; n < m_nSlotsQnt; n++) {
                    ...
                    if (tmpStr != nullptr) {
                        ...
                        if ((m_Image[i].saveName[n] = new char[len]) == nullptr) { throw std::runtime_error("allocate memory error"); }
...
float CXI_VIMAGESCROLL::ChangeDinamicParameters(float fYDelta)
{
    ...
    auto const lbBorderDist = m_rect.bottom - m_pCenter.y;
    auto const ltBorderDist = m_pCenter.y - m_rect.top;
    ...
    while (true) {
            ...
            for (n = 0; n < m_nSlotsQnt; n++) {
                if (m_Image[curImage].saveName[n] != nullptr) {
                    if (m_Image[curImage].ptex[n] == -1) {
                        m_Image[curImage].ptex[n] =
                            GetTexFromEvent(core->Event("GetInterfaceTexture", "sl", m_Image[curImage].saveName[n], curImage));
                        break;
                    }
                }
            }
            if (n == m_nSlotsQnt || m_Image[curImage].ptex[n] != -1) break;
        ...
        auto const bpos = curYCenter + m_ImageSize.y * .5f;
        ...
        } else if (!bIncrement && tpos < m_rect.top + m_nVDelta)  // end pass to left is all end
        {
            ...
            if (m_pScroll->next == nullptr) { m_pScroll->next = nullptr; }
            break;
        }
...
void CXI_VIMAGESCROLL::ChangeScroll(int nScrollItemNum)
{
    ...
    if (pAttr != nullptr) {
        ...
        if (nScrollItemNum == -1
            || m_nListSize != static_cast<int32_t>(pAttr->GetAttributeAsDword("NotUsed", 0) + pAttr->GetAttributeAsDword("ListSize", 0))) {
            RefreshScroll();
            return;
        }
            ...
            if (pAttribute != nullptr) {
                for (n = 0; n < m_nStringQuantity; n++) {
                    ...
                    if (sStringName != nullptr && sStringName[0] == '#') {
                        ...
                        if (m_Image[i].strSelf[n] == nullptr) throw std::runtime_error("allocate memory error");
...
void CXI_VIMAGESCROLL::DeleteImage(int imgNum)
{
    ...
    m_Image                 = new IMAGEDESCRIBE[m_nListSize];
    if (m_Image == nullptr) throw std::runtime_error("memory allocate error");
    if (imgNum > 0) memcpy(m_Image, pOldImgs, imgNum * sizeof(IMAGEDESCRIBE));
    if (imgNum < m_nListSize) memcpy(&m_Image[imgNum], &pOldImgs[imgNum + 1], (m_nListSize - imgNum) * sizeof(IMAGEDESCRIBE));
    delete pOldImgs;
...
int CXI_VIMAGESCROLL::FindTexGroupFromOld(char** pGroupList, char const* groupName, int listSize)
{
...
int32_t CXI_VIMAGESCROLL::GetMousePointedPictureNum()
{
    ...
    float const curYCenter = m_pScroll ? m_pScroll->pCenter.y : (m_rect.top + m_rect.bottom) / 2;
    int32_t     n          = 0;
    if (mp.y < curYCenter) {
        // number reduction
