...
void CXI_BORDER::ChangePosition(XYRECT& rNewPos)
{
    ...
    if (m_pBackImage) m_pBackImage->SetPosition(m_rect);
...
void CXI_BORDER::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char param[256];
    ...
    if (m_nCaptionHeight > 0 && ReadIniString(ini1, name1, ini2, name2, "captionimage", param, sizeof(param), "")) {
        ...
        if (m_pCaptionImage) { m_pCaptionImage->LoadAccordingToString(param); }
    }
...
void CXI_BORDER::WriteVertexForSquare(
    XI_ONETEX_VERTEX* pV, FXYRECT& UVRect, uint32_t dwColor, int32_t left, int32_t top, int32_t right, int32_t bottom)
{
    pV[0].color = dwColor;
