...
void CXI_CHECKBUTTONS::Draw(bool bSelected, uint32_t Delta_Time)
{
    ...
    for (auto& button: m_aButton) {
        ...
        if (button->bChoose) dwColor = m_dwSelectFontColor;
        if (button->bDisable) dwColor = m_dwDisableFontColor;
        if (!m_bSelected) dwColor = m_dwDisableFontColor;
        ...
        if (button->pImg) {
            button->pImg->SetDifferentPosition(
                static_cast<int32_t>(fX + m_fpIconOffset.x),
                static_cast<int32_t>(fY + m_fpIconOffset.y),
                static_cast<int32_t>(m_fpIconSize.x),
                static_cast<int32_t>(m_fpIconSize.y));
            button->pImg->Draw();
        }
...
void CXI_CHECKBUTTONS::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    ...
    for (int32_t n = 0; n < 100; n++) {
        ...
        char const* pTmpChar = param;
        auto const  bSelect  = CXI_UTILS::StringGetInt(pTmpChar) != 0;
        auto const  bDisable = CXI_UTILS::StringGetInt(pTmpChar) != 0;
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "alignment", param, sizeof(param), "")) {
        if (storm::iEquals(param, "center")) m_nFontAlignment = PR_ALIGN_CENTER;
        if (storm::iEquals(param, "right")) m_nFontAlignment = PR_ALIGN_RIGHT;
    }
...
int CXI_CHECKBUTTONS::CommandExecute(int wActCode)
{
    if (m_bUse) {
        ...
        case ACTION_MOUSECLICK: CheckMouseClick(ptrOwner->GetMousePoint()); break;
        }
    }
    return -1;
}
...
bool CXI_CHECKBUTTONS::IsClick(int buttonID, int32_t xPos, int32_t yPos)
{
    for (auto& btn: m_aButton)
        if (btn->pImg && btn->pImg->IsPointInside(xPos, yPos)) return true;
    if (!m_bUse || !m_bClickable || xPos < m_rect.left || xPos > m_rect.right || yPos < m_rect.top || yPos > m_rect.bottom) return false;
        ...
        for (int32_t n = 0; n < m_aButton.size(); n++) {
            ...
            if (m_aButton[n]->bSetPos) {
                sprintf_s(
                    ...
                    sizeof(pcWriteParam),
                    "%d,%d",
                    static_cast<int>(m_aButton[n]->pos.x),
...
uint32_t CXI_CHECKBUTTONS::MessageProc(int32_t msgcode, MESSAGE& message)
{
    ...
    case 0:  // add button
    {
        ...
        auto const         bSelect  = (message.Long() != 0);
        auto const         bDisable = (message.Long() != 0);
    ...
    {
        auto const         nButtonNum = message.Long() - 1;
        std::string const& param      = message.String();
    ...
    {
        auto const nButtonNum = message.Long() - 1;
        bool const bChoose    = (message.Long() != 0);
        SetCheckToButton(nButtonNum, bChoose);
    } break;

    case 3:  // get the state of the button
    {
        int32_t const nButtonNum = message.Long() - 1;
    ...
    {
        for (int32_t n = 0; n < m_aButton.size(); n++)
            if (m_aButton[n]->bChoose) return n;
    } break;

    case 5:  // disable / enable button
    {
        int32_t const nButtonNum = message.Long() - 1;
        bool const    bDisable   = (message.Long() != 0);
        if (nButtonNum < 0 || nButtonNum >= m_aButton.size()) return 0;
        m_aButton[nButtonNum]->bDisable = bDisable;
        m_aButton[nButtonNum]->pImg->LoadFromBase(
            m_sIconGroupName.c_str(), PicName(bDisable, m_aButton[nButtonNum]->bChoose).c_str(), false);
    } break;
    }
...
void CXI_CHECKBUTTONS::SetInternalName(std::string& sName)
{
    if (sName == "all") {
        ...
        m_nEditableSectionIndex = -1;
        if (storm::iEquals(sName.c_str(), "btn", 3)) { m_nEditableSectionIndex = atoi(&sName.c_str()[3]) - 1; }
    }
}

void CXI_CHECKBUTTONS::AddButton(char const* pcText, bool bDisable, bool bSelect)
{
    ...
    if (pcText && pcText[0] == '#') {
        ...
        auto const strNum = pStringService->GetStringNum(pcText);
        if (strNum < 0) {
...
void CXI_CHECKBUTTONS::ChangeText(int32_t nButtonNum, char const* pcText)
{
...
void CXI_CHECKBUTTONS::CheckMouseClick(const FXYPOINT& pntMouse)
{
    if (m_bClickIntoTextActive) {
        if (!m_bIndividualPos) {
            ...
            int32_t n;
            for (n = 0; n < m_aButton.size(); n++) {
                int32_t const nHeight = static_cast<int32_t>(m_aButton[n]->aStr.size() * m_fTextLineHeight);
                if (!m_aButton[n]->bDisable) {
                    if (pntMouse.y >= nY && pntMouse.y <= nY + nHeight) {
                        if (m_bExclusiveChoose) {
                            if (!m_aButton[n]->bChoose) SetCheckToButton(n, true);
                        } else
...
{
    for (int32_t n = 0; n < m_aButton.size(); n++)
        if (!m_aButton[n]->bDisable && m_aButton[n]->bChoose) SetButtonOff(n);
}

void CXI_CHECKBUTTONS::SetCheckToButton(int32_t nButtonNum, bool bCheck)
{
    if (nButtonNum < 0 || nButtonNum >= m_aButton.size()) return;  // there is no such button
    if (m_aButton[nButtonNum]->bChoose == bCheck) return;          // already set this state
