        ...
        if (Delta_Time > 0) {
            if (m_pPrevNode != ptrOwner->GetCurrentNode()) {
                ...
                if (m_pPrevNode) {
                    ...
                    if (m_bShowGlow) { SetRectanglesToPosition(m_pPrevNode->GetCursorRect()); }
                }
            } else {
                if (m_pPrevNode) {
                    m_bShowGlow = m_pPrevNode->IsShowGlowCursor();
                    if (m_bShowGlow && m_pPrevNode->IsGlowChanged()) {
                        auto const rectXY = m_pPrevNode->GetCursorRect();
                        if (static_cast<int>(m_pTexVert[0].pos.x) != rectXY.left || static_cast<int>(m_pTexVert[0].pos.y) != rectXY.top
...
void CXI_GLOWCURSOR::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int      i;
    char     param[255];
    FXYPOINT fPnt;
...
void CXI_GLOWCURSOR::SetRectanglesToPosition(const XYRECT& rectXY)
{
    ...
    m_pTexVert[1].pos.x = m_pTexVert[3].pos.x = m_pTexVert[7].pos.x = m_pTexVert[9].pos.x = static_cast<float>(rectXY.right);
    m_pTexVert[2].pos.y = m_pTexVert[3].pos.y = m_pTexVert[9].pos.y = m_pTexVert[11].pos.y = static_cast<float>(rectXY.bottom);
