...
CXI_QUESTTITLE::CXI_QUESTTITLE() : m_iconWidth(0), m_iconHeight(0), m_iconVOffset(0), m_texId(0), m_vertOffset(0)
{
...
void CXI_QUESTTITLE::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char param[255];
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "iconGroup", param, sizeof(param), "")) {
        ...
        if (m_iconGroupName == nullptr) { throw std::runtime_error("allocate memory error"); }
...
void CXI_QUESTTITLE::SetNewTopQuest(ATTRIBUTES* pA, int topNum)
{
    ...
    // if(ptrOwner->QuestFileReader())
    {
        ...
        if (m_stringQuantity > m_allStrings) m_stringQuantity = m_allStrings;
        if (m_curIdx >= m_stringQuantity) m_curIdx = m_stringQuantity - 1;

        // creating an array of strings
        if (m_stringQuantity <= 0) return;
        if ((m_strList = new STRING_DESCRIBER[m_stringQuantity]) == nullptr) { throw std::runtime_error("allocate memory error"); }
        ...
        for (i = 0; i < m_stringQuantity; i++) {
            ...
            char const* pTmpQuestRecordID = pAttr->GetAttribute("LogName");
            ...
            if (ptrOwner->QuestFileReader()->GetQuestTitle(pTmpQuestRecordID, pAttr->GetThisName(), param)) {
                size_t const titleSize = param.size();
                if (titleSize == 0)
                    m_strList[i].lineQuantity = 0;
                else {
                    ...
                    while (GetLineNext(m_idFont, pstr, lineName.data(), lineName.size())) {
                        auto const len        = lineName.size() + 1;
...
{
    float const left  = static_cast<float>(m_rect.left) - m_selectOffset;
    auto const  right = static_cast<float>(m_rect.right);
