...
CXI_SCROLLTEXT::CXI_SCROLLTEXT() : m_idFont(0)
{
    m_pScroller    = nullptr;
    m_nMaxStringes = 0;
    ...
    if (m_bUse) {
        for (auto i = 0; i < m_nPiceQuantity; i++) {
            ...
            case PICE_TYPE_STRING: {
                auto const chStart = m_pStrList[i].data.strDescr.startOffset;
    ...
    INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2, /*VDX9RENDER*/ void* rs, XYRECT& hostRect, XYPOINT& ScreenSize)
{
    ...
    SetText(
        "proba string ^fnormal continue proba string one, two, three etc. Continue next stringes from full screen "
        "filling");
    SetGlowCursor(false);
    return true;
}
...
void CXI_SCROLLTEXT::SetText(char const* newText)
{
    char        param[256];
    ...
    if (newText != nullptr) {
        auto const len = strlen(newText) + 1;
        if ((m_pText = new char[len]) == nullptr) throw std::runtime_error("allocate memory error");
        ...
        while (*pCh != 0) {
            if (*pCh == '^') {
                ...
                if (nBeg != nEnd) startWidth = FillPices(m_pText, nBeg, nEnd - nBeg, idx, spl, startWidth);
...
void CXI_SCROLLTEXT::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    m_pScroller    = nullptr;
    m_nMaxStringes = 0;
    m_dwFontColor  = storm::Color {255, 255, 255, 255}.to_hex();
...
int32_t CXI_SCROLLTEXT::FillPices(char* pt, size_t beg, size_t size, int32_t& idx, STRING_PICE* spl, int32_t wid)
{
    ...
    while (true) {
        ...
        resStr[0]              = 0;
        while (true) {
            auto const sw = GetStringWord(&pstr[chQuantity], param, sizeof(param) - 1);
            if (sw == 0) break;
            chQuantity += sw;
