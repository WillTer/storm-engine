...
int CXI_FOURIMAGE::CommandExecute(int wActCode)
{
    ...
    if (m_bUse) {
        ...
        switch (wActCode) {
        case ACTION_RIGHTSTEP: newSelectItem++; break;
        case ACTION_LEFTSTEP: newSelectItem--; break;
        case ACTION_UPSTEP: {
            auto* pvdat = core->Event("FI_UpCom", "l", m_nSelectItem);
            if (pvdat == nullptr || pvdat->GetInt() == 0) newSelectItem -= 2;
        } break;
        case ACTION_DOWNSTEP: newSelectItem += 2; break;
        case ACTION_ACTIVATE: break;
        case ACTION_SPEEDRIGHT: newSelectItem++; break;
        case ACTION_SPEEDLEFT: newSelectItem--; break;
        case ACTION_DEACTIVATE: break;
        case ACTION_MOUSECLICK:
            int i;
            for (i = 0; i < 4; i++)
                if (m_MousePoint.x >= m_imgRect[i].left && m_MousePoint.x <= m_imgRect[i].right && m_MousePoint.y >= m_imgRect[i].top
                    && m_MousePoint.y <= m_imgRect[i].bottom)
                    break;
            if (i < 4) {
                newSelectItem = i;
                if (m_nSelectItem == newSelectItem && IsCurrentNode()) { retVal = ACTION_ACTIVATE; }
            }
            break;
        }
        ...
        if (m_bUsed[newSelectItem]) m_nSelectItem = newSelectItem;
        ...
        if (bUseOneString || bUseTwoString) {
            ...
            for (auto i = 0; i < 4; i++) {
                ...
                if (bUseOneString) {
                    ...
                    //         posY,
                    //         "%s",
                    //         pStringService->GetString(m_oneStr[i]));
                }
                if (bUseTwoString) {
                    ...
                    //         posY,
                    //         "%s",
                    //         pStringService->GetString(m_twoStr[i]));
                }
            }
        }
    }
...
void CXI_FOURIMAGE::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    int         i;
    char        param[256];
    char        param1[256];
    ...
    // get base color
    m_dwBaseColor = GetIniARGB(ini1, name1, ini2, name2, "commonColor", storm::Color {255, 128, 128, 128}.to_hex());

    // get color for light select image
    m_dwLightSelectColor = GetIniARGB(ini1, name1, ini2, name2, "lightSelectCol", storm::Color {255, 255, 255, 255}.to_hex());

    // get color for dark select image
    m_dwDarkSelectColor = GetIniARGB(ini1, name1, ini2, name2, "darkSelectCol", storm::Color {255, 150, 150, 150}.to_hex());
    ...
    if (bUseOneString) {
        ...
        //     if ((m_oneStrFont = m_rs->LoadFont(param1)) == -1) core->Trace("can not load font:'%s'", param1);

        m_xOneOffset = m_nOneStrOffset = 0;
        if (GetMidStr(param, param1, sizeof(param1), "off:(", ")")) GetDataStr(param1, "ll", &m_xOneOffset, &m_nOneStrOffset);

        m_foreColOneStr = storm::Color {255, 255, 255, 255}.to_hex();
        if (GetMidStr(param, param1, sizeof(param1), "fc:{", "}")) { m_foreColOneStr = GetColorFromStr(param1, m_foreColOneStr); }

        m_backColOneStr = storm::Color {0, 0, 0, 0}.to_hex();
        if (GetMidStr(param, param1, sizeof(param1), "bc:{", "}")) { m_backColOneStr = GetColorFromStr(param1, m_backColOneStr); }
    } else {
        m_nOneStrOffset = 0;
        m_foreColOneStr = storm::Color {255, 255, 255, 255}.to_hex();
        m_backColOneStr = storm::Color {0, 0, 0, 0}.to_hex();
    ...
    if (bUseTwoString) {
        m_twoStrFont = -1;
        // if (GetMidStr(param, param1, sizeof(param1), "font:", ","))
        //     if ((m_twoStrFont = m_rs->LoadFont(param1)) == -1) core->Trace("can not load font:'%s'", param1);

        m_xTwoOffset = m_nTwoStrOffset = 0;
        if (GetMidStr(param, param1, sizeof(param1), "off:(", ")")) GetDataStr(param1, "ll", &m_xTwoOffset, &m_nTwoStrOffset);

        m_foreColTwoStr = storm::Color {255, 255, 255, 255}.to_hex();
        if (GetMidStr(param, param1, sizeof(param1), "fc:{", "}")) { m_foreColTwoStr = GetColorFromStr(param1, m_foreColTwoStr); }

        m_backColTwoStr = storm::Color {0, 0, 0, 0}.to_hex();
        if (GetMidStr(param, param1, sizeof(param1), "bc:{", "}")) { m_backColTwoStr = GetColorFromStr(param1, m_backColTwoStr); }
    } else {
        m_nTwoStrOffset = 0;
        m_foreColTwoStr = storm::Color {255, 255, 255, 255}.to_hex();
        m_backColTwoStr = storm::Color {0, 0, 0, 0}.to_hex();
    ...
    if (pAttribute != nullptr) {
        ...
        if (pA == nullptr)
            ...
            if (m_sGroupName == nullptr || m_nTextureId == nullptr) { throw std::runtime_error("Allocate memory error"); }
            for (i = 0; i < m_nTexturesQuantity; i++) {
                ...
                if (stmp == nullptr)
                    ...
                    if ((m_sGroupName[i] = new char[len]) == nullptr) { throw std::runtime_error("Allocate memory error"); }
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (pAttrTmp != nullptr) {
                ...
                if (m_twoTexID[i] != -1 && m_twoTexID[i] < m_nTexturesQuantity)
                    ...
                    m_twoImgID[i] = -1;
                char const* tmps = pAttrTmp->GetAttribute("str1");
                if (tmps != nullptr && *tmps == '#') {
                    auto const len = strlen(tmps);
                    if ((m_pOneStr[i] = new char[len]) == nullptr) throw std::runtime_error("allocate memory error");
                ...
                if (tmps != nullptr && *tmps == '#') {
                    auto const len = strlen(tmps);
                    if ((m_pTwoStr[i] = new char[len]) == nullptr) throw std::runtime_error("allocate memory error");
    ...
    auto const bRelativeRect = !GetIniLong(ini1, name1, ini2, name2, "bAbsoluteRectangle");
    for (i = 0; i < 4; i++) {
        sprintf_s(param, "position%d", i + 1);
        m_imgRect[i] = GetIniLongRect(ini1, name1, ini2, name2, param, XYRECT(0, 0, 0, 0));
        if (bRelativeRect) GetRelativeRect(m_imgRect[i]);
    }

    // get border picture
    if (ReadIniString(ini1, name1, ini2, name2, "border", param, sizeof(param), "")) {
        tmpstr = GetSubStr(param, param1, sizeof(param1));
        if ((m_sBorderGroupName = new char[sizeof param1]) == nullptr) throw std::runtime_error("allocate memory error");
...
void CXI_FOURIMAGE::Update(bool bSelected, uint32_t DeltaTime)
{
    ...
    //                 }
    //
    //                 auto const ad = ALPHA(m_dwDarkSelectColor);
    //                 auto const rd = RED(m_dwDarkSelectColor);
    //                 auto const gd = GREEN(m_dwDarkSelectColor);
    //                 auto const bd = BLUE(m_dwDarkSelectColor);
    //                 auto const al = ALPHA(m_dwLightSelectColor);
    //                 auto const rl = RED(m_dwLightSelectColor);
    //                 auto const gl = GREEN(m_dwLightSelectColor);
    //                 auto const bl = BLUE(m_dwLightSelectColor);
    //                 uint32_t   a, r, g, b;
    //                 if (m_bColorType) {
...
{
    for (auto i = 0; i < 4; i++)
        if (xPos >= m_imgRect[i].left && xPos <= m_imgRect[i].right && yPos >= m_imgRect[i].top && yPos <= m_imgRect[i].bottom
            && m_bClickable && m_bUse) {
            m_MousePoint.x = xPos;
            m_MousePoint.y = yPos;
...
{
    auto const nXOffset = rNewPos.left - m_rect.left;
    auto const nYOffset = rNewPos.top - m_rect.top;
    auto const nXToGrow = rNewPos.right - m_rect.right;
    auto const nYToGrow = rNewPos.bottom - m_rect.bottom;
