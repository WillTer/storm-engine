...
void CXI_IMGCOLLECTION::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char    param[256];
    int32_t n;
    ...
    // Get images quantity
    auto imgQuantity = 0;
    if (ini1->ReadString(name1, "picture", param, sizeof(param) - 1, "")) do {
            if (!storm::iEquals(param, "editsection:", 12)) imgQuantity++;
...
void CXI_IMGCOLLECTION::SetBuffers(
    XI_ONETEX_VERTEX* pVBuf, uint16_t* pIBuf, int rectNum, XYRECT& scrRect, FXYRECT& texRect, uint32_t dwColor)
{
...
void CXI_IMGCOLLECTION::SaveParametersToIni()
{
    ...
    for (n = 0; n < m_aEditInfo.size(); n++)
        if (m_aEditInfo[n].bNative) break;
    ...
    if (n < m_aEditInfo.size()) {
        ...
        for (n = 0; n < m_aEditInfo.size(); n++) {
            ...
            sprintf_s(
                pcWriteParam,
                sizeof(pcWriteParam),
                "%s,col:{%d,%d,%d,%d},pos:{%d,%d,%d,%d}",
                m_aEditInfo[n].sName.c_str(),
                ALPHA(m_aEditInfo[n].dwColor),
                RED(m_aEditInfo[n].dwColor),
                GREEN(m_aEditInfo[n].dwColor),
                BLUE(m_aEditInfo[n].dwColor),
                m_aEditInfo[n].nLeft,
                m_aEditInfo[n].nTop,
                m_aEditInfo[n].nRight,
                m_aEditInfo[n].nBottom);
            pIni->AddString(m_nodeName, "picture", pcWriteParam);
        }
    }
}
...
uint32_t CXI_IMGCOLLECTION::MessageProc(int32_t msgcode, MESSAGE& message)
{
    ...
    case 0:  // add icon
    {
        ...
        // picture color
        auto const dwColor = message.Long();
        // picture position
        XYRECT pos;
    ...
    {
        auto const     nImgNum = message.Long();
