...
CXI_QUESTTEXTS::STRING_DESCRIBER* CXI_QUESTTEXTS::STRING_DESCRIBER::Add(char const* ls, bool complete)
{
...
bool GetNextIdFromList(char*& sptr, char* bufQuestID, size_t nSizeBufQuestID, char* buf, size_t bufSize, char* dataBuf)
{
    ...
    if (sstart[0] == '@') {
        ...
        if (sstart[0] < '0' || sstart[0] > '9') {
            ...
            while (sstart[0] && sstart[0] != '@') {
                if (n < static_cast<int32_t>(nSizeBufQuestID)) bufQuestID[n++] = sstart[0];
                sstart++;
                idSize--;
            }
            bufQuestID[n] = '\0';
    ...
    if (idSize == 0) return false;
    if (static_cast<size_t>(idSize) >= bufSize) idSize = bufSize - 1;
...
CXI_QUESTTEXTS::CXI_QUESTTEXTS() : m_vertOffset(0)
{
...
int CXI_QUESTTEXTS::CommandExecute(int wActCode)
{
    if (m_bUse) {
        ...
        switch (wActCode) {
        case ACTION_UPSTEP:
            if (m_listCur->prev != nullptr) m_listCur = m_listCur->prev;
            break;
        case ACTION_DOWNSTEP:
            if (m_listCur->next != nullptr) m_listCur = m_listCur->next;
            break;
        case ACTION_SPEEDUP: {
...
void CXI_QUESTTEXTS::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char param[255];
...
void CXI_QUESTTEXTS::GetStringListForQuestRecord(std::vector<std::string>& stringList, char const* recText, char const* userData) const
{
    char pcQuestID[1024];
    char pcTextID[1024];
    char pcDate[256];
    ...
    if (GetNextIdFromList(pcTmp, pcQuestID, sizeof(pcQuestID), pcTextID, sizeof(pcTextID) - 1, pcDate)) {
        stringList.clear();
        if (pcDate[0]) stringList.push_back(pcDate);
