...
void CXI_PCEDITBOX::ChangePosition(XYRECT& rNewPos)
{
    ...
    auto const nHeight      = m_rect.bottom - m_rect.top;
    if (m_pLeftImage) {
...
void CXI_PCEDITBOX::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    char param[2048];
            ...
            for (int32_t n = 0; n < core->Controls->GetKeyBufferLength(); n++) {
                if (pKeys[n].bSystem) {
                    ...
                    case VK_BACK:
                        ...
                        break;
                    case VK_END: m_nEditPos = strLength; break;
                    ...
                    case VK_DELETE:
                        ...
                        break;
                    case VK_LEFT:
                        if (m_nEditPos > 0) m_nEditPos--;
                        break;
                    case VK_RIGHT:
                        if (m_nEditPos < strLength) m_nEditPos++;
                        break;
                    }
                } else
...
void CXI_PCEDITBOX::ShowCursorPosition(std::string& str)
{
    ...
    int strLength = utf8::Utf8StringLength(str.c_str());
    if (m_nEditPos > strLength) m_nEditPos = strLength;
...
void CXI_PCEDITBOX::InsertSymbol(std::string& str, utf8::u8_char chr)
{
    ...
    if (m_nMaxSize >= 0 && strLength >= m_nMaxSize) return;
