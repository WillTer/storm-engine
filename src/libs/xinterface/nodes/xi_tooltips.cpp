...
void CXI_ToolTip::SetByFormatString(XYRECT& rectOwner, INIFILE* pDefIni, char const* pFmtStr)
{
    if (!pFmtStr) return;
    char    tokenID[256];
    ...
    for (auto* pcParam = (char*)pFmtStr; pcParam && pcParam[0];) {
        ...
        if (CXI_UTILS::StringGetTokenString(pcParam, tokenString, sizeof(tokenString))) {
            ...
            case InterfaceToken_width: m_nMaxStrWidth = CXI_UTILS::StringGetInt(pStr);
            }
        }
    }
...
{
    if (m_nMouseX != nX || m_nMouseY != nY || nX < m_rActiveZone.left || nX > m_rActiveZone.right || nY < m_rActiveZone.top
        || nY > m_rActiveZone.bottom) {
        m_nMouseX      = nX;
        m_nMouseY      = nY;
        m_bDisableDraw = true;
...
{
    if (!m_pV) return;
    CXI_UTILS::WriteSquareToVertexBuffer(
        &m_pV[0],
        m_dwBackColor,
        m_uvBackMiddle,
        m_rPos.left + m_nLeftSideWidth,
        m_rPos.top,
        m_rPos.right - m_nRightSideWidth,
        m_rPos.bottom);
    CXI_UTILS::WriteSquareToVertexBuffer(
        &m_pV[4], m_dwBackColor, m_uvBackLeft, m_rPos.left, m_rPos.top, m_rPos.left + m_nLeftSideWidth, m_rPos.bottom);
    CXI_UTILS::WriteSquareToVertexBuffer(
        &m_pV[8], m_dwBackColor, m_uvBackRight, m_rPos.right - m_nRightSideWidth, m_rPos.top, m_rPos.right, m_rPos.bottom);
}
...
void CXI_ToolTip::ReplaceRectangle(int32_t x, int32_t y)
{
    ...
    if (m_rPos.right > m_pntScreenSize.x) m_rPos.right = m_pntScreenSize.x;
