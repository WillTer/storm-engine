...
CXI_FORMATEDTEXT::STRING_DESCRIBER::STRING_DESCRIBER(char* ls) : color(0)
{
    ...
    if (len == 1)
        ...
        if ((lineStr = new char[len]) == nullptr) { throw std::runtime_error("allocate memory error"); }
...
CXI_FORMATEDTEXT::STRING_DESCRIBER* CXI_FORMATEDTEXT::STRING_DESCRIBER::Add(char* ls, int groupNum)
{
    ...
    if (newSD == nullptr) { throw std::runtime_error("allocate memory error"); }
...
int CXI_FORMATEDTEXT::CommandExecute(int wActCode)
{
    if (m_bUse && !m_bFrized) {
        ...
        if (wActCode == ACTION_MOUSECLICK || wActCode == ACTION_MOUSERCLICK) {
            auto const fpntMouse = ptrOwner->GetMousePoint();
            if (m_bUpEnable && m_idUpEnableTexture >= 0) {
            ...
            if (m_bDownEnable && m_idDownEnableTexture >= 0) {
                if (fpntMouse.x >= m_frDownPos.left && fpntMouse.x <= m_frDownPos.right && fpntMouse.y >= m_frDownPos.top
                    && fpntMouse.y <= m_frDownPos.bottom) {
        ...
        if (!m_bSelectableCursor) {
            ...
            switch (wActCode) {
            case ACTION_UPSTEP: SetVertexToNewGroup(true, FindUpGroup(m_nCurGroupNum - 1), FindDownGroup(m_nCurGroupNum - 1)); break;
            case ACTION_DOWNSTEP: SetVertexToNewGroup(false, FindUpGroup(m_nCurGroupNum + 1), FindDownGroup(m_nCurGroupNum + 1)); break;
            case ACTION_SPEEDUP: SetVertexToNewGroup(true, FindUpGroup(m_nCurGroupNum - 1), FindDownGroup(m_nCurGroupNum - 1)); break;
            case ACTION_SPEEDDOWN: SetVertexToNewGroup(false, FindUpGroup(m_nCurGroupNum + 1), FindDownGroup(m_nCurGroupNum + 1)); break;
...
bool CXI_FORMATEDTEXT::IsClick(int buttonID, int32_t xPos, int32_t yPos)
{
    ...
    int32_t const nNum = static_cast<int32_t>((yPos - m_rect.top) / m_vertOffset + .1f);
...
void CXI_FORMATEDTEXT::LoadIni(INIFILE* ini1, char const* name1, INIFILE* ini2, char const* name2)
{
    ...
    int const nPosMorph = GetIniLong(ini1, name1, ini2, name2, "bAbsoluteRectangle");
...
bool CXI_FORMATEDTEXT::GetLineNext(int fontNum, char const*& pInStr, char* buf, int bufSize) const
{
    if (pInStr == nullptr || buf == nullptr) return false;

    char const* pStart  = pInStr;
    int         bYesEOL = 0;
    while (*pInStr != 0) {
        if (pInStr[0] == '\\' && (pInStr[1] == 'n' || pInStr[1] == 'N')) break;
    ...
    size_t const lineSize = pInStr - pStart;
    if (lineSize > 0) {
...
void CXI_FORMATEDTEXT::GetOneLine(int fontNum, char const* pStr, char* buf, int bufSize) const
{
...
void CXI_FORMATEDTEXT::SetPointer(float fPos)
{
    if (m_bSelectableCursor)
        SetVertexToNewGroup(
            true,
            FindUpGroup(static_cast<int32_t>(m_nStringGroupQuantity * fPos)),
...
{
    if (m_bSelectableCursor) {
        if (m_nStringGroupQuantity > 1) return 1.f / static_cast<float>(m_nStringGroupQuantity - 1);
        return 0.f;
    }
    if (m_nAllTextStrings > 0) return 1.f / static_cast<float>(m_nAllTextStrings);
    return 0.f;
    return 0.f;
}
...
{
    if (m_bSelectableCursor) {
        if (m_nStringGroupQuantity > 1) return static_cast<float>(m_nCurGroupNum) / static_cast<float>(m_nStringGroupQuantity - 1);
        return 0.f;
    }
    if (m_nAllTextStrings > 0) return static_cast<float>(m_listCur->strNum) / static_cast<float>(m_nAllTextStrings);
    return 0.f;
    return 0.f;
}
...
int32_t CXI_FORMATEDTEXT::AddFormatedText(char const* str)
{
    ...
    if (m_bUseOneStringAdding) {
        ...
        if (m_listRoot == nullptr) {
            if ((dscrTmp = m_listRoot = new STRING_DESCRIBER(newStr)) == nullptr) { throw std::runtime_error("allocate memory error"); }
...
{
    char const* str      = pStrDescr->lineStr;
    char const* tagBegin = str;
...
uint32_t CXI_FORMATEDTEXT::MessageProc(int32_t msgcode, MESSAGE& message)
{
    ...
    case 0:  // add text to the formatted list and return the number of occupied lines
    {
        ...
        if (pvdat != nullptr) {
            int const retVar = AddFormatedText(pvdat->GetString());
            CheckScrollButtons();
            return retVar;
        }
    } break;
    ...
    {
        int32_t const nGrNum = message.Long();
        if (m_bSelectableCursor) SetVertexToNewGroup(true, FindUpGroup(nGrNum), FindDownGroup(nGrNum));
        CheckScrollButtons();
    } break;
    ...
    case 8:  // Set color for group
    {
        ...
        uint32_t const newColor = message.Long();
    ...
    case 9:  // Find out the number of lines that a line will take
    {
        VDATA* pvdat = message.ScriptVariablePointer();
        if (pvdat != nullptr) {
            char const* tmpStr = pvdat->GetString();
            char        buf[512];
            int         n;
...
void CXI_FORMATEDTEXT::SetVertexToNewGroup(bool bUpDirect, int32_t upIdx, int32_t downIdx)
{
    ...
    if (pv != nullptr) {
        ...
        if (m_bBackRectangle) {
            pv[12].pos.y = pv[14].pos.y = static_cast<float>(m_rect.top - m_nUpRectOffset - m_rBorderOffset.top + m_vertOffset * upIdx);
...
{
    bool const oldUp   = m_bUpEnable;
    bool const oldDown = m_bDownEnable;
    if (m_listCur == nullptr) {
...
void CXI_FORMATEDTEXT::SetSpecialStrings(ATTRIBUTES* pARoot)
{
    ...
    int const q = pARoot->GetAttributesNum();
    for (int i = 0; i < q; i++) {
        ATTRIBUTES* pA = pARoot->GetAttributeClass(i);
        if (pA == nullptr) continue;
        char const* tmpstr = pA->GetAttribute("str");
        if (tmpstr == nullptr) continue;
        int const pos = pA->GetAttributeAsDword("pos", -1);
        while (pos > m_nAllTextStrings)
            AddFormatedText("\n");
        if (pos >= 0) {
            int const oldCompareWidth = m_nCompareWidth;  // to avoid hyphenation
...
void CXI_FORMATEDTEXT::SetCurrentGroupNum(int32_t nFirstNum, int32_t nSelectNum)
{
    ...
    for (pSD = m_listRoot; pSD; pSD = pSD->next)
        if (pSD->strGroup == nFirstNum) break;
...
void CXI_FORMATEDTEXT::ReplaceString(int32_t nGrpNum, char const* pSrcStr)
{
    if (nGrpNum < 0) return;
    ...
    uint32_t const dwOldColor = dscrCur->color;
    ...
    if (!dscrCur) {
        ...
        for (dscrCur = m_listRoot; dscrCur; dscrCur = dscrCur->next)
            if (dscrCur->strGroup == nGrpNum) dscrCur->color = dwOldColor;
        return;
    }
...
void CXI_FORMATEDTEXT::InsertStringBefore(STRING_DESCRIBER* pNextDescr, char const* pSrcStr, int32_t nGrpNum, uint32_t dwColor)
{
    ...
    while (GetLineNext(m_idFont, pstr, newStr, sizeof(newStr))) {
        ...
        if (!pNewDescr) { throw std::runtime_error("allocate memory error"); }
