...
void STRSERVICE::SetLanguage(char const* sLanguage)
{
    ...
    STORM_DELETE(m_sLanguage);
    auto const len = strlen(sLanguage) + 1;
    if ((m_sLanguage = new char[len]) == nullptr) { throw std::runtime_error("Allocate memory error"); }
    ...
    while (true) {
        ...
        if (langIni->ReadString("DIRECTORY", m_sLanguage, param, sizeof(param) - 1, "")) {
            ...
            if ((m_sLanguageDir = new char[len]) == nullptr) { throw std::runtime_error("Allocate memory error"); }
        ...
        if (langIni->ReadString("COMMON", "strings", param, sizeof(param) - 1, "")) {
            auto const len = strlen(param) + 1;
            if ((m_sIniFileName = new char[len]) == nullptr) { throw std::runtime_error("Allocate memory error"); }
        ...
        if (langIni->ReadString("COMMON", "defaultLanguage", param, sizeof(param) - 1, "")) {
            if (storm::iEquals(m_sLanguage, param)) break;
            core.Trace("WARNING! Language %s not exist some ini parameters. Language set to default %s", m_sLanguage, param);
            STORM_DELETE(m_sLanguage);
            auto const len = strlen(param) + 1;
            m_sLanguage    = new char[len];
            if (m_sLanguage == nullptr) { throw std::runtime_error("Allocate memory error"); }
    ...
    // get string quantity
    auto newSize = 0;
    if (ini->ReadString(nullptr, "string", param, sizeof(param) - 1, "")) do {
            newSize++;
        } while (ini->ReadStringNext(nullptr, "string", param, sizeof(param) - 1));

    // check to right of ini files
    if (newSize != m_nStringQuantity && m_nStringQuantity != 0) core.Trace("WARNING: language %s ini file has different size", sLanguage);
    ...
    if (newSize > 0) {
        ...
        if (m_psStrName == nullptr || m_psString == nullptr) throw std::runtime_error("Allocate memory error");
    } else {
        m_psString  = nullptr;
        m_psStrName = nullptr;
    }
    ...
    for (i = 0; i < m_nStringQuantity; i++) {
        if (GetStringDescribe(param, strName, string)) {
            ...
            if (m_psStrName[i] == nullptr) throw std::runtime_error("allocate memory error");
    ...
    for (auto* pUSB = pOldURoot; pUSB != nullptr; pUSB = pUSB->next) {
        ...
        int32_t const     newID = OpenUsersStringFile(pUSB->fileName);
        ...
        if (pUTmp->nStringsQuantity != pUSB->nStringsQuantity) {
            ...
            for (itmp1 = 0; itmp1 < pUTmp->nStringsQuantity; itmp1++) {
                ...
                for (itmp2 = 0; itmp2 < pUSB->nStringsQuantity; itmp2++) {
                    ...
                    if (storm::iEquals(pUSB->psStrName[itmp2], pUTmp->psStrName[itmp1])) break;
                }
                if (itmp2 >= pUSB->nStringsQuantity) core.Trace(">>> string <%s> not found into strings file", pUTmp->psStrName[itmp1]);
            }
            ...
            for (itmp1 = 0; itmp1 < pUSB->nStringsQuantity; itmp1++) {
                ...
                for (int itmp2 = 0; itmp2 < pUTmp->nStringsQuantity; itmp2++) {
                    ...
                    if (storm::iEquals(pUTmp->psStrName[itmp2], pUSB->psStrName[itmp1])) break;
                }
                if (itmp2 >= pUTmp->nStringsQuantity) core.Trace(">>> string <%s> is new into strings file", pUSB->psStrName[itmp1]);
            }
        }
    }
    ...
    if (stringName != nullptr)
        for (int i = 0; i < m_nStringQuantity; i++)
            if (storm::iEquals(m_psStrName[i], stringName)) {
                ...
                if (len > 0) strcpy_s(sBuffer, bufferSize, m_psString[i]);
...
void STRSERVICE::LoadIni()
{
    ...
    if (sGlobalUserFileName[0]) g_idGlobLanguageFileID = OpenUsersStringFile(sGlobalUserFileName);
...
int32_t STRSERVICE::GetStringNum(char const* stringName)
{
    ...
    if (stringName != nullptr)
        for (int32_t i = 0; i < m_nStringQuantity; i++)
            if (storm::iEquals(m_psStrName[i], stringName)) return i;
    return -1L;
...
int32_t STRSERVICE::OpenUsersStringFile(char const* fileName)
{
    ...
    for (itUSB = m_pUsersBlocks; itUSB != nullptr; itUSB = itUSB->next) {
        if (itUSB->fileName != nullptr && storm::iEquals(itUSB->fileName, fileName)) { break; }
        pPrev = itUSB;
    }
    if (itUSB != nullptr) {
        itUSB->nref++;
    ...
    pUSB->nref     = 1;
    auto const len = strlen(fileName) + 1;
    pUSB->fileName = new char[len];
    if (pUSB->fileName == nullptr) { throw std::runtime_error("Allocate memory error"); }
    ...
    if (pUSB->nStringsQuantity == 0) {
        ...
        if (pUSB->psStrName == nullptr || pUSB->psString == nullptr) { throw std::runtime_error("Allocate memory error"); }
        stridx = 0;
        for (i = 0; i < pUSB->nStringsQuantity; i++) {
    ...
    int32_t const block_id = pUSB->blockID;
    pUSB->next             = nullptr;
    if (pPrev == nullptr) {
...
char* STRSERVICE::TranslateFromUsers(int32_t id, char const* inStr)
{
    ...
    for (i = 0; i < pUSB->nStringsQuantity; i++)
        if (pUSB->psStrName[i] != nullptr && storm::iEquals(pUSB->psStrName[i], inStr)) break;
...
bool STRSERVICE::GetNextUsersString(char* src, int32_t& idx, char** strName, char** strData) const
{
    ...
    if (strName != nullptr && nameBeg <= nameEnd) {
        *strName = new char[nameEnd - nameBeg + 2];
        if (*strName == nullptr) throw std::runtime_error("Allocate memory error");
    ...
    if (strData != nullptr && dataBeg <= dataEnd) {
        *strData = new char[dataEnd - dataBeg + 2];
        if (*strData == nullptr) throw std::runtime_error("Allocate memory error");
...
uint32_t _Language_OpenFile(VS_STACK* pS)
{
    ...
    char const* strLngFileName = nullptr;
    pLngFileName->Get(strLngFileName);

    // get the ID for the given file
    int32_t const nLngFileID = g_StringServicePointer->OpenUsersStringFile(strLngFileName);
...
uint32_t _InterfaceMakeNode(VS_STACK* pS)
{
    ...
    int32_t const nPriority = pDat->GetInt();

    pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* sNodeName = pDat->GetString();

    pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* sNodeType = pDat->GetString();
...
uint32_t _InterfaceDeleteFolder(VS_STACK* pS)
{
    ...
    int32_t const nSuccess = DeleteFolderWithCantainment(sFolderName);
    pDat                   = (VDATA*)pS->Push();
    if (!pDat) return IFUNCRESULT_FAILED;
    pDat->Set(nSuccess);
    return IFUNCRESULT_OK;
...
uint32_t _InterfaceFindFolders(VS_STACK* pS)
{
    ...
    pDat = (VDATA*)pS->Push();
    if (!pDat) { return IFUNCRESULT_FAILED; }
    int32_t const nSuccess = (pA->GetAttributesNum() > 0);
    pDat->Set(nSuccess);
    return IFUNCRESULT_OK;
}
...
uint32_t _DialogAssembleStr(VS_STACK* pS)
{
    ...
    pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* pcParam = pDat->GetString();
    pDat                = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
...
uint32_t _DialogAddParamToStr(VS_STACK* pS)
{
    ...
    pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* pcDatVal = pDat->GetString();
    pDat                 = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* pcDatID = pDat->GetString();
    pDat                = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* pcSrcStr = pDat->GetString();
...
{
    auto data = stack->Pop();
    if (!data) { return IFUNCRESULT_FAILED; }

    auto const key_code = data->GetInt();
...
uint32_t _AddControlTreeNode(VS_STACK* pS)
{
    auto pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    float const fTimeOut = pDat->GetFloat();

    pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* pcOutControl;
    pDat->Get(pcOutControl);

    pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    char const* pcBaseControl;
    pDat->Get(pcBaseControl);

    pDat = (VDATA*)pS->Pop();
    if (!pDat) return IFUNCRESULT_FAILED;
    int32_t const nParent = pDat->GetInt();

    int32_t const nNodIdx = core.Controls->AddControlTreeNode(nParent, pcBaseControl, pcOutControl, fTimeOut);
