...
char* SkipToken(char* str, char const* cmp)
{
    int         flag;
    char*       str1;
    char const* cmp1;
...
char* FindToken(char* str, char const* cmp)
{
    int         flag;
    char*       str1;
    char const* cmp1;
...
char* GetTokenWhile(char* src, char* dst, char const* skip)
{
...
SRSPARAM MYD3DFOGMODE[] = {{"none", D3DFOG_NONE}, {"exp", D3DFOG_EXP}, {"exp2", D3DFOG_EXP2}, {"linear", D3DFOG_LINEAR}};
...
SRSPARAM MYD3DMATERIALCOLORSOURCE[] = {{"material", D3DMCS_MATERIAL}, {"color1", D3DMCS_COLOR1}, {"color2", D3DMCS_COLOR2}};
...
SRSPARAM PARAM_TYPES[] = {{"matrix", PARAM_MATRIX}, {"texture", PARAM_TEXTURE}, {"dword", PARAM_DWORD}, {"float", PARAM_FLOAT}};
...
uint32_t CTechnique::GetSTSSIndex(char* pStr)
{
    ...
    for (uint32_t i = 0; i < dwNumParam; i++)
        if (storm::iEquals(pStr, TexturesStageStates[i].cName)) return i;
...
uint32_t CTechnique::GetSAMPIndex(char* pStr)
{
    ...
    for (uint32_t i = 0; i < dwNumParam; i++)
        if (storm::iEquals(pStr, SampleStates[i].cName)) return i;
...
uint32_t CTechnique::AddShader(char* pShaderName)
{
    ...
    for (uint32_t i = 0; i < dwNumShaders; i++)
        if (storm::iEquals(pShaders[i].pName, pShaderName)) return i;
    ...
    { \
        ...
        if (SkipToken(*pStr, TRANSFORM_CHECK)) {
            ...
            if (dwCode == 0xFFFFFFFF)  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            {
                ...
                if (nullptr == (pTemp = SkipToken(*pStr, WORLD_TRANSFORM_CHECK))) throw std::runtime_error("transform. error!");
...
uint32_t CTechnique::ProcessVertexDeclaration(shader_t* pS, char* pFile, uint32_t dwSize, char** pStr)
{
    ...
    while (nullptr != (*pStr = GetString(pFile, dwSize, *pStr))) {
        ...
        if (SkipToken(*pStr, VDECL_FLOAT_CHECK)) {
            ...
            if (dwTemp != ~0) CurDeclElement(pS)->UsageIndex = dwIndexes[dwTemp]++;
        }
...
uint32_t CTechnique::ProcessShaderAsm(shader_t* pS, char* pFile, uint32_t dwSize, char** pStr, uint32_t dwShaderType, bool HLSL)
{
    ...
    if (D3D_OK != hr && ErrorShader) {
        auto const len     = ErrorShader->GetBufferSize() + 1;
...
bool CTechnique::ExecutePassStart()
{
    ...
    if (it == htBlocks.end()) return false;
...
uint32_t CTechnique::GetPassParameter(uint32_t dwParam, uint32_t dwFlags)
{
    if (dwFlags & FLAGS_CODE_IN_PARAM) {
        if (dwParam >= dwCurNumParams) throw std::runtime_error("Technique: Exceeded parameter numbers!");
...
bool CTechnique::ExecutePass(bool bStart)
{
    ...
    while (uint32_t(pPass - pPassStart) < dwCurPassSize) {
        ...
        case CODE_TEXTURE:  // SetTexture(
        {
            ...
            if (iTextureIndex >= 0) iTextureIndex = (int32_t)GetPassParameter(*pPass++, dwSubCode);
        ...
        case CODE_SVS: {
            ...
            if (dwShaderIndex != INVALID_SHADER_INDEX && pShaders[dwShaderIndex].pVertexShader != nullptr) {
                ...
                if (pShaders[dwShaderIndex].pVertexDecl != nullptr) pRS->SetVertexDeclaration(pShaders[dwShaderIndex].pVertexDecl);
        ...
        case CODE_RESTORE:  // restore states
            ...
            case SUBCODE_RESTORE_CLEAR: ClearSavedStates(); break;
            case SUBCODE_RESTORE_STATES: RestoreSavedStates(); break;
            }
            break;
        }
    }
...
void CTechnique::SetCurrentBlock(char const* name, uint32_t _dwNumParams, void* pParams)
{
    if (name && name[0]) {
