...
void Effects::compile(char const* fxPath)
{
    debugMsg_ = fxPath;
    ID3DXEffect* fx;
    ...
    while (technique != nullptr) {
        ...
        auto const  len = strlen(desc.Name);
...
bool Effects::begin(std::string const& techniqueName)
{
    // transform to lowercase to be compliant with the original code
    std::string name_in_lowercase;
...
ID3DXEffect* Effects::getEffectPointer(std::string const& techniqueName)
{
    // transform to lowercase to be compliant with the original code
    std::string name_in_lowercase;
    name_in_lowercase.reserve(techniqueName.length());
