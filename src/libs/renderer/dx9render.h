...
#define COLOR2VECTOR4(a) \
    CVECTOR4( \
        float((a & 0xFF0000) >> 0x10) / 255.0f, \
        float((a & 0xFF00) >> 0x8) / 255.0f, \
        float(a & 0xFF) / 255.0f, \
...
public:
    ...
    virtual void RenderAnimation(
        int32_t ib, void* src, int32_t numVrts, int32_t minv, int32_t numv, int32_t startidx, int32_t numtrg, bool isUpdateVB) = 0;
    ...
    virtual HRESULT SetClipPlane(uint32_t Index, float const* pPlane) = 0;
    ...
    virtual int32_t TextureCreate(char const* fname)                                                                    = 0;
    ...
    virtual int32_t Print(int32_t x, int32_t y, char const* format, ...)                                   = 0;
    virtual int32_t Print(int32_t nFontNum, uint32_t color, int32_t x, int32_t y, char const* format, ...) = 0;
    virtual int32_t ExtPrint(
        int32_t     nFontNum,
        uint32_t    foreColor,
        uint32_t    backColor,
        int         wAlignment,
        ...
        char const* format,
        ...)                                                                                                                    = 0;
    virtual int32_t StringWidth(std::string_view const& string, int32_t nFontNum = 0, float fScale = 1.f, int32_t scrWidth = 0) = 0;
    ...
    virtual int32_t LoadFont(char const* fontName)          = 0;  // returns the number \ font id, or -1 on error
    virtual bool    UnloadFont(char const* fontName)        = 0;  // returns true if the font is still in use
    ...
    virtual bool    SetCurFont(char const* fontName)        = 0;  // returns true if the given font is installed
    ...
    virtual bool    SetFontIniFileName(char const* iniName) = 0;

    // DX9Render: Techniques Section
    virtual bool TechniqueExecuteStart(char const* cBlockName) = 0;
    ...
    virtual void DrawRects(
        RS_RECT*    pRSR,
        uint32_t    dwRectsNum,
        char const* cBlockName     = nullptr,
        uint32_t    dwSubTexturesX = 1,
        uint32_t    dwSubTexturesY = 1,
        float       fScaleX        = 1.0f,
        float       fScaleY        = 1.0f)                                                                                                = 0;
    virtual void DrawSprites(RS_SPRITE* pRSS, uint32_t dwSpritesNum, char const* cBlockName = nullptr)                       = 0;
    virtual void DrawLines(RS_LINE* pRSL, uint32_t dwLinesNum, char const* cBlockName = nullptr)                             = 0;
    virtual void DrawVector(const CVECTOR& v1, const CVECTOR& v2, uint32_t dwColor, char const* pTechniqueName = "DXVector") = 0;
    virtual void DrawLines2D(RS_LINE2D* pRSL2D, size_t dwLinesNum, char const* cBlockName = nullptr)                         = 0;
    ...
    virtual void DrawBuffer(
        ...
        size_t      startidx,
        size_t      numtrg,
        char const* cBlockName = nullptr) = 0;
    virtual void DrawIndexedPrimitiveNoVShader(
    ...
    virtual CVideoTexture* GetVideoTexture(char const* sVideoName)       = 0;
    virtual void           ReleaseVideoTexture(CVideoTexture* pVTexture) = 0;

    // DX9Render: Vertex/Index Buffers Section
    virtual int32_t CreateVertexBuffer(int32_t type, size_t nverts, uint32_t usage, uint32_t dwPool = D3DPOOL_DEFAULT) = 0;
    ...
    virtual HRESULT
    CreateVertexBuffer(UINT Length, uint32_t Usage, uint32_t FVF, D3DPOOL Pool, IDirect3DVertexBuffer9** ppVertexBuffer)       = 0;
    virtual HRESULT VBLock(IDirect3DVertexBuffer9* pVB, UINT OffsetToLock, UINT SizeToLock, uint8_t** ppbData, uint32_t Flags) = 0;
    ...
    virtual HRESULT GetCubeMapSurface(
        IDirect3DCubeTexture9* ppCubeTexture, D3DCUBEMAP_FACES FaceType, UINT Level, IDirect3DSurface9** ppCubeMapSurface) = 0;
    virtual HRESULT
    CreateTexture(UINT Width, UINT Height, UINT Levels, uint32_t Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DTexture9** ppTexture) = 0;
    virtual HRESULT CreateCubeTexture(
        UINT EdgeLength, UINT Levels, uint32_t Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DCubeTexture9** ppCubeTexture) = 0;
    ...
    virtual HRESULT CreateDepthStencilSurface(
        UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, IDirect3DSurface9** ppSurface) = 0;
    ...
    virtual HRESULT LockRect(
        ...
        const RECT*            pRect,
        uint32_t               Flags)                                                                                                                = 0;
    virtual HRESULT LockRect(IDirect3DTexture9* ppTexture, UINT Level, D3DLOCKED_RECT* pLockedRect, const RECT* pRect, uint32_t Flags) = 0;
    ...
    virtual HRESULT CreatePixelShader(uint32_t const* pFunction, IDirect3DPixelShader9** ppShader)                          = 0;
    virtual HRESULT CreateVertexShader(uint32_t const* pFunction, IDirect3DVertexShader9** ppShader)                        = 0;
    ...
    virtual HRESULT SetVertexShaderConstantF(UINT StartRegister, float const* pConstantData, UINT Vector4iCount) = 0;
    virtual HRESULT SetPixelShaderConstantF(UINT StartRegister, float const* pConstantData, UINT Vector4iCount)  = 0;
    ...
    virtual HRESULT Clear(uint32_t Count, const D3DRECT* pRects, uint32_t Flags, D3DCOLOR Color, float Z, uint32_t Stencil) = 0;
    virtual HRESULT BeginScene()                                                                                            = 0;
    virtual HRESULT EndScene()                                                                                              = 0;

    virtual HRESULT ImageBlt(char const* pName, RECT* pDstRect = nullptr, RECT* pSrcRect = nullptr)  = 0;
    virtual HRESULT ImageBlt(int32_t nTextureId, RECT* pDstRect = nullptr, RECT* pSrcRect = nullptr) = 0;

    virtual void SetProgressImage(char const* image)     = 0;
    virtual void SetProgressBackImage(char const* image) = 0;
    virtual void SetTipsImage(char const* image)         = 0;
    ...
    virtual bool
    SetRenderTarget(IDirect3DCubeTexture9* pCubeTex, uint32_t dwFaceType, uint32_t dwLevel, IDirect3DSurface9* pNewZStencil) = 0;
    virtual void           SetView(CMatrix const& mView)                                                                     = 0;
    virtual void           SetWorld(CMatrix const& mView)                                                                    = 0;
    virtual void           SetProjection(CMatrix const& mView)                                                               = 0;
    virtual CMatrix const& GetView()                                                                                         = 0;
    virtual CMatrix const& GetWorld()                                                                                        = 0;
    virtual CMatrix const& GetProjection()                                                                                   = 0;

    virtual IDirect3DVolumeTexture9* CreateVolumeTexture(
        uint32_t Width, uint32_t Height, uint32_t Depth, uint32_t Levels, uint32_t Usage, D3DFORMAT Format, D3DPOOL Pool) = 0;
