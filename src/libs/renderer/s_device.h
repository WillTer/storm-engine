...
public:
    ...
    void RenderAnimation(
        int32_t ib, void* src, int32_t numVrts, int32_t minv, int32_t numv, int32_t startidx, int32_t numtrg, bool isUpdateVB) override;
    ...
    HRESULT SetClipPlane(uint32_t Index, float const* pPlane) override;
    ...
    int32_t TextureCreate(char const* fname) override;
    ...
    int32_t Print(int32_t x, int32_t y, char const* format, ...) override;
    int32_t Print(int32_t nFontNum, uint32_t color, int32_t x, int32_t y, char const* format, ...) override;
    int32_t ExtPrint(
        int32_t     nFontNum,
        uint32_t    foreColor,
        uint32_t    backColor,
        ...
        char const* format,
        ...) override;
    int32_t StringWidth(std::string_view const& string, int32_t nFontNum = 0, float fScale = 1.f, int32_t scrWidth = 0) override;
    ...
    int32_t LoadFont(char const* fontName) override;    // returns the number \ font id, or -1 on error
    bool    UnloadFont(char const* fontName) override;  // returns true if the font is still in use
    ...
    bool    SetCurFont(char const* fontName) override;  // returns true if the given font is installed
    ...
    bool    SetFontIniFileName(char const* iniName) override;

    // DX9Render: Techniques Section
    bool TechniqueExecuteStart(char const* cBlockName) override;
    ...
    void DrawRects(
        ...
        float       fScaleX        = 1.0f,
        float       fScaleY        = 1.0f) override;
    void DrawSprites(RS_SPRITE* pRSS, uint32_t dwSpritesNum, char const* cBlockName = nullptr) override;
    void DrawLines(RS_LINE* pRSL, uint32_t dwLinesNum, char const* cBlockName = nullptr) override;
    void DrawLines2D(RS_LINE2D* pRSL2D, size_t dwLinesNum, char const* cBlockName = nullptr) override;
    ...
    void DrawBuffer(
        ...
        size_t      startidx,
        size_t      numtrg,
        char const* cBlockName = nullptr) override;
    void DrawIndexedPrimitiveNoVShader(
    ...
    CVideoTexture* GetVideoTexture(char const* sVideoName) override;
    ...
    HRESULT GetCubeMapSurface(
        IDirect3DCubeTexture9* ppCubeTexture, D3DCUBEMAP_FACES FaceType, UINT Level, IDirect3DSurface9** ppCubeMapSurface) override;
    HRESULT CreateTexture(
        UINT Width, UINT Height, UINT Levels, uint32_t Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DTexture9** ppTexture) override;
    HRESULT CreateCubeTexture(
        UINT EdgeLength, UINT Levels, uint32_t Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DCubeTexture9** ppCubeTexture) override;
    HRESULT CreateOffscreenPlainSurface(UINT Width, UINT Height, D3DFORMAT Format, IDirect3DSurface9** ppSurface) override;
    HRESULT CreateDepthStencilSurface(
        UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, IDirect3DSurface9** ppSurface) override;
    ...
    HRESULT CreatePixelShader(uint32_t const* pFunction, IDirect3DPixelShader9** ppShader) override;
    HRESULT CreateVertexShader(uint32_t const* pFunction, IDirect3DVertexShader9** ppShader) override;
    ...
    HRESULT SetVertexShaderConstantF(UINT StartRegister, float const* pConstantData, UINT Vector4iCount) override;
    HRESULT SetPixelShaderConstantF(UINT StartRegister, float const* pConstantData, UINT Vector4iCount) override;
    ...
    HRESULT ImageBlt(char const* pName, RECT* pDstRect, RECT* pSrcRect) override;
    ...
    void SetProgressImage(char const* image) override;
    void SetProgressBackImage(char const* image) override;
    void SetTipsImage(char const* image) override;
    void StartProgressView() override;
    void ProgressView() override;
    void EndProgressView() override;

    static uint32_t const   rectsVBuffer_SizeInRects;
    ...
    void MakeDrawVector(
        RS_LINE*       pLines,
        uint32_t       dwNumSubLines,
        CMatrix const& mMatrix,
        CVECTOR        vUp,
        CVECTOR        v1,
        CVECTOR        v2,
        float          fScale,
        uint32_t       dwColor);
    void DrawVector(const CVECTOR& v1, const CVECTOR& v2, uint32_t dwColor, char const* pTechniqueName = "DXVector") override;
    ...
    void SetView(CMatrix const& mView) override;
    void SetWorld(CMatrix const& mView) override;
    void SetProjection(CMatrix const& mView) override;
    CMatrix const& GetView() override;
    CMatrix const& GetWorld() override;
    CMatrix const& GetProjection() override;

    IDirect3DVolumeTexture9* CreateVolumeTexture(
        uint32_t Width, uint32_t Height, uint32_t Depth, uint32_t Levels, uint32_t Usage, D3DFORMAT Format, D3DPOOL Pool) override;
