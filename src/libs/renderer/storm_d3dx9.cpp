...
#include <libs/util/platform/d3dx9.hpp>
...
const struct pixel_format_desc* get_format_info(D3DFORMAT format)
{
    ...
    while (formats[i].format != format && formats[i].format != D3DFMT_UNKNOWN)
        i++;
    if (formats[i].format == D3DFMT_UNKNOWN) { WARN("Unknown format %#x.\n", format); }
    return &formats[i];
}
...
HRESULT
lock_surface(IDirect3DSurface9* surface, const RECT* surface_rect, D3DLOCKED_RECT* lock, IDirect3DSurface9** temp_surface, bool write)
{
    ...
    *temp_surface = NULL;
    if (FAILED(hr = IDirect3DSurface9_LockRect(surface, lock, surface_rect, lock_flag))) {
        ...
        hr = write
            ? IDirect3DDevice9_CreateOffscreenPlainSurface(device, width, height, desc.Format, D3DPOOL_SYSTEMMEM, temp_surface, NULL)
            : IDirect3DDevice9_CreateRenderTarget(device, width, height, desc.Format, D3DMULTISAMPLE_NONE, 0, TRUE, temp_surface, NULL);
        if (FAILED(hr)) {
            WARN(
        ...
        if (write || SUCCEEDED(hr = IDirect3DDevice9_StretchRect(device, surface, surface_rect, *temp_surface, NULL, D3DTEXF_NONE)))
...
{
    if (format->type == FORMAT_ARGB || format->type == FORMAT_ARGBF16 || format->type == FORMAT_ARGBF || format->type == FORMAT_DXT)
        return TRUE;
...
static void init_argb_conversion_info(
    const struct pixel_format_desc* srcformat, const struct pixel_format_desc* destformat, struct argb_conversion_info* info)
{
    UINT i;
...
static void get_relevant_argb_components(const struct argb_conversion_info* info, const BYTE* col, DWORD* out)
{
    ...
    for (i = 0; i < 4; ++i) {
        if (!info->process_channel[i]) continue;
...
static void format_to_vec4(const struct pixel_format_desc* format, const BYTE* src, struct vec4* dst)
{
    ...
    for (c = 0; c < 4; ++c) {
        static unsigned int const component_offsets[4] = {3, 0, 1, 2};
        float*                    dst_component        = (float*)dst + component_offsets[c];

        if (format->bits[c]) {
            mask = ~0u >> (32 - format->bits[c]);

            memcpy(&tmp, src + format->shift[c] / 8, min(sizeof(DWORD), (format->shift[c] % 8 + format->bits[c] + 7) / 8));

            if (format->type == FORMAT_ARGBF16) WARN("Unimplemented for FORMAT_ARGBF16");
...
static void format_from_vec4(const struct pixel_format_desc* format, const struct vec4* src, BYTE* dst)
{
    ...
    for (c = 0; c < 4; ++c) {
        static unsigned int const component_offsets[4] = {3, 0, 1, 2};
        float const               src_component        = *((float const*)src + component_offsets[c]);
            ...
            for (x = 0; x < min_width; x++) {
                if (!src_format->to_rgba && !dst_format->from_rgba && src_format->type == dst_format->type
                    ...
                    if (color_key) {
                        ...
                        if (ck_pixel == color_key) val &= ~conv_info.destmask[0];
                    }
                    ...
                    if (ck_format) {
                        ...
                        if (ck_pixel == color_key) tmp.w = 0.0f;
                    }
    ...
    D3DCOLOR            color_key)
{
    ...
    if (!dst_surface || !src_memory || !src_rect) { return D3DERR_INVALIDCALL; }
    if (src_format == D3DFMT_UNKNOWN || src_rect->left >= src_rect->right || src_rect->top >= src_rect->bottom) { return E_FAIL; }
    ...
    if (dst_rect_aligned.right & (destformatdesc->block_width - 1) && dst_rect_aligned.right != surfdesc.Width)
        dst_rect_aligned.right =
            std::min((dst_rect_aligned.right + destformatdesc->block_width - 1) & ~(destformatdesc->block_width - 1), surfdesc.Width);
    if (dst_rect_aligned.bottom & (destformatdesc->block_height - 1) && dst_rect_aligned.bottom != surfdesc.Height)
        dst_rect_aligned.bottom =
            std::min((dst_rect_aligned.bottom + destformatdesc->block_height - 1) & ~(destformatdesc->block_height - 1), surfdesc.Height);
    ...
    if (src_format == surfdesc.Format && dst_size.width == src_size.width && dst_size.height == src_size.height && color_key == 0
        && !(src_rect->left & (srcformatdesc->block_width - 1)) && !(src_rect->top & (srcformatdesc->block_height - 1))
        && !(dst_rect->left & (destformatdesc->block_width - 1)) && !(dst_rect->top & (destformatdesc->block_height - 1))) {
        copy_pixels(
            static_cast<const BYTE*>(src_memory),
            src_pitch,
            0,
            static_cast<BYTE*>(lockrect.pBits),
            lockrect.Pitch,
            0,
            &src_size,
            srcformatdesc);
    } else /* Stretching or format conversion. */
    {
        ...
        {
            if ((filter & 0xf) != D3DX_FILTER_POINT) { WARN("Unhandled filter %#x.\n", filter); }
    ...
    D3DCOLOR            color_key)
{
    ...
    if (!dst_surface || !src_surface) return D3DERR_INVALIDCALL;
    ...
    if (!src_rect) {
        ...
        src_rect->left > src_rect->right || src_rect->right > src_desc.Width || src_rect->left < 0 || src_rect->left > src_desc.Width
        || src_rect->top > src_rect->bottom || src_rect->bottom > src_desc.Height || src_rect->top < 0 || src_rect->top > src_desc.Height) {
    ...
    if (!dst_rect) {
        ...
        dst_rect->left > dst_rect->right || dst_rect->right > dst_desc.Width || dst_rect->left < 0 || dst_rect->left > dst_desc.Width
        || dst_rect->top > dst_rect->bottom || dst_rect->bottom > dst_desc.Height || dst_rect->top < 0 || dst_rect->top > dst_desc.Height) {
        WARN("Invalid dst_rect specified.\n");
        return D3DERR_INVALIDCALL;
    }
    ...
    if (!dst_palette && !src_palette && !color_key) {
        if (src_desc.Format == dst_desc.Format && dst_size.width == src_size.width && dst_size.height == src_size.height && color_key == 0
            && !(src_rect->left & (src_format_desc->block_width - 1)) && !(src_rect->top & (src_format_desc->block_height - 1))
            && !(dst_rect->left & (dst_format_desc->block_width - 1)) && !(dst_rect->top & (dst_format_desc->block_height - 1))) {
            d3d_filter = D3DTEXF_NONE;
        } else {
            switch (filter) {
            case D3DX_FILTER_NONE: d3d_filter = D3DTEXF_NONE; break;

            case D3DX_FILTER_POINT: d3d_filter = D3DTEXF_POINT; break;

            case D3DX_FILTER_LINEAR: d3d_filter = D3DTEXF_LINEAR; break;

            default: d3d_filter = D3DTEXF_FORCE_DWORD; break;
            }
        }
        ...
        if (d3d_filter != D3DTEXF_FORCE_DWORD) {
            ...
            IDirect3DDevice9_Release(device);
            if (SUCCEEDED(hr)) return D3D_OK;
        }
    }

    if (FAILED(lock_surface(src_surface, NULL, &lock, &temp_surface, FALSE))) return D3DXERR_INVALIDDATA;

    hr = D3DXLoadSurfaceFromMemory(
        dst_surface, dst_palette, dst_rect, lock.pBits, src_desc.Format, lock.Pitch, src_palette, src_rect, filter, color_key);

    if (FAILED(unlock_surface(src_surface, NULL, temp_surface, FALSE))) return D3DXERR_INVALIDDATA;
