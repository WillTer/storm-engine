...
IVBufferManager::IVBufferManager(
    VDX9RENDER* renderer_, int32_t vertex_type, int vertex_size, size_t index_count, size_t vertex_count, size_t max_size)
    : renderer_(renderer_)
    , max_size_(max_size)
    , index_buffer_(renderer_->CreateIndexBuffer(max_size * index_count * sizeof(uint16_t)))
    , vertex_buffer_(renderer_->CreateVertexBuffer(vertex_type, max_size * vertex_count * vertex_size, D3DUSAGE_WRITEONLY))
...
IVBufferManager::~IVBufferManager()
{
    if (vertex_buffer_) { renderer_->ReleaseVertexBuffer(vertex_buffer_); }
    if (index_buffer_) { renderer_->ReleaseIndexBuffer(index_buffer_); }
}
...
int32_t IVBufferManager::ReserveElement()
{
    for (size_t i = 0; i < max_size_; i++) {
        if (!used_[i]) {
            used_[i] = true;
            if (actual_size_ == i) { ++actual_size_; }
            return i;
        }
    }
...
void IVBufferManager::GetPointers(int32_t _i, uint16_t** iPointer, void** vPointer, int32_t* vOffset /* = 0*/) const
{
    ...
    *vPointer = static_cast<uint8_t*>(vertices_) + _i * vertex_size_ * vertex_count_;
    if (vOffset) { *vOffset = _i * vertex_count_; }
}

//--------------------------------------------------------------------
void IVBufferManager::DrawBuffers(char const* _technique) const
{
    if (locked_ || !actual_size_) return;

    renderer_->DrawBuffer(
        vertex_buffer_, vertex_size_, index_buffer_, 0, actual_size_ * vertex_count_, 0, actual_size_ * index_count_ / 3, _technique);
}
