...
bool MakeLong(char** pDataPointer, int32_t* result)
{
    ...
    while (true) {
        ...
        index++;
        if (index > 8) throw std::runtime_error("inf loop");
    }
    return false;
}
...
FONT::FONT(VDX9RENDER& renderer, IDirect3DDevice9& device) : charDescriptors_(USED_CODES), renderService_(renderer), device_(device) {}
...
{
    if (initialized_) { throw std::runtime_error("Font already initialized"); }
    initialized_ = true;
    ...
    if (ini->ReadString(font_name, "Texture", buffer, sizeof(buffer) - 1, "")) { textureName_ = buffer; }
    if (ini->ReadString(font_name, "Techniques", buffer, sizeof(buffer) - 1, "")) { techniqueName_ = buffer; }
    ...
    if (ini->TestKey(font_name, "Shadow", nullptr)) drawShadows_ = true;
    ...
    for (codepoint = 30; codepoint < USED_CODES; codepoint++) {
        ...
        pData = buffer;
        if (!MakeLong(&pData, &ltmp)) throw std::runtime_error("invalid font record");
        ...
        if (!MakeLong(&pData, &ltmp)) throw std::runtime_error("invalid font record");
        charDescriptors_[codepoint].Pos.y1 = static_cast<float>(height_ - static_cast<int32_t>(ltmp * verticalAspectRatio_));
    ...
    device_.CreateVertexBuffer(
        sizeof(FONT_CHAR_VERTEX) * MAX_SYMBOLS * SYM_VERTEXS,
        D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY,
        FONT_CHAR_FVF,
        D3DPOOL_SYSTEMMEM,
        &vertexBuffer_,
        nullptr);
    if (vertexBuffer_ == nullptr) throw std::runtime_error("vbuffer error");
...
int32_t FONT::GetStringWidth(std::string_view const& text, std::optional<float> scale_override) const
{
    if (text.empty()) return 0;
    float         xoffset = 0;
    int32_t const s_num   = text.size();

    float const scale = scale_override.value_or(scale_);
    ...
    for (int32_t i = 0; i < s_num; i += utf8::u8_inc(text.data() + i)) {
        ...
        if (Codepoint > USED_CODES) {
            ...
            if constexpr (storm::kIsDebug) { throw std::runtime_error(std::format("Invalid codepoint: {}", Codepoint)); }
            continue;
        }
        ...
        if (Codepoint == ' ') { xoffset += spacebarWidth_ * scale; }
    }
...
int32_t FONT::UpdateVertexBuffer(int32_t x, int32_t y, char* data_PTR, int utf8length, float scale, uint32_t color)
{
    ...
    for (int i = 0, curLetter = 0; i < s_num; i += utf8::u8_inc(data_PTR + i), curLetter++) {
        ...
        pVertex[n + 0].color = pVertex[n + 1].color = pVertex[n + 2].color = pVertex[n + 3].color = pVertex[n + 4].color =
            pVertex[n + 5].color                                                                  = color;

        pVertex[n + 0].rhw = pVertex[n + 1].rhw = pVertex[n + 2].rhw = pVertex[n + 3].rhw = pVertex[n + 4].rhw = pVertex[n + 5].rhw = scale;
    }
    vertexBuffer_->Unlock();
    return static_cast<int32_t>(xoffset);
}
...
{
    if (data_PTR == nullptr || techniqueName_.empty()) return 0;
    auto    xoffset = 0L;
    int32_t s_num   = utf8::Utf8StringLength(data_PTR);
    if (s_num == 0) return 0;

    auto const bDraw = renderService_.TechniqueExecuteStart(techniqueName_.c_str());
    ...
    bool const     drawShadows = overrides.shadow.value_or(drawShadows_);
    float const    scale       = overrides.scale.value_or(scale_);
    uint32_t const color       = overrides.color.value_or(color_);
    if (drawShadows) {
...
{
    if (textureHandle_ != -1L) renderService_.TextureRelease(textureHandle_);
    textureHandle_ = -1L;
}

void FONT::RepeatInit()
{
    if (textureHandle_ == -1L) textureHandle_ = renderService_.TextureCreate(textureName_.c_str());
}
