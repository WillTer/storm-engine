...
void TCarcass::RebuildLevels(tCarcassVertex* _vBuffer, bool _firstDraw, uint32_t _dTime)
{
    ...
    if (_firstDraw) lengthK = 0.2f * sqrtf(~(levelStarts[(levelsCount - 1) /**measure.pointsCount*/] - levelStarts[0]));

    for (auto level = 0; level < (levelsCount); level++) {
        auto const levelK = static_cast<float>(level) / (levelsCount - 1);
        for (auto measurePoint = 0; measurePoint < measure.pointsCount; measurePoint++) {
            ...
            tempVertex->pos =
                (levelStarts[level]
                 + CVECTOR((1.f + WIDEN_K * levelK) * measure.deltaPointX[measurePoint], heightK * measure.deltaPointY[measurePoint], 0.f));
            tempVertex->pos.y += .01f;

            auto measureK = 0.f;
            //((float) abs(measurePoint - (measure.pointsCount >> 1))) / ((measure.pointsCount >> 1));
            if (!measurePoint) measureK = 1.f;
            ...
            if (_firstDraw) {
                ...
                tempVertex->color = (static_cast<uint32_t>(speedA * sqrtf(levelA) * (1.f - measureK) * 0xFF) << 24) | 0x00FFFFFF;
                auto const dv     = fmodf(_dTime * vSpeed, 1.f);
                auto const du     = fmodf(_dTime * uSpeed, 1.f);
