...
void NoiseCloud::Update(float dltTime)
{
    for (int32_t i = 0; i < sizeof(rect) / sizeof(Rect); i++) {
        ...
        auto const k   = rect[i].r / TRND_CLDRADIUS;
        auto       rot = dltTime * (2.0f - powf(k, 0.3f) * 2.2f);
        if (rot < 0.0f) rot = 0.0f;
        rect[i].a += rot;
        if (rect[i].a > 2.0f * TRND_PI) rect[i].a -= 2.0f * TRND_PI;
        rect[i].angle += rot;
        if (rect[i].angle > 2.0f * TRND_PI) rect[i].angle -= 2.0f * TRND_PI;
        rect[i].r += dltTime * TRND_CLDRADIUS * 0.1f;
        if (rect[i].r < TRND_CLDRADIUS * 0.2f) rect[i].alpha = rect[i].r / (TRND_CLDRADIUS * 0.2f);
        if (rect[i].r > TRND_CLDRADIUS * 0.6f) rect[i].alpha = 1.0f - (rect[i].r - TRND_CLDRADIUS * 0.6f) / (TRND_CLDRADIUS * 0.4f);
        if (rect[i].alpha < 0.0f) rect[i].alpha = 0.0f;
        if (rect[i].alpha > 1.0f) rect[i].alpha = 1.0f;
...
void NoiseCloud::Draw(VDX9RENDER* rs)
{
    ...
    for (int32_t i = 0; i < sizeof(rect) / sizeof(Rect); i++) {
        ...
        // Position
        auto const sn = sinf(rect[i].angle);
        auto const cs = cosf(rect[i].angle);
        ...
        buf[i * 6 + 5].v   = 1.0f;
        // Colour
        auto const clr   = rect[i].light * 200.0f;
        auto const color = (static_cast<int32_t>(clr * 1.0f) << 16) | (static_cast<int32_t>(clr * 0.95f) << 8)
