            ...
            if (IsShip()) {
                ...
                if (soundService) {
                    if (lastPlayTime <= 0.0f) {
                        ...
                        soundService->play("TornadoCrackSound", PCM_3D, VOLUME_FX, false, false, 0, &pos);
...
void Debris::AddModel(char const* modelName, float prt, float spd)
{
...
{
    auto const rnd = rand() * (1.0f / RAND_MAX);
    auto       sum = 0.0f;
    int32_t    i;
