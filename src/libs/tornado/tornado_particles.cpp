...
void TornadoParticles::Update(float dltTime)
{
    ...
    for (int32_t i = 0; i < sizeof(groundPrt) / sizeof(GroundParticle); i++) {
        ...
        auto const t = groundPrt[i].t;
        if (t == 0) continue;  // eddy. don't need an assert crash
        // Assert(t);
        ...
        // Position
        auto const r       = groundPrt[i].r * ((1.0f - t) * (1.0f - t) * 0.7f + 0.3f);
    ...
    for (int32_t i = 0; i < sizeof(pillarPrt) / sizeof(PillarParticle); i++) {
        ...
        auto const k  = pillarPrt[i].k;
        pillarPrt[i].pos.y += k * dltTime * (20.0f + 40.0f * kh * kh);
        if (pillarPrt[i].pos.y >= pillar.GetHeight()) pillarPrt[i].pos.y = 0.0f;
        auto const x = pillar.GetX(pillarPrt[i].pos.y);
        auto const z = pillar.GetZ(pillarPrt[i].pos.y);
        auto const r = pillar.GetRaduis(pillarPrt[i].pos.y) - pillarPrt[i].size * 0.25f;
...
inline void TornadoParticles::DrawParticles(VDX9RENDER* rs, void* prts, int32_t num, int32_t size, int32_t texture, char const* tech)
{
    ...
    for (int32_t i = 0; i < num; i++) {
        ...
        auto const color        = (static_cast<int32_t>(parts->alpha * galpha) << 24) | 0x00ffffff;
