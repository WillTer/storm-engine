...
bool MODELR::Init(std::shared_ptr<storm::ServiceLocator> const& service_locator)
{
...
uint64_t MODELR::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_MODEL_SET_TECHNIQUE: {
        str = message.String();
        if (root) SetChildrenTechnique(root, str.c_str());
    } break;
