...
bool NODER::Clip()
{
    ...
    auto const sph_dist = ~(*clip_c - glob_mtx * center);
    ...
    if (flags & CLIP_ENABLE && sph_dist <= (geo_radius + clip_r) * (geo_radius + clip_r)) {
        ...
        for (int32_t p = 0; p < clip_nps; p++) {
            auto const x     = clip_p[p].D * clip_p[p].Nx - glob_mtx.m[3][0];
            auto const y     = clip_p[p].D * clip_p[p].Ny - glob_mtx.m[3][1];
            auto const z     = clip_p[p].D * clip_p[p].Nz - glob_mtx.m[3][2];
            auto const Nx    = glob_mtx.m[0][0] * clip_p[p].Nx + glob_mtx.m[0][1] * clip_p[p].Ny + glob_mtx.m[0][2] * clip_p[p].Nz;
            auto const Ny    = glob_mtx.m[1][0] * clip_p[p].Nx + glob_mtx.m[1][1] * clip_p[p].Ny + glob_mtx.m[1][2] * clip_p[p].Nz;
            auto const Nz    = glob_mtx.m[2][0] * clip_p[p].Nx + glob_mtx.m[2][1] * clip_p[p].Ny + glob_mtx.m[2][2] * clip_p[p].Nz;
            auto const lx    = glob_mtx.m[0][0] * x + glob_mtx.m[0][1] * y + glob_mtx.m[0][2] * z;
            auto const ly    = glob_mtx.m[1][0] * x + glob_mtx.m[1][1] * y + glob_mtx.m[1][2] * z;
            auto const lz    = glob_mtx.m[2][0] * x + glob_mtx.m[2][1] * y + glob_mtx.m[2][2] * z;
            clip_gp[p].nrm.x = Nx;
            clip_gp[p].nrm.y = Ny;
            clip_gp[p].nrm.z = Nz;
    ...
    if (flags & CLIP_ENABLE_TREE)
        for (int32_t l = 0; l < next.size(); l++)
            if (next[l] != nullptr && static_cast<NODER*>(next[l])->Clip() == true) retval = true;
    return retval;
}
...
float NODER::Update(CMatrix& mtx, CVECTOR& cnt)
{
    ...
    for (int32_t l = 0; l < next.size(); l++)
        if (next[l] != nullptr) {
            ...
            auto const r   = static_cast<NODER*>(next[l])->Update(glob_mtx, cnt);
            auto const rad = sqrtf(~(cnt - center)) + r;
...
{
    if (isReleased) return 2.0f;
    // check for bounding spheres intersection
    auto const lmn       = dst - src;
    auto const dist2ray2 = ~((glob_mtx * center - src) ^ lmn);
    auto const dlmn      = ~(lmn);
    // hierarchy test
    ...
    if (flags & TRACE_ENABLE_TREE)
        for (int32_t n = 0; n < next.size(); n++) {
            ...
            auto const d = static_cast<NODER*>(next[n])->Trace(src, dst);
            if (d < best_dist) {
                best_dist     = d;
    ...
    if (flags & TRACE_ENABLE && dist2ray2 < dlmn * geo_radius * geo_radius) {
        ...
        auto const ds = geo->Trace((GEOS::VERTEX&)_src, (GEOS::VERTEX&)_dst);
        if (ds < best_dist) {
            best_dist     = ds;
            bestTraceNode = this;
        }
    ...
    char const* lightPath, char const* pname, char const* oname, CMatrix const& m, CMatrix const& globm, NODER* par, char const* lmPath)
{
    ...
    if (lightPath) sys_LightPath = lightPath;

    if (lmPath) sys_lmPath = lmPath;
    ...
    auto const idGeo      = geo->FindName("geometry");
...
void NODER::RestoreGeometry()
{
    ...
    auto const* const tPath  = gs->GetTexturePath();
    auto const        len    = strlen(tPath) + 1;
...
void NODER::Draw()
{
    ...
    auto const cnt = glob_mtx * center;

    // visibility check
    int32_t p;
    for (p = 0; p < 4; p++) {
        auto const dist = cnt.x * ViewPlane[p].nrm.x + cnt.y * ViewPlane[p].nrm.y + cnt.z * ViewPlane[p].nrm.z - ViewPlane[p].d;
    ...
    if (flags & VISIBLE) {
        ...
        int32_t p;
        for (p = 0; p < 4; p++) {
            float const dist = cnt.x * ViewPlane[p].nrm.x + cnt.y * ViewPlane[p].nrm.y + cnt.z * ViewPlane[p].nrm.z - ViewPlane[p].d;
        ...
        if (p == 4) {
            ...
            for (p = 0; p < 4; p++) {
                ...
                float const y = ViewPlane[p].d * ViewPlane[p].nrm.y - glob_mtx.m[3][1];
                float const z = ViewPlane[p].d * ViewPlane[p].nrm.z - glob_mtx.m[3][2];
                float const Nx =
                    glob_mtx.m[0][0] * ViewPlane[p].nrm.x + glob_mtx.m[0][1] * ViewPlane[p].nrm.y + glob_mtx.m[0][2] * ViewPlane[p].nrm.z;
                float const Ny =
                    glob_mtx.m[1][0] * ViewPlane[p].nrm.x + glob_mtx.m[1][1] * ViewPlane[p].nrm.y + glob_mtx.m[1][2] * ViewPlane[p].nrm.z;
                float const Nz =
                    glob_mtx.m[2][0] * ViewPlane[p].nrm.x + glob_mtx.m[2][1] * ViewPlane[p].nrm.y + glob_mtx.m[2][2] * ViewPlane[p].nrm.z;
                float const lx      = glob_mtx.m[0][0] * x + glob_mtx.m[0][1] * y + glob_mtx.m[0][2] * z;
                float const ly      = glob_mtx.m[1][0] * x + glob_mtx.m[1][1] * y + glob_mtx.m[1][2] * z;
                float const lz      = glob_mtx.m[2][0] * x + glob_mtx.m[2][1] * y + glob_mtx.m[2][2] * z;
    ...
    if (flags & VISIBLE_TREE)
        for (int32_t l = 0; l < next.size(); l++)
            if (next[l] != nullptr) static_cast<NODER*>(next[l])->Draw();
}

//----------------------------------------------------------
// NODE get node by number
//----------------------------------------------------------
NODER* NODER::FindNode(char const* cNodeName)
{
    if (storm::iEquals(cNodeName, name)) return this;
    for (int32_t i = 0; i < next.size(); i++) {
        if (!next[i]) continue;
...
//
//-------------------------------------------------------------------
void NODER::SetTechnique(char const* name)
{
...
void NODER::SetMaxViewDist(float fDist)
{
    max_view_dist = fDist;
    for (int32_t n = 0; n < next.size(); n++)
        if (next[n]) static_cast<NODER*>(next[n])->SetMaxViewDist(fDist);
}
