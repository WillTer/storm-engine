#pragma once

#include <libs/collide/object.h>
#include <libs/geometry/geos.h>
#include <libs/math/matrix.h>

class Animation;
class NODER;
class NODE
{
public:
    CMatrix loc_mtx;
    CMatrix glob_mtx;

    enum FLAGS {
        VISIBLE           = (1 << 0),
        VISIBLE_TREE      = (1 << 1),
        CLIP_ENABLE       = (1 << 2),
        CLIP_ENABLE_TREE  = (1 << 3),
        TRACE_ENABLE      = (1 << 4),
        TRACE_ENABLE_TREE = (1 << 5)
    };

    int32_t flags;

    GEOS*               geo;
    GEOS::MATERIAL_FUNC geoMaterialFunc;
    // children
    std::vector<NODE*> next;
    // 0 for root
    NODE* parent;

    virtual ~NODE() {};
    // name of this node - locator's name
    virtual char const* GetName() = 0;

    // unlink node
    virtual NODE* Unlink() = 0;
    // unlink node to model
    virtual entid_t Unlink2Model() = 0;
    // link node to node
    virtual void Link(NODE* node) = 0;
    // link model to node
    virtual void Link(entid_t model, bool transform = true) = 0;

    virtual void        SetTechnique(char const* name) = 0;
    virtual char const* GetTechnique()                 = 0;
    virtual bool        Init(
               char const*    lightPath,
               char const*    pname,
               char const*    oname,
               CMatrix const& m,
               CMatrix const& globm,
               NODER*         par,
               char const*    lmPath) = 0;

    virtual float Trace(const CVECTOR& src, const CVECTOR& dst) = 0;

    virtual void SubstituteGeometry(std::string const& new_model) = 0;
};

class MODEL: public COLLISION_OBJECT
{
public:
    class RenderTuner
    {
        friend MODEL;

    public:
        RenderTuner() {};

        virtual ~RenderTuner() {};

        virtual void Set(MODEL* model, /*VDX9RENDER*/ void* rs) {};

        virtual void Restore(MODEL* model, /*VDX9RENDER*/ void* rs) {};
    };

    void SetRenderTuner(RenderTuner* rt)
    {
        renderTuner = rt;
    };

    RenderTuner* GetRenderTuner(RenderTuner* rt) const
    {
        return renderTuner;
    };

protected:
    RenderTuner* renderTuner;

public:
public:
    MODEL()
    {
        renderTuner = nullptr;
    };

    ~MODEL() override
    {
        renderTuner = nullptr;
    };

    virtual NODE*      GetNode(int32_t n)              = 0;
    virtual NODE*      FindNode(char const* cNodeName) = 0;
    virtual void       Update()                        = 0;
    virtual Animation* GetAnimation()                  = 0;

    float Trace(const CVECTOR& src, const CVECTOR& dst) override                                                             = 0;
    bool  Clip(const PLANE* planes, int32_t nplanes, const CVECTOR& center, float radius, ADD_POLYGON_FUNC addpoly) override = 0;

    char const* GetCollideMaterialName() override               = 0;
    bool        GetCollideTriangle(TRIANGLE& triangle) override = 0;

    virtual NODE* GetCollideNode() = 0;
};
