...
public:
    ...
    NODER*      FindNode(char const* cNodeName);
    ...
    char const* GetTechnique() override;

    // replace only this node model without touching anything else
    void SubstituteGeometry(std::string const& new_model) override;
...
public:
    ...
    NODE*      FindNode(char const* cNodeName) override;
