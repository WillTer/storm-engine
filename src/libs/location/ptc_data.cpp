...
PtcData::PtcData() : isSlide(false), slideDir(), isBearing(false), stepPos {}
{
...
{
    // Position on the map
    auto const mapX = static_cast<int32_t>((pos.x - min.x) / ws);
    auto const mapZ = static_cast<int32_t>((pos.z - min.z) / ls);
    ...
    for (int32_t i = 0; i < m.size; i++) {
        ...
        for (j = 0; j < 3; j++) {
            ...
            // Edge normal
            float const nx = -(ve.z - vs.z);
            float const nz = (ve.x - vs.x);
            // If outside the triangle, break
            if (pos.x * nx + pos.z * nz > vs.x * nx + vs.z * nz) break;
        }
        if (j == 3) {
            // Check the height
            float const d = FindHeight(indeces[m.start + i], pos.x, pos.z);
            if (dist >= 0.0f) {
...
// Find the direction of the path
bool PtcData::FindPathDir(int32_t step, int32_t curNode, const CVECTOR& cur, int32_t toNode, const CVECTOR& to, int32_t& node, CVECTOR& pos)
{
    ...
    uint8_t const v    = (line[toNode >> 2] >> ((toNode & 3) * 2)) & 3;
    if (v == 3) return false;
    // Edge
    ...
    int32_t const nb = triangle[curNode].nb[v];
...
CVECTOR PtcData::FindEdgePoint(const CVECTOR& vs, const CVECTOR& ve, const CVECTOR& cur, const CVECTOR& to)
{
    ...
    if (nl > 0.0f) {
        // Plane
        nrm *= 1.0f / sqrtf(nl);
        float const dist = nrm | vs;
        // Destination position
        float const dTo = (to | nrm) - dist;
        if (dTo <= 0.0f) {
            // Destination behind the edge
            float const dCur = (cur | nrm) - dist;
...
float PtcData::Trace(const CVECTOR& s, const CVECTOR& d) const
{
    ...
    int32_t const mnX = static_cast<int32_t>((pmin.x - min.x) / ws);
    int32_t const mnZ = static_cast<int32_t>((pmin.z - min.z) / ls);
    int32_t const mxX = static_cast<int32_t>((pmax.x - min.x) / ws);
    int32_t const mxZ = static_cast<int32_t>((pmax.z - min.z) / ls);
    ...
    for (int32_t zi = mnZ; zi <= mxZ; zi++) {
        ...
        for (int32_t xi = mnX; xi <= mxX; xi++) {
            ...
            for (int32_t i = 0; i < m.size; i++) {
                float const kn = Trace(triangle[ids[i]], s, d);
                if (kn < k) k = kn;
            }
        }
    }
...
float PtcData::Trace(PtcTriangle& trg, const CVECTOR& s, const CVECTOR& d) const
{
    ...
    float const dst = n | *(CVECTOR*)&vertex[trg.i[0]];
    // Distance of the segment to the plane
    float const d1 = (n | s) - dst;
    float const d2 = (n | d) - dst;
...
void PtcData::FindForce(int32_t curNode, CVECTOR& force) const
{
    ...
    for (int32_t i = 0; i < 3; i++) {
        if (nb[i] >= 0) continue;
        // Normal to the edge
        int32_t const s = triangle[curNode].i[i];
        int32_t const e = triangle[curNode].i[i < 2 ? i + 1 : 0];
        // Edge vertices
        ...
        float       nx = (ve.z - vs.z);
        float       nz = -(ve.x - vs.x);
        float const nl = sqrtf(nx * nx + nz * nz);
        if (!nl) continue;
        nx /= nl;
        nz /= nl;
        // Repulsive force
...
// Get node material
char const* PtcData::GetMaterial(int32_t curNode)
{
    if (!materials) return nullptr;
    if (curNode < 0 || curNode >= numTriangles) return nullptr;
    int32_t const mtl = triangle[curNode].mtl;
