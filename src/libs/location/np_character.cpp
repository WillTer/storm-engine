...
uint32_t NPCharacter::ChlProcessMessage(int32_t messageID, MESSAGE& message)
{
    ...
    case MSG_NPCHARACTER_PUSHTASK: return PushTask();
    case MSG_NPCHARACTER_POPTASK: return PopTask();
...
void NPCharacter::Move(float dltTime)
{
    // process the task
    switch (task.task) {
    case npct_none: break;
    case npct_stay: break;
    case npct_gotopoint: break;
    case npct_followcharacter: break;
    case npct_fight: break;
    case npct_escape: break;
    case npct_dead: break;
    }
...
void NPCharacter::Update(float dltTime)
{
    ...
    switch (task.task) {
    case npct_none: break;
    case npct_stay: SetFightMode(false); break;
    case npct_gotopoint: break;
    case npct_followcharacter: UpdateFollowCharacter(dltTime); break;
    case npct_fight: UpdateFightCharacter(dltTime); break;
    case npct_escape: UpdateEscapeCharacter(dltTime); break;
    case npct_dead: break;
    }
    ...
    if (location->IsDebugView()) {
        ...
        location->Print(
            curPos + CVECTOR(0.0f, height, 0.0f),
            rad,
            2,
            1.0f,
            0xffffff,
            ...
            GetTaskName(lastSetTask));
        if (isDebugEx) location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, 1, 1.0f, 0xffffff, 0.5f, "target(%s)", fid);
        if (isDebugEx)
            location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, 0, 1.0f, 0xffffff, 0.5f, "cmd(%s)", GetCommandName(command.cmd));
        if (isDebugEx)
            location->Print(
                curPos + CVECTOR(0.0f, height, 0.0f), rad, -1, 1.0f, 0xffffff, 0.5f, "pos(%.2f, %.2f, %.2f)", curPos.x, curPos.y, curPos.z);
        if (isDebugEx)
            location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, -2, 1.0f, 0xffffff, 0.5f, "isJump = %s", isJump ? "true" : "false");
        int32_t line = 4;
        ...
        if (isDebugEx && AttributesPointer) {
            ...
            if (atr) {
                for (int32_t i = atr->GetAttributesNum() - 1; i >= 0; i--) {
                    ...
                    if (fid && id)
                        location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, line++, 1.0f, 0xffffff, 0.5f, "tmpl.%s(%s)", fid, id);
                }
                if (atr->HasValue()) {
                    location->Print(
                        curPos + CVECTOR(0.0f, height, 0.0f),
                        ...
                        0.5f,
                        "tmpl(%s)",
                        static_cast<char const*>(atr->GetThisAttr()));
                }
            }
            ...
            if (atr) {
                for (int32_t i = atr->GetAttributesNum() - 1; i >= 0; i--) {
                    ...
                    id  = atr->GetAttribute(i);
                    if (fid && id)
                        location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, line++, 1.0f, 0xffffff, 0.5f, "type.%s(%s)", fid, id);
                }
                if (atr->HasValue())
                    location->Print(
                        curPos + CVECTOR(0.0f, height, 0.0f),
            ...
            if (atr) {
                id = "";
                if (atr->HasValue()) id = atr->GetThisAttr();
                location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, line++, 1.0f, 0xffffff, 0.5f, "group(\"%s\")", id);
            }
        }
        location->Print(
            curPos + CVECTOR(0.0f, height, 0.0f), rad, line++, 1.0f, 0xffffff, 0.5f, "wantToAttack = %s", wantToAttack ? "true" : "false");
        location->Print(
            curPos + CVECTOR(0.0f, height, 0.0f),
            rad,
            line++,
        ...
        location->Print(
            curPos + CVECTOR(0.0f, height, 0.0f), rad, line++, 1.0f, 0xffffff, 0.5f, "wantToFire = %s)", wantToFire ? "true" : "false");
    }
}
...
bool NPCharacter::SetNewTask(NPCTask tsk, MESSAGE& message)
{
    ...
    case npct_none:
        ...
        CmdStay();
        return CmdNone();
    case npct_stay: SetFightMode(false); return CmdStay();
    case npct_gotopoint:
        SetFightMode(false);
        SetRunMode(false);
    ...
    case npct_fight: task.target = message.EntityID(); return InitFightChartacter(task.target);
    case npct_escape:
...
void NPCharacter::UpdateFollowCharacter(float dltTime)
{
    ...
    float const dst = ~(c->curPos - curPos);
...
void NPCharacter::UpdateEscapeCharacter(float dltTime)
{
    ...
    float const fDist = (bMusketer) ? fMusketerDistance * 0.7f : NPC_STOP_ESCAPE;
...
void NPCharacter::UpdateFightCharacter(float dltTime)
{
    ...
    if (fgtCurType == fgt_none && priorityAction.name && shot.name) {
        if (storm::iEquals(priorityAction.name, shot.name)) { bCurrentActionIsFire = true; }
    }

    bool const bGunLoaded = IsGunLoad();

    if (fgtSetType == fgt_fire || fMusketerFireTime > 0.0f) bCurrentActionIsFire = true;
    ...
    bool const bVisTarget = VisibleTest(c);
    bool const bFarTarget = dst > NPC_FIGHT_FIRE_DIST * NPC_FIGHT_FIRE_DIST;
    ...
    if (bMusketer && bTryFire && fMusketerCheckFireTime <= 0.0f) {
        ...
        if (!bVisTarget && !target) {
            ...
            float const   fCurDist = sqrtf(~(vOurPos - vPos));
            const CVECTOR vDir     = !(vOurPos - vPos);
    ...
    // if (bTryFire)
    {
        if (command.cmd != aicmd_gotopoint && !bCurrentActionIsFire) {
            float const fDist   = (bMusketer && !bMusketerNoMove) ? fMusketerDistance * 0.9f : NPC_FIGHT_GO_DIST;
            float const fDistTo = (bMusketer && !bMusketerNoMove) ? fMusketerDistance * 0.7f : NPC_FIGHT_GO_DIST;
            if (dst > fDist * fDist) {
                // must come closer
        ...
        if (bTryFire && wantToFire && isFireEnable) {
            if (dst < NPC_FIGHT_FIRE_DIST * NPC_FIGHT_FIRE_DIST && (dst > NPC_FIGHT_GO_DIST * NPC_FIGHT_GO_DIST || fightLevel > 0.6f)) {
                if (IsGunLoad()) {
                    if (l > 0.3) {
                        l = 1.0 / sqrt(l);
                        dx *= l;
        ...
        if (bMusketer && command.cmd == aicmd_stay) { Turn(c->curPos.x - curPos.x, c->curPos.z - curPos.z); }
        //}
    }
    SetExCharacter(c);
    if (fgtCurType == fgt_none && priorityAction.name && shot.name) {
        if (storm::iEquals(priorityAction.name, shot.name)) {
            ...
            if (target) {
                if (target != c) {
                    float const tdx = curPos.x - target->curPos.x;
                    float const tdz = curPos.z - target->curPos.z;
                    float const edx = curPos.x - c->curPos.x;
                    float const edz = curPos.z - c->curPos.z;
...
void NPCharacter::DoFightActionAnalysisNone(float dltTime, NPCharacter* enemy)
{
    if (!enemy) return;
    // look at our desires
    bool const wishAttact  = wantToAttack;
    bool const wishDefence = wantToDefence;
    ...
    if (isDodgeEnabled) {
        ...
        if (!fndChr.empty()) {
            for (size_t i = 0; i < fndChr.size(); i++) {
                ...
                int32_t const grp = chrGroup->FindGroupIndex(chr->group);
    ...
    static float const backAng    = -cosf(45.0f * (3.1415926535f / 180.0f));
    for (size_t i = 0; i < fndCharacter.size(); i++) {
        ...
        fc.d2 = sqrtf(fc.d2);
        if (isFreeBack) {
            float const cs = dir.x * fc.dx + dir.z * fc.dz;
            if (cs < 0.0f) {
                if (-cs >= backAng * fc.d2) { isFreeBack = false; }
            }
        }
    ...
    if (wishAttact) {
        ...
        for (size_t i = 0; i < enemies.size(); i++) {
            ...
            float const k  = es.state * 1.0f + (es.dir + 1.0f) * 0.5f;
            if (enemy) {
                if (kSel < k) {
                    kSel  = k;
                    enemy = es.chr;
            ...
            static float const dangerAng = cosf(0.7f * CHARACTER_ATTACK_ANG * (3.1415926535f / 180.0f));
    ...
    int32_t            attacked      = 0;
    static float const attackAng     = cosf(0.5f * CHARACTER_ATTACK_ANG * (3.1415926535f / 180.0f));
    for (size_t i = 0; i < enemies.size(); i++) {
    ...
    if (!wantToDefence) { return; }
    // If the blow is piercing
    if (isBreakAttack) {
        // Probability of recognizing a penetrating attack
        float const prbDetect = fightLevel * fightLevel * 0.9f;
        if (PrTest(prbDetect)) {
            if (!isFreeBack) {
                DoFightBlock(true);
            } else {
...
void NPCharacter::DoFightAttack(Character* enemy, int32_t enemyCounter, bool wishDefence)
{
    ...
    float const r = rand() * (max / RAND_MAX);
    float       p = 0.0f;
    int32_t     i;
    for (i = 0; i < count; i++) {
        p += attack[i].prb;
...
{
    wantToDefence   = false;
    float const sum = defencePrbBlock + defencePrbParry;
    if (sum <= 0.0f) return;
    float const parryEnergy = GetActEnergy("parry");
    float const used        = GetEnergy() - parryEnergy;
    float const minTime     = 0.8f + (1.0f - fightLevel) * 0.8f;
    float const maxTime     = minTime + (1.1f - fightLevel) * 2.5f;
    ...
    float const noParryThreshold = (fightLevel * 0.3f + rand() * (0.3f / RAND_MAX));
...
{
    if (isInBlock) { blockTime -= fightLevel * 0.01f; }
}
...
bool NPCharacter::PopTask()
{
    ...
    switch (task.task) {
    case npct_none: return CmdNone();
    case npct_stay: return CmdStay();
    case npct_gotopoint: SetRunMode(false); return CmdGotoPoint(task.to.x, task.to.y, task.to.z, NPC_STOP_GOTODIST);
    case npct_runtopoint: SetRunMode(true); return CmdGotoPoint(task.to.x, task.to.y, task.to.z, NPC_STOP_GOTODIST);
    case npct_followcharacter: return InitFollowChartacter(task.target);
    case npct_fight: return InitFightChartacter(task.target); break;
    case npct_escape: break;
    case npct_dead:
        CmdStay();
        Dead();
...
NPCharacter::NPCTask NPCharacter::GetTaskID(char const* taskName)
{
    if (!taskName || !taskName[0]) return npct_unknow;
    for (int32_t i = 0; i < npct_max; i++) {
        char const* task = GetTaskName(static_cast<NPCTask>(i));
        if (storm::iEquals(task, taskName)) return static_cast<NPCTask>(i);
    }
    return npct_unknow;
}

// Get task name by type
char const* NPCharacter::GetTaskName(NPCTask t)
{
    switch (t) {
