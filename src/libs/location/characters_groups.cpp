...
CharactersGroups::String::String(char const* str)
{
    name  = nullptr;
    len   = 0;
    max   = 0;
...
bool CharactersGroups::String::Cmp(char const* str, int32_t l, int32_t h) const
{
    if (!name || !name[0]) {
...
int32_t CharactersGroups::String::GetHash(char const* str)
{
    if (!str) return 0;
    return LocatorArray::CalcHashString(str);
}

int32_t CharactersGroups::String::GetLen(char const* str)
{
    if (!str) return 0;
    return strlen(str);
}
...
// Initialization
bool CharactersGroups::Init(std::shared_ptr<entt::registry> const& registry)
{
...
void CharactersGroups::Execute(uint32_t delta_time)
{
    ...
    // perform groups
    auto const playerGroup  = FindGroupIndex("Player");
    ...
    for (int32_t i = 0; i < numGroups; i++) {
        ...
        for (int32_t j = 0; j <= i; j++) {
            ...
            auto const lastActive = rel[j].isActive;
...
void CharactersGroups::FindEnemyFromFindList(Character* chr, Group* grp, bool visCheck)
{
    ...
    for (size_t i = 0; i < fnd.size(); i++) {
        ...
        auto const gi = GetCharacterGroup(fnd[i].c);
        if (gi < 0) continue;
        ...
        // Found an enemy, add
        if (AddEnemyTarget(chr, fnd[i].c)) { targets.push_back(fnd[i].c); }
    }
    ...
    if (!targets.empty() && !(fnd = location->supervisor.FindCharacters(chr, grp->say)).empty()) {
        for (size_t i = 0; i < fnd.size(); i++) {
            ...
            auto const cgrp = GetCharacterGroup(c);
            if (cgrp < 0) continue;
            // Found character group
            ...
            for (size_t j = 0; j < targets.size(); j++) {
                if (grp->index != cgrp) {
                    auto const tgrp = GetCharacterGroup(targets[j]);
                    if (tgrp >= 0) {
    ...
    for (int32_t i = 0; i < chr->numTargets;) {
        ...
        if (c && (trg.time < trg.timemax || trg.timemax < 0.0f)) {
            if (!c->IsDead()) {
                ...
                auto const gc = GetCharacterGroup(c);
...
{
    std::string const& cmd = message.String();
    if (cmd.empty()) return 0;
    if (storm::iEquals(cmd, "VldTrg")) { return MsgIsValidateTarget(message); }
    if (storm::iEquals(cmd, "GetTrg")) { return MsgGetOptimalTarget(message); }
    if (storm::iEquals(cmd, "IsEnemy")) { return MsgIsEnemy(message); }
    if (storm::iEquals(cmd, "MoveChr")) { return MoveCharacterToGroup(message); }
    ...
    if (storm::iEquals(cmd, "SetGroupLook")) { return MsgSetGroupLook(message); }
    if (storm::iEquals(cmd, "SetGroupHear")) { return MsgSetGroupHear(message); }
    if (storm::iEquals(cmd, "SetGroupSay")) { return MsgSetGroupSay(message); }
    if (storm::iEquals(cmd, "SetGroupPriority")) { return MsgSetGroupPriority(message); }
    ...
    if (storm::iEquals(cmd, "SetAlarm")) { return MsgSetAlarm(message); }
    if (storm::iEquals(cmd, "SetAlarmDown")) { return MsgSetAlarmDown(message); }
...
bool CharactersGroups::MsgIsValidateTarget(MESSAGE& message)
{
    ...
    c->GetPosition(vP1);
    en->GetPosition(vP2);
    auto const fDistance = sqrtf(~(vP1 - vP2));
...
bool CharactersGroups::MsgGetOptimalTarget(MESSAGE& message) const
{
    ...
    if (c->numTargets > 1) {
        CVECTOR pos, p;
        c->GetPosition(pos);
        auto const numChr = location->supervisor.character.size();
...
void CharactersGroups::MsgAddTarget(MESSAGE& message)
{
    ...
    auto const g2 = GetCharacterGroup(enemy);
    if (g1 < 0 || g2 < 0) return;
    auto  isSelf = false;
    auto& r      = FindRelation(g1, g2, &isSelf);
    if (isSelf) return;
...
// Register a group
int32_t CharactersGroups::RegistryGroup(char const* groupName)
{
    auto const idxgrp = FindGroupIndex(groupName);
...
void CharactersGroups::ReleaseGroup(char const* groupName)
{
    auto const idxgrp = FindGroupIndex(groupName);
    if (idxgrp < 0) return;  // no group nothing to delete

    Group* oldGroup       = groups[idxgrp];
    groups[idxgrp]        = groups[numGroups - 1];
    groups[numGroups - 1] = nullptr;

    if (oldGroup->relations) delete oldGroup->relations;
    delete oldGroup;
...
bool CharactersGroups::MoveCharacterToGroup(MESSAGE& message)
{
    ...
    if (!grp) RegistryGroup(grpName.c_str());
...
void CharactersGroups::MsgSetAlarmReaction(MESSAGE& message)
{
    ...
    auto& r      = FindRelation(message, &isSelf);
    if (isSelf) return;
    std::string const& act      = message.String();
    std::string const& rel      = message.String();
    auto               actState = rs_enemy;
...
CharactersGroups::Group* CharactersGroups::FindGroup(char const* name)
{
    auto const gi = FindGroupIndex(name);
    if (gi < 0) return nullptr;
    return groups[gi];
}

// Find a group by name
int32_t CharactersGroups::FindGroupIndex(char const* name)
{
    if (!name) return -1;
    auto const l = String::GetLen(name);
    auto const h = String::GetHash(name);
...
{
    std::string const& grpName1 = message.String();
    std::string const& grpName2 = message.String();
    return FindRelation(grpName1.c_str(), grpName2.c_str(), selfgroup);
}

// Find group relationship
CharactersGroups::Relation& CharactersGroups::FindRelation(char const* name1, char const* name2, bool* selfgroup)
{
...
CharactersGroups::Relation& CharactersGroups::FindRelation(int32_t g1, int32_t g2, bool* selfgroup)
{
    ...
    if (g1 <= g2) { return groups[g2]->relations[g1]; }
...
int32_t CharactersGroups::GetCharacterGroup(Character* c)
{
    ...
    if (c->groupID >= 0 && c->groupID < numGroups) {
        if (storm::iEquals(c->group, groups[c->groupID]->name.name)) { return c->groupID; }
    }
...
void CharactersGroups::LoadDataRelations()
{
    ...
    auto* saveData = AttributesPointer->FindAClass(AttributesPointer, "savedata");
    if (!saveData) return;
    int32_t const numG = saveData->GetAttributesNum();
    for (int32_t i = 0; i < numG; i++) {
        auto* grp = saveData->GetAttributeClass(i);
        // Registering the first group
        char const* gname = grp->GetAttribute("name1");
        if (!gname) gname = "";
        auto const g1        = RegistryGroup(gname);
        ...
        if (!gname) gname = "";
        auto const g2        = RegistryGroup(gname);
...
void CharactersGroups::RestoreStates()
{
    for (int32_t i = 0; i < numGroups; i++) {
        for (int32_t j = 0; j < i; j++) {
            auto&      r        = FindRelation(i, j);
            auto const oldState = r.isActive;
...
char const* CharactersGroups::GetTextState(RelState state)
{
    switch (state) {
    case rs_friend: return "friend";
    case rs_neitral: return "neitral";
    case rs_enemy: return "enemy";
    default: break;
    }
    return "unknow value";
}
