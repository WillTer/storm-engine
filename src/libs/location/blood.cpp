...
// Initialization
bool Blood::Init(std::shared_ptr<entt::registry> const& registry)
{
...
void Blood::Execute(uint32_t delta_time)
{
    ...
    auto const fDeltaTime = delta_time * 0.001f;
    for (int32_t n = 0; n < aBlood.size(); n++) {
        ...
        if (aBlood[n].fLiveTime <= 0.f) {
            if (aBlood[n].nStartIdx == nStartT) {
                ...
                if (nStartT >= MAX_BLOOD_TRIANGLES) nStartT -= MAX_BLOOD_TRIANGLES;
...
{
    auto const fDistPow2 = fDist * fDist;
    int32_t    nCurQ     = 0;
    for (int32_t n = 0; n < aBlood.size(); n++) {
