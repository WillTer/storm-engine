...
void Lights::Realize(uint32_t delta_time)
{
    ...
    for (int32_t i = 0, n = 0; i < numLights; i++)
    {
        ...
        if (collide)
        {
            const auto dist = collide->Trace(core.GetEntityIds(SUN_TRACE), pos, CVECTOR(ls.pos.x, ls.pos.y, ls.pos.z),
                                             lampModels, numLampModels);
            isVisible = dist > 1.0f;
        }
...
void Lights::PrintDebugInfo()
{
    for (uint32_t i = 0; i < numLights; ++i)
    {
        ...
        rs->ExtPrint(FONT_DEFAULT, D3DCOLOR_ARGB(255, 255, 255, 255), 0x00000000, PR_ALIGN_CENTER, false, scale, 0, 0,
        ...
        rs->ExtPrint(FONT_DEFAULT, color, 0x00000000, PR_ALIGN_CENTER, false, scale, 0, 0, static_cast<int32_t>(vrt.x),
                     static_cast<int32_t>(vrt.y), fmt::format("{}", i).c_str());
