...
void Lights::Realize(uint32_t delta_time)
{
    ...
    for (int32_t i = 0, n = 0; i < numLights; i++) {
        ...
        d += 1.0f;
        // Current size
        auto const size = d * l.coronaSize;
        // Transparency
        alpha *= d;
...
// Find source index
int32_t Lights::FindLight(char const* name)
{
    if (!name || !name[0]) return -1;
    for (int32_t i = 0; i < numTypes; i++)
        if (storm::iEquals(name, types[i].name)) return i;
    return -1;
}
...
// Add lantern model
bool Lights::AddLampModel(entid_t const lampModel)
{
...
// Add portable source
int32_t Lights::AddMovingLight(char const* type, const CVECTOR& pos)
{
    int32_t idx;
    ...
    auto const nType = FindLight(type);
    if (nType < 0) return -1;
