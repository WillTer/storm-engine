...
// Initialization
bool Location::Init(std::shared_ptr<entt::registry> const& registry)
{
...
void Location::Update(uint32_t delta_time)
{
    ...
    uint32_t const max_delta_time = 500;
    auto const     maxDltTime     = 0.1f;
...
uint64_t Location::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_LOCATION_ADD_MODEL: {
        ...
        std::string const& tech = message.String();
        level                   = message.Long();

        // Dynamic lights are optional
        auto const dynamicLightsOn = message.GetFormat().size() > 4 ? message.Long() : 0;
    ...
    case MSG_LOCATION_GET_MODEL:
        ...
        if (!core.GetEntityPointer(model.ID(lastLoadStaticModel))) return 0;
    ...
    case MSG_LOCATION_MODEL_SET_POS: {
        ...
        std::string const& name2 = message.String();
    ...
    case MSG_LOCATION_UPDATELOCATORS: UpdateLocators(); return 1;
    ...
    case MSG_LOCATION_SHADOWPATH:
        ...
        return 1;
    case MSG_LOCATION_PAUSE: isPause = message.Long() != 0; return 1;
    ...
    case MSG_LOCATION_LOC_EXISTS: {
        std::string const& name = message.String();
        if (CheckIfLocatorExists(name.c_str())) return 1;
        return 0;
    }
    ...
    case MSG_LOCATION_SETCHRPOSITIONS: supervisor.SetSavePositions(); break;
    case MSG_LOCATION_TLPCHRPOSITIONS: supervisor.DelSavePositions(true); break;
    case MSG_LOCATION_CLRCHRPOSITIONS: supervisor.DelSavePositions(false); break;
    ...
    case MSG_LOCATION_VIEWSTATEBARS: bDrawBars = message.Long() != 0; return 1;
    }
    return 0;
}

// Find locator group
LocatorArray* Location::FindLocatorsGroup(char const* gName)
{
    if (!gName || !gName[0]) return nullptr;
    auto const hash = LocatorArray::CalcHashString(gName);
    for (int32_t i = 0; i < numLocators; i++) {
...
bool Location::CheckIfLocatorExists(char const* lName)
{
    for (int32_t i = 0; i < numLocators; i++) {
        if (locators[i]->FindByName(lName) != -1) { return true; }
    }

    return false;
}

int32_t Location::LoadStaticModel(char const* modelName, char const* tech, int32_t level, bool useDynamicLights)
{
    lights        = static_cast<Lights*>(core.GetEntityPointer(lightsid));
    auto const im = model.CreateModel(modelName, tech, level, true, useDynamicLights ? GetLights() : nullptr);
    if (im < 0) return -1;
    // Pointer to geometry
    auto* mdl = model[im];
    if (!mdl) {
    ...
    for (int32_t i = 0; i < ginfo.nlabels; i++) {
        ...
        auto const hash = LocatorArray::CalcHashString(label.group_name);
        int32_t    j;
        ...
        auto const locIndex = locators[j]->FindByName(label.name);
        if (locIndex < 0) {
            auto& mtxx = *((CMatrix*)label.m);
            for (int32_t me = 0; me < 16; me++)
                if (isnan(mtxx.matrix[me])) {
                    core.Trace("Location: locator %s::%s in position have NaN value, reset it!", label.group_name, label.name);
                    mtxx.SetIdentity();
                    break;
                }
...
bool Location::LoadJumpPatch(char const* modelName)
{
...
bool Location::LoadGrass(char const* modelName, char const* texture)
{
    core.EraseEntity(grass);
...
bool Location::MessageEx(char const* name, MESSAGE& message)
{
    if (storm::iEquals(name, "DelAllLights")) {
        lights->DelAllLights();
    } else if (storm::iEquals(name, "AddFlys")) {
        auto const effects = core.GetEntityId("LocationEffects");
        ...
        int32_t const      n         = model.FindModel(modelname.c_str());
        ...
        float const fScale          = message.Float();
        float const fMaxWidth       = message.Float();
        float const fMaxHeight      = message.Float();
        float const fMinVisibleDist = message.Float();
        float const fMaxVisibleDist = message.Float();
        float const fMinGrassLod    = message.Float();
        core.Send_Message(
            grass, "lffffff", MSG_GRASS_SET_PARAM, fScale, fMaxWidth, fMaxHeight, fMinVisibleDist, fMaxVisibleDist, fMinGrassLod);
                ...
                for (int32_t j = 0; j < locators[i]->Num(); j++) {
                    ...
                    if (a) {
                        ...
                        if (v) {
                            ...
                            core.Trace("Location: Can't create attribute 'locators.%s.%s.vz'!", groupName, locators[i]->Name(j));
                        }
                        // vy
                        ...
                        if (v) {
                            ...
                            core.Trace("Location: Can't create attribute 'locators.%s.%s.vy'!", groupName, locators[i]->Name(j));
                        }
                        // vx
                        ...
                        if (v) {
                            ...
                            core.Trace("Location: Can't create attribute 'locators.%s.%s.vx'!", groupName, locators[i]->Name(j));
                        }
                    } else {
                        core.Trace("Location: Can't create attribute 'locators.%s.%s'!", groupName, locators[i]->Name(j));
                    }
                }
            } else {
...
#define CalcKColor(ind) \
    { \
        kColor = light | !CVECTOR(sphereVertex[t * 3 + ind].v.x, sphereVertex[t * 3 + ind].v.y, sphereVertex[t * 3 + ind].v.z); \
...
void Location::AddDamageMessage(const CVECTOR& pos3D, float hit, float curhp, float maxhp)
{
    ...
    if (k < 0.0f) k = 0.0f;
    if (k > 1.0f) k = 1.0f;
    float const r1 = 0.2f, g1 = 1.0f, b1 = 0.2f;
    float const r2 = 1.0f, g2 = 0.2f, b2 = 0.2f;
    float const r = r2 + (r1 - r2) * k;
    float const g = g2 + (g1 - g2) * k;
    float const b = b2 + (b1 - b2) * k;
...
{
    if (enemyBarsCount >= sizeof(enemyBar) / sizeof(enemyBar[0])) { return; }
...
{
    char               buf[4096];
    std::string const& bufStr = message.String();
    ...
    for (int32_t i = 0; i < num; i++) {
        ...
        if (ptc.FindNode(pos, y) < 0) {
            ...
            float const ldist = pos.y - y;
            if (fabsf(ldist) > 0.2f) {
                sprintf_s(
                    buf, sizeof(buf), "Warning: Locator '%s':'%s' very far from patch: %fm", la->GetGroupName(), la->LocatorName(i), ldist);
                buf[sizeof(buf) - 1] = 0;
...
void Location::CorrectBar(float v, float start, float end, BarVertex* vrt)
{
    ...
    float const dx     = vrt[1].p.x - vrt[0].p.x;
    float const du     = vrt[1].u - vrt[0].u;
    float const startX = vrt[0].p.x + dx * start;
    float const startU = vrt[0].u + du * start;
    float const endX   = vrt[0].p.x + dx * end;
    float const endU   = vrt[0].u + du * end;
    vrt[0].p.x         = startX;
    vrt[0].u           = startU;
    vrt[1].p.x         = endX;
    vrt[1].u           = endU;
