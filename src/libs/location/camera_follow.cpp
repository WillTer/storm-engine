...
CameraFollow::CameraFollow() : camPos(), lookTo(), camradius(0), rotSpd(0), rotAcc(0)
{
    lc           = nullptr;
    isBrn        = true;
    camay        = 0.0f;
...
void CameraFollow::MoveCamera(float dltTime)
{
    ...
    if (lc->isSpecialMode) { likeCamay = camay + lc->dAy; }
    lookTo = lc->pos;
    lookTo.y += lc->lheight;
    // decide what to do
    auto const pi   = 3.14159265359f;
    auto       chay = likeCamay - static_cast<int32_t>(likeCamay / pi) * 2.0f * pi;
    auto const cay  = camay - static_cast<int32_t>(camay / pi) * 2.0f * pi;
    if (cay - chay > pi) chay += 2 * pi;
    if (cay - chay < -pi) chay -= 2 * pi;
    auto const dAng = chay - cay;
...
void CameraFollow::BornCamera()
{
    ...
    if constexpr (kp < 1.0f) camPos = lookTo + (camPos - lookTo) * (kp * 0.9f);
}
...
{
    static auto const pi  = 3.14159265359f;
    static auto const day = pi * 0.25f;  // Vertical deviation
    static auto const dax = pi * 0.16f;  // Vertical deviation
...
bool CameraFollow::ApplyPoly(const CVECTOR* v, int32_t n)
{
    ...
    auto const pos1 = fndCamPos - norm * d;
    // looking for the closest point to the point of intersection of the ray direction to the camera
    auto const cs = norm | fndCamDir;
    if (cs != 0.0f) {
        d /= -cs;
        if (d >= 0.0f) {
            auto const pos2 = fndCamPos + fndCamDir * d;
            auto const dlt  = pos2 - pos1;
