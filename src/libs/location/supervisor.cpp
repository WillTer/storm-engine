...
Supervisor::Supervisor() : isDelete(false)
{
    time      = 0.0f;
    waveTime  = 0.0f;
    curUpdate = 0;
...
void Supervisor::Update(float dltTime)
{
    ...
    for (i = 0; i < character.size() - 1; i++) {
        ...
        auto const radius = character[i].c->radius;
        for (auto j = i + 1; j < character.size(); j++) {
            ...
            if ((~ci->impulse) > 0.0001f && !moveI) { moveI = ((ci->impulse.x * dx + ci->impulse.z * dz) < 0.0f); }
            auto moveJ = cj->IsMove();
            if ((~cj->impulse) > 0.0001f && !moveJ) { moveJ = ((cj->impulse.x * dx + cj->impulse.z * dz) > 0.0f); }
            if (ci->IsFight()) {
                if (moveI == moveJ) {
                    ci->curPos.x += dx * 0.5f;
                    ci->curPos.z += dz * 0.5f;
...
bool Supervisor::CheckPosition(float x, float y, float z, Character* c) const
{
    for (size_t i = 0; i < character.size(); i++) {
        ...
        auto const dx = x - character[i].c->curPos.x;
        auto const dy = y - character[i].c->curPos.y;
        auto const dz = z - character[i].c->curPos.z;
...
std::vector<Supervisor::FindCharacter>
Supervisor::FindCharacters(Character* chr, float radius, float angTest, float nearPlane, float ax, bool isSort, bool lookCenter) const
{
    if (!chr || radius < 0.0f) return {};
    ...
    if (isSort) {
        auto comparator = [](auto const& lhs, auto const& rhs) { return lhs.d2 < rhs.d2; };
...
int32_t Supervisor::FindForvardLocator(LocatorArray* la, const CVECTOR& pos, const CVECTOR& norm, bool lookChr) const
{
    ...
    if (l >= 0 && la->GetLocatorPos(l, lpos.x, lpos.y, lpos.z)) {
        if (!CheckPosition(lpos.x, lpos.y, lpos.z, nullptr)) { return FindForvardLocator(la, pos, norm, true); }
    }
    return l;
}
