...
std::vector<Supervisor::FindCharacter> Supervisor::FindCharacters(Character *chr, float radius, float angTest,
                                                                  float nearPlane, float ax, bool isSort,
                                                                  bool lookCenter) const
{
    ...
    if (isSort)
    {
        auto comparator = [](const auto &lhs, const auto &rhs) { return lhs.d2 < rhs.d2; };
