...
class Character: public Entity
{
    ...
    struct ActionCharacter {
        ...
        void ChangeName(char const* _name);
...
public:
    ...
    bool Teleport(char const* group, char const* locator);
...
public:
    ...
    virtual void ActionEvent(char const* actionName, Animation* animation, int32_t index, int32_t eventID, AnimationEvent event);
    virtual void ActionEvent(Animation* animation, int32_t playerIndex, char const* eventName);

    // The character attacks, analyze to whom to apply damage
    // virtual void CharacterNowIsAttacked(){};

    int32_t PlaySound(char const* soundName, bool isLoop = false, bool isCached = false);
    void    PlayStep();
...
private:
    ...
    bool     SetAction(char const* actionName, float tstart = 0.0f, float movespeed = 0.0f, float trnspeed = 0.0f, bool forceStart = false);
    void     UpdateAnimation();
    char const* FindIdleAnimation(float& tstart);
    char const* FindFightIdleAnimation(float& tstart);
    float       GetCurrentPlayerPos();
    float       GetAniPlayTime();
    bool        SetPriorityAction(char const* action);
...
protected:
    ...
    char const* deadName;       // Death action name
    ...
    static char const* fightNamesTbl[fgt_max];
    ...
    char const* m_pcHandLightLocator;
...
{
    return (priorityAction.name && (storm::iEquals(priorityAction.name, jump.name) || storm::iEquals(priorityAction.name, fall.name)));
}
