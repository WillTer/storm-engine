...
// Initialization
bool LocationCamera::Init(std::shared_ptr<storm::ServiceLocator> const& service_locator)
{
    ...
    if (!isLockFPMode) {
        ...
        case cwm_follow:  // Follow The Character
            ...
            if (isELook) {
                ...
                if (kMorph >= 1.0f) character->SetCameraAlpha(0.0f);
...
uint64_t LocationCamera::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_CAMERA_FREE:
        ...
        kMorph    = 1.0f;
        return 1;
    case MSG_CAMERA_SLEEP: isSleep = message.Long() != 0; return 1;
    ...
    // internal
    case -1: {
        auto const fSpeed    = message.Float();
        auto const fTime     = message.Float();
        auto const fMin      = message.Float();
        auto const fMax      = message.Float();
        auto const fAngSpeed = message.Float();
        auto const fAngMax   = message.Float();
        TurnOnDynamicFov(fSpeed, fTime, fMin, fMax, fAngSpeed, fAngMax);
        return 1;
    }

    case -2: {
        std::string const& trackname  = message.String();
        auto const         fTrackTime = message.Float();
    ...
    case -3: TurnOffTrackCamera(); return 1;
...
void LocationCamera::ExecuteFree(float dltTime)
{
    ...
    if (freeAx > pi * 0.4f) freeAx = pi * 0.4f;
    if (freeAy > pi * 2.0f) freeAy -= pi * 2.0f;
    if (freeAy < -pi * 2.0f) freeAy += pi * 2.0f;
...
void LocationCamera::Clip(PLANE* p, int32_t numPlanes, CVECTOR& cnt, float rad, bool (*fnc)(const CVECTOR* vtx, int32_t num)) const
{
    if (location) location->Clip(p, numPlanes, cnt, rad, fnc);
}

void LocationCamera::TurnOnDynamicFov(float fSpeed, float fTime, float fRelationMin, float fRelationMax, float fAngSpeed, float fAngMax)
{
...
void LocationCamera::ProcessDynamicFov(float fDeltaTime, const CVECTOR& vFrom, const CVECTOR& vTo, CVECTOR& vUp)
{
    ...
    if (dynamic_fog.fFogTimeMax > 0.f && dynamic_fog.fFogTimeCur >= dynamic_fog.fFogTimeMax) {
        ...
        auto const v    = vTo - vFrom;
        auto const fAng = atan2f(v.x, v.z);
...
bool LocationCamera::LoadCameraTrack(char const* pcTrackFile, float fTrackTime)
{
    m_fTrackCurTime  = 0.f;
    ...
    if (!m_track.Load(pcTrackFile)) return false;
...
{
    auto const fTrackTime = TrackPauseProcess();
