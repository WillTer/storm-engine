...
void LocationEffects::CreateSplash(const CVECTOR& pos, float power)
{
    ...
    for (i = 0; i < LFX_SPLASHES_NUM; i++)
        if (chrSplash[i].time < 0.0f) break;
    ...
    for (i = 0; i < LFX_SPLASHES_P_NUM; i++) {
        auto const ang   = rand() * (LFX_PI * 2.0f / RAND_MAX);
        auto const r     = rand() * (2.0f * LFX_SPLASHES_SRAD / static_cast<float>(RAND_MAX));
        auto const s     = 1.0f + rand() * (3.0f / static_cast<float>(RAND_MAX));
...
{
    SGInited();
    int32_t const max = sizeof(flinders) / sizeof(flinders[0]);
    int32_t const num = 4 + (rand() & 3);
    for (int32_t i = 0; i < num && numFlinders < max; i++) {
        ...
        float const a             = rand() * 6.283185307f / (RAND_MAX + 1);
...
{
    SGInited();
    int32_t const max = sizeof(blood) / sizeof(blood[0]);
    int32_t const num = 16 + (rand() & 7);
...
{
    SGInited();
    int32_t const max = sizeof(smoke) / sizeof(smoke[0]);
    int32_t const num = 5 + (rand() & 3);
...
void LocationEffects::ProcessedShotgun(float dltTime)
{
    ...
    numFlinders = j;
    //
    if (numFlinders) { DrawParticles(flinders, numFlinders, sizeof(flinders[0]), texFlinders, "ShootParticles"); }
