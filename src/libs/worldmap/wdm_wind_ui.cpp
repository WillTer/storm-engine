...
void WdmWindUI::SetAttributes(ATTRIBUTES* apnt)
{
    ...
    char const* txPath;
    char const* barTxPath;
    char const* maskTxPath;
    // Screen parameters
    float w, h;
...
void WdmWindUI::LRender(VDX9RENDER* rs)
{
    ...
    CVECTOR windDir;
    resizeRatio         = wdmObjects->resizeRatio;
    auto const widForce = wdmObjects->GetWind(x, y, windDir);
    auto const ang      = static_cast<float>(atan2(windDir.x, windDir.z));
    // Buffer for drawing
    Vertex buf[(3 * 2) * 2];
