...
void WdmEnemyShip::Update(float dltTime)
{
    ...
    auto const dx = wdmObjects->playerShip->mtx.Pos().x - mtx.Pos().x;
    auto const dz = wdmObjects->playerShip->mtx.Pos().z - mtx.Pos().z;
    auto const d  = sqrtf(dx * dx + dz * dz);
    // Visibility
...
void WdmEnemyShip::FindShipsForce()
{
    sx = 0.0f;
    sz = 0.0f;
    for (auto const& ship: wdmObjects->ships) {
        if (!ship) continue;
...
void WdmEnemyShip::Move(float dltTime)
{
    ...
    auto const dl = dx * dx + dz * dz;
    // Our direction
    auto const vx = mtx.Vz().x;
    auto const vz = mtx.Vz().z;
    auto const vl = vx * vx + vz * vz;
...
bool WdmEnemyShip::GeneratePosition(float objRadius, float brnDltAng, float& x, float& z)
{
    ...
    auto const psx  = wdmObjects->playerShip->mtx.Pos().x;
    auto const psz  = wdmObjects->playerShip->mtx.Pos().z;
    auto const psay = static_cast<WdmEnemyShip*>(wdmObjects->playerShip)->ay;
    // Choice fields
    uint8_t field[32];
    ...
    while (true) {
        // define the angle
        int32_t const ang = rand() & 31;
        if (field[ang] != 0xff) {
            auto const angle = psay + brnDltAng * (0.5f - ang / 31.0f);
            // Determine the radius
            int32_t rad;
            for (rad = rand() & 7; field[ang] & (1 << rad); rad = rand() & 7)
                ;
            auto const radius =
                wdmObjects->enemyshipBrnDistMin + (wdmObjects->enemyshipBrnDistMax - wdmObjects->enemyshipBrnDistMin) * rad / 7.0f;
            // Coordinates
...
// Get attribute name
char const* WdmEnemyShip::GetAttributeName() const
{
