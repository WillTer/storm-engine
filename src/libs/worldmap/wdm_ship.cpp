...
bool WdmShip::Load(char const* modelName)
{
    auto const res = WdmRenderModel::Load(modelName);
    if (geo) {
        GEOS::INFO ginfo;
        geo->GetInfo(ginfo);
...
{
    auto const pi2 = 3.14159265f * 2.0f;
    ...
    // Find a new position
    CVECTOR     pos     = mtx.Pos();
    float const dltMove = speed * kWind * dltTime;
...
void WdmShip::LRender(VDX9RENDER* rs)
{
    if (wmtexture >= 0 && isWMRender) {
        ...
        float const   y = 0.01f;
        uint32_t      color =
            (static_cast<uint32_t>(al) << 16) | (static_cast<uint32_t>(al) << 8) | (static_cast<uint32_t>(al) << 0) | 0xff000000;
        ...
        for (int32_t i = 0; i < WDM_SHIP_WMSZ; i++) {
            ...
            float const dx = -cosf(lines[i].ay) * k;
            float const dz = sinf(lines[i].ay) * k;
...
{
    float const inrtPos  = 5.5f;
    float const inrtAng  = 3.5f;
    float const inrtSize = 12.0f;
...
bool WdmShip::CheckPosition(float x, float z, float objRadius)
{
    ...
    for (i = 0; i < wdmObjects->ships.size(); i++) {
        ...
        float const dx = wdmObjects->ships[i]->mtx.Pos().x - x;
        float const dz = wdmObjects->ships[i]->mtx.Pos().z - z;
