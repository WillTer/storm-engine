...
class WindField
{
    ...
    struct Wind {
        ...
        void Lerp(Wind const& w1, Wind const& w2, float k)
        {
            x = w1.x + (w2.x - w1.x) * k;
            z = w1.z + (w2.z - w1.z) * k;
        };
...
public:
    ...
    WindField()
    {
        auto const minX = 0.0f, minZ = 0.0f, maxX = 1.0f, maxZ = 1.0f;
        baseX      = minX;
        baseZ      = minZ;
    ...
    {
        switch (step) {
        case cs_initors: SubStep1(); break;
        case cs_copy1: SubStep2(); break;
        case cs_lines: SubStep3(); break;
        case cs_copy2: SubStep4(); break;
        }
        updateTime += dltTime;
    }
...
private:
    void SubStep1()
    {
        if (steps >= WindFieldSteps) {
            ...
            for (int32_t i = 0; i < 4; i++) {
                auto const ang  = rand() * (3.14159265358979f * 2.0f / RAND_MAX);
                auto const amp  = rand() * (4.0f / RAND_MAX);
        ...
        for (int32_t i = 0; i < WindFieldSize; i++) {
            auto const w2                 = i * 1.0f / (WindFieldSize - 1);
            auto const w1                 = 1.0f - w2;
    ...
    void SubStep3()
    {
        ...
        auto needLines = static_cast<int32_t>(updateTime * (static_cast<float>(WindFieldSize) / WindFieldUpdateTime) + 0.99f - curLine);
        if (needLines < 1) needLines = 1;
        for (; needLines > 0; needLines--) {
            // count along the lines
            auto const i = curLine++;
    ...
    void SubStep4()
    {
        if (updateTime < WindFieldUpdateTime) { return; }
        updateTime = 0.0f;
        curWind++;
        if (curWind > 2) curWind = 0;
        auto const    frame = (curWind + 1) % 3;
        auto*         from  = &field[0][0];
        auto*         to    = &wind[frame][0][0];
        int32_t const count = WindFieldSize * WindFieldSize;
