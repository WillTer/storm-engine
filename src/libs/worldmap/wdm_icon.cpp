...
void WdmIcon::Update(float dltTime)
{
    if (lastShip != wdmObjects->enemyShip) {
        ...
        if (lastShip != nullptr) {
            ...
            case wdmest_unknow: icon[0].u = 0.0f; break;
            case wdmest_merchant: icon[0].u = 1.0f / 8.0f; break;
            case wdmest_warring: icon[0].u = 2.0f / 8.0f; break;
            case wdmest_follow: icon[0].u = 3.0f / 8.0f; break;
            }
        } else
            icon[0].u = -1.0f;
    }
