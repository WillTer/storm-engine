...
void WdmInterfaceObject::FillRectCoord(Vertex* vrt, float x, float y, float w, float h, float ang)
{
    ...
    if (ang == 0.0f) {
        ...
        // With a turn
        auto const sn  = -0.5f * sinf(ang);
        auto const cs  = -0.5f * cosf(ang);
        auto const dwx = cs * w;
        auto const dwy = sn * w;
        auto const dhx = -sn * h;
        auto const dhy = cs * h;
...
void WdmInterfaceObject::FillRectUV(Vertex* vrt, float tu, float tv, float tw, float th, float ang)
{
    ...
    if (ang == 0.0f) {
        ...
        auto const sn  = 0.5f * sinf(ang);
        auto const cs  = 0.5f * cosf(ang);
        auto const dwu = cs * tw;
        auto const dwv = sn * tw;
        auto const dhu = -sn * th;
        auto const dhv = cs * th;
        tu += 0.5f * tw;
        tv += 0.5f * th;
        vrt[0].tu = tu - dwu + dhu;
        vrt[0].tv = tv - dwv + dhv;
...
void WdmInterfaceObject::Print(int32_t font, int32_t color, float xleft, float xright, float y, char const* format, ...)
{
    va_list args;
