...
bool WorldMap::Init()
{
    ...
    if (AttributesPointer) {
        ...
        char const* s                    = AttributesPointer->GetAttribute("debug");
    ...
    if (aDate) {
        int32_t const sec = aDate->GetAttributeAsDword("sec", 1);
        int32_t const min = aDate->GetAttributeAsDword("min", 1);
    ...
    if (saveData) {
        auto const num = saveData->GetAttributesNum();
        for (uint32_t i = 0; i < num; i++) {
            auto* a = saveData->GetAttributeClass(i);
            if (!a) continue;
            char const* type      = a->GetAttribute("type");
            char const* modelName = a->GetAttribute("modelName");
            if (!type || !type[0]) {
            ...
            if (storm::iEquals(type, "Follow") && modelName && modelName[0]) {
                if (!CreateFollowShip(modelName, 1.0f, -1.0f, a)) { core.Trace("WoldMap: not loaded follow encounter."); }
                continue;
            }
            if (storm::iEquals(type, "Warring") && modelName && modelName[0]) {
                char const* attacked = a->GetAttribute("attacked");
                if (attacked) {
                    auto* a1 = saveData->FindAClass(saveData, attacked);
                    if (a1) {
                        char const* modelName1 = a1->GetAttribute("modelName");
            ...
            if (storm::iEquals(type, "Attacked")) { continue; }
            if (storm::iEquals(type, "Storm")) {
                auto const isTornado = (a->GetAttributeAsDword("isTornado", 0) != 0);
                if (!CreateStorm(isTornado, -1.0f, a)) { core.Trace("WoldMap: not loaded storm encounter."); }
                continue;
            }
            saveData->DeleteAttributeClassX(a);
        }
...
void WorldMap::Realize(uint32_t delta_time)
{
    if (AttributesPointer && wdmObjects->playerShip) {
        ...
        auto const force = wdmObjects->GetWind(x, z, wind);
    ...
    //---------------------------------------------------------
    auto const dltTime = 0.001f * delta_time;
    // Updating the date
    if (hour < 0.0f) hour = 0.0f;
    hour += dltTime * timeScale;
    auto days         = static_cast<int32_t>(hour / 24.0f);
    hour              = (hour / 24.0f - days) * 24.0f;
    auto const dtHour = static_cast<int32_t>(hour);
    auto const dtMin  = static_cast<int32_t>((hour - dtHour) * 60.0f);
    auto const dtSec  = static_cast<int32_t>(((hour - dtHour) * 60.0f - dtMin) * 60.0f);
    ...
    //
    char const* tmp = aDate->GetAttribute("sec");
    ...
    auto ay       = 0.0f;
    wdmObjects->playerShip->GetPosition(x, z, ay);
    auto const wind_force = wdmObjects->GetWind(x, z, wind_dir);
    auto const ang        = atan2(wind_dir.x, wind_dir.z);
    core.Event("WorldMap_GetWindParameters", "ff", wind_force, ang);
    // Checking the update attribute of the encounter
    if (AttributesPointer) {
        char const* upd = AttributesPointer->GetAttribute("addQuestEncounters");
        if (upd && upd[0] != 0) { core.Event("WorldMap_AddQuestEncounters"); }
    }
}
...
uint64_t WorldMap::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_WORLDMAP_CREATESTORM: {
        auto const isTornado = message.Long() != 0;
        CreateStorm(isTornado);
    } break;
    case MSG_WORLDMAP_CREATEENC_MER: {
        std::string const& sName   = message.String();
        std::string const& buf     = message.String();
        std::string const& sName2  = message.String();
        auto const         kSpeed  = message.Float();
        auto const         timeOut = message.Float();                                            // boal
        return CreateMerchantShip(sName.c_str(), buf.c_str(), sName2.c_str(), kSpeed, timeOut);  // boal
    } break;
        // boal 04/01/06 -->
    case MSG_WORLDMAP_CREATEENC_MER_XZ: {
        std::string const& sName   = message.String();
        auto const         fx1     = message.Float();
        auto const         fz1     = message.Float();
        auto const         fx2     = message.Float();
        auto const         fz2     = message.Float();
        auto const         kSpeed  = message.Float();
        auto const         timeOut = message.Float();
    ...
    case MSG_WORLDMAP_CREATEENC_RELEASE: ReleaseEncounters(); break;
    case MSG_WORLDMAP_LAUNCH_EXIT_TO_SEA:
        if (wdmObjects->playerShip) {
            ...
            core.Event("ExitFromWorldMap");
        break;
    case MSG_WORLDMAP_SET_NATION_FLAG: wdmObjects->nationFlagIndex = message.Long(); break;
    case MSG_WORLDMAP_SET_COORDINATES: wdmObjects->coordinate = message.String(); break;
    case MSG_WORLDMAP_WIND_UI_SET_SHOW: wdmObjects->showWindUI = (message.Long() != 0); break;
    case MSG_WORLDMAP_UPDATE: wdmObjects->forceUpdate = true; break;
    }
    return 0;
}
...
uint32_t WorldMap::AttributeChanged(ATTRIBUTES* apnt)
{
    ...
    if (storm::iEquals(apnt->GetThisName(), "deleteUpdate")) {
        ...
        if (pa == aStorm) {
            auto const cur = static_cast<int32_t>(pa->GetAttributeAsDword("cur"));
        ...
        auto* pa = apnt->GetParent();
        if (pa == aInfo) { pa->SetAttributeUseDword("playerInStorm", static_cast<int32_t>(wdmObjects->playarInStorm)); }
    } else {
...
WdmRenderObject* WorldMap::AddObject(WdmRenderObject* obj, int32_t level)
{
    ...
    auto const i = GetObject(firstObject, level);
    object[i].ro = obj;
    return obj;
}
...
{
    if (!obj) return;
    auto const i = GetObject(firstPrObject, level);
    object[i].ro = obj;
}
...
{
    if (!obj) return;
    int32_t const i = GetObject(firstMrObject, level);
    object[i].ro    = obj;
}
...
{
    if (!obj) return;
    int32_t const i = GetObject(firstLrObject, level);
    object[i].ro    = obj;
}
...
int32_t WorldMap::GetObject(int32_t& first, int32_t level)
{
    ...
    int32_t const i = firstFreeObject;
...
WdmRenderObject*
WorldMap::CreateModel(WdmRenderModel* rm, char const* modelName, bool pr, bool mr, bool lr, int32_t objectLevel, int32_t drawLevel)
{
...
bool WorldMap::CreateMerchantShip(
    char const* modelName, char const* locNameStart, char const* locNameEnd, float kSpeed, float time, ATTRIBUTES* save)
{
    ...
    if (!wdmObjects->islands) { core.Trace("World map: Islands not found"); }
    CVECTOR gpos;
    if (!locNameEnd || !locNameEnd[0]) {
        if (wdmObjects->islands) {
            if (!wdmObjects->islands->GetRandomMerchantPoint(gpos)) { core.Trace("World map: Locators <Merchants:...> not found"); }
        }
    } else {
        if (wdmObjects->islands) {
    ...
    if (save) { save->SetAttribute("modelName", modelName); }
    static_cast<WdmEnemyShip*>(ship)->SetSaveAttribute(save);
    return true;
}

// boal Create a merchant's ship in coordinates
bool WorldMap::CreateMerchantShipXZ(
    char const* modelName, float x1, float z1, float x2, float z2, float kSpeed, float time, ATTRIBUTES* save)
{
    if (kSpeed < 0.1f) kSpeed = 0.1f;
    WdmShip* ship = new WdmMerchantShip();
    if (ship->killMe) {
...
bool WorldMap::CreateFollowShip(char const* modelName, float kSpeed, float time, ATTRIBUTES* save)
{
    if (kSpeed < 0.1f) kSpeed = 0.1f;
    ...
    if (isSkipEnable) {
        ...
        if (isSkipEnable->Get(skipEnable)) { static_cast<WdmEnemyShip*>(ship)->canSkip = skipEnable != 0; }
    }
...
bool WorldMap::CreateWarringShips(char const* modelName1, char const* modelName2, float time, ATTRIBUTES* save1, ATTRIBUTES* save2)
{
    ...
    float const moveRadius = (ship1->modelRadius + ship2->modelRadius) * (0.4f + (rand() & 3) * (0.1f / 3.0f));
    float const fullRadius = 0.6f * (moveRadius + 2.0f * std::max(ship1->modelRadius, ship2->modelRadius));
    // General position
    float x, z;
    if (!WdmEnemyShip::GeneratePosition(fullRadius, 1.5f, x, z)) return false;
    // General angle
    float const angl = rand() * 2.0f * pi / (RAND_MAX + 1);
    // Center offset
    float const dx = moveRadius * cosf(angl);
    float const dz = -moveRadius * sinf(angl);
    ...
    if (save1 && save2) { save1->SetAttribute("attacked", save2->GetThisName()); }
    if (save1) { save1->SetAttribute("modelName", modelName1); }
    if (save2) { save2->SetAttribute("modelName", modelName2); }
...
bool WorldMap::FindIslandPosition(char const* name, float& x, float& z, float& r)
{
    return false;
}
...
void WorldMap::ResetScriptInterfaces() const
{
    ...
    if (aInfo) { aInfo->SetAttributeUseDword("playerInStorm", 0); }
}
...
ATTRIBUTES* WorldMap::GetEncSaveData(char const* type, char const* retName)
{
    ...
    // Save the name
    if (AttributesPointer) { AttributesPointer->SetAttribute(retName, atrName); }
    return a;
}
