...
void WdmSea::Update(float dltTime)
{
    ...
    float const maxAni = sizeof(aniTextures) / sizeof(int32_t);
    aniFrame /= maxAni;
    ...
    auto const flareBurnTime = 0.0005f;
...
void WdmSea::LRender(VDX9RENDER* rs)
{
    ...
    auto const curFrame  = static_cast<int32_t>(aniFrame);
    auto       nextFrame = curFrame + 1;
    if (nextFrame >= sizeof(aniTextures) / sizeof(int32_t)) nextFrame = 0;
