...
// Create geometry
GEOS* WdmObjects::CreateGeometry(char const* path)
{
    ...
    for (i = entryModels[i]; i >= 0; i = models[i].next) {
        if (models[i].hash == hash) {
            if (storm::iEquals(models[i].path, path)) { return models[i].geo; }
        }
    }
    // Loading geometry
    modelPath = "worldmap/";
...
void WdmObjects::DrawVector(const CVECTOR& start, const CVECTOR& end, uint32_t color) const
{
    ...
    len = sqrtf(len);
    dir *= 1.0f / len;
    auto const p = 0.9f * len;
    auto const r = 0.03f * len;

    int32_t t = 0;
    for (int32_t i = 0, imax = 8; i < imax; i++) {
        auto const y1 = r * sinf(2.0f * WdmObjects_myPI * i / static_cast<float>(imax));
        auto const x1 = r * cosf(2.0f * WdmObjects_myPI * i / static_cast<float>(imax));
        auto const y2 = r * sinf(2.0f * WdmObjects_myPI * (i + 1) / static_cast<float>(imax));
        auto const x2 = r * cosf(2.0f * WdmObjects_myPI * (i + 1) / static_cast<float>(imax));
...
// Get save string
char const* WdmObjects::GetWindSaveString(std::string& windData)
{
    windData           = "v02_";
    int32_t const size = sizeof(windField);
    ...
    auto const* buf = (uint8_t const*)&windField;
...
// Set save string
void WdmObjects::SetWindSaveString(char const* str)
{
    if (!str) {
        windField.Reinit();
        return;
    ...
    for (int32_t i = 0; i < size; i++) {
        auto const data = GetDataFromString(str);
        if (data < 0) {
            windField.Reinit();
            return;
        }
...
int32_t WdmObjects::GetDataFromString(char const*& cur)
{
    uint32_t tmp = 0;
