...
bool WdmClouds::Cloud::Reset(bool isFirstTime)
{
    ...
    if (count < 1) count = 1;
    // Base position
    auto const baseX = (rand() * (1.0f / RAND_MAX) - 0.5f) * wdmObjects->worldSizeX;
    auto const baseZ = (rand() * (1.0f / RAND_MAX) - 0.5f) * wdmObjects->worldSizeZ;
    // Scaling factor
    auto       scaleX = (rand() * (0.8f / RAND_MAX) + 0.2f);
    auto       scaleZ = (rand() * (0.8f / RAND_MAX) + 0.2f);
    auto const nrm    = 1.0f / sqrtf(scaleX * scaleX + scaleZ * scaleZ);
    scaleX *= nrm;
    scaleZ *= nrm;
...
{
    if (count <= 0) return;
    auto const minDist   = WdmCloudsCloudRad * 0.5f;
    auto const midDist   = WdmCloudsCloudRad * 1.0f;
    auto const maxDist   = WdmCloudsCloudRad * 1.5f;
    auto const minWorldX = -0.5f * wdmObjects->worldSizeX;
    auto const maxWorldX = 0.5f * wdmObjects->worldSizeX;
    auto const minWorldZ = -0.5f * wdmObjects->worldSizeZ;
    auto const maxWorldZ = 0.5f * wdmObjects->worldSizeZ;
    // Moving the clouds
    ...
    for (int32_t i = 0; i < count; i++) {
        ...
        if (cld.pos.x < minWorldX || cld.pos.x > maxWorldX || cld.pos.z < minWorldZ || cld.pos.z > maxWorldZ) { outsideCount++; }
        // Taking into account the particle in the bounding sphere
        auto const cldr = cld.size * 1.5f;
        if (i) {
            dir          = center - cld.pos;
            auto const r = sqrtf(dir.x * dir.x + dir.z * dir.z);
...
void WdmClouds::Cloud::Kill(Cloud const& cld)
{
    if (count && cld.count) {
        auto const dx      = center.x - cld.center.x;
        auto const dz      = center.z - cld.center.z;
        auto const d       = dx * dx + dz * dz;
        auto const minDist = (radius + cld.radius) * 0.2f;
        if (d < minDist * minDist) { lifeTime = 0.0f; }
    }
}
