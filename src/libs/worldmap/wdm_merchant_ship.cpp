...
WdmMerchantShip::WdmMerchantShip() : isEnableSetDir(false)
{
...
// Setting parameters
bool WdmMerchantShip::Load(char const* modelName)
{
...
void WdmMerchantShip::Goto(float x, float z, float rad)
{
    ...
    if (isEnableSetDir) {
        double const sn = x - mtx.Pos().x;
        double       cs = z - mtx.Pos().z;
        auto const   l  = sn * sn + cs * cs;
        if (l > 0.0) cs /= sqrt(l);
        if (cs > 1.0) cs = 1.0;
        if (cs < -1.0) cs = -1.0;
        cs = acos(cs);
...
{
    auto const dx = gotoPos.x - mtx.Pos().x;
    auto const dz = gotoPos.z - mtx.Pos().z;
