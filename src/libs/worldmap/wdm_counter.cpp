...
void WdmCounter::LRender(/*VDX9RENDER*/ void* rs)
{
    ...
    // Numbers
    // Day
    auto const day     = wdmObjects->wm->day - 1;
    auto const mon     = wdmObjects->wm->mon - 1;
    auto const days    = wdmObjects->wm->month[mon + 1];
    auto const year    = wdmObjects->wm->year;
    auto const kHeight = wdmObjects->wm->hour / 24.0f;
    auto       kNext   = 0.0f;
    float      low, dlt, kMove;
    if (days == 30) {
        low   = day % 10 + kHeight;
        kMove = 0.0f;
        if (day < 29) {
            if (low > 8.0f) kMove = low - 8.0f;
            if (kMove > 1.0f) kMove = 1.0f;
        } else {
