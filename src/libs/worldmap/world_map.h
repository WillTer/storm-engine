...
public:
    ...
    WdmRenderObject* CreateModel(
        WdmRenderModel* rm,
        char const*     modelName,
        bool            pr          = false,
        bool            mr          = true,
        bool            lr          = true,
        int32_t         objectLevel = 0,
        int32_t         drawLevel   = 0);
...
private:
    ...
    bool CreateMerchantShip(
        char const* modelName,
        char const* locNameStart,
        char const* locNameEnd,
        float       kSpeed,
        float       time = -1.0f,
        ATTRIBUTES* save = nullptr);
    // boal Create a merchant's ship in coordinates
    bool CreateMerchantShipXZ(
        char const* modelName, float x1, float z1, float x2, float z2, float kSpeed, float time = -1.0f, ATTRIBUTES* save = nullptr);
    // Create a ship that follows us
    bool CreateFollowShip(char const* modelName, float kSpeed, float time = -1.0f, ATTRIBUTES* save = nullptr);
    // Create a pair of warring ships
    bool CreateWarringShips(
        char const* modelName1, char const* modelName2, float time = -1.0f, ATTRIBUTES* save1 = nullptr, ATTRIBUTES* save2 = nullptr);
    // Delete all encounters
    void ReleaseEncounters();
    // Create an attribute to save the encounter parameters
    ATTRIBUTES* GetEncSaveData(char const* type, char const* retName);

    // Find coordinates and radius by destination
    bool FindIslandPosition(char const* name, float& x, float& z, float& r);
