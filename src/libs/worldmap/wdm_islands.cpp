...
WdmIslands::WdmIslands()
{
    ...
    for (int32_t i = 0; i < ginfo.nlabels; i++) {
        ...
        if (!storm::iEquals(label.group_name, "islands")) continue;
        // Skip if added
        int32_t j;
        ...
        if (model) {
            ...
            name += "_palms";
            isl.palms =
                static_cast<WdmRenderModel*>(wdmObjects->wm->CreateModel(new WdmRenderModel(), name.c_str(), false, false, true, 4, 800));
            if (isl.palms) {
                static auto const* techName = "WdmModelDrawStdAlphaTest";
...
{
    auto const maxHeightInTest = 0.5f;
    // Radius of a rectangle on a plane
    auto const boxRadius = sqrtf(length * length + width * width);
    ...
    for (int32_t i = 0; i < islands.size(); i++) {
        ...
        float const dist2   = ~(locMtx.Pos() - island.model->center);
        float const maxDist = boxRadius + island.model->radius;
...
bool WdmIslands::AddEdges(const GEOS::VERTEX* vrt, int32_t numVrt)
{
    ...
    float const   d = (v | curPos) - (v | CVECTOR(vrt[0].x, vrt[0].y, vrt[0].z));
    if (d < 0) return true;
    // Adding edges
    numEdges += numVrt;
...
void WdmIslands::SetIslandsData(ATTRIBUTES* apnt, bool isChange)
{
    ...
    if (!apnt) return;
    // Number of labels
    uint32_t const numAttr = apnt->GetAttributesNum();
    ...
    for (uint32_t i = 0; i < numAttr; i++) {
        ...
        char const*    id         = a->GetAttribute("id");
        char const*    locator    = a->GetAttribute("locator");
        char const*    text       = a->GetAttribute("text");
        int32_t const  icon       = static_cast<int32_t>(a->GetAttributeAsDword("icon", static_cast<uint32_t>(-1)));
        char const*    font       = a->GetAttribute("font");
        float const    pivotX     = -0.5f;
        float const    pivotY     = -0.5f;
        float const    heightView = a->GetAttributeAsFloat("heightView", 250.0);
        ...
        if (index < 0) {
            ...
            // Adding a new label
            int32_t const iEntry = hash & (sizeof(labelsEntry) / sizeof(labelsEntry[0]) - 1);
            // index = labels.Add();
            ...
            if (!storm::iEquals(labels[index].locatorName, locator)) {
                if (LabelsFindLocator(locator, pos)) { labels[index].locatorName = locator; }
            }
        }
        // Text
...
int32_t WdmIslands::LabelsFind(char const* id, uint32_t hash)
{
    if (!id) id = "";
    int32_t i = hash & (sizeof(labelsEntry) / sizeof(labelsEntry[0]) - 1);
    for (i = labelsEntry[i]; i >= 0; i = labels[i].next) {
        if (labels[i].idHash == hash) {
            if (storm::iEquals(labels[i].id, id)) return i;
        }
    }
    return -1;
}

bool WdmIslands::LabelsFindLocator(char const* name, CVECTOR& pos) const
{
...
int32_t WdmIslands::LabelsAddFont(char const* name)
{
    if (!name) name = "";
    for (int32_t i = 0; i < fonts.size(); i++) {
        if (storm::iEquals(fonts[i].name, name)) { return fonts[i].id; }
    }
...
void WdmIslands::FindDirection(const CVECTOR& position, const CVECTOR& destination, CVECTOR& direction) const
{
    ...
    float         y    = 0.0f;
    int32_t const from = patch->FindNode(position, y);
    y                  = 0.0f;
...
void WdmIslands::FindReaction(const CVECTOR& position, CVECTOR& reaction) const
{
    if (patch) {
        float         y = 0.0f;
        int32_t const n = patch->FindNode(position, y);
        if (n >= 0) {
...
bool WdmIslands::GetQuestLocator(char const* locName, CVECTOR& p)
{
...
bool WdmIslands::CheckIslandArea(char const* islandName, float x, float z)
{
    for (int32_t i = 0; i < islands.size(); i++) {
        if (!islands[i].area) continue;
        if (islands[i].modelName == islandName) { return IsShipInArea(i, CVECTOR(x, 0.0f, z)); }
    }
    return false;
}

// Get the closest point to the island zone
void WdmIslands::GetNearPointToArea(char const* islandName, float& x, float& z)
{
    // Looking for an area
    int32_t i;
    for (i = 0; i < islands.size(); i++) {
        if (!islands[i].area) continue;
        if (islands[i].modelName == islandName) { break; }
    }
    if (i >= islands.size()) { return; }
    ...
    v.y = 0.0f;
    v.z = 0.0f;
    islands[i].area->geo->Clip(
        nullptr, 0, v, wdmObjects->worldSizeX * wdmObjects->worldSizeX + wdmObjects->worldSizeZ * wdmObjects->worldSizeZ, FindNearPoint);
    if (checkMode) {
        x = centPos.x;
        z = centPos.z;
    }
...
bool WdmIslands::FindNearPoint(const GEOS::VERTEX* vrt, int32_t numVrt)
{
    // Skip broken Data
    for (int32_t i = 0; i < numVrt; i++) {
        float const dx  = vrt[i].x - curPos.x;
        float const dz  = vrt[i].z - curPos.z;
        float const dst = dx * dx + dz * dz;
...
{
    static float const testRadius = 0.01f;
    // No area, no intersection
    if (!islands[islIndex].area) return false;
    // Checking the contact of the radii
    float const dist2 = ~(pos - islands[islIndex].area->center);
    float const rad   = testRadius + islands[islIndex].area->radius;
