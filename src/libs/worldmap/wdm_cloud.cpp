...
WdmCloud::~WdmCloud()
{
    ...
    // if (lightning >= 0) wdmObjects->rs->TextureRelease(lightning);
}
...
void WdmCloud::Update(float dltTime)
{
    ...
    if (d < 100.0f) {
        ...
        if (d > 250.0f) { move[i].v -= (move[i].pos - move[i].cent) * (0.5f * move[i].kSpd * dltTime); }
        // Decay rate
        d = 0.01f * dltTime;
        if (d > 1.0f) d = 1.0f;
        move[i].v -= move[i].v * d;
...
void WdmCloud::PRender(/*VDX9RENDER*/ void* rs) {}

void WdmCloud::LRender(/*VDX9RENDER*/ void* rs)
{
    // Render(rs);
}
...
//     uint32_t const lightningColor = (static_cast<uint8_t>(globalAlpha * 255.0f) << 24) | 0x00ffffff;
//     for (int32_t i = 0; i < numRects; i++) {
//         auto& r = rect[i];
//         if (r.dwColor & 0x0000ff00) {
//             if (r.dwColor & 0x00000400) {
//                 auto const kFrames = 1.0f / 4.0f;
//                 auto const width   = 10.0f;
//                 auto const u       = ((r.dwColor >> 8) & 3) * kFrames;
//                 auto&      vx      = view.Vx();
...
//                 lght[3].v          = 0.99f;
//                 rs->DrawPrimitiveUP(
//                     D3DPT_TRIANGLESTRIP, D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1, 2, lght, sizeof(Vertex), "WdmLightning");
//             }
...
void WdmCloud::BuildCloud(int32_t n)
{
    ...
    for (int32_t i = 0; i < n; i++, numRects++) {
        ...
        if (rand() & 1) move[i].dx = -move[i].dx;
        if (rand() & 1) move[i].dz = -move[i].dz;
        move[i].dTime = 0.0f;
    }
    pos = 0.0f;
    dir = 0.0f;
...
void WdmCloud::CreateVertexDeclaration(/*VDX9RENDER*/ void* rs)
{
