...
public:
    ...
    // Create geometry
    GEOS* CreateGeometry(char const* path);
    ...
    // Get save string
    char const* GetWindSaveString(std::string& windData);
    // Set save string
    void SetWindSaveString(char const* str);
    // Add float to string
    void AddDataToString(std::string& str, uint8_t d);
    // Get float from string
    int32_t GetDataFromString(char const*& cur);
