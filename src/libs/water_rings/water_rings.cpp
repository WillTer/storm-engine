...
//------------------------------------------------------------------------------------
bool WaterRings::Init(std::shared_ptr<entt::registry> const& registry)
{
...
uint64_t WaterRings::ProcessMessage(MESSAGE& message)
{
    ...
    for (auto i = 0; i < waterrings::MAX_RINGS; i++) {
        if (!rings[i].active) {
            ...
            auto const a  = message.Float() + PI + randCentered(PId2 / 1.5f);
            rings[i].cosA = cosf(a);
            rings[i].sinA = sinf(a);

            auto const walk = message.Long() != 0;
            auto const run  = message.Long() != 0;
            auto const swim = message.Long() != 0;
            if (!(walk || run || swim)) {
                rings[i].activeTime = 0;
                rings[i].active     = false;
                return 0;
...
void WaterRings::UpdateGrid(int _ringI, uint16_t* _iPointer, RING_VERTEX* _vPointer, int32_t _vOffset)
{
    ...
    float const  midX       = (waterrings::GRID_STEPS_COUNT - 1) / 2.f;
    float const  midZ       = (waterrings::GRID_STEPS_COUNT - 1) / 2.f;
    RING_VERTEX* ringVertex = _vPointer;
    float        gX, gZ;
    if (ring->active) {
        uint32_t const texA = static_cast<uint32_t>(a * 50) << 24;
        float const    r    = .4f + 1.5f * ring->activeTime / (waterrings::FADE_IN_TIME + waterrings::FADE_OUT_TIME);
