...
OctTree::OctTree() : vertsPos(), vertsPosMin(), vertsPosMax(), vertsR(0)
{
    root     = nullptr;
    numVerts = 0;
    maxVerts = 0;
...
int32_t OctTree::Check(OTNode* node, Vertex* v, int32_t num)
{
    if (node->vrt) {
        ...
        for (int32_t i = 0; i < 8; i++)
            if (node->node[i]) num += Check(node->node[i], v, 0);
    }
    return num;
}
...
bool OctTree::AddVertex(OTNode* node, Vertex* v)
{
    ...
    if (node->vrt) {
        ...
        auto const cnt = (node->min + node->max) * 0.5f;
...
void OctTree::FindVerts(OTNode* node)
{
    ...
    if (node->vrt == nullptr) {
        for (int32_t i = 0; i < 8; i++)
            if (node->node[i]) FindVerts(node->node[i]);
    } else {
        for (int32_t i = 0; i < node->num; i++) {
            auto const r = ~(node->vrt[i]->p - vertsPos);
            if (r < vertsR) {
