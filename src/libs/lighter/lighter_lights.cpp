...
void LighterLights::AddPointLight(
    const CVECTOR& color, const CVECTOR& pos, float att0, float att1, float att2, float range, char const* group)
{
...
void LighterLights::PostInit()
{
    ...
    for (int32_t i = 0; i < numLights; i++) {
        ...
        int32_t j;
        for (j = 0; j < numGrp; j++)
            if (storm::iEquals(grp[j], light[i].group)) break;
