...
void LightProcessor::Process()
{
    if (shadowTriangle >= 0) {
        ...
        if (shadowTriangle == -1) {
            ...
            auto const  numVrt    = geometry->numVrt;
            auto const  numLights = lights->Num();
    ...
    if (smoothVertex >= 0) {
        ...
        if (smoothVertex == -1) {
            ...
            if (geometry->numVrt) window->smoothPrc = smoothVertex / static_cast<float>(geometry->numVrt);
        }
    }
    if (blurVertex >= 0) {
        BlurLight();
        if (blurVertex == -1) {
            ...
            if (geometry->numVrt) window->blurPrc = blurVertex / static_cast<float>(geometry->numVrt);
        }
    }
...
void LightProcessor::ApplyTriangleShadows(Triangle& t)
{
    ...
    for (int32_t i = 0; i < num; i++) {
        ...
        case Light::t_sky:
            ...
            if (t.n.y >= 0.0f) {
                float      sky = 0.0;
                auto const rad = geometry->radius;
                auto const rdx = geometry->radius * 0.2f;
...
{
    auto const lookNorm   = window->smoothNorm;
    auto const smoothRad  = window->smoothRad;
    auto const kSmoothRad = 1.0f / smoothRad;
    ...
    for (int32_t i = 0; i < LIGHTPRC_SMOOTH_NUM && smoothVertex < geometry->numVrt; i++, smoothVertex++) {
        ...
        auto const  numVerts = octtree->numVerts;
...
void LightProcessor::BlurLight()
{
    ...
    auto const  blurRad  = window->blurRad;
...
void LightProcessor::CalcLights(int32_t lit, bool isCos, bool isAtt, bool isSdw)
{
    ...
    for (int32_t n = 0; n < geometry->numVrt; n++) {
        ...
        for (int32_t i = 0; i < num; i++) {
            ...
            case Light::t_sun:
                // Cosine of an angle
                if (isCos) { shw.csatt = lt.cosine * shw.cs + (1.0f - lt.cosine); }
                // Shading coefficient
                if (isSdw) {
            ...
            case Light::t_point:
                ...
                // Cosine of an angle
                if (isCos || isAtt) { shw.csatt = (lt.cosine * shw.cs + (1.0f - lt.cosine)) * shw.att; }
                // Shading coefficient
                if (isSdw) {
                    vl = (v.shadow[i].sm - 0.5) * lt.curct + 0.5;
                    if (vl < 0.0f) vl = 0.0f;
