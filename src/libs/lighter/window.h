...
private:
    ...
    void Print(int32_t color, float xleft, float xright, float y, float scale, bool isAlign, char const* format, ...);
    void DrawCursor();

    bool    Slider(int32_t id, float y, char const* text, float& value, float min = 0.0f, float max = 1.0f, bool drawVal = false);
    bool    ColorPicker(int32_t id, float y, CVECTOR& ref, float st, CVECTOR& res);
    bool    Button(float x, float y, float w, float h, char const* text, int32_t* act = nullptr, int32_t init = -1);
    void    Checker(float x, float y, char const* text, bool& res);
    int32_t SelPreset();

    void SavePreset(int32_t prs);
    void LoadPreset(int32_t prs);

    char* GenerateName(char const* f, char const* n);
