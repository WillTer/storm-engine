...
void Window::InitList(LighterLights& ls)
{
    ...
    for (int32_t i = 0; i < maxSize; i++) {
        ...
        case Light::t_group: {
            auto const len            = strlen(ls[i].group) + sizeof "Group ";
...
void Window::Draw(float dltTime)
{
    ...
    // Flap button
    auto const clsx = winx + winw - 20.0f;
    auto const clsy = winy + 5.0f;
    auto const clsw = 15.0f;
    auto const clsh = 15.0f;
    auto       c    = selColor;
    auto       cl   = frmColor;
    ...
    // List header
    auto const lstx  = winx + 5.0f;
    auto       lsty  = winy + 5.0f;
    auto const lstw  = 240.0f;
    auto       lsth  = 22.0f;
    auto const lstbx = lstx + lstw;
    auto const lstbw = lsth;
    c                = selColor;
    cl               = frmColor;
    if (listWait <= 0.0f) {
    ...
    if (selected >= 0) {
        ...
        case ListElement::t_quick:
            ...
            DrawLRect(
                winx + 15.0f,
                winy + winh + list[selected].h - 30.0f,
                winx + 15.0f + (winw - 30.0f) * (tracePrc + smoothPrc) * 0.5f,
                winy + winh + list[selected].h - 10.0f,
                isLockCtrl ? 0xff000080 : 0xff0000c0,
                0);
            DrawLRect(
                winx + 15.0f,
                winy + winh + list[selected].h - 30.0f,
        ...
        case ListElement::t_trace:
            DrawLRect(
                winx + 15.0f,
                winy + winh * 0.5f - 10.0f,
                winx + 15.0f + (winw - 30.0f) * tracePrc,
                winy + winh * 0.5f + 10.0f,
                isLockCtrl ? 0xff000080 : 0xff0000c0,
                0);
        ...
        case ListElement::t_smooth:
            ...
            DrawLRect(
                winx + 15.0f,
                winy + 110.0f,
                winx + 15.0f + (winw - 30.0f) * smoothPrc,
                winy + 130.0f,
                isLockCtrl ? 0xff000080 : 0xff0000c0,
                0);
    ...
    if (isList) {
        ...
        // Rendering loop
        if (listPos > numElements - numLines / 2) listPos = static_cast<float>(numElements - numLines / 2);
        if (listPos < 0.0f) listPos = 0.0f;
        auto const str = static_cast<int32_t>(listPos);
        int32_t    sel = -1;
        ...
        for (int32_t i = str, cnt = 0; i < numElements && cnt < numLines; i++, cnt++) {
            auto const clr = textColor;
            if (cnt == sel) {
                DrawLRect(lstx + 3, y, lstx + lstw - 3, y + lstbw, 0xff808080, 0);
                actItem = i;
            }
            if (i == selected) { DrawLRect(lstx + 3, y, lstx + lstw - 3, y + lstbw, 0, 0xff800000); }
...
void Window::Print(int32_t color, float xleft, float xright, float y, float scale, bool isAlign, char const* format, ...)
{
    va_list args;
...
{
    Vertex      v[3];
    auto const  p1x = cursx + 15.0f, p1y = cursy + 20.0f;
    auto const  p2x = cursx + 15.0f * 1.2f - 20.0f * 0.4f, p2y = cursy + 20.0f * 1.2f + 15.0f * 0.4f;
    float const p3x = cursx + 15.0f * 1.2f + 20.0f * 0.4f, p3y = cursy + 20.0f * 1.2f - 15.0f * 0.4f;
    v[0].x   = cursx;
    v[0].y   = cursy;
    v[0].z   = 0.5f;
    v[0].rhw = 2.0f;
...
bool Window::Slider(int32_t id, float y, char const* text, float& value, float min, float max, bool drawVal)
{
    bool  isUpdate = false;
    ...
    float const szx = isSmallSlider ? 3.0f : 8.0f;
    float const szy = isSmallSlider ? 3.0f : 5.0f;
    // Engine
    ...
    if (!isPikerActive && !isList) {
        if (isActiveMouseState) {
            ...
            if (slidID != id && cursy >= winy + y && cursy <= winy + y + 5.0f) {
                if (cursx >= x && cursx <= x + sldLen) { v = (cursx - x) / sldLen; }
            }
        } else if (isMouseDown) {
            if (slidID == id) {
...
bool Window::ColorPicker(int32_t id, float y, CVECTOR& ref, float st, CVECTOR& res)
{
    ...
    UpdateColors();
    // Draw a rectangle
    float const x = winx + 60.0f + sldLen;
    float const w = 50.0f;
    float const h = 50.0f;
    ...
    knrm                 = 255.0f / knrm;
    uint32_t const color = (static_cast<uint32_t>(res.x * knrm) << 16) | (static_cast<uint32_t>(res.y * knrm) << 8)
    ...
    if (isPikerActive) {
        ...
        float const x2 = x - 4.0f;
        float const y2 = winy + y + 256.0f + 3.0f;
        struct Vrt {
            float x, y;
            float z, rhw;
            float u, v;
...
bool Window::Button(float x, float y, float w, float h, char const* text, int32_t* act, int32_t init)
{
    bool     isInside = false;
    uint32_t c        = selColor;
...
void Window::Checker(float x, float y, char const* text, bool& res)
{
    uint32_t    c = selColor;
    float const s = 10.0f;
    float const h = 20.0f;
    ...
    if (res) {
        ...
        DrawLine(
            winx + x + s + 1.0f,
            winy + y + h * 0.5f - s * 0.5f - 5.0f,
            winx + x + s * 0.5f,
            winy + y + h * 0.5f + s * 0.5f - 3.0f,
            frmColor);
    }
...
void Window::LoadPreset(int32_t prs)
{
    ...
    for (int32_t i = 0; i < numElements; i++) {
        ...
        case ListElement::t_light:
            ...
            *list[i].cosine = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_cosine"), *list[i].cosine));
            *list[i].shadow = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_shadow"), *list[i].shadow));
            *list[i].bright = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_bright"), *list[i].bright));
            *list[i].contr  = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_contr"), *list[i].contr));
            *list[i].gamma  = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_gamma"), *list[i].gamma));
            *list[i].isOn   = ini->GetInt(sect, GenerateName(list[i].name, "_isOn"), static_cast<int32_t>(*list[i].isOn)) != 0;
            break;
        case ListElement::t_glight:
            list[i].st      = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_intens"), list[i].st));
            list[i].c.x     = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_clrR"), list[i].c.x));
            ...
            *list[i].contr  = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_contr"), *list[i].contr));
            *list[i].gamma  = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_gamma"), *list[i].gamma));
            *list[i].att0   = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_att0"), *list[i].att0));
            *list[i].att1   = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_att1"), *list[i].att1));
            *list[i].att2   = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_att2"), *list[i].att2));
            *list[i].range  = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_range"), *list[i].range));
            *list[i].isOn   = ini->GetInt(sect, GenerateName(list[i].name, "_isOn"), static_cast<int32_t>(*list[i].isOn)) != 0;
            break;
        default: break;
        }
...
char* Window::GenerateName(char const* f, char const* n)
{
