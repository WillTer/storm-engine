...
{
    auto const xx = x * cos + z * sin;
    auto const zz = z * cos - x * sin;
    x             = xx;
    z             = zz;
}
...
inline bool IntersectLines2D(const CVECTOR& v1, const CVECTOR& v2, const CVECTOR& v3, const CVECTOR& v4, CVECTOR& vRes)
{
    ...
    auto const half1 = 1.0f / 4.0f * (SQR(v2.x - v1.x) + SQR(v2.z - v1.z));
    auto const d1    = SQR(vRes.x - (v1.x + v2.x) / 2.0f) + SQR(vRes.z - (v1.z + v2.z) / 2.0f);

    if (d1 > half1) return false;

    auto const half2 = 1.0f / 4.0f * (SQR(v4.x - v3.x) + SQR(v4.z - v3.z));
    auto const d2    = SQR(vRes.x - (v3.x + v4.x) / 2.0f) + SQR(vRes.z - (v3.z + v4.z) / 2.0f);

    if (d2 > half2) return false;
