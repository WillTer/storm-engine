...
{
    // Position
    auto const radius = SHARK_PULL_DIST * (0.4f + rand() * 1.6f / RAND_MAX);
    auto const ang    = SHARK_PI * rand() * (2.0f / RAND_MAX);
...
inline void Sharks::Shark::Reset(float cam_x, float cam_z)
{
    ...
    auto const dx  = cam_x - pos.x;
    auto const dz  = cam_z - pos.z;
    auto const max = SHARK_PULL_DIST * 4.0f;
...
{
    auto const dx = x - spos.x;
    auto const dz = z - spos.z;
    auto const d  = dx * dx + dz * dz;
    auto       fy = 0.0f;
    if (d < r2) {
        auto const k = sqrtf(d / r2);
        fy           = -(1.0f - k) * 100.0f;
...
inline void Sharks::Shark::Coordination(float cam_x, float cam_z, float dltTime, SEA_BASE* sb, ISLAND_BASE* ib)
{
    ...
    rTime -= dltTime;
    if (rTime < 0.0f) {
        auto const radius = rand() * 10.0f / RAND_MAX;
        auto const ang    = SHARK_PI * rand() * (2.0f / RAND_MAX);
    ...
    dirTime -= dltTime;
    if (dirTime <= 0.0f) {
        auto const p = rand() * 1.0f / RAND_MAX;
    ...
    auto const sx = sinf(angs.y);
    auto const sz = cosf(angs.y);
    vx            = fforce.x;
    vz            = fforce.z;
    l             = vx * vx + vz * vz;
    if (l > 0.0f) {
    ...
    auto const sn = sx * vz - sz * vx;
    auto const cs = sx * vx + sz * vz;
    ...
    if (sb) {
        CVECTOR    n;
        auto const seaY = sb->WaveXZ(spos.x, spos.z, &n);
...
{
    auto const step = 2.0f * SHARK_PI / numPnt;
    auto       vx   = 0.0f;
    auto       vz   = 0.0f;
    auto*      mdl  = static_cast<MODEL*>(core.GetEntityPointer(ib->GetSeabedEID()));
    if (!mdl) return;
    for (int32_t i = 0; i < numPnt; i++) {
        auto const x  = sinf(i * step);
        auto const z  = cosf(i * step);
        auto const xp = spos.x + rad * x;
        auto const zp = spos.z + rad * z;
...
void Sharks::Shark::Event(Animation* animation, int32_t index, int32_t eventID, AnimationEvent event)
{
    ...
    static auto const* actStand = "stand";
    static auto const* actSwim  = "Shark_Swim";
    static auto const* actJump  = "Shark_Jump";
    auto const* const  act      = animation->Player(0).GetAction();
    int32_t const      rnd      = rand();
...
int32_t Sharks::Shark::GenerateTrack(uint16_t* inds, Vertex* vrt, uint16_t base, SEA_BASE* sb)
{
    ...
    k = (1.2f + k) / 1.2f;
    // Fin parameters
    auto const length = 2.0f * 3.0f * k;
    auto const width  = 1.0f * 1.5f * k;
    // Indexes
...
// Initialization
bool Sharks::Init(std::shared_ptr<entt::registry> const& registry)
{
    ...
    // Execution layers
    int32_t const emdl = AttributesPointer->GetAttributeAsDword("executeModels", 77);
    int32_t const rmdl = AttributesPointer->GetAttributeAsDword("realizeModels", 77);
    int32_t const eprt = AttributesPointer->GetAttributeAsDword("executeParticles", 77);
    int32_t const rprt = AttributesPointer->GetAttributeAsDword("realizeParticles", 100000);
    ...
    if (v) {
        ...
        if (root) {
            auto const time = root->GetAttributeAsFloat("time");
            if (time > 9.0f && time < 20.0f) {
                root = root->FindAClass(root, "date");
                if (root) {
                    auto const year = root->GetAttributeAsFloat("year");
                    if (year >= 1633.0f) {
                        auto const month = root->GetAttributeAsDword("month");
                        if (month & 1) {
                            auto const day = root->GetAttributeAsDword("day");
                            if (day == 7) {
                                if ((SDL_GetTicks() & 7) == 5) { waitPTime = 60.0f + rand() * 500.0f / RAND_MAX; }
                            }
                        }
                    }
                }
...
void Sharks::Execute(uint32_t delta_time)
{
    ...
    for (auto ent: entities) {
        ...
        if (!ship) break;
        // Ship position
        auto const shipPos = ship->GetMatrix()->Pos();
        // Ship size
        auto const s   = ship->GetBoxsize();
        auto const rd2 = (s.x * s.x + s.z * s.z) * 3.0f;
