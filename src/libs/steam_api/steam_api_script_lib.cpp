...
{
    auto* pV = pS->Push();
    if (!pV) { return IFUNCRESULT_FAILED; }
...
uint32_t getDlcEnabled(VS_STACK* pS)
{
    auto*   pV = pS->Pop();
    int32_t dlc {};
    if (!pV->Get(dlc)) { return IFUNCRESULT_FAILED; }

    auto const dlc_active = SteamApi::getInstance().isDLCActive(dlc);
    pV                    = pS->Push();
    if (!pV) { return IFUNCRESULT_FAILED; }
...
uint32_t getDlcData(VS_STACK* pS)
{
    ...
    auto const dlc_data = SteamApi::getInstance().getDLCDataByIndex(dlc);
    pV                  = pS->Push();
    if (!pV) { return IFUNCRESULT_FAILED; }
...
uint32_t activateGameOverlayDlc(VS_STACK* pS)
{
    ...
    auto const success = SteamApi::getInstance().activateGameOverlayDLC(dlc);
    pV                 = pS->Push();
    if (!pV) { return IFUNCRESULT_FAILED; }
...
{
    auto       pStr = pS->Pop();
    auto const nm   = pStr->GetString();
    if (!nm) { return IFUNCRESULT_FAILED; }

    auto const name = std::string(nm);

    auto pReturn = pS->Push();
    if (!pReturn) { return IFUNCRESULT_FAILED; }

    int32_t const ret = SteamApi::getInstance().SetAchievementState(name.c_str());
    pReturn->Set(ret);
...
uint32_t slGetAchievement(VS_STACK* pS)
{
    ...
    int32_t const ret = SteamApi::getInstance().GetAchievementState(name.c_str());
    pReturn->Set(ret);
...
uint32_t slSetStat(VS_STACK* pS)
{
    ...
    int32_t const ret = SteamApi::getInstance().SetStatValue(name.c_str(), val);
    pReturn->Set(ret);
...
uint32_t slGetStat(VS_STACK* pS)
{
    ...
    int32_t const ret = SteamApi::getInstance().GetStatValue(name.c_str());
    pReturn->Set(ret);
...
uint32_t slStoreStats(VS_STACK* pS)
{
    ...
    int32_t const ret = SteamApi::getInstance().StoreStats();
...
uint32_t slClearAchievement(VS_STACK* pS)
{
    ...
    int32_t const ret = SteamApi::getInstance().ClearAchievement(name.c_str());
    pReturn->Set(ret);
...
uint32_t slResetStats(VS_STACK* pS)
{
    ...
    int32_t const ret = SteamApi::getInstance().ResetStats(val);
    pReturn->Set(ret);
