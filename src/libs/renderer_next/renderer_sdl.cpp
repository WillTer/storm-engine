...
std::unique_ptr<ITexture> RendererSDL::load_texture(TextureAsset const& asset)
{
    ...
    auto       texture    = std::make_unique<TextureSDL>(*this, copy_pass, asset);
    return texture;
}
...
void RendererSDL::test_draw()
{
    auto vertex_binding   = SDL_GPUBufferBinding {};
    vertex_binding.buffer = m_vertex_buffer.get();
    vertex_binding.offset = 0;
    SDL_BindGPUVertexBuffers(m_current_render_pass.get(), 0, &vertex_binding, 1);

    auto index_binding   = SDL_GPUBufferBinding {};
    index_binding.buffer = m_index_buffer.get();
    index_binding.offset = 0;
    SDL_BindGPUIndexBuffer(m_current_render_pass.get(), &index_binding, SDL_GPU_INDEXELEMENTSIZE_16BIT);

    SDL_DrawGPUIndexedPrimitives(m_current_render_pass.get(), 6, 1, 0, 0, 0);
}
...
void RendererSDL::start_pass()
{
    ...
    SDL_BindGPUGraphicsPipeline(m_current_render_pass.get(), m_pipeline.get());
}
