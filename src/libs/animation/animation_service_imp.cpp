...
{
    if (core.Controls->GetDebugAsyncKeyState(VK_F4)) return;
...
Animation* AnimationServiceImp::CreateAnimation(char const* animationName)
{
    // looking for animation, load if not found
    int32_t i;
    for (i = 0; i < ainfo.size(); i++)
        if (ainfo[i]) {
            if (*ainfo[i] == animationName) break;
        }
    if (i == ainfo.size()) {
        i = LoadAnimation(animationName);
        if (i < 0) return nullptr;
    }
    auto const aniIndex = i;
    // The animation is loaded, creating an animation manager
    for (i = 0; i < animations.size(); i++)
        if (!animations[i]) break;
    if (i == animations.size()) { animations.emplace_back(nullptr); }
...
// Event
void AnimationServiceImp::Event(char const* eventName)
{
...
int32_t AnimationServiceImp::LoadAnimation(char const* animationName)
{
    ...
    for (auto isHaveSection = ani->GetSectionName(path, 63); isHaveSection; isHaveSection = ani->GetSectionNameNext(path, 63)) {
        ...
        if (ani->ReadString(path, ASKW_EVENT, key, 256, "")) {
            do {
                ...
                if (tm < stime) tm = static_cast<float>(stime);
                if (tm > etime) tm = static_cast<float>(etime);
                // Reading the event type
                auto ev = eae_normal;
                if (key[p]) {
                    // Looking for a start
                    for (p++; key[p]; p++)
                        if ((key[p] >= 'A' && key[p] <= 'Z') || (key[p] >= 'a' && key[p] <= 'z')) break;
                    em = key + p;
                    ...
                    if (em[0] == 0) {
                        ...
                        core.Trace(
                            "Warning: Incorrect %s <%s> in action [%s] of animation file %s.ani,\nunknow event "
    ...
    if (i == ainfo.size()) ainfo.emplace_back(nullptr);
...
void AnimationServiceImp::LoadUserData(
    INIFILE* ani, char const* sectionName, std::unordered_map<std::string, std::string>& data, char const* animationName)
{
