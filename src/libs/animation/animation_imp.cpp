...
AnimationImp::AnimationImp(int32_t id, AnimationInfo* animationInfo)
{
    ...
    headBoneIndex       = 16;
    char const* dataStr = GetData("Head bone index");
    if (dataStr) headBoneIndex = atol(dataStr);
    customHeadAX = 0.0f;
    customHeadAY = 0.0f;
}
...
{
    if (eventID < 0) return;
    auto const event  = eventID >> 16;
    auto const lindex = eventID & 0xffff;
    Assert(event < ae_numevents);
...
char const* AnimationImp::GetData(char const* dataName) const
{
    auto const& userData = aniInfo->GetUserData();
...
void AnimationImp::CopyPlayerState(int32_t indexSrc, int32_t indexDst, bool copyTimerState)
{
    ...
    for (int32_t i = 0; i < ANI_MAX_ACTIONS; i++) {
        auto isInv = false;
        if (timer[i].IsUsedPlayer(indexSrc, &isInv)) { timer[i].SetPlayer(indexDst, isInv); }
    }
}
...
{
    auto const b       = isControllableHead;
...
{
    for (int32_t i = 0; i < ANIIMP_MAXLISTENERS; i++) {
        if (ae_listeners[event][i]) { ae_listeners[event][i]->Event(this, index, (event << 16) | i, event); }
    }
}
