...
static char const* lightName  = "Saber_light";
static char const* bladeStart = "start";
static char const* bladeEnd   = "end";

static char const* gunHandName = "gun_hand";
static char const* gunBeltName = "gun_belt";
static char const* gunFire     = "gun_fire";

static char const* sabergunHandName = "sabergun_hand";
...
uint64_t Blade::ProcessMessage(MESSAGE& message)
{
    ...
    switch (message.Long()) {
    case MSG_BLADE_SET: return LoadBladeModel(message); break;
    ...
    case 1001:
        ...
        AddTieItem(message);
        break;
    case 1002: DelTieItem(message); break;
    case 1003: DelAllTieItem(); break;
    }
    return 0;
}
...
int32_t Blade::FindTieItemByIndex(int32_t n)
{
    if (n < 0) return -1;
    for (int32_t i = 0; i < ITEMS_INFO_QUANTITY; i++)
        if (items[i].nItemIndex == n) return i;
    return -1;
}
...
bool Blade::TIEITEM_INFO::LoadItemModel(char const* mdlName, char const* locName)
{
    ...
    auto const len = strlen(locName) + 1;
