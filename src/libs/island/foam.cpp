...
bool CoastFoam::Init(std::shared_ptr<entt::registry> const& registry)
{
            ...
            if (bSelected && (vStart.y > 0.0f && vStart.y > vEnd.y)) {
                ...
                if (pln.Intersection(vS1, vE1, vR1)) {
                    ...
                    if (bMenu) {
                        ...
                        if (iEditFoamVertex == 0 && bShift) pFP->v[1] = pFP->v[0] + vDiff;
                    }
        ...
        if (csIns.state == CST_ACTIVATED) {
            if (bShift) {
                ...
                if (iEditFoamPart == pF->aFoamParts.size() - 1) {
                    auto const foam1 = pF->aFoamParts.end() - 2, foam2 = pF->aFoamParts.end() - 1;
...
void CoastFoam::ExecuteFoamType2(Foam* pF, float fDeltaTime)
{
    ...
    for (int32_t k = 0; k < pF->iNumFoams; k++) {
        ...
        case 1:
            ...
            if (pF->fSpeed[k] < 0.01f) { pF->iMode[k] = 2; }
            break;
        case 2:
        ...
        for (int32_t y = 0; y < 8; y++) {
            ...
            auto const fAlpha = pF->fAlpha[k] * Clampf(2.5f * (1.0f - dy) * dy);
...
bool CoastFoam::IsClipped(Foam* pF)
{
    ...
    for (uint32_t i = 0; i < dwSize; i++) {
        ...
        for (uint32_t j = 0; j < 4; j++) {
            auto const fD1 = (v0 | vP[j]) - pFrustumPlanes[j].D;
            auto const fD2 = (v1 | vP[j]) - pFrustumPlanes[j].D;
...
void CoastFoam::ExecuteFoamType1(Foam* pF, float fDeltaTime)
{
    ...
    for (int32_t y = 0; y < 8; y++) {
        ...
        for (int32_t x = 0; x < iLen; x++) {
            ...
            auto fAlpha1 = fAlpha;
            if (pWP->p[y].fPos > 0.9f) { fAlpha1 = fAlpha * Clampf(1.0f - pWP->p[y].fPos) / 0.1f; }
            if (x <= 4) fAlpha1 *= static_cast<float>(x) / 4.0f;
...
void CoastFoam::RecalculateFoam(int32_t iFoam)
{
    ...
    if (!pF->aFoamParts.empty()) {
        for (size_t i = 0; i < pF->aFoamParts.size() - 1; i++) {
            ...
            auto const dx = (pF1->v[0] - pF2->v[0]).GetLength() / static_cast<float>(iFoamDivides);
            for (int32_t j = 0; j < iFoamDivides; j++) {
                ...
                pWP->v[0] = pF1->v[1] + static_cast<float>(j) / static_cast<float>(iFoamDivides - 1) * (pF2->v[1] - pF1->v[1]);
                pWP->v[1] = pF1->v[0] + static_cast<float>(j) / static_cast<float>(iFoamDivides - 1) * (pF2->v[0] - pF1->v[0]);

                auto const fStartPos = sinf(ii / 14.0f * PI) * 0.1f;
                for (int32_t k = 0; k < 8; k++) {
...
void CoastFoam::Load()
{
    ...
    clear();
    auto const iNumFoams = pI->GetInt(nullptr, "NumFoams", 0);
    ...
    for (int32_t i = 0; i < iNumFoams; i++) {
        ...
        int32_t const iNumParts = pI->GetInt(cSection, "NumParts", 0);
