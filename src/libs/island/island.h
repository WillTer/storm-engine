...
private:
    ...
    // depth map section
    bool         CreateHeightMap(std::string_view const& pDir, std::string_view const& pName);
    ...
    bool Mount(std::string_view const& fname, std::string_view const& fdir, entid_t* eID);
    void Uninit();

    void CalcBoxParameters(CVECTOR& vBoxCenter, CVECTOR& vBoxSize);

    void SetName(std::string_view const& pIslandName)
    {
    ...
    void AddLocationModel(entid_t eid, std::string_view const& pIDStr, std::string_view const& pStr);
...
public:
    ...
    char const* GetCollideMaterialName()
    {
        return nullptr;
    };
