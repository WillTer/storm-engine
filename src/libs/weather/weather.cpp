...
bool WEATHER::Init(std::shared_ptr<entt::registry> const& registry)
{
...
uint32_t WEATHER::AttributeChanged(ATTRIBUTES* pAttribute)
{
    ...
    if (*pParent == "time") {
        ...
        if (*pAttribute == "speed") {
            ...
            if (fFloats[whf_time_speed] != 0.f) fFloats[whf_time_speed] = 0.001f / fFloats[whf_time_speed];
            return 0;
        }
    ...
    if (*pAttribute == "isDone") { SetCommonStates(); }
    return 0;
}
