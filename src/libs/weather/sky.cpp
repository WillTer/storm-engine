...
bool SKY::Init(std::shared_ptr<entt::registry> const& registry)
{
...
void SKY::UpdateFogSphere(bool const initialize)
{
    ...
    for (size_t y = 0; y < kNumLevels; y++) {
        auto const h  = y * kR / kNumLevels;
        auto const R1 = sqrtf(SQR(kR) - SQR(h));
        for (size_t x = 0; x < kNumAngles; x++) {
            auto const fCos = cosf(static_cast<float>(x) / static_cast<float>(kNumAngles) * PIm2);
            auto const fSin = sinf(static_cast<float>(x) / static_cast<float>(kNumAngles) * PIm2);
...
void SKY::FillSkyDirArray(ATTRIBUTES* pAttribute)
{
    ...
    if (q > 1) {
        for (int32_t n = 0; n < q; n++) {
            auto const* const attrName = pAttribute->GetAttributeName(n);
            if (attrName && attrName[0] == 'd' && attrName[1] >= '0' && attrName[1] <= '9') {
                auto const i = atol(&attrName[1]);
                if (i < q) aSkyDirArray[i] = to_string(pAttribute->GetAttribute(n));
            }
        }
    } else {
...
void SKY::GetSkyDirStrings(std::string& sSkyDir, std::string& sSkyDirNext)
{
    ...
    if (n1 >= 0) {
        if (n1 >= static_cast<int32_t>(aSkyDirArray.size())) n1 -= aSkyDirArray.size();
        auto n2 = n1 + 1;
        if (n2 >= static_cast<int32_t>(aSkyDirArray.size())) n2 -= aSkyDirArray.size();
...
void SKY::UpdateTimeFactor()
{
    ...
    auto const nNext = static_cast<int32_t>(fTimeFactor);
    if (nPrev != nNext) {
        // re-read textures
