...
{
    auto const fDist  = 1.5f + FRAND(fRainRadius);
    auto const fAngle = FRAND(PIm2);
...
void Rain::GenerateRain()
{
    ...
    for (i = 0; i < dwNumRainBlocks; i++) {
        ...
        auto const jitter               = fRainWindSpeedJitter;  // Weather->GetFloat(whf_rain_wind_speed_jitter);
    ...
    // iVBSeaDrops =
    //     rs->CreateVertexBuffer(D3DSEADROPVERTEX_FORMAT, NUM_SEA_DROPS * 4 * sizeof(SEADROPVERTEX), D3DUSAGE_DYNAMIC |
    //     D3DUSAGE_WRITEONLY);
    ...
    // rainbow
    // if (bRainbowEnable) { iRainbowTex = rs->TextureCreate(sRainbowTexture.c_str()); }
    //
...
bool Rain::Init()
{
    cs = static_cast<COLLIDE*>(core->GetService("CollideService"));
...
{
    auto const fDist            = 6.0f * 5.4f * fWindPower;
...
void Rain::RealizeDrops(uint32_t Delta_Time)
{
    ...
    static auto dwShipName = entt::hashed_string::value("Ship");
    ...
    if (auto&& entities = core->GetEntityIds(RAIN_DROPS); !entities.empty()) {
        for (int32_t i = 0; i < iNumNewDrops1 + iNumNewDrops2; i++) {
            ...
            if (pSea) {
                ...
                if (y >= vDst.y && y <= vSrc.y) fTest2 = (vSrc.y - y) / (vSrc.y - vDst.y);
            }
            float fTest = fTest2;
            ...
            if (fTest <= 1.0f) {
                ...
                if (pShip) {
                    ...
                    for (k = 0; k < aShips.size(); k++)
                        if (aShips[k].pShip == pShip) break;
                    if (k == aShips.size()) { aShips.push_back(ship_t {pShip->GetId(), pShip}); }
...
void Rain::Realize(uint32_t Delta_Time)
{
    ...
    //         for (i = 0; i < dwNumRainBlocks; i++) {
    //             auto dwAlpha =
    //                 static_cast<uint32_t>(255.0f * static_cast<float>(pRainBlocks[i].dwTime) / static_cast<float>(dwRainTimeBlend));
    ...
    //             pRainBlocks[i].vPos.x += (fRainSpeed * fRainWindSpeed + f) * static_cast<float>(Delta_Time) / 60.0f * sinf(fWindAngle);
    //             pRainBlocks[i].vPos.z += (fRainSpeed * fRainWindSpeed + f) * static_cast<float>(Delta_Time) / 60.0f * cosf(fWindAngle);
...
{
    switch (message.Long()) {
    case 1: bShow = (0 != message.Long()); break;
    }
    return 0;
}
