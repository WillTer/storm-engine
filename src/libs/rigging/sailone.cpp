...
void SAILONE::goWave(SAILVERTEX* pv, uint32_t Delta_Time)
{
    ...
    if (ss.eSailType == SAIL_TREANGLE) {
        ...
        // fluttering rectangular sails
            ...
            VertIdx = HorzIdx;
            if (!sroll) {
                auto const delta = static_cast<float>(Delta_Time) * pp->FLEXSPEED;
                if (WindUp) {
                    SumWind -= delta;
...
void SAILONE::FillVertex(SAILVERTEX* pv)
{
    ...
    if (ss.eSailType == SAIL_TREANGLE) {
        ...
        if (ss.eSailType == SAIL_TRAPECIDAL) dnorm = ss.boundSphere.c - sgeo.cv.normL * ss.boundSphere.r;
        // enumeration of sections along the sail and looping through them
        // ===============================================================
...
void SAILONE::GoTWave(SAILVERTEX* pv)
{
    ...
    auto WindAmplitude = pp->TsailWindDepend * ss.boundSphere.r * (pp->globalWind.base + .3f) / static_cast<float>((SAIL_ROW_MAX - 1))
        / static_cast<float>((SAIL_ROW_MAX - 1));
    ...
    for (ix = 0; ix < SAIL_ROW_MAX;) {
        ...
        if (ix == SAIL_ROW_MAX - 1)
            if (sailtrope.pnttie[1]) *sailtrope.pPos[1] = pcur;
        ...
        for (iy = 0; iy <= ix; iy++, idx++) {
            ...
            if (ix == 0 || ix == 4 || ix == 8 || ix == 12 || ix == 16)
                if (iy == 0 || iy == 4 || iy == 8 || iy == 12 || iy == 16) SailPnt[svNum++] = pcur;
    ...
    // Set rope point
    if (sailtrope.pnttie[2]) *sailtrope.pPos[2] = pcur - dV;
...
void SAILONE::GoVWave(SAILVERTEX* pv)
{
    ...
    if (ss.bYesLimitPoint) { WindAmplitude = (ss.fDeepH + ss.fDeepZ) * 0.15f; }
    ...
    if (sailtrope.pnttie[0]) *sailtrope.pPos[0] = StartPoint;
    ...
    for (ix = 0; ix < SAIL_COL_MAX;) {
        ...
        ddV = sgeo.cv.ddVv - WindAdd * (2.f - k) / static_cast<float>(SAIL_ROW_MAX) - SailDownVect * 2.f / static_cast<float>(SAIL_ROW_MAX);
        ...
        // Set the anchor point of the rope
        if (ix == 0 && sailtrope.pnttie[2]) *sailtrope.pPos[2] = pcur - dV;
    ...
    // Set the anchor point of the rope
    if (sailtrope.pnttie[3]) *sailtrope.pPos[3] = pcur - dV;
}
...
void SAILONE::SetGeometry()
{
    ...
    auto const tmpCol = 1.f / static_cast<float>((SAIL_COL_MAX - 1));
    auto const tmpRow = 1.f / static_cast<float>((SAIL_ROW_MAX - 1));
    ...
    if (ss.eSailType == SAIL_TREANGLE) {
        ...
        pG->cv.dVh =
            (normLD * ss.fDeepH - p13 * ss.fDeepVh + pG->cv.normR * ss.fDeepVz) * tmpRow * (1.f - ss.holeCount * pp->fSHoleFlexDepend);
    ...
    if (sroll != nullptr) sgeo.cv.dopV = sroll->oldgeo.cv.dopV;
}

void SAILONE::SetRolling(bool bRoll)
{
    if (sroll) {
        if (sroll->rollup == bRoll) return;  // do not re-do
    ...
    {
        auto const locPos = hostNode->loc_mtx.Pos();
...
{
    if (!sroll) return;  // no sail folding
        ...
        if (bRolling) {
            ...
            for (auto i = 0; i < 2; i++)
                if (sailtrope.rrs[i]) {
                    ...
                    case 3: begVect = ss.hardPoints[1]; break;
                    }
...
bool SAILONE::GetGrid(CVECTOR& cam, float perspect)
{
    ...
    if (bRolling) {
        ...
        for (auto i = 0; i < SAIL_FAR_IDX; i++) {
            dwCol = farGrid[i].col;
            dwRow = farGrid[i].row;
            if (distance < farGrid[i].len) break;
        }
    }
...
void SAILONE::DoSRollSail(SAILVERTEX* pv)
{
    ...
    auto const dp   = (ss.hardPoints[1] - ss.hardPoints[0]) / static_cast<float>(m_dwCol - 1);

    auto const p1 = (SAIL_ROW_MAX - 1) / 4;
    auto const p2 = p1 * 2;
    auto const p3 = p1 * 3;
    auto const p4 = p1 * 4;

    auto       windVal = pp->WindVect[VertIdx];  // sinf((float)VertIdx/(float)WINDVECTOR_QUANTITY*2.f*PI);
    auto const dz      = pp->ROLL_Z_VAL * sailHeight;
    auto const dy      = pp->ROLL_Y_VAL * sailHeight;
...
void SAILONE::DoTRollSail(SAILVERTEX* pv)
{
    ...
    auto       pcur = ss.hardPoints[0];
    auto const dp   = (ss.hardPoints[1] - ss.hardPoints[0]) * .25f;
    ...
    if (sailtrope.pnttie[2]) *sailtrope.pPos[2] = ss.hardPoints[1];
}
...
float SAILONE::Trace(const CVECTOR& src, const CVECTOR& dst, bool bCannonTrace)
{
    ...
    auto const minp = ss.boxCenter - ss.boxSize;
    auto const maxp = ss.boxCenter + ss.boxSize;
    ...
    if (ss.eSailType == SAIL_TREANGLE) return TSailTrace(sv, dv, bCannonTrace);
...
float SAILONE::SSailTrace(CVECTOR& src, CVECTOR& dst, bool bCannonTrace)
{
    ...
    if (DoHole) {
        ...
        if (pp->gdata[HostNum].bDeleted != true && pp->gdata[HostNum].bYesShip) {
            ...
            auto  charIdx = -1;
            if (pVai != nullptr && pVai->GetACharacter() != nullptr) charIdx = pVai->GetACharacter()->GetAttributeAsDword("index");
            if (charIdx != -1)
                core->Event(
                    "DoSailHole",
                    "llssllllf",
...
void SAILONE::CalculateSailWind()
{
    ...
    if (bRolling)
        ...
        switch (ss.eSailType) {
        case SAIL_TREANGLE: curSpeed *= pp->ts_min + xtmp * pp->ts_xdep + ztmp * pp->ts_zdep; break;
        case SAIL_TRAPECIDAL: curSpeed *= pp->fs_min + xtmp * pp->fs_xdep + ztmp * pp->fs_zdep; break;
        default: curSpeed *= pp->ss_min + xtmp * pp->ss_xdep + ztmp * pp->ss_zdep;
        }

        // take into account the folding of sails
        if (sroll) curSpeed *= sroll->delta;
    }
}
...
float SAILONE::CheckSailSquar(int i, CVECTOR& va, CVECTOR& vb, CVECTOR& vc, CVECTOR& vsrc, CVECTOR& vdst, bool bCannonTrace)
{
    ...
    auto const  fA = d1.y * d2.z - d1.z * d2.y;
    auto const  fB = d1.z * d2.x - d1.x * d2.z;
    float const fC = d1.x * d2.y - d1.y * d2.x;
    float const fD = fA * va.x + fB * va.y + fC * va.z;

    float const srcDist = fA * vsrc.x + fB * vsrc.y + fC * vsrc.z - fD;
    float const dstDist = fA * vdst.x + fB * vdst.y + fC * vdst.z - fD;
    ...
    if (retVal <= 1.f) {
        ...
        d1 = kA * cv.x + kB * cv.y;
        d2 = kA * vc.x + kB * vc.y;
        if ((d1 != kD) && (d2 != kD) && ((d1 > kD) ^ (d2 > kD))) return 2.f;
        // for bc
        kA = -vbc.y;
        kB = vbc.x;
...
void SAILONE::DoSFreeSail(SAILVERTEX* pv)
{
    ...
    for (int ix = 0; ix < 4; ix++) {
        ...
        for (int iy = 0; iy < 5; iy++) {
            ...
            float mul;
            if (iy)
                mul = (pp->FALL_SSAIL_ADD_MIN + static_cast<float>(rand()) / static_cast<float>(RAND_MAX) * pp->FALL_SSAIL_ADD_RAND)
                    / sqrtf(~dvec);
            else
                mul = 1.f;
...
void SAILONE::DoTFreeSail(SAILVERTEX* pv)
{
    ...
    for (int ix = 0; ix < 5; ix++) {
        ...
        for (int iy = 0; iy <= ix; iy++) {
            ...
            float mul;
            if (iy)
                mul = (pp->FALL_TSAIL_ADD_MIN + static_cast<float>(rand()) / static_cast<float>(RAND_MAX) * pp->FALL_TSAIL_ADD_RAND)
                    / sqrtf(~dvec);
            else
                mul = 1.f;
            SailPnt[gidx] += dvec * mul;
...
{
    int const maxHoleQ = GetMaxHoleCount();
    ss.holeCount       = 0;
...
void SAILONE::SetTurnLimits()
{
    ...
    float const kx = cv1.z - cv0.z;
    float const ky = cv1.x - cv0.x;
    float const kc = cv0.x * kx - cv0.z * ky;

    float const x0 = ss.hardPoints[0].x;
    float const y0 = ss.hardPoints[0].z;

    float      minA  = -1.f;
    float      maxA  = -1.f;
    bool const bV    = (x0 * kx - y0 * ky + kc) >= 0.f;
    bool       bVADD = bV ^ (kx >= 0.f);
    float      nextV;

    float const fstep = 0.01f;
    float const fAdding =
        sqrtf(kx * kx + ky * ky) * (pp->TsailWindDepend * ss.boundSphere.r * 1.3f + inf.boxsize.x + ss.fDeepH + ss.fDeepZ);

    for (float fA = pp->MAXTURNANGL; fA > 0.f; fA -= fstep) {
        float const ca = cosf(fA);
        float const sa = sinf(fA);
...
float SAILONE::GetDistanceFromPointTo3Point(const CVECTOR& v, const CVECTOR& vB1, const CVECTOR& vB2, const CVECTOR& vB3)
{
    ...
    float const   fD = -(vN | vB2);

    float const f = v | vN + fD;
    return f;
}
