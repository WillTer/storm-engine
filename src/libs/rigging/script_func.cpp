...
uint32_t _ShipSailState(VS_STACK* pS)
{
    ...
    auto const nChrIdx = pChrIdx->GetInt();
            ...
            if (formatStr[i] == '#' || !formatStr[i]) {
                if (bBuildAccessString) {
                    if (accessString.size() > 1) {
                        ...
                        if (accessString[0] == 'f' && accessString[1] == '.') { nAttrNameStart = 3; }
                        ATTRIBUTES* pA = pAttr->FindAClass(pAttr, accessString.c_str() + nAttrNameStart);
                        if (pA != nullptr && pA->HasValue()) {
                            char const* writeStr = pA->GetThisAttr();
                            switch (accessString[0]) {
                            case 's': retString += writeStr; break;
                            case 'f': {
                                char  tmpp[256];
                                float ftmp = 0.f;
...
uint32_t _funcGetSailSpeed(VS_STACK* pS)
{
    ...
    float const fSailPow = pSailPow->GetFloat();

    auto*         pHoleMax = (VDATA*)pS->Pop();
    int32_t const nHoleMax = pHoleMax->GetInt();

    auto*         pHoleQ = (VDATA*)pS->Pop();
    int32_t const nHoleQ = pHoleQ->GetInt();
...
uint32_t _RandomHole2Sail(VS_STACK* pS)
{
    ...
    if (!(pData = (VDATA*)pS->Pop())) return IFUNCRESULT_FAILED;
    uint32_t const _holeData = pData->GetInt();
    if (!(pData = (VDATA*)pS->Pop())) return IFUNCRESULT_FAILED;
    int _maxHole = pData->GetInt();
    if (!(pData = (VDATA*)pS->Pop())) return IFUNCRESULT_FAILED;
    int const _groupNum = pData->GetInt();
    if (!(pData = (VDATA*)pS->Pop())) return IFUNCRESULT_FAILED;
    char const* _reyName = pData->GetString();
    if (!(pData = (VDATA*)pS->Pop())) return IFUNCRESULT_FAILED;
    int const _chrIdx = pData->GetInt();
    ...
    for (int i = 0; holeMask > 0; i++, holeMask >>= 1)
        if (!(holeMask & 1)) holeIdx[holeArraySize++] = i;

    holeMask = _holeData;
    while (holeArraySize > 0 && _addHoleQ > 0) {
        int const i = rand() % holeArraySize;
...
uint32_t _DeleteOneSailHole(VS_STACK* pS)
{
    ...
    char const* _reyName = pData->GetString();
    if (!(pData = (VDATA*)pS->Pop())) return IFUNCRESULT_FAILED;
    char const* _groupName = pData->GetString();
    if (!(pData = (VDATA*)pS->Pop())) return IFUNCRESULT_FAILED;
    int const _chrIdx = pData->GetInt();
