...
bool SAIL::Init(std::shared_ptr<entt::registry> const& registry)
{
...
void SAIL::Execute(uint32_t Delta_Time)
{
    ...
    if (gdata && core.Controls->GetDebugAsyncKeyState(VK_MENU) < 0 && core.Controls->GetDebugAsyncKeyState(VK_CONTROL) < 0
            ...
            if ((pTmpMdl = static_cast<MODEL*>(core.GetEntityPointer(gdata[0].modelEI))) != nullptr) {
                ...
                if (auto* nod = pTmpMdl->FindNode(pcTmpMastName)) {
                    ...
                    if (eiMastTmp = core.CreateEntity("MAST"); eiMastTmp != 0) {
                        ...
                        core.Send_Message(eiMastTmp, "lpii", MSG_MAST_SETGEOMETRY, nod, gdata[0].shipEI, gdata[0].modelEI);
                    }
                }
            }
        }
    ...
    if (m_nLastUpdate <= 0) { m_nLastUpdate = GROUP_UPDATE_TIME; }
    ...
    if (bUse) {
        ...
        for (i = 0; i < sailQuantity; i++) {
            ...
            if (slist[i]->sroll && !slist[i]->bFreeSail) slist[i]->DoRollingStep(Delta_Time);
            if (slist[i]->sroll && !slist[i]->bFreeSail) gdata[slist[i]->HostNum].bFinalSailDo = true;
            ...
            if (slist[i]->ss.turningSail) slist[i]->TurnSail(fMaxTurnAngl);
        ...
        for (i = 0; i < groupQuantity; i++) {
            ...
            if (gdata[i].bFinalSailDoOld != gdata[i].bFinalSailDo) {
                if (gdata[i].bYesShip) {
                    ATTRIBUTES* pA = static_cast<VAI_OBJBASE*>(core.GetEntityPointer(gdata[i].shipEI))->GetACharacter();
            ...
            if (gdata[i].bYesShip) {
                ...
                if (pVai != nullptr && pVai->GetACharacter() != nullptr) {
                    ...
                    if (pA != nullptr) pA->SetAttributeUseDword("SP", fftoi(curSP));
...
uint64_t SAIL::ProcessMessage(MESSAGE& message)
{
        ...
        if ((mdl = static_cast<MODEL*>(core.GetEntityPointer(gdata[groupQuantity - 1].modelEI))) != nullptr) {
            ...
            for (int j = 0; true; j++) {
                ...
                for (i = 0; i < gi.nlabels; i++) {
                    ...
                    if (!strncmp(gl.name, "sail", 4)) AddSailLabel(gl, nod, bSailUp);
                }
            }
        } else {
                ...
                if (so->ss.turningSail && posNum != 0)  // setting for turning sails only
                {
                    if (tmpEI = core.GetEntityId("rope"); tmpEI != 0) {
                        if (so->sailtrope.rrs[0] == nullptr) {
                            ...
                            CVECTOR bpos;
                            switch (posNum) {
                            case 1: bpos = so->ss.hardPoints[0]; break;
                            case 2: bpos = so->ss.hardPoints[0]; break;
                            case 3: bpos = so->ss.hardPoints[1]; break;
                            }
            ...
            for (i = 0; i < groupQuantity; i++)
                if (tmpEI == gdata[i].shipEI) {
                    ...
                    if (maxVal < 0.3f) {
                        ...
                        if (gdata[i].curSailSet != ROLLTYPE_FAST) { yesChange = true; }
                    } else {
    ...
    case MSG_SAIL_MAST_PROCESSING:
        ...
        if (m_nMastCreatedCharacter != -1) {
            ...
            if (!param.empty()) {
                if ((m_sMastName = new char[param.size() + 1]) == nullptr) { throw std::runtime_error("allocate memory error"); }
...
void SAIL::AddSailLabel(GEOS::LABEL& lbl, NODE* nod, bool bSailUp)
{
    ...
    if (i == sailQuantity)  // there is no such sail - need to create a new one
    {
        ...
        if (!strncmp(nod->GetName(), "rey", 3)) {
            if (nod->parent)
                if (!strncmp(nod->parent->GetName(), "mast", 4)) cs->mastNum = atoi(&nod->parent->GetName()[4]);
            if (nod->GetName()[4] == 'a' || cs->mastNum == 1) cs->rollType = ROLLTYPE_FAST;
        }
    ...
    switch (lbl.name[4]) {
    case '1': cs->ss.hardPoints[0] = vtmp; break;
    case '2': cs->ss.hardPoints[1] = vtmp; break;
    case '3':
        ...
        break;
    case '4': cs->ss.hardPoints[2] = vtmp; break;
    case '0': cs->ss.bYesLimitPoint = true; cs->ss.LimitPoint = vtmp;
    }
}
...
void SAIL::SetAllSails(int groupNum)
{
    ...
    if (gdata[groupNum].sailQuantity > 0) {
        ...
        int idx                 = 0;
        for (int i = 0; i < sailQuantity; i++)
            if (slist[i]->HostNum == groupNum) { gdata[groupNum].sailIdx[idx++] = i; }
        // see the presence of holes
        if (gdata[groupNum].bYesShip && !gdata[groupNum].bDeleted) {
            ATTRIBUTES* pACh = static_cast<VAI_OBJBASE*>(core.GetEntityPointer(gdata[groupNum].shipEI))->GetACharacter();
            ...
            if (pA != nullptr) {
                std::string const param = std::to_string(gdata[groupNum].maxHole);
                pA->SetValue(param);
...
void SAIL::LoadSailIni()
{
    ...
    ini->ReadString(section, "rollSSailForm", param, sizeof(param) - 1, "0.2,0.8,1.0,0.8,0.4,1.0,1.3,1.0,0.4,0.8,1.0,0.8,0.2");
    sscanf(
        param,
    ...
    sscanf(param, "%f,%f,%f,%f,%f", &TSailRollForm[0], &TSailRollForm[1], &TSailRollForm[2], &TSailRollForm[3], &TSailRollForm[4]);
...
float SAIL::Trace(const CVECTOR& src, const CVECTOR& dst)
{
    ...
    for (int i = 0; i < groupQuantity; i++) {
        ...
        if (!(src.x <= maxp.x && src.x >= minp.x && src.y <= maxp.y && src.y >= minp.y && src.z <= maxp.z && src.z >= minp.z)
            && !(dst.x <= maxp.x && dst.x >= minp.x && dst.y <= maxp.y && dst.y >= minp.y && dst.z <= maxp.z && dst.z >= minp.z)) {
            ...
            if (src.x < minp.x) {
                ...
                if (vmed.y >= minp.y && vmed.y <= maxp.y && vmed.z >= minp.z && vmed.z <= maxp.z) bYesTrace = true;
            }
            ...
            if (!bYesTrace && src.z < minp.z) {
                ...
                if (vmed.y >= minp.y && vmed.y <= maxp.y && vmed.x >= minp.x && vmed.x <= maxp.x) bYesTrace = true;
            }
            ...
            if (!bYesTrace && src.y < minp.y) {
                ...
                if (vmed.z >= minp.z && vmed.z <= maxp.z && vmed.x >= minp.x && vmed.x <= maxp.x) bYesTrace = true;
            }
        ...
        if (retVal <= 1.f) break;  // trace only one ship at a time
    }

    return retVal;
}

char const* SAIL::GetCollideMaterialName()
{
    return "sail";
}
            ...
            if (slist[sn]->sroll == nullptr && !slist[sn]->bRolling) {
                ...
                for (i = 0; i < 2; i++) {
                    if (slist[sn]->sailtrope.rrs[i]) {
                        int const tieNum = slist[sn]->sailtrope.rrs[i]->tiePoint;
                        CVECTOR   endVect;
                        ...
                        case 3: begVect = slist[sn]->ss.hardPoints[1]; break;
                        }
...
float SAIL::Cannon_Trace(int32_t iBallOwner, const CVECTOR& src, const CVECTOR& dst)
{
    ...
    float const retVal = Trace(src, dst);
...
void SAIL::DoSailToNewHost(entid_t newModelEI, entid_t newHostEI, int grNum, NODE* nod, entid_t oldModelEI)
{
    ...
    if (oldg == groupQuantity) return;  // no old model - return without doing anything

    // find a sail
    int sn;
    for (sn = 0; sn < sailQuantity; sn++)
        if (slist[sn]->hostNode == nod && slist[sn]->HostNum == oldg && (grNum == 0 || slist[sn]->groupNum == grNum)) break;
    if (sn == sailQuantity) return;  // there is no such sail - return without result

    // find a link to our sail in the old owner
    int idx;
    for (idx = 0; idx < gdata[oldg].sailQuantity; idx++)
        if (gdata[oldg].sailIdx[idx] == sn) break;
    if (idx == gdata[oldg].sailQuantity) return;  // no sail in the group - return without result
    ...
    if (gn == groupQuantity)  // there is no such owner - create a new one
    {
        GROUPDATA* oldgdata = gdata;
        gdata               = new GROUPDATA[groupQuantity + 1];
        if (gdata == nullptr) throw std::runtime_error("Not memory allocation");
        memcpy(gdata, oldgdata, sizeof(GROUPDATA) * groupQuantity);
    ...
    if (m_nMastCreatedCharacter >= 0 && slist[sn] != nullptr)
        core.Event(
            ...
            "llssllllf",
            -1,
            m_nMastCreatedCharacter,
            (m_sMastName == nullptr ? "#" : m_sMastName),
            slist[sn]->hostNode->GetName(),
            slist[sn]->groupNum,
    ...
    if (i == gdata[gn].sailQuantity) {
        ...
        if (gdata[gn].sailQuantity == 0) {
            ...
            if ((gdata[gn].sailIdx = new int[gdata[gn].sailQuantity + 1]) == nullptr) { throw std::runtime_error("Not memory allocation"); }
    ...
    slist[sn]->sailtrope.pnttie[0] = slist[sn]->sailtrope.pnttie[1] = slist[sn]->sailtrope.pnttie[2] = slist[sn]->sailtrope.pnttie[3] =
        false;
    slist[sn]->sailtrope.pPos[0] = slist[sn]->sailtrope.pPos[1] = slist[sn]->sailtrope.pPos[2] = slist[sn]->sailtrope.pPos[3] = nullptr;
...
void SAIL::DeleteSailGroup()
{
    ...
    int const old_sailQuantity  = sailQuantity;
    int const old_groupQuantity = groupQuantity;
    ...
    if (sailQuantity == 0) sailQuantity = 0;
    ...
    if (sailQuantity == 0 || groupQuantity == 0) {
        ...
        slist = new SAILONE*[sailQuantity];
        gdata = new GROUPDATA[groupQuantity];
        if (slist == nullptr || gdata == nullptr) { throw std::runtime_error("allocate memory error"); }
        ...
        for (gn = 0; gn < old_groupQuantity; gn++) {
            ...
            for (sn = 0; sn < old_sailQuantity; sn++)
                if (oldslist[sn] != nullptr && oldslist[sn]->HostNum == gn) nsn++;
            if (nsn == 0) continue;
            // in a new location, create a record about the sail group
            memcpy(&gdata[groupQuantity], &oldgdata[gn], sizeof(GROUPDATA));
            if ((gdata[groupQuantity].sailIdx = new int[nsn]) == nullptr) { throw std::runtime_error("allocate memory error"); }
...
{
    entid_t rope_id;
    if (!(rope_id = core.GetEntityId("rope"))) return;  // no ropes, no concert
    ...
    for (int si = 0; si < gdata[ogn].sailQuantity; si++) {
        int const sn  = gdata[ogn].sailIdx[si];
        NODE*     nod = nullptr;
                ...
                if (atoi(&gl.group_name[5]) == slist[sn]->groupNum)                   // with the correct group number
                    if (!strncmp(gl.name, "rope", 4) || !strncmp(gl.name, "fal", 3))  // and the sail is tied with a rope
                    {
                        ...
                        if (rb->IsAbsentRope(gdata[ogn].modelEI, nRopeNum)) {
                            ...
                            if (slist[sn]->sailtrope.pnttie[0] || slist[sn]->sailtrope.pnttie[1] || slist[sn]->sailtrope.pnttie[2]
                            ...
                            for (int j = 0; j < iMax; j++)
                                nroot->glob_mtx.MulToInv(
                                    slist[sn]->hostNode->glob_mtx * slist[sn]->ss.hardPoints[j], slist[sn]->ss.hardPoints[j]);
...
SAILONE* SAIL::FindSailFromData(int gn, char const* nodeName, char const* grName) const
{
    ...
    for (int i = 0; i < gdata[gn].sailQuantity; i++) {
        int const sn = gdata[gn].sailIdx[i];
        if (slist[sn]->hostNode != nullptr && strcmp(slist[sn]->hostNode->GetName(), nodeName) == 0 && slist[sn]->groupNum == grNum)
            return slist[sn];
    }
    return nullptr;
}
...
int SAIL::FindGroupForCharacter(int chrIdx) const
{
    for (int gn = 0; gn < groupQuantity; gn++) {
        ...
        if (pA != nullptr)
            if (static_cast<int>(pA->GetAttributeAsDword("index", -1)) == chrIdx) return gn;
    }
    return -1;
}
...
{
    ATTRIBUTES* pA = nullptr;
    if (gdata[grNum].bYesShip) pA = static_cast<VAI_OBJBASE*>(core.GetEntityPointer(gdata[grNum].shipEI))->GetACharacter();
    if (pA != nullptr) return static_cast<int>(pA->GetAttributeAsDword("index", -1));
    return -1;
}

SAILONE_BASE* SAIL::FindSailForCharacter(int chrIdx, char const* nodeName, int grNum)
{
    if (nodeName == nullptr) return nullptr;

    int const gn = FindGroupForCharacter(chrIdx);

    if (gn >= 0 && gn < groupQuantity) {
        for (int idx = 0; idx < gdata[gn].sailQuantity; idx++) {
            int const sn = gdata[gn].sailIdx[idx];
            if (slist[sn]->hostNode != nullptr && slist[sn]->groupNum == grNum && strcmp(slist[sn]->hostNode->GetName(), nodeName) == 0)
                return slist[sn];
        }
    }
...
uint32_t SAIL::ScriptProcessing(char const* name, MESSAGE& message)
{
    if (name == nullptr) return 0;

    if (storm::iEquals(name, "RandomSailsDmg")) {
        int32_t const chrIdx = message.Long();
        float const   fDmg   = message.Float();
        int const     gn     = FindGroupForCharacter(chrIdx);
    ...
    if (storm::iEquals(name, "SailRollSpeed")) {
        int32_t const chrIdx = message.Long();
        float const   fSpeed = message.Float();
        int const     gn     = FindGroupForCharacter(chrIdx);
        if (gn >= 0 && gn < groupQuantity) gdata[gn].fRollingSpeed = fSpeed * ROLLINGSPEED;
    }
...
void SAIL::DoRandomsSailsDmg(int chrIdx, int gn, float fDmg)
{
    ...
    for (int _sailsQnt = gdata[gn].sailQuantity; _sailsQnt > 0;) {
        int const tmp = rand() % _sailsQnt;
        int const sn  = _sailsIdx[tmp];
        ...
        VDATA* pvd = core.Event(
            ...
            slist[sn]->groupNum,
            fDmg,
            static_cast<float>(slist[sn]->maxSpeed) / gdata[gn].speed_m,
            nNewHoleCount,
            slist[sn]->GetMaxHoleCount(),
            slist[sn]->GetHoleDword());
