...
uint64_t ROPE::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_ROPE_INIT: {
        ...
        auto const wFirstRope = ropeQuantity;
        if (gdata != nullptr) {
        ...
        for (j = 0; true; j++) {
            ...
            for (i = 0; i < gi.nlabels; i++) {
                ...
                if (!strncmp(gl.name, "rope", 4)) AddLabel(gl, nod, false);
                if (!strncmp(gl.name, "fal", 3)) AddLabel(gl, nod, true);
        ...
        if (gdata[groupQuantity - 1].ropeIdx == nullptr) { throw std::runtime_error("allocate memory error"); }
        int idx = 0;
        for (int rn = wFirstRope; rn < ropeQuantity; rn++)
            if (rlist[rn]->HostGroup == groupQuantity - 1) gdata[groupQuantity - 1].ropeIdx[idx++] = rn;
    } break;
...
void ROPE::SetVertexes(ROPEDATA* pr, float dtime) const
{
    ...
    if (pr->angDeep > 2.f * PI) pr->angDeep = 0.f;
    pr->angRot += ROTATE_SPEED * dtime;
    if (pr->angRot > 2.f * PI) pr->angRot = 0.f;
    ...
    for (int segn = 0; segn <= pr->segquant; segn++) {
        CVECTOR cv = cvb;
        if (pr->bMakeWave) cv += deepV * static_cast<float>(segn * (pr->segquant - segn));
    ...
    if (rd->bMatWorld != nullptr && rd->eMatWorld != nullptr) {
        ...
        if (rd->len < .1f || rd->len > 200.f) {
            ...
            if (ropeQuantity == 1) {
                ...
                ROPEDATA** oldrlist = rlist;
                rlist               = new ROPEDATA*[ropeQuantity - 1];
                if (rn > 0) memcpy(rlist, oldrlist, sizeof(ROPEDATA*) * rn);
                if (rn < ropeQuantity - 1) memcpy(&rlist[rn], &oldrlist[rn + 1], sizeof(ROPEDATA*) * (ropeQuantity - 1 - rn));
                delete oldrlist;
            }
            ropeQuantity--;
        } else {
            ...
            cvert            = norm.z / rd->len;
            svert            = norm.y / rd->len;
            float const ftmp = sqrtf(norm.x * norm.x + norm.z * norm.z);
            if (ftmp >= 0.0001f) {
                chorz = norm.z / ftmp;
                shorz = norm.x / ftmp;
...
void ROPE::SetTextureGrid(ROPEDATA* pv) const
{
    ...
    float const tvMax = sqrtf(~(*pv->eMatWorld * pv->pEnd - *pv->bMatWorld * pv->pBeg));
    ...
    float const dtv = (tvMax / ROPE_TEX_LEN - tv) / static_cast<float>(pv->segquant);
    for (int j = 0; j <= pv->segquant; j++) {
        for (int i = 0; i < ROPE_EDGE; i++) {
            if (i <= ROPE_EDGE / 2)
                vertBuf[iv + i].tu = static_cast<float>(i) / static_cast<float>((ROPE_EDGE / 2));
            else
                vertBuf[iv + i].tu = 1.f - static_cast<float>(i - ROPE_EDGE / 2) / static_cast<float>((ROPE_EDGE + 1) / 2);
            vertBuf[iv + i].tv = tv;
        }
        iv += ROPE_EDGE;
        tv += dtv;
    ...
    for (int rn = firstNum; rn < ropeQuantity; rn++) {
        ...
        while (rlist[rn]->bMatWorld == nullptr || rlist[rn]->eMatWorld == nullptr) {
            ...
            if (ropeQuantity) {
                ...
                if (rlist == nullptr) rlist = oldrlist;
                if (rn > 0) memcpy(rlist, oldrlist, sizeof(ROPEDATA*) * rn);
                if (rn < ropeQuantity) memcpy(&rlist[rn], &oldrlist[rn + 1], sizeof(ROPEDATA*) * (ropeQuantity - rn));
                if (oldrlist != rlist) delete oldrlist;
            } else {
                delete rlist;
                rlist = nullptr;
            }
...
void ROPE::DoSTORM_DELETE()
{
    ...
    for (gn = 0; gn < groupQuantity; gn++) {
        ...
        if (rq < gdata[gn].ropeQuantity) {
            ...
            if (gdata[gn].ropeIdx == nullptr) gdata[gn].ropeIdx = oldropeIdx;

            gdata[gn].ropeQuantity = rq;
            memcpy(gdata[gn].ropeIdx, oldropeIdx, sizeof(int) * rq);
            if (gdata[gn].ropeIdx != oldropeIdx) delete oldropeIdx;
        }
...
void ROPE::DoDeleteUntie(entid_t mdl_id, NODE* rnod, int gNum)
{
    ...
    if (gn < groupQuantity) {
        for (int ri = 0; ri < gdata[gn].ropeQuantity; ri++) {
            int const rn = gdata[gn].ropeIdx[ri];
