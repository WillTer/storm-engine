...
float TSplash::HeightF(uint32_t _time, float _r, float _k)
{
    ...
    auto const rK     = (GRID_LENGTH - _r) / GRID_LENGTH;
...
bool TSplash::Process(uint32_t _dTime)
{
    ...
    auto const stepSize = static_cast<float>(GRID_LENGTH) / static_cast<float>(GRID_STEPS);
    auto const halfSize = GRID_LENGTH / 2.0f;
    CVECTOR    curPos;
    float      localHeight;

    // calculate alpha
    int const dt = time - SPLASH_FRAMES_COUNT * SPLASH_FRAME_DELAY / 2;
    uint32_t  alpha;
    if (time < SPLASH_FADE_IN_TIME)
        ...
        alpha = static_cast<uint32_t>(255.f - dt * 255.f / (SPLASH_FADE_TIME - SPLASH_FRAMES_COUNT * SPLASH_FRAME_DELAY / 2));
...
bool TSplash::Process2(uint32_t _dTime)
{
    ...
    if (time > (SPLASH_FRAMES_COUNT - 1) * SPLASH_FRAME_DELAY) return false;
