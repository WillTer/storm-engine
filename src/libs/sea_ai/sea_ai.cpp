...
void SEA_AI::Execute(uint32_t Delta_Time)
{
    ...
    for (size_t i = 0; i < AIGroup::AIGroups.size(); ++i) {
        auto const group = AIGroup::AIGroups[i];
...
uint64_t SEA_AI::ProcessMessage(MESSAGE& message)
{
    ...
    case AI_MESSAGE_CANNON_RELOAD: AIShip::ReloadCannons(message.AttributePointer()); break;
    case AI_MESSAGE_SET_CAMERAS_ATTRIBUTE: AIHelper::pASeaCameras = message.AttributePointer(); break;
    ...
    case AI_MESSAGE_GROUP_SET_LOCATION_OTHER_GROUP: {
        std::string const& cGroupName      = message.String();
        std::string const& cOtherGroupName = message.String();
        AIGroup::GroupSetLocationNearOtherGroup(cGroupName.c_str(), cOtherGroupName.c_str());
        break;
    }
    case AI_MESSAGE_GROUP_SET_TYPE: {
        std::string const& cGroupName = message.String();
        std::string const& cTemp      = message.String();
    ...
    case AI_MESSAGE_GROUP_SET_TASK: {
        ...
        case AITASK_ATTACK: {
            std::string const& cGroupName          = message.String();
            std::string const& cGroupAttackingName = message.String();
    ...
    case AI_MESSAGE_UPDATE_RELATIONS: Helper.CalculateRelations(); break;
    ...
    case AI_MESSAGE_CHARACTER_DEAD: {
        ...
        if (pAIObj) {
            pAIObj->SetDead(true);
            if (pAIObj->GetObjType() != AIOBJ_FORT) static_cast<AIShip*>(pAIObj)->GetShipBasePointer()->SetDead();
        }
    } break;
...
void SEA_AI::Load(char const* pStr)
{
    ...
    auto const dwNumGroups = SL.LoadDword();
...
uint32_t SEA_AI::AttributeChanged(ATTRIBUTES* pAttribute)
{
    ...
    if (*pAttribute == "DistanceBetweenGroupShips") { AIGroup::fDistanceBetweenGroupShips = pAttribute->GetAttributeAsFloat(); }
...
void SEA_AI::AddShip(entid_t eidShip, ATTRIBUTES* pCharacter, ATTRIBUTES* pAShip)
{
    ...
    // search group
    char const* pGName = pG->GetAttribute("Name");
    Assert(pGName);
...
void SEA_AI::SetCompanionEnemy(ATTRIBUTES* pACharacter)
{
    ...
    Assert(pSeaAIG);
    char const* pGName = pSeaAIG->GetAttribute("Name");
    Assert(pGName);
