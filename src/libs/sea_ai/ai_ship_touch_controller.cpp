...
CVECTOR* AIShipTouchController::GetPentagonBox()
{
    ...
    auto const vBoxSize = GetAIShip()->GetBoxsize() / 2.0f;
...
void AIShipTouchController::TestCollision(AIShip* pOtherShip)
{
    ...
    auto const fSpeed = GetAIShip()->GetShipBasePointer()->State.vSpeed.z;
    ...
    for (i = 0; i < aTouchRays.size(); i++) {
        auto const fAng = GetAIShip()->GetAng().y + static_cast<float>(i) / static_cast<float>(aTouchRays.size()) * PIm2;
        ...
        for (uint32_t j = 0; j < 5; j++) {
            ...
            if (IntersectLines2D(vOurPos, vPos, vBox[j], vBox[(j == 4) ? 0 : j + 1], vRes)) {
                ...
                auto const fDist   = sqrtf(~(vOurPos - vRes));
...
void AIShipTouchController::Execute(float fDeltaTime)
{
    ...
    auto const fSpeed = KNOTS2METERS(pShip->State.vSpeed.z);
    ...
    if (dtTouchTime.Update(fDeltaTime)) {
        ...
        if (AIHelper::pIsland) {
            for (i = 0; i < aTouchRays.size(); i++) {
                auto const fAng = GetAIShip()->GetAng().y + static_cast<float>(i) / static_cast<float>(aTouchRays.size()) * PIm2;
                auto       vPos = vOurPos + fRaySize * CVECTOR(sinf(fAng), 0.0f, cosf(fAng));
                auto const fRes = static_cast<COLLISION_OBJECT*>(AIHelper::pIsland)->Trace(vOurPos, vPos);
                if (fRes < 1.0f) { aTouchRays[i].fDist *= fRes; }
            }
        }
        ...
        for (i = 0; i < aTouchRays.size() / 2; i++) {
            for (j = 0; j < 2; j++) {
                ...
                auto const fTriRotator = (aTouchRays[ii].fDist + aTouchRays[i1].fDist + aTouchRays[i2].fDist) / 3.0f;
...
float AIShipTouchController::GetBestRotateDirection()
{
    ...
    uint32_t const iBortNum = (aTouchRays.size() - 1) / 2;
...
{
    auto const dwNum = pSL->LoadDword();
