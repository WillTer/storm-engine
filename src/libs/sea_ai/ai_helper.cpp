...
AIHelper::AIHelper() : dwRelationSize(0)
{
    pRelations = nullptr;
}
...
uint32_t AIHelper::GetRelationSafe(ATTRIBUTES* pA1, ATTRIBUTES* pA2) const
{
    ...
    auto const dw1 = FindIndex(pA1);
    if (dw1 == INVALID_ARRAY_INDEX) return RELATION_NEUTRAL;
    auto const dw2 = FindIndex(pA2);
...
void AIHelper::Print(float x, float y, float fScale, char const* pFormat, ...)
{
    char cBuffer[2048];
...
void AIHelper::Save(CSaveLoad* pSL)
{
    ...
    pSL->SaveBuffer((char const*)pRelations, dwRelationSize * dwRelationSize * sizeof(uint32_t));
