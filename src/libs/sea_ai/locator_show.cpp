...
char const* SeaLocatorShow::GetRealLocatorName(ATTRIBUTES* pA)
{
    return pA->GetThisName();
}

char const* SeaLocatorShow::GetLocatorName(ATTRIBUTES* pA)
{
    char const* pName = pA->GetAttribute("name");
    if (!pName) pName = pA->GetThisName();
    return pName;
}

char const* SeaLocatorShow::GetLocatorGroupName(ATTRIBUTES* pA)
{
...
#define Color ((CLerp(255.0f, 0.5f) << 24) | (CLerp(255.0f, 0.7f) << 16) | (CLerp(255.0f, 0.7f) << 8) | (CLerp(255.0f, 0.7f) << 0));
    ...
    auto const    myPI = 3.1415926535897932f;
    int32_t const a1   = 32;
    auto const    a2   = (a1 / 2);
    ...
    auto const light = !CVECTOR(0.0f, 0.0f, 1.0f);
    float      kColor;
    // Filling the vertices
    for (int32_t i = 0, t = 0; i < a2; i++) {
        auto const r1 = sinf(myPI * i / static_cast<float>(a2));
        auto const y1 = cosf(myPI * i / static_cast<float>(a2));
        auto const r2 = sinf(myPI * (i + 1) / static_cast<float>(a2));
        auto const y2 = cosf(myPI * (i + 1) / static_cast<float>(a2));
        for (int32_t j = 0; j < a1; j++) {
            auto const x1 = sinf(2.0f * myPI * j / static_cast<float>(a1));
            auto const z1 = cosf(2.0f * myPI * j / static_cast<float>(a1));
            auto const x2 = sinf(2.0f * myPI * (j + 1) / static_cast<float>(a1));
            auto const z2 = cosf(2.0f * myPI * (j + 1) / static_cast<float>(a1));
            // 0
