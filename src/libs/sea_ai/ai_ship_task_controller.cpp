...
bool AIShipTaskController::CheckCurrentTaskComplete()
{
    ...
    case AITASK_DEFEND:
        ...
        break;
    case AITASK_MOVE:
    case AITASK_RUNAWAY: break;
    case AITASK_FOLLOW:
        if (!GetCurrentTaskAIObj() || GetCurrentTaskAIObj()->isDead()) return true;
        break;
    case AITASK_DRIFT: break;
    }
    return false;
}
...
{
    auto const vFirePos = GetCurrentTaskAIObj()->GetPos();
    auto const vOurDir  = CVECTOR(sinf(GetAIShip()->GetAng().y), 0.0f, cosf(GetAIShip()->GetAng().y));

    auto* pCC = GetAIShip()->GetCannonController();

    auto const bortIt = pCC->GetBestFireBortOnlyDistance(vFirePos, 20.0f + FRAND(50.0f));
    if (pCC->IsValid(bortIt)) {
        auto const vBortDir = pCC->GetBortDirection(*bortIt);
        auto const fRotate  = ((vBortDir | vOurDir) > 0.0f) ? -1.2f : 1.2f;
...
void AIShipTaskController::FindRunAwayPoint()
{
    ...
    auto const fShipK = 1.0f;   // impact factor from ships
    auto const fFortK = 1.0f;   // fort impact factor
    auto const fWindK = 10.0f;  // impact factor for wind

    // check ships
    for (auto& i: AIShip::AIShips) {
        if (GetAIShip() != i) {
            if (!Helper.isEnemy(i->GetACharacter(), GetAIShip()->GetACharacter())) continue;
            auto       vDir      = i->GetPos() - GetAIShip()->GetPos();
            auto const fDistance = sqrtf(~vDir);
    ...
    if (iNumPoints) vRAPoint = vRAPoint / static_cast<float>(iNumPoints);
...
void AIShipTaskController::DoTask(float fDeltaTime)
{
    ...
    case AITASK_DEFEND:
        ...
        break;
    case AITASK_MOVE: SetDestinationPoint(pTask->vTaskPnt); break;
    case AITASK_RUNAWAY: FindRunAwayPoint(); break;
    case AITASK_BRANDER:
...
bool AIShipTaskController::isAttack(ATTRIBUTES* pAOtherCharacter)
{
    ...
    if (pTask && pTask->pATaskCharacter == pAOtherCharacter)  //~!~
    {
        switch (pTask->dwTaskType) {
        case AITASK_ABORDAGE:
        case AITASK_ATTACK: return true; break;
        }
    }
    return false;
}
