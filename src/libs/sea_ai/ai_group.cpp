...
AIGroup::AIGroup(char const* pGroupName)
{
    Assert(pGroupName);
...
void AIGroup::AddShip(entid_t eidShip, ATTRIBUTES* pACharacter, ATTRIBUTES* pAShip)
{
    ...
    if (auto const* pAMode = pACharacter->FindAClass(pACharacter, "Ship.Mode")) {
    ...
    if (char const* event = GetCommanderACharacter()->GetAttribute("GroupShipPos_event")) {
        ...
        vShipPos = CVECTOR(vInitGroupPos.x, vInitGroupPos.y, vInitGroupPos.z)
            - (aGroupShips.size() * fDistanceBetweenGroupShips) * CVECTOR(sinf(vInitGroupPos.y), 0.0f, cosf(vInitGroupPos.y));
    }
...
bool AIGroup::isMainGroup()
{
    for (auto& aGroupShip: aGroupShips)
        if (aGroupShip->isMainCharacter()) return true;
...
void AIGroup::Execute(float fDeltaTime)
{
    ...
    if (bFirstExecute) {
        if (!sLocationNearOtherGroup.empty()) {
            auto const  fNewAng = FRAND(PIm2);
            auto* const pG      = FindGroup(sLocationNearOtherGroup.c_str());
            if (pG) {
                auto vNewGroupPos = pG->vInitGroupPos + ((100.0f + FRAND(200.0f)) * CVECTOR(sinf(fNewAng), 0.0f, cosf(fNewAng)));
        ...
        if (isMainGroup()) {
            for (auto& aGroupShip: aGroupShips)
                if (!aGroupShip->isMainCharacter())
                    aGroupShip->GetTaskController()->SetNewTask(PRIMARY_TASK, AITASK_DEFEND, GetCommanderACharacter());
        }
    ...
    if (!isMainGroup()) {
        ...
        for (auto& aGroupShip: aGroupShips) {
            auto const fCurSpeed = aGroupShip->GetShipBasePointer()->GetCurrentSpeed();
            if (fCurSpeed < fMinimalSpeed) fMinimalSpeed = fCurSpeed;
        }

        auto const bSetFixedSpeed = sCommand == "move";
...
{
    for (auto& aGroupShip: aGroupShips)
        if (!aGroupShip->isDead()) return false;
    return true;
}
...
AIGroup* AIGroup::CreateNewGroup(char const* pGroupName)
{
    Assert(pGroupName);
...
AIGroup* AIGroup::FindGroup(char const* pGroupName)
{
    for (auto& AIGroup: AIGroups)
        if (AIGroup->GetName() == pGroupName) return AIGroup;
    return nullptr;
}
...
{
    for (auto& AIGroup: AIGroups)
        if (AIGroup->isMainGroup()) return AIGroup;
    return nullptr;
}
...
void AIGroup::SailMainGroup(CVECTOR vPos, float fAngle, ATTRIBUTES* pACharacter)
{
    ...
    for (auto pAIShip: pMG->aGroupShips) {
        ...
        pAIShip->SetPos(vPos);
        if (fAngle > -10000.0f) pAIShip->SetAngleY(fAngle);
...
AIGroup* AIGroup::FindOrCreateGroup(char const* pGroupName)
{
    AIGroup* pAIGroup = FindGroup(pGroupName);
    return (pAIGroup) ? pAIGroup : CreateNewGroup(pGroupName);
}

void AIGroup::GroupSetMove(char const* pGroupName, CVECTOR& vMovePoint)
{
...
void AIGroup::GroupSetRunAway(char const* pGroupName)
{
...
float AIGroup::GetAttackHP(char const* pGroupName, float fDistance)
{
...
void AIGroup::GroupSetAttack(char const* pGroupName, char const* pGroupAttackingName)
{
...
void AIGroup::SetXYZ_AY(char const* pGroupName, CVECTOR vPos, float _fAY)
{
    AIGroup* pAIGroup = FindOrCreateGroup(pGroupName);
...
void AIGroup::GroupSetType(char const* pGroupName, char const* cGroupType)
{
    AIGroup* pG = FindOrCreateGroup(pGroupName);
    Assert(pG);
...
void AIGroup::GroupHelpMe(char const* pGroupName, AIShip* pMe, AIShip* pEnemy)
{
    AIGroup* pG = FindGroup(pMe->GetACharacter());
    Assert(pG);
}

void AIGroup::ShipChangeGroup(ATTRIBUTES* pACharacter, char const* pGroupName)
{
...
void AIGroup::GroupSetCommander(char const* pGroupName, ATTRIBUTES* _pACommander)
{
    AIGroup* pG = FindOrCreateGroup(pGroupName);
    Assert(pG);
...
void AIGroup::GroupSetLocationNearOtherGroup(char const* pGroupName, char const* pOtherGroupName)
{
    AIGroup* pG = FindOrCreateGroup(pGroupName);
    Assert(pG);
