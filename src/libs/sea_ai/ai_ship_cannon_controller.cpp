...
{
    auto const bEnoughBalls = isHaveEnoughBallsForBort(bort);
    if (!bEnoughBalls) bNotEnoughBalls = true;
    if (!bort.isCharged() || bort.aCannons.empty() || bort.isBortDamaged()) return false;

    auto vTempFirePos = vFirePos + CVECTOR(0.0f, fFireHeight, 0.0f);

    auto* pVData =
        core->Event(SHIP_GET_BORT_FIRE_DELTA, "afff", GetAIShip()->GetACharacter(), vTempFirePos.x, vTempFirePos.y, vTempFirePos.z);
    auto const fRadius = pVData->GetFloat();
    auto const fAng    = FRAND(PIm2);
    vTempFirePos += CVECTOR(fRadius * sinf(fAng), 0.0f, fRadius * cosf(fAng));

    auto const vTempFireDir = !(vTempFirePos - pOurAIShip->GetPos());

    core->Event(
        SHIP_BORT_FIRE,
        "lisffffff",
        GetIndex(GetAIShip()->GetACharacter()),
        GetAIShip()->GetShipEID(),
        const_cast<char*>(bort.sName.c_str()),
        vTempFirePos.x,
        vTempFirePos.y,
        vTempFirePos.z,
        vTempFireDir.x,
    ...
    for (auto& cannon: bort.aCannons) {
        if (!cannon.isDamaged()) {
            ...
            if (debugDrawToggle) {
                debugFirePositions.emplace_back(vTempFirePos, storm::Color {0xFF, 0xFF, 0xFF, 0x00}.to_hex(), float {});
            }
...
{
    auto const vOurPos  = GetAIShip()->GetPos();
    auto const fSpeedV0 = GetSpeedV0();

    auto const vTempDir = !CVECTOR(vFireDir.x, 0.0f, vFireDir.z);
    ...
    while (IsValid(bortIt)) {
        float const fMaxDist = AICannon::CalcMaxFireDistance(bortIt->fOurBortFireHeight + vOurPos.y, fSpeedV0, fAngle);
...
bool AIShipCannonController::Fire(AIShip* pEnemy)
{
    ...
    Assert(pVData);
    float const fFireHeight = pVData->GetFloat();

    pVData = core->Event(CANNON_GET_FIRE_TIME, "a", GetAIShip()->GetACharacter());
    Assert(pVData);
    float const fFireTime = pVData->GetFloat();

    float const fDistance    = GetAIShip()->GetDistance(*pEnemy);
    float const fSpeedV0     = GetSpeedV0();
    float const fRealSpeedV0 = fSpeedV0 * AIBalls::pAIBalls->GetMultiplier();
    float const fSpeedZ      = pEnemy->GetShipBasePointer()->GetCurrentSpeed();
    float const fAng         = pEnemy->GetShipBasePointer()->GetAng().y;

    auto const vEnemyPos       = pEnemy->GetPos();
    auto const vTargetEnemyPos = vEnemyPos + CVECTOR {0.0f, fFireHeight, 0.0f};
    ...
    auto bortIt = GetFirstFireBort(vFirePos);
    while (IsValid(bortIt)) {
        auto const& cannons = bortIt->aCannons;
        if (!cannons.empty()) {
            auto const& midCannon = cannons[cannons.size() / 2];

            // calc average real speed
            float const fRealAng = midCannon.CalcHeightFireAngle(fSpeedV0, !(vTargetEnemyPos - midCannon.GetPos()), vTargetEnemyPos);
            float const fSpeedV  = fRealSpeedV0 * cosf(fRealAng);

            // calc precise pos
            TOUCH_PARAMS touchParamsNew;
            pEnemy->GetShipBasePointer()->TouchMove(
                static_cast<uint32_t>((fDistance / fSpeedV + fFireTime) * 1000.0f), nullptr, &touchParamsNew);
            vFirePos = touchParamsNew.vPos;

            if (debugDrawToggle) {
                debugFirePositions.emplace_back(vEnemyPos, storm::Color {0xFF, 0xFF, 0x00, 0x00}.to_hex(), float {});
                debugFirePositions.emplace_back(
                    vFirePos + CVECTOR {0.0f, fFireHeight, 0.0f}, storm::Color {0xFF, 0x00, 0xFF, 0x00}.to_hex(), float {});
            }
...
void AIShipCannonController::Execute(float fDeltaTime)
{
    ...
    if (bReload) {
        ...
        for (auto& bort: aShipBorts) {
            bool const bEnoughBalls = isHaveEnoughBallsForBort(bort);
            if (!bEnoughBalls) bNotEnoughBalls = true;
    ...
    if (GetAIShip()->isMainCharacter()) {
        bNotEnoughBalls = false;
        for (auto& bort: aShipBorts) {
            if (isHaveEnoughBallsForBort(bort)) continue;
    ...
    for (auto& bort: aShipBorts) {
        if (!bort.aCannons.empty()) {
            ...
            float const fMaxBortFireDistance =
            ...
            auto const intactCannons = GetBortIntactCannonsNum(bort);
            if (intactCannons > 0) {
            ...
            // if (GetAIShip()->isMainCharacter())
            {
                ...
                pACurBort->SetAttributeUseFloat(
                    "DamageRatio",
                    1.0f
                        - (static_cast<float>(GetBortIntactCannonsNum(bort)) + static_cast<float>(GetBortDisabledCannonsNum(bort)))
...
float AIShipCannonController::GetFireDistance(bool bMaxFireDistance) const
{
    ...
    for (auto const& bort: aShipBorts) {
        if (!bort.aCannons.empty()) {
            float const fMaxFireDistance =
                AICannon::CalcMaxFireDistance(bort.fOurBortFireHeight + vOurPos.y, fSpeedV0, GetBortHeightAngle(bort) + bort.fFireAngMax);
...
bool AIShipCannonController::UpdateParameters()
{
    ...
    for (auto& bort: aShipBorts) {
        if (!bort.aCannons.empty()) {
            ...
            bort.fMaxFireDistance =
                AICannon::CalcMaxFireDistance(bort.fOurBortFireHeight + vOurPos.y, fSpeedV0, GetBortHeightAngle(bort) + bort.fFireAngMax);
            float const fDir = bort.fFireDir;
...
CVECTOR AIShipCannonController::GetFirePos(const CVECTOR& vFireDir, float fDistance)
{
    ...
    auto const bortIt = GetFirstFireBort(vTempFirePos);
    if (!IsValid(bortIt)) { return {1e5f, 0.0f, 1e5f}; }

    float const fMaxDist = AICannon::CalcMaxFireDistance(bortIt->fOurBortFireHeight + vOurPos.y, fSpeedV0, fAngle);
    if (fDistance > fMaxDist) fDistance = fMaxDist;

    float const t = fDistance / (fSpeedV0 * cosf(fAngle));
    float const y = fSpeedV0 * t * sinf(fAngle) - AIHelper::fGravity * SQR(t) / 2.0f;
...
bool AIShipCannonController::isCanFire(const CVECTOR& vCamDir) const
{
    ...
    for (auto const& bort: aShipBorts) {
        if (!bort.aCannons.empty()) {
            ...
            float const fSub = v2.y - v1.y;
            float const fAng = acosf(Clamp(v1 | v2));
            if (fSub > 0.0f && fAng > fabsf(bort.fFireAngMin)) continue;
            if (fSub < 0.0f && fAng > fabsf(bort.fFireAngMax)) continue;
...
bool AIShipCannonController::isCanFireBort(const AISHIP_BORT& bort, const CVECTOR& vFirePos, float* pfZapasDistance) const
{
    if (bort.aCannons.empty()) return false;
    if (bort.isBortDamaged()) return false;

    CVECTOR vFireDir      = (vFirePos - pOurAIShip->GetPos());
    vFireDir              = !CVECTOR(vFireDir.x, 0.0f, vFireDir.z);
    float const fFireDist = sqrtf(~(vFirePos - pOurAIShip->GetPos()));

    // check distance limit
    if (fFireDist > bort.fMaxFireDistance) return false;

    float const fY0 = vFireDir | bort.vDirection;
    if (fY0 <= bort.fCosFireZone) return false;
    ...
    if (pfZapasDistance) *pfZapasDistance = bort.fMaxFireDistance - fFireDist;

    return true;
}

auto AIShipCannonController::GetBestFireBortOnlyDistance(CVECTOR vFirePos, float fZapasDistance) -> decltype(aShipBorts)::iterator
{
    float const   fFireDist  = sqrtf(~(vFirePos - pOurAIShip->GetPos()));
    auto          bestBortIt = std::end(aShipBorts);
    const CVECTOR vOurPos    = GetAIShip()->GetPos();
    auto const    vOurDir    = CVECTOR(sinf(GetAIShip()->GetAng().y), 0.0f, cosf(GetAIShip()->GetAng().y));
    float         fBestAng   = 1e8f;
    for (auto bort = std::begin(aShipBorts); bort != std::end(aShipBorts); ++bort) {
        if (!bort->aCannons.empty()) {
            if (!bort->isCharged() || bort->isBortDamaged()) continue;

            float const fSpeedV0         = GetSpeedV0();
            float const fMaxFireDistance = AICannon::CalcMaxFireDistance(
                bort->fOurBortFireHeight + vOurPos.y, fSpeedV0, GetBortHeightAngle(*bort) + bort->fFireAngMax);
            ...
            float const fAng     = acosf(Clamp(vOurDir | vBortDir));
            if (fAng < fBestAng) {
                fBestAng   = fAng;
                bestBortIt = bort;
            }
...
auto AIShipCannonController::GetFirstFireBort(const CVECTOR& vFirePos, float* pfZapasDistance) -> decltype(aShipBorts)::iterator
{
    for (auto it = std::begin(aShipBorts); it != std::end(aShipBorts); ++it) {
        if (!it->aCannons.empty() && isCanFireBort(*it, vFirePos, pfZapasDistance) && !it->isBortDamaged()) return it;
    }

    return std::end(aShipBorts);
}

auto AIShipCannonController::GetNextFireBort(const decltype(aShipBorts)::iterator bortIt, const CVECTOR& vFirePos, float* pfZapasDistance)
...
void AIShipCannonController::Realize(float fDeltaTime)
{
    ...
    for (auto it = debugFirePositions.begin(); it != debugFirePositions.end();) {
        ...
        if (auto& [pos, color, dt] = *it; dt <= kDebugFadeTime) {
            ...
            auto const dist = (pos - cPos).GetLength();

            float radius = 0.3f;
            if (dist > 1.0f) { radius *= sqrtf(dist); }
    ...
    for (auto const& bort: aShipBorts) {
        for (auto const& cannon: bort.aCannons) {
            if (!cannon.isDamaged()) {
                constexpr auto red   = storm::Color {0xFF, 0xDC, 0x14, 0x3C}.to_hex();
                constexpr auto green = storm::Color {0xFF, 0x7C, 0xFC, 0x00}.to_hex();

                auto const&& vPos = cannon.GetPos();
    ...
    for (auto const& bort: aShipBorts) {
        if (!bort.aCannons.empty()) {
            ...
            // draw baba
            float const fDir   = bort.fFireDir;
            float const fRastr = bort.fFireZone;
            float const fDA    = PId2 / 36.0f;
            for (float fAng = fDir - fRastr / 2.0f; fAng < fDir + fRastr / 2.0f; fAng += fDA) {
                ...
                constexpr auto color = storm::Color {0x0F, 0x90, 0xEE, 0x90}.to_hex();
    ...
    if (!Verts.empty()) {
        ...
        //     D3DPT_TRIANGLELIST, D3DFVF_XYZ | D3DFVF_DIFFUSE, Verts.size() / 3, &Verts[0], sizeof(tr_vertex), "ShipCannonAngles");
    }
}
...
bool AIShipCannonController::ScanShipForCannons()
{
        ...
        for (size_t i = 0; i < static_cast<uint32_t>(info.nlabels); i++) {
            ...
            for (auto& bort: aShipBorts)
                ...
                if (storm::iEquals(bort.sName, label.group_name)) {
                    ...
                    float const fDamage = pADamages->GetAttributeAsFloat(buf, 0.0f);
                    pADamages->SetAttributeUseFloat(buf, fDamage);
                    pCannon->SetDamage(fDamage);
                    if (pCannon->isDamaged()) bort.dwNumDamagedCannons++;
    ...
    for (auto& bort: aShipBorts) {
        if (!bort.aCannons.empty()) {
            ...
            bool const bHaveEnoughBalls = isHaveEnoughBallsForBort(bort);
...
uint32_t AIShipCannonController::GetBortIntactCannonsNum(const AISHIP_BORT& bort) const
{
    ...
    for (auto const& aCannon: bort.aCannons)
        if (!aCannon.isDamaged()) dwCannons++;
    return dwCannons;
}
...
uint32_t AIShipCannonController::GetBortDisabledCannonsNum(const AISHIP_BORT& bort) const
{
    uint32_t dwCannons = 0;
    for (auto const& aCannon: bort.aCannons)
        if (aCannon.isDisabled()) dwCannons++;
    return dwCannons;
}
...
bool AIShipCannonController::isHaveEnoughBallsForBort(const AISHIP_BORT& bort) const
{
    ...
    Assert(pvData);
    if (static_cast<int32_t>(GetBortIntactCannonsNum(bort)) > pvData->GetInt()) return false;
    return true;
}
...
void AIShipCannonController::CheckCannonsBoom(float fTmpCannonDamage, const CVECTOR& vPnt)
{
    ...
    for (auto& bort: aShipBorts) {
        ...
        for (uint32_t j = 0; j < bort.aCannons.size(); j++) {
            ...
            float const fDistance = sqrtf(~(vPnt - vCPos));

            if (fDistance > fMaxCannonDamageDistance) continue;
            VDATA* pVData = core->Event(
                CANNON_DAMAGE,
                "affffff",
                GetAIShip()->GetACharacter(),
                fTmpCannonDamage,
                pC->GetDamage(),
                fDistance,
                vPnt.x,
...
void AIShipCannonController::ResearchCannons()
{
    ...
    for (auto& aShipBort: aShipBorts) {
        ...
        for (uint32_t j = 0; j < aShipBort.aCannons.size(); j++) {
            ...
            sprintf_s(str, "c%d", j);
            float const fDamage = pADamages->GetAttributeAsFloat(str, 0.0f);
            ...
            if (pC->isDamaged()) {
                ...
                pACurBort->SetAttributeUseFloat(
                    "DamageRatio",
                    1.0f - static_cast<float>(GetBortIntactCannonsNum(aShipBort)) / static_cast<float>(aShipBort.aCannons.size()));
            }
        }
    }
}
...
void AIShipCannonController::Load(CSaveLoad* pSL)
{
    ...
    uint32_t const dwNum = pSL->LoadDword();
    for (uint32_t i = 0; i < dwNum; i++) {
        ...
        uint32_t const dwNumCannons = pSL->LoadDword();
