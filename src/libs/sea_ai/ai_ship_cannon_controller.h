...
private:
    struct AISHIP_BORT {
        ...
        AISHIP_BORT() : fOurBortFireHeight(0.0f)
        {
            ClearCharge();
        }

        int operator==(char const* pStr) const
        {
            Assert(pStr && pStr[0]);
...
public:
    ...
    // bort section
    [[nodiscard]] decltype(aShipBorts)::iterator GetFirstFireBort(const CVECTOR& vFirePos, float* pfZapasDistance = nullptr);
    [[nodiscard]] decltype(aShipBorts)::iterator
                       GetNextFireBort(decltype(aShipBorts)::iterator bortIt, const CVECTOR& vFirePos, float* pfZapasDistance = nullptr);
    ...
    [[nodiscard]] bool  isCanFireBort(const AISHIP_BORT& bort, const CVECTOR& vFirePos, float* pfZapasDistance = nullptr) const;
    ...
    [[nodiscard]] decltype(aShipBorts)::iterator GetBestFireBortOnlyDistance(CVECTOR vFirePos, float fZapasDistance);
