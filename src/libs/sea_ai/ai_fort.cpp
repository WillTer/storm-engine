...
void AIFort::Execute(uint32_t Delta_Time)
{
    ...
    auto const bFiredTimer = dtFiredTimer.Update(fDeltaTime);
    ...
    for (uint32_t k = 0; k < aForts.size(); k++) {
        ...
        auto const iMax                = pF->GetAllCannonsNum();  // boal fix
        for (uint32_t i = 0; i < iMax; i++) {
            auto* pC = pF->GetCannon(i);

            auto const dwNewCurrentCannonType = pF->GetCannonType(i);
            ...
            if (!pC->isFired() && pC->isReady2Fire() && bFiredTimer) {
                ...
                auto const fMaxFireDistance = AICannon::CalcMaxFireDistance(vCPos.y, fSpeedV0, 0.35f);  // FIX-ME
                for (auto& j: AIShip::AIShips)
                    if (!j->isDead() && Helper.isEnemy(pF->GetACharacter(), j->GetACharacter())) {
                        auto const fDistance = j->GetDistance(vCPos);
                ...
                if (pFireAIShip) pC->Fire(fSpeedV0, pFireAIShip->GetPos());
            }
...
bool AIFort::AddFort(ATTRIBUTES* pIslandAP, ATTRIBUTES* pFortLabelAP, ATTRIBUTES* pFortCharacter, entid_t eidModel, entid_t eidBlot)
{
    ...
    char const* pModelName = pModelAP->GetThisAttr();
    Assert(pModelName);
    char const* pLocatorsName = pLocatorsAP->GetThisAttr();
    Assert(pLocatorsName);
    char const* pModelsDir = pModelsDirAP->GetThisAttr();
    Assert(pModelsDir);
...
void AIFort::AddFortHit(int32_t iCharacterIndex, CVECTOR& vHitPos)
{
    ...
    for (i = 0; i < aForts.size(); i++) {
        ...
        for (j = 0; j < iMax; j++) {
            ...
            auto       vPos      = pC->GetPos();
            auto const fDistance = sqrtf(~(vPos - vHitPos));
            ...
            auto* pVData = core.Event(
                FORT_CANNON_DAMAGE,
                "llallfffff",
                iCharacterIndex,
                GetIndex(pF->GetACharacter()),
                pF->pFortLabelAP,
                iMax,
                pF->GetDamagedCannonsNum(),
                vPos.x,
                vPos.y,
                vPos.z,
                fDistance,
...
{
    for (auto& aFort: aForts) {
        if (core.GetEntityPointer(aFort->GetModelEID()) == core.GetEntityPointer(eidModel)) return aFort;
    }
    return nullptr;
}
...
uint64_t AIFort::ProcessMessage(MESSAGE& message)
{
    ...
    case AI_MESSAGE_FORT_SET_LIGHTS:
        ...
        if (pShipsLights && pFort) pShipsLights->SetLights(&pFort->tmpObject);
        break;
    case AI_MESSAGE_FORT_UNSET_LIGHTS:
        pFort = FindFort(message.EntityID());
        if (pShipsLights && pFort) pShipsLights->UnSetLights(&pFort->tmpObject);
        break;
    }
    return 0;
}
...
bool AIFort::ScanFortForCannons(AI_FORT* pFort, char const* pModelsDir, char const* pLocatorsName) const
{
    entid_t     model_id;
    GEOS::LABEL label;
    GEOS::INFO  info;
...
float AIFort::Trace(const CVECTOR& vSrc, const CVECTOR& vDst)
{
    ...
    for (uint32_t i = 0; i < GetNumForts(); i++) {
        ...
        Assert(pModel);
        auto const fRes = pModel->Trace(vSrc, vDst);
        if (fRes > 1.0f) continue;
        auto const fDistance = fRes * sqrtf(~(vDst - vSrc));
...
float AIFort::Cannon_Trace(int32_t iBallOwner, const CVECTOR& vSrc, const CVECTOR& vDst)
{
    ...
    for (uint32_t i = 0; i < GetNumForts(); i++) {
        ...
        float const fRes = pModel->Trace(vSrc, vDst);
        ...
        if (fRes <= 1.0f) {
            ...
            core.Send_Message(GetFort(i)->GetBlotEID(), "lffffff", MSG_BLOTS_HIT, vTemp.x, vTemp.y, vTemp.z, vDir.x, vDir.y, vDir.z);
        }
    }
    return fBestRes;
}
...
{
    float const    fDistance = pObj->GetMaxFireDistance();
    uint32_t const iMax      = GetAllCannonsNum();  // boal fix
    for (uint32_t i = 0; i < iMax; i++) {
        AICannon* pC = GetCannon(i);
        if (pC->isDamaged()) continue;

        const CVECTOR vCPos = pC->GetPos();
        float const   fDirY = pC->GetDirY();
