...
void AIShipCameraController::Execute(float fDeltaTime)
{
    ...
    if (isCameraOutside()) {
        ...
        if (GetAIShip()->GetCannonController()->isCanFire(vZ)) {
            ...
            auto const vFireDir  = !(vFirePos - vOurPos);
            auto const fFireDist = sqrtf(~(vFirePos - vOurPos));
            vFirePos.y           = 0.0f;
            // check Ships
            ...
            if (AIFort::pAIFort)
                for (i = 0; i < AIFort::pAIFort->GetNumForts(); i++) {
                    ...
                    for (j = 0; j < iMax; j++) {
                        ...
                        auto const fDistance = pC->GetDistance(vOurPos);
                        if (fDistance > fFireDist) continue;
                        if (fDistance > fBestDistance) continue;
                        auto const fDot = vFireDir | (!(pC->GetPos() - vOurPos));
                        if (fDot < 0.0f) continue;

                        auto       vFire2CannonPos = GetAIShip()->GetCannonController()->GetFirePos(vZ, fDistance);
                        auto const fRealDistance   = ~(vFire2CannonPos - pC->GetPos());
