...
Vector4 const& DataUV::GetValue(uint32_t FrameNum)
{
    uint32_t const TotalFrames = Frames.size();
    FrameNum                   = FrameNum % TotalFrames;
    return Frames[FrameNum];
}

// Set values
void DataUV::SetValues(Vector4 const* _Frames, uint32_t FramesCount)
{
    Frames.clear();
...
char const* DataUV::GetName() const
{
    return Name.c_str();
}
