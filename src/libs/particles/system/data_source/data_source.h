...
private:
    ...
    int FindEmitter(char const* Name);

protected:
    virtual ~DataSource();

public:
    FieldList* CreateEmptyPointEmitter(char const* EmitterName);
    FieldList* CreateBillBoardParticle(char const* ParticleName, char const* EmitterName);
    FieldList* CreateModelParticle(char const* ParticleName, char const* EmitterName);
