...
public:
    ...
    char const* GetRandomName(char const* names)
    {
        Tokenize(names);
    ...
    char const* GetTokenByIndex(uint32_t Index)
    {
        return Tokens[Index];
    }

    void Tokenize(char const* names)
    {
        Tokens.clear();
        strncpy_s(NamesString, names, MAX_STRING_SIZE);
        int StrLen = strlen(NamesString);
        if (StrLen > 0 && NamesString[StrLen - 1] != ';') strcat_s(NamesString, ";");
        ...
        for (auto n = 0; n < StrLen; n++) {
            if (NamesString[n] == ';') {
                ...
                if (ModelName[0] != '\0') { Tokens.push_back(ModelName); }
                FromChar = n + 1;
            }
        }
    }
