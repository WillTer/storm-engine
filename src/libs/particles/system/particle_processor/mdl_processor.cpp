...
ModelProcessor::ModelProcessor(ParticleManager* pManager) : Parser()
{
...
void ModelProcessor::AddParticle(
    ...
    uint32_t        dwGUID)
{
    ...
    auto const* const GeomNames = pFields->GetString(PARTICLE_GEOM_NAMES);
    auto const* const pGeomName = Parser.GetRandomName(GeomNames);
    ...
    auto const VelocityPower = pFields->GetRandomGraphVal(PARTICLE_VELOCITY_POWER, EmitterTime, EmitterLifeTime);
    ...
    auto const* const pEmitterName = pFields->GetString(ATTACHEDEMITTER_NAME);
...
void ModelProcessor::Process(float DeltaTime)
{
    ...
    for (uint32_t n = 0; n < Particles.size(); n++) {
        ...
        auto const Time     = Particles[n]->ElapsedTime;
        auto const LifeTime = Particles[n]->LifeTime;
        ...
        auto const GravK = Particles[n]->graph_GravK->GetValue(Time, LifeTime, Particles[n]->GravKK);
