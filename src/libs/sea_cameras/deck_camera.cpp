...
bool DeckCamera::LoadState(ENTITY_STATE* state)
{
    SetDevice();
    return true;
}

void DeckCamera::Execute(uint32_t Delta_Time)
{
    if (!isOn()) return;
...
void DeckCamera::Realize(uint32_t Delta_Time)
{
    if (!isOn()) return;
}

void DeckCamera::Move(uint32_t DeltaTime)
{
    if (!isActive()) return;
    ...
    if (speed != 0.f) {
        ...
        while (true) {
            ...
            if (htmp >= -500.f && vRes.y - htmp < MEN_STEP_UP && htmp - vRes.y < MEN_STEP_UP) {
                ...
                if (bNoFinded) break;  // standing where there is no floor
                ...
                if (len > 0.f)
                    if (speed >= len) {
                        ...
                        if (MultiTrace(src, dst, camera_pos.y) <= 1.f) {
                            ...
                            src.x = dst.x = vRes.x;
                            src.z = dst.z = vRes.z;
                            if (MultiTrace(src, dst, camera_pos.y) > 1.f) break;
                        }
                    } else {
        ...
        if (!bNoFinded) {
            ...
            if (fTmp <= 1.f)
                ...
                if (MultiTrace(src, dst, camera_pos.y) <= 1.f) camera_pos = prev_res;
            }
        }
    }
...
uint64_t DeckCamera::ProcessMessage(MESSAGE& message)
{
    switch (message.Long()) {
...
bool DeckCamera::GetCrossXZ(CVECTOR& spos, CVECTOR& dv, CVECTOR& p1, CVECTOR& p2, CVECTOR& res)
{
    auto bNoCross = false;
    ...
    if (!bNoCross)
        if (EQU_FLOAT(p1.z, p2.z))
            if (bZset) {
                if (p1.z != zRes) bNoCross = true;
            } else {
                bZset = true;
                zRes  = p1.z;
            }

    if (bNoCross) return false;  // no intersection
    if (bXset && bZset)
        if ((spos.x - p1.x) * (p2.z - p1.z) != (p2.x - p1.x) * (spos.z - p1.z)) return false;  // no intersection
    ...
    if (!bZset && !bXset) {
        xRes =
            (p1.z - spos.z + spos.x * dv.z / dv.x - p1.x * (p2.z - p1.z) / (p2.x - p1.x)) / (dv.z / dv.x - (p2.z - p1.z) / (p2.x - p1.x));
    ...
    // on X
    if (p1.x < p2.x) {
        if (xRes < p1.x || xRes > p2.x) return false;
    }  // no intersection on the segment
    else if (xRes < p2.x || xRes > p1.x)
        return false;
    // on Z
    if (p1.z < p2.z) {
        if (zRes < p1.z || zRes > p2.z) return false;
    ...
    if ((xRes > spos.x && dv.x < 0.f) || (xRes < spos.x && dv.x > 0.f) || (zRes > spos.z && dv.z < 0.f) || (zRes < spos.z && dv.z > 0.f))
        return false;
    res.x = xRes;
    res.y = yRes;
    res.z = zRes;
...
uint32_t DeckCamera::AttributeChanged(ATTRIBUTES* pAttr)
{
...
void DeckCamera::SetViewPoint(CVECTOR& cViewPoint)
{
...
float DeckCamera::MultiTrace(const CVECTOR& cvUp, const CVECTOR& cvDown, float fHBase)
{
...
void DeckCamera::Save(CSaveLoad* pSL)
{
    pSL->SaveBuffer((char const*)&tri, sizeof(tri));
    ...
    pSL->SaveFloat(h_eye);
    pSL->SaveBuffer((char const*)&Screen_Rect, sizeof(Screen_Rect));
    pSL->SaveLong(lock_x);
    pSL->SaveLong(lock_y);
...
void DeckCamera::Load(CSaveLoad* pSL)
{
    pSL->Load2Buffer(&tri);
