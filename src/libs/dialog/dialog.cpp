...
void DIALOG::DlgTextDescribe::ChangeText(std::string_view const text)
{
    ...
    for (;;) {
        size_t const next_break = current_span.find_first_of('\\', current_offset);
        if (next_break != std::string_view::npos && next_break < current_span.size() - 1) {
            auto const next_char = current_span[next_break + 1];
            if (storm::ichar_traits<char>::eq(next_char, 'n')) {
                auto const temp_text = std::string(current_span.substr(0, next_break)) + "...";
...
int32_t DIALOG::DlgTextDescribe::GetShowHeight()
{
    ...
    for (n = 0; n < pageBreaks_.size(); n++)
        if (currentLine_ < pageBreaks_[n]) break;
...
void DIALOG::DlgTextDescribe::Show(int32_t nY)
{
    ...
    i = 0;
    for (n = 0; n < pageBreaks_.size(); n++)
        if (pageBreaks_[n] > currentLine_) break;
    if (n < pageBreaks_.size())
        nEnd = pageBreaks_[n];
    else
        nEnd = asText.size();
...
bool DIALOG::DlgTextDescribe::IsLastPage()
{
    ...
    for (n = 0; n < pageBreaks_.size(); n++)
        if (pageBreaks_[n] > currentLine_) break;
    if (n >= pageBreaks_.size() || pageBreaks_[n] >= asText.size()) return true;
    return false;
}
...
void DIALOG::DlgTextDescribe::NextPage()
{
    ...
    for (n = 0; n < pageBreaks_.size(); n++)
        if (pageBreaks_[n] > currentLine_) break;
    if (n < pageBreaks_.size() && pageBreaks_[n] < asText.size()) currentLine_ = pageBreaks_[n];
}
...
void DIALOG::FillButtons()
{
    ...
    if ((!m_DlgText.IsLastPage()) || linkDescribe_.CanScrollDown()) m_dwButtonState |= BUTTON_STATE_DOWNENABLE;

    if (m_DlgText.IsLastPage() && linkDescribe_.CanScrollUp()) m_dwButtonState |= BUTTON_STATE_UPENABLE;
...
void DIALOG::Realize(uint32_t Delta_Time)
{
    ...
    // play speech
    auto const& snd = m_registry->ctx().get<SoundServicePtr>();
    assert(snd);
    ...
    if (cs.state == CST_ACTIVATED || cs2.state == CST_ACTIVATED || cs1.state == CST_ACTIVATED)  // boal
    {
        ...
        if (m_DlgText.IsLastPage()) {
            ...
            if (pA) pA = pA->GetAttributeClass(linkDescribe_.GetSelectedLine());
            if (pA) {
                char const* goName = pA->GetAttribute("go");
...
uint32_t DIALOG::AttributeChanged(ATTRIBUTES* pA)
{
    ...
    if (par != nullptr) {
        char const* parname = par->GetThisName();
        if (parname != nullptr && storm::iEquals(parname, "Links")) parLinks = true;
    }

    char const* nm = pA->GetThisName();
...
void DIALOG::UpdateDlgTexts()
{
    ...
    std::string const& text = AttributesPointer->GetAttribute("Text");
