...
constexpr float const   DIVIDER_HEIGHT     = 10;
constexpr int32_t const DIALOG_LINE_HEIGHT = 26;
...
constexpr std::array SPRITE_DATA = {
    ...
    // Dialog lines
    SpriteInfo {
        .position = {-39, static_cast<float>(479 - 67 + 39 - DIALOG_LINE_HEIGHT), 639 + 39, static_cast<float>(479 - 67 + 39)},
    ...
    // Divider
    SpriteInfo {
        .position = {35, static_cast<float>(450 - (DIVIDER_HEIGHT / 2)), 605, static_cast<float>(450 + (DIVIDER_HEIGHT + 2))},
...
void LegacyDialog::Realize(uint32_t deltaTime)
{
    ...
    if (backNeedsUpdate_) { UpdateBackBuffers(); }

    bool const shouldDrawDivider = linkDescribe_.GetShowHeight() != 0;
...
uint32_t LegacyDialog::AttributeChanged(ATTRIBUTES* attributes)
{
    ...
    if (storm::iEquals(attributeName, "texture")) {
        ...
        std::string const mood = attributes->GetThisAttr();
        mood_                  = mood;
    } else if (storm::iEquals(attributeName, "greeting")) {
        std::string const soundName = attributes->GetThisAttr();
...
void LegacyDialog::UpdateBackBuffers()
{
    ...
    auto createSpriteMesh = [hScale, vScale](SpriteInfo const& sprite) {
        std::array const mesh = {
            XI_TEX_VERTEX {
                .pos {hScale * sprite.position.left, vScale * sprite.position.top, 1.f},
                .rhw   = 0.5f,
                .color = COLOR_NORMAL,
                .u     = sprite.uv.left,
                .v     = sprite.uv.top},
            XI_TEX_VERTEX {
                .pos {hScale * sprite.position.right, vScale * sprite.position.top, 1.f},
                .rhw   = 0.5f,
                .color = COLOR_NORMAL,
                .u     = sprite.uv.right,
                .v     = sprite.uv.top},
            XI_TEX_VERTEX {
                .pos {hScale * sprite.position.left, vScale * sprite.position.bottom, 1.f},
                .rhw   = 0.5f,
                .color = COLOR_NORMAL,
                .u     = sprite.uv.left,
                .v     = sprite.uv.bottom},
            XI_TEX_VERTEX {
                .pos {hScale * sprite.position.right, vScale * sprite.position.bottom, 1.f},
                .rhw   = 0.5f,
                .color = COLOR_NORMAL,
                .u     = sprite.uv.right,
                .v     = sprite.uv.bottom},
    ...
    // Top of main dialog
    {
        size_t const text_lines = formattedDialogText_.size();
        ...
        if (linkDescribe_.GetShowHeight() != 0) { textureLines_ += 1; }

        sprite_data       = SPRITE_DATA[8];
        auto const offset = DIALOG_LINE_HEIGHT * static_cast<float>(textureLines_);
        sprite_data.position.top -= offset;
        sprite_data.position.bottom -= offset;
    ...
    // Divider
    {
        ...
        auto const offset = linkDescribe_.GetShowHeight() / vScale;
        sprite_data.position.top -= offset;
        sprite_data.position.bottom -= offset;
...
{
    if (headModel_ == invalid_entity) return;
...
{
    size_t const previous_lines = formattedDialogText_.size();

    char const* text_attr = AttributesPointer->GetAttribute("Text");
