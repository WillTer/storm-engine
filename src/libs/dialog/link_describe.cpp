...
void DlgLinkDescribe::ChangeText(ATTRIBUTES* pALinks)
{
    ...
    size_t const number_of_links = pALinks->GetAttributesNum();
    for (size_t i = 0; i < number_of_links; ++i) {
        auto* pA = pALinks->GetAttributeClass(i);
        if (pA) {
            if (pA->GetAttribute("edit")) {
                bool const has_var_target = !iEquals(to_string(pA->GetAttribute("edit")), "string");
                edit_                     = EditConfig {
...
int32_t DlgLinkDescribe::GetShowHeight()
{
    ...
    if (n > maxLinesPerPage_) n = maxLinesPerPage_;
...
void DlgLinkDescribe::Show(int32_t nY)
{
    ...
    if (selectedLine_ < lineBreaks_.size() && selectedLine_ >= 0) {
        if (selectedLine_ > 0) nBeg = lineBreaks_[selectedLine_ - 1];
        ...
        if (pKeys) {
            for (int32_t n = 0; n < nKeyQ; n++) {
                if (pKeys[n].bSystem) {
                    ...
                    case VK_LEFT:
                        if (edit_->charIndex > 0) edit_->charIndex--;
                        break;
                    case VK_BACK:
                    ...
                    case VK_RIGHT: {
                        ...
                        if (edit_->charIndex < strLength) edit_->charIndex++;
                    } break;
                    case VK_DELETE: {
                        int strLength = utf8::Utf8StringLength(textLines_[nTextIdx].c_str());
...
DlgLinkDescribe& DlgLinkDescribe::MoveUp()
{
    if (CanMoveUp()) {
        --selectedLine_;
        if (selectedLine_ > 0) {
            if (startIndex_ > lineBreaks_[selectedLine_ - 1]) { startIndex_ = lineBreaks_[selectedLine_ - 1]; }
        } else {
            startIndex_ = 0;
        }
    }
...
DlgLinkDescribe& DlgLinkDescribe::MoveDown()
{
    if (CanMoveDown()) {
        ++selectedLine_;
        if (startIndex_ + maxLinesPerPage_ < lineBreaks_[selectedLine_]) { startIndex_ = lineBreaks_[selectedLine_] - maxLinesPerPage_; }
    }
    return *this;
}
