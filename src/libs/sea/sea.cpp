...
bool SEA::Init(std::shared_ptr<entt::registry> const& registry)
{
    ...
    for (i = 0; i < FRAMES; i++) {
        ...
        for (uint32_t y = 0; y < YWIDTH; y++)
            for (uint32_t x = 0; x < XWIDTH; x++) {
                auto const dwAddress = x + y * YWIDTH;
...
#define GET_MASSIVE(dx, dy) (float(pMassive[((x + dx) & (XWIDTH - 1)) + ((y + dy) & (XWIDTH - 1)) * XWIDTH] & 0xFF) / 255.0f)
                ...
                if (bSimpleSea) { vRes1 = !(vRes * CVECTOR(100.0f, 1.0f, 100.0f)); }
...
bool SEA::EditMode_Update()
{
    ...
    float const fReflection   = 0.8f;
    float const fTransparency = 0.7f;
    float       fFrenel       = 0.75f;
    float const fAttenuation  = 0.9f;
...
void SEA::BuildTree(int32_t iTX, int32_t iTY, int32_t iLev)
{
    ...
    float const   fGSize = fGridStep * iSize;
...
float SEA::WaveXZ(float x, float z, CVECTOR* pNormal)
{
    ...
    float const fDistance = Sqr(x - vCamPos.x) + Sqr(z - vCamPos.z);
    if (fDistance > fMaxSeaDistance * fMaxSeaDistance) {
        if (pNormal) *pNormal = CVECTOR(0.0f, 1.0f, 0.0f);
        return 0.0f;
    }

    float const x1 = (x + vMove1.x) * fScale1;
    float const z1 = (z + vMove1.z) * fScale1;
    iX11 = ffloor(x1 + 0.0f), iX12 = iX11 + 1;
    iY11 = ffloor(z1 + 0.0f), iY12 = iY11 + 1;
    float const fX1 = (x1 - iX11);
    float const fZ1 = (z1 - iY11);
    iX11 &= (XWIDTH - 1);
    iX12 &= (XWIDTH - 1);
    iY11 &= (XWIDTH - 1);
    iY12 &= (XWIDTH - 1);

    float const x2 = (x + vMove2.x) * fScale2;
    float const z2 = (z + vMove2.z) * fScale2;
    iX21 = ffloor(x2 + 0.0f), iX22 = iX21 + 1;
    iY21 = ffloor(z2 + 0.0f), iY22 = iY21 + 1;
    float const fX2 = (x2 - iX21);
    float const fZ2 = (z2 - iY21);
    iX21 &= (XWIDTH - 1);
    iX22 &= (XWIDTH - 1);
    iY21 &= (XWIDTH - 1);
    iY22 &= (XWIDTH - 1);
    ...
    float const nX1 = (nx1 + fX1 * (nx2 - nx1) + fZ1 * (nx3 - nx1) + fX1 * fZ1 * (nx4 + nx1 - nx2 - nx3));
    float const nZ1 = (nz1 + fX1 * (nz2 - nz1) + fZ1 * (nz3 - nz1) + fX1 * fZ1 * (nz4 + nz1 - nz2 - nz3));
    ...
    float const nX2 = (nx1 + fX2 * (nx2 - nx1) + fZ2 * (nx3 - nx1) + fX2 * fZ2 * (nx4 + nx1 - nx2 - nx3));
    float const nZ2 = (nz1 + fX2 * (nz2 - nz1) + fZ2 * (nz3 - nz1) + fX2 * fZ2 * (nz4 + nz1 - nz2 - nz3));
    // float nX2 = 0.0f;
    // float nZ2 = 0.0f;
    ...
    if (pNormal) {
        ...
        pNormal->y = 1.0f;
        pNormal->z = 0.0f;*/
        float const nY1 = sqrtf(1.0f - (Sqr(nX1) + Sqr(nZ1)));
        float const nY2 = sqrtf(1.0f - (Sqr(nX2) + Sqr(nZ2)));
...
void SEA::PrepareIndicesForBlock(uint32_t dwBlockIndex)
{
    ...
    float const fStep = fGridStep * static_cast<float>(1 << pB->iLOD);
    float       fSize = fGridStep * pB->iSize;
    int32_t     x, y, size0 = pB->iSize >> pB->iLOD;

    pB->iIStart = iIStart;

    float const x1 = static_cast<float>(pB->iTX * pB->iSize) * fGridStep;
    float const y1 = static_cast<float>(pB->iTY * pB->iSize) * fGridStep;
    float const x2 = x1 + static_cast<float>(size0) * fStep;
    float const y2 = y1 + static_cast<float>(size0) * fStep;
    ...
    for (i = 0; i < static_cast<int32_t>(dwBlockIndex); i++)  // if (i == 6)
    {
        ...
        if (bTestedUp || bTestedDown) {
            ...
            if ((pB->iX1 >= pB2->iX1 && pB->iX2 <= pB2->iX2) || (pB->iX1 <= pB2->iX1 && pB->iX2 >= pB2->iX2)) {
                int32_t const iMinX = Max(pB->iX1, pB2->iX1);
                int32_t const iMaxX = Min(pB->iX2, pB2->iX2);

                int32_t const iStartDstX = pB->iSize0 * (iMinX - pB->iX1) / (pB->iX2 - pB->iX1);
                int32_t const iStartSrcX = pB2->iSize0 * (iMinX - pB2->iX1) / (pB2->iX2 - pB2->iX1);

                int32_t const iEndDstX = pB->iSize0 * (iMaxX - pB->iX1) / (pB->iX2 - pB->iX1);
        ...
        if (bTestedLeft || bTestedRight) {
            ...
            if ((pB->iY1 >= pB2->iY1 && pB->iY2 <= pB2->iY2) || (pB->iY1 <= pB2->iY1 && pB->iY2 >= pB2->iY2)) {
                int32_t const iMinY = Max(pB->iY1, pB2->iY1);
                int32_t const iMaxY = Min(pB->iY2, pB2->iY2);

                int32_t const iStartDstY = pB->iSize0 * (iMinY - pB->iY1) / (pB->iY2 - pB->iY1);
                int32_t const iStartSrcY = pB2->iSize0 * (iMinY - pB2->iY1) / (pB2->iY2 - pB2->iY1);

                int32_t const iEndDstY = pB->iSize0 * (iMaxY - pB->iY1) / (pB->iY2 - pB->iY1);
...
void SEA::SSE_WaveXZBlock(SeaBlock& pB)
{
    ...
    int32_t const iIFirst  = pB.iIFirst;
    int32_t const iILast   = pB.iILast;

    // calculate
    for (cz = y1, y = 0; y <= size0; y++, cz += fStep) {
        for (cx = x1, x = 0; x <= size0; x++, cx += fStep) {
            uint32_t const dwVIndex = pIndices[iIStart1];
...
void SEA::CalculateNormalMap(float fFrame, float fAmplitude, float* pfOut, std::vector<uint32_t*>& aFrames)
{
    ...
    float const fDelta = fFrame - iFrame1;
    ...
    for (int32_t y = 0; y < YWIDTH; y++)
        for (int32_t x = 0; x < XWIDTH; x++) {
            uint32_t const dw1 = pB1[x + y * XWIDTH];
            uint32_t const dw2 = pB2[x + y * XWIDTH];
            float const    nx1 = static_cast<float>(static_cast<short>(dw1)) / 32767.5f;
            float const    nx2 = static_cast<float>(static_cast<short>(dw2)) / 32767.5f;
            float const    nz1 = static_cast<float>(static_cast<short>(dw1 >> 0x10)) / 32767.5f;
            float const    nz2 = static_cast<float>(static_cast<short>(dw2 >> 0x10)) / 32767.5f;
...
void SEA::CalculateHeightMap(float fFrame, float fAmplitude, float* pfOut, std::vector<uint8_t*>& aFrames)
{
    ...
    for (int32_t y = 0; y < YWIDTH; y++)
        for (int32_t x = 0; x < XWIDTH; x++) {
            float const f1 = pB1[x + y * XWIDTH];
            float const f2 = pB2[x + y * XWIDTH];
...
void SEA::Realize(uint32_t dwDeltaTime)
{
    ...
    if (vCamPos.y < 0.0f) bUnderSea = true;

    if (vCamPos.y < WaveXZ(vCamPos.x, vCamPos.z && bStarted)) bUnderSea = true;
    ...
    vSeaCenterPos      = CVECTOR(
        fBlockSize * (static_cast<int32_t>(vCamPos.x / fBlockSize) - iNumBlocks),
        fMaxSeaHeight * 0.5f,
    ...
    if (iVStart && iTStart) {
        ...
        auto const vec3 = CVECTOR4(fFoamV, fFoamK, fFoamUV, 6.0f);
        auto const vec4 = CVECTOR4(fTmp, fTmp, fTmp, fTmp);
        auto const vec5 = CVECTOR4(vCamPos.x - vWorldOffset.x, vCamPos.y, vCamPos.z - vWorldOffset.z, 1.0f);
        auto const vec6 = CVECTOR4(1.0f, 0.0f, 0.0f, 1.0f);
        auto const vec7 = CVECTOR4(fFrenel, 1.0f, 0.5f, 1.0f);
...
float SEA::Trace(const CVECTOR& vSrc, const CVECTOR& vDst)
{
    ...
    float         fRes      = 2.0f;
    float const   fDV       = 1.0f / static_cast<float>(iNumTests - 1);
    ...
    for (int32_t i = 0; i < iNumTests; i++) {
        ...
        float const   fWaveY = WaveXZ(vTemp.x, vTemp.z, nullptr);

        if (fWaveY > vTemp.y) return static_cast<float>(i) * fDV;
    }
...
uint32_t SEA::AttributeChanged(ATTRIBUTES* pAttribute)
{
    ...
    if (*pAttribute == "Stop") { bStop = AttributesPointer->GetAttributeAsDword("Stop") != 0; }

    if (*pAttribute == "UnderWater") { bUnderSeaEnable = AttributesPointer->GetAttributeAsDword("UnderWater") != 0; }

    if (*pAttribute == "MaxSeaHeight") {
        fMaxSeaHeight = AttributesPointer->GetAttributeAsFloat("MaxSeaHeight", 50.0f);

        float const fScale = (fMaxSeaHeight >= _fAmp1 + _fAmp2) ? 1.0f : fMaxSeaHeight / (_fAmp1 + _fAmp2);
    ...
    if (*pAttribute == "MaxSeaDistance") { fMaxSeaDistance = AttributesPointer->GetAttributeAsFloat("MaxSeaDistance"); }
