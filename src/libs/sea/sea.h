...
private:
    ...
    struct SeaBlock {
        ...
        static bool QSort(SeaBlock const& b1, SeaBlock const& b2)
        {
            return (b1.iLOD > b2.iLOD);
        }

        inline bool operator<(SeaBlock const& B) const
        {
            return (B.iLOD < iLOD);
        };
    };
    ...
    inline float CalcLod(float const& x, float const& y, float const& z);
