...
inline int Utf8ToCodepoint(char const* utf8)
{
...
inline int Utf8StringLength(char const* s)
{
    int32_t s_num = 0;
    while (*s)
        s_num += (*s++ & 0xC0) != 0x80;
...
inline int u8_inc(char const* s)
{
    int i = 1;
...
inline int u8_dec(char const* s)
{
    int i = 1;
    while (s[-i] && (s[-i] & 0xC0) == 0x80) {
        i++;
...
/* charnum => byte offset */
inline int u8_offset(char const* str, int charnum)
{
    int offs = 0;
