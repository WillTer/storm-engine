...
bool SEA_OPERATOR::Init(std::shared_ptr<entt::registry> const& registry)
{
...
uint64_t SEA_OPERATOR::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_SEA_OPERATOR_BALL_UPDATE: {
        ...
        if (myShip->GetACharacter() != attr) break;

        auto const ballAlive = message.Long() != 0;
        if (ballAlive && !ballTracked) {
            StartNewAction();
            ballTracked = true;
        }

        if (!ballAlive) { StartNewAction(); }
        ...
        if (!ballAlive) finalBallPosition = ballPosition;
    } break;

    case MSG_SEA_OPERATOR_SHIP_HIT: {
        if (!IsTimeToActivate()) break;
        ...
        if (shipAttr == myShip->GetACharacter()) HandleShipHit();
    } break;
    }
    return 0;
}
...
void SEA_OPERATOR::Execute(uint32_t _dTime)
{
    ...
    if (((rand() % IDLE_ACTION_SEED) == 1) && IsTimeToActivate()) HandleShipIdle();

    if (!active) sinceLastActionTime += _dTime;
...
void SEA_OPERATOR::HandleShipIdle()
{
    ...
    auto const startAngle    = randCentered(PId2);
    auto const startDistance = randUpper(2.0f * myShip->GetCurrentSpeed() * action.actionTime / 1e3f);
...
void SEA_OPERATOR::HandleShipFire(entid_t _shipID, char const* _bortName, const CVECTOR& _destination, const CVECTOR& _direction)
{
    ...
    auto const shipDirection     = CVECTOR(sinf(ship->GetAng().y), 0.0f, cosf(ship->GetAng().y));
...
void SEA_OPERATOR::ShowAttackerBort(tAction* _action)
{
    ...
    auto const timeDistance      = static_cast<float>(_action->timePassed) / 7e2f;
    auto const shipDirection     = CVECTOR(sinf(_action->attackerShip->GetAng().y), 0.0f, cosf(_action->attackerShip->GetAng().y));
    auto       shipDirectionPerp = CVECTOR(shipDirection.z, 0.0f, -1.0f * shipDirection.x);
    auto const directionPerp     = CVECTOR(_action->direction.z, 0.0f, -1.0f * _action->direction.x);
...
void SEA_OPERATOR::ShowBallAtMyShip(tAction* _action)
{
    ...
    auto const minY      = 1.0f + sea->WaveXZ(cameraPos.x, cameraPos.z);
    auto const timeScale = MIN_TIME_DELTA + (1.0f - MIN_TIME_DELTA) * powf(timeK, 0.37f);
...
bool SEA_OPERATOR::IsTimeToActivate(bool _testControls)
{
    ...
    if (sinceLastActionTime < TIME_BETWEEN_ACTIONS) return false;
