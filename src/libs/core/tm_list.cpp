...
void TM_LIST::Initialize(HWND hwnd, HINSTANCE hinst, uint32_t style, uint32_t style_ex)
{
    ...
    hOwn = CreateWindowEx(
        ...
        hInst,
        nullptr);
    if (hOwn == nullptr) throw std::runtime_error("cant create list view");
...
void TM_LIST::AddColumn(char const* name, int32_t length)
{
    ...
    if (ListView_InsertColumn(hOwn, Columns_Num, &lvc) == -1) throw std::runtime_error("cant add column");
    Columns_Num++;
}

void TM_LIST::AddItem(char const* name)
{
    LVITEM  lvi;
    wchar_t string[MAX_STR_SIZE];
...
void TM_LIST::SetItemText(int32_t Item_index, int32_t Subitem_index, char const* text)
{
    if (text) {
...
void TM_LIST::GetItemText(int32_t Item_index, int32_t Subitem_index, char const* text, int32_t max_size)
{
    if (text) {
        std::wstring TextW = utf8::ConvertUtf8ToWide(text);
...
void TM_LIST::UpdatePosition()
{
    ...
    uint32_t const width  = r.right - r.left;
    uint32_t const height = r.bottom - r.top;
...
void TM_LIST::SelectItem(char const* name)
{
    if (!name) return;
    auto const items = GetItemsCount();
...
void TM_LIST::ProcessMessageBase(uint64_t iMsg, uint64_t wParam, uint64_t lParam)
{
    ...
    switch (iMsg) {
    case WM_SYSKEYDOWN:
        if (wParam == VK_F10) { CDebug->SetTraceMode(TMODE_MAKESTEP_OVER); }
        break;
    case WM_KEYDOWN:
        if (wParam == VK_F4) StartEditSelectedItem();
        if (wParam == VK_F10) { CDebug->SetTraceMode(TMODE_MAKESTEP_OVER); }
        if (wParam == VK_F11) { CDebug->SetTraceMode(TMODE_MAKESTEP); }
        if (wParam == VK_F5) {
    ...
    case WM_COMMAND:
        ...
        case EN_UPDATE:
            ...
            if (hEdit == (HWND)lParam) {
                uint32_t const lines = SendMessage(hEdit, EM_GETLINECOUNT, 0, 0);
                if (lines > 1) {
                    if (edit_item >= 0 && edit_subitem >= 0) {
                        ...
                        for (int32_t i = 0; i < static_cast<int32_t>(lines); i++) {
                            ...
                            *(uint16_t*)TextEditBuffer = MAX_STR_SIZE - 2;
                            int32_t const chars        = SendMessage(hEdit, EM_GETLINE, i, (LPARAM) static_cast<LPCSTR>(TextEditBuffer));
        ...
        case NM_DBLCLK:
            ...
            if (lpnmlv) {
                if (lpnmlv->iItem >= 0) {
                    ...
                    auto const list_rect = GetPosition();
                    ...
                    hEdit = CreateWindowEx(
                        ...
                        GetInstance(),
                        nullptr);
                    if (hFont) { SendMessage(hEdit, WM_SETFONT, (WPARAM)hFont, 0); }
...
void TM_LIST::StartEditSelectedItem()
{
    ...
    auto const iSelected = ListView_GetSelectionMark(GetWindowHandle());
...
void TM_LIST::SetCharID(char const* text)
{
