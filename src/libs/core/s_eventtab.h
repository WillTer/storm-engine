...
public:
    ...
    void     SetStatus(char const* event_name, uint32_t func_code, uint32_t status);
    uint32_t AddEventHandler(char const* event_name, uint32_t func_code, uint32_t func_segment_id, int32_t flag, bool bStatic = false);
    bool     DelEventHandler(char const* event_name, uint32_t func_code);
    ...
    uint32_t FindEvent(char const* event_name);
    void     ProcessFrame();
};
