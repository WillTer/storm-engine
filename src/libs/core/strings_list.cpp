...
bool STRINGS_LIST::AddString(char const* _char_PTR)
{
    // GUARD(STRINGS_LIST::AddString)
    if (_char_PTR == nullptr) throw std::runtime_error("zero string");
    auto const hash = MakeHashValue(_char_PTR);
    ...
    {
        String_Table_PTR = static_cast<char**>(realloc(String_Table_PTR, (List_size + SL_BLOCK_SIZE) * sizeof(char*)));
        List_size        = List_size + SL_BLOCK_SIZE;
    }

    auto const len            = strlen(_char_PTR) + 1;
...
uint32_t STRINGS_LIST::GetStringCode(char const* _char_PTR)
{
    if (Strings <= 0 || String_Table_PTR == nullptr || _char_PTR == nullptr) { return INVALID_ORDINAL_NUMBER; }
    auto const hash = MakeHashValue(_char_PTR);
...
bool STRINGS_LIST::AddUnicalString(char const* _char_PTR)
{
...
uint32_t STRINGS_LIST::MakeHashValue(char const* string)
{
    uint32_t hval = 0;
