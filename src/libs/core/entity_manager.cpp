...
void EntityManager::EraseAndFree(EntityInternalData& data)
{
    for (layer_index_t i = 0; i < kMaxLayerNum; ++i) {
        if (data.mask >> i & 1) { RemoveFromLayer(i, data); }
    }
    delete data.ptr;
}
...
{
    data.state       = kExists;
    auto const index = static_cast<entid_index_t>(data.id);
    deletedIndices_.push(index);
}

hash_t EntityManager::GetClassCode(entid_t const id) const
{
    auto const idx = GetEntityDataIdx(id);
    if (!EntIdxValid(idx)) { return {}; }

    return entities_[idx].hash;
}

void EntityManager::AddToLayer(layer_index_t const index, EntityInternalData& data, priority_t const priority)
{
    ...
    auto const targetIdx = std::distance(std::begin(priorities), std::ranges::upper_bound(priorities, priority));

    priorities.insert(std::begin(priorities) + targetIdx, priority);
    entity_ids.insert(std::begin(entity_ids) + targetIdx, data.id);
}

void EntityManager::RemoveFromLayer(layer_index_t const index, EntityInternalData& data)
{
    assert(index < kMaxLayerNum);

    auto& mask = data.mask;

    auto const priority = data.priorities[index];
    ...
    auto const ssize = std::ssize(priorities);

    auto const lowerIdx = std::distance(std::begin(priorities), std::ranges::lower_bound(priorities, priority));
...
void EntityManager::EraseEntity(entid_t const id)
{
    auto const idx = GetEntityDataIdx(id);
    if (!EntIdxValid(idx)) { return; }
    ...
    if (data.state != kValid) { return; }

    // check id mismatch
    if (data.ptr->GetId() != id) { return; }
...
entptr_t EntityManager::GetEntityPointer(entid_t const id) const
{
    auto const idx = GetEntityDataIdx(id);
    if (!EntIdxValid(idx)) { return nullptr; }

    return entities_[idx].ptr;
}

entity_container_cref EntityManager::GetEntityIds(layer_type_t const type) const
{
    ...
    for (auto it = std::begin(layers_); it != std::end(layers_); ++it) {
        ...
        if (it->type == type) { std::ranges::copy(it->entity_ids, std::back_inserter(result)); }
    }
...
{
    static std::vector<entid_t> const null;
...
entity_container_cref EntityManager::GetEntityIds(layer_index_t const index) const
{
    assert(index < kMaxLayerNum);
...
{
    auto const index = GetEntityDataIdx(id);
    return EntIdxValid(index) && entities_[index].state == kValid;
}

entid_t EntityManager::GetEntityId(uint32_t const hash) const
{
    for (auto it = std::begin(entities_); it != std::end(entities_); ++it) {
        if (it->hash == hash && it->state == kValid) { return it->id; }
    }

    return invalid_entity;
}

bool EntityManager::EntIdxValid(size_t const idx)
{
    return idx != invalid_entity_idx_t {};
}

entid_t EntityManager::CalculateEntityId(size_t const idx)
{
    // assert for idx exceeding 2^32
    assert(static_cast<size_t>(static_cast<uint32_t>(idx)) == idx);

    auto const stamp = ++previousStamp_;
    auto const id    = static_cast<entid_t>(stamp) << 32 | idx;

    return id;
}

layer_type_t EntityManager::GetLayerType(layer_index_t const index) const
{
    assert(index < kMaxLayerNum);
...
void EntityManager::SetLayerType(layer_index_t const index, layer_type_t const type)
{
    assert(index < kMaxLayerNum);
...
void EntityManager::SetLayerFrozen(layer_index_t const index, bool const freeze)
{
    assert(index < kMaxLayerNum);
...
bool EntityManager::IsLayerFrozen(layer_index_t const index) const
{
    assert(index < kMaxLayerNum);
...
{
    if (auto const entity_idx = GetEntityDataIdx(entity_id); EntIdxValid(entity_idx)) {
        if (!(entities_[entity_idx].mask >> layer_idx & 1)) { AddToLayer(layer_idx, entities_[entity_idx], priority); }
    }
}
...
{
    if (auto const entity_idx = GetEntityDataIdx(entity_id); EntIdxValid(entity_idx)) {
        if (entities_[entity_idx].mask >> layer_idx & 1) { RemoveFromLayer(layer_idx, entities_[entity_idx]); }
    }
}
...
void EntityManager::NewLifecycle()
{
    ...
    while (!deletedIndices_.empty()) {
        auto const entity_idx = deletedIndices_.top();
...
size_t EntityManager::GetEntityDataIdx(entid_t const id) const
{
    if (id == invalid_entity) { return invalid_entity_idx_t {}; }

    // check bounds
    auto const index = static_cast<entid_index_t>(id);
    if (index >= std::size(entities_)) { return invalid_entity_idx_t {}; }
    ...
    // }
    if (data.state == kNotExists) { return invalid_entity_idx_t {}; }

    // check if already replaced by another entity
    if (data.ptr->GetId() != id) { return invalid_entity_idx_t {}; }
...
entid_t EntityManager::InsertEntity(entptr_t ptr, hash_t hash)
{
    ...
    if (freeIndices_.empty()) {
        entid_index_t const index = std::size(entities_);
        id                        = CalculateEntityId(index);
        entities_.emplace_back(EntityInternalData {{}, {}, kValid, ptr, id, hash});
    } else {
        entid_index_t const index = freeIndices_.top();
        id                        = CalculateEntityId(index);
        freeIndices_.pop();
...
void EntityManager::ForEachEntity(std::function<void(entptr_t)> const& f)
{
    std::ranges::for_each(entities_, [&](EntityInternalData const& data) {
        if (data.state == kValid) {
            if (auto const entity_ptr = GetEntityPointer(data.id)) { f(entity_ptr); }
        }
    });
}
