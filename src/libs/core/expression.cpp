...
bool COMPILER::BC_ProcessExpression(DATA* value)
{
    ...
    if (TokenIs(AND)) {
        ...
        auto const var_code = *((uint32_t*)&pRunCodeBase[TLR_DataOffset]);
        if (TokenType() == VARIABLE) {
...
void COMPILER::BC_ProcessExpression_L0(DATA* value)
{
    ...
    while (TokenIs(OP_BOOL_AND) || TokenIs(OP_BOOL_OR)) {
        ...
        default: SetError("Invalid expression break"); return;
        }
    }
}
...
void COMPILER::BC_ProcessExpression_L1(DATA* value, bool bSkip)
{
    ...
    while (TokenIs(OP_NOT_EQUAL) || TokenIs(OP_LESSER_OR_EQUAL) || TokenIs(OP_LESSER) || TokenIs(OP_GREATER_OR_EQUAL) || TokenIs(OP_GREATER)
           || TokenIs(OP_BOOL_EQUAL)) {
        auto const op = TokenType();
        switch (op) {
        case SQUARE_CLOSE_BRACKET: break;
        case CLOSE_BRACKET: break;
...
void COMPILER::BC_ProcessExpression_L2(DATA* value, bool bSkip)
{
    ...
    while (TokenIs(OP_PLUS) || TokenIs(OP_MINUS)) {
        ...
        if (!bSkip) {
            switch (op) {
            case OP_PLUS: value->Plus(&TempVal); break;
            case OP_MINUS: value->Minus(&TempVal); break;
            }
        }
    }
}
...
void COMPILER::BC_ProcessExpression_L3(DATA* value, bool bSkip)
{
    ...
    while (TokenIs(OP_MULTIPLY) || TokenIs(OP_DIVIDE) || TokenIs(OP_MODUL)) {
        ...
        if (!bSkip) {
            switch (op) {
            case OP_MULTIPLY: value->Multiply(&TempVal); break;
            case OP_DIVIDE: value->Divide(&TempVal); break;
            case OP_MODUL: value->Modul(&TempVal); break;
            }
        }
    }
}
...
void COMPILER::BC_ProcessExpression_L5(DATA* value, bool bSkip)
{
    ...
    if (!bSkip) {
        if (op == OP_MINUS)
            ...
            case VAR_FLOAT: value->Convert(VAR_INTEGER);
            case VAR_INTEGER:
                value->Get(iVal);
                if (iVal != 0)
                    iVal = 0;
            ...
            case VAR_STRING:
                ...
                value->Set(iVal);
                break;
            default: SetError("Invalid argument type for '!' opearator"); break;
            }
        }
    }
}
...
void COMPILER::BC_ProcessExpression_L6(DATA* value, bool bSkip)
{
    if (TokenIs(OPEN_BRACKET)) {
        ...
        if (!TokenIs(CLOSE_BRACKET)) SetError("No matching ')' in expression");
        while (BC_TokenGet() == DEBUG_LINE_CODE) {}
    } else
...
void COMPILER::BC_ProcessExpression_L7(DATA* value, bool bSkip)
{
    ...
    case CALL:
        if (bSkip) {
            ...
            do {
                switch (BC_TokenGet()) {
                case OPEN_BRACKET: local_in++; break;
                case CLOSE_BRACKET: local_in--; break;
                }
            } while (local_in > 0);
            BC_TokenGet();
            return;
...
bool COMPILER::CompileExpression_L0(SEGMENT_DESC& Segment)
{
    ...
    while (sttOpType == OP_BOOL_AND || sttOpType == OP_BOOL_OR) {
        ...
        uint32_t const dwUpdateOffset1 = Segment.BCode_Program_size + 2;
        ...
        uint32_t const dwUpdateOffset2 = Segment.BCode_Program_size + 2;
        dwJumpOffset2                  = 0;
...
bool COMPILER::CompileExpression_L1(SEGMENT_DESC& Segment)
{
    ...
    while (sttOpType == OP_NOT_EQUAL || sttOpType == OP_LESSER_OR_EQUAL || sttOpType == OP_LESSER || sttOpType == OP_GREATER_OR_EQUAL
           || sttOpType == OP_GREATER || sttOpType == OP_BOOL_EQUAL) {
        sttOpType = Token.GetType();
    ...
    case UNKNOWN:
        ...
        case VARIABLE:
            ...
            if (sttResult == SQUARE_OPEN_BRACKET) {
                ...
                // after top of the stack must be array index
                Token.Get();
                if (!CompileExpression_L2(Segment)) return false;
                // test for ']'
                if (Token.GetType() != SQUARE_CLOSE_BRACKET) {
                    SetError("missing ']'");
                    return false;
