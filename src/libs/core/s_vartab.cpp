...
VarInfo::VarInfo(VarInfo const& other) : name(other.name), type(other.type), value(), elements(other.elements), segment_id(other.segment_id)
{
}

VarInfo& VarInfo::operator=(VarInfo const& other)
{
    if (this != &other) {
        name       = other.name;
        type       = other.type;
...
size_t VarTable::AddVar(VarInfo const& vi)
{
    if (vi.name.empty()) { return INVALID_VAR_CODE; }
...
VarInfo const* VarTable::GetVar(size_t var_index) const
{
    if (var_index >= vars_.size()) { return nullptr; }

    if (vars_[var_index].segment_id == INVALID_SEGMENT_ID) { return nullptr; }

    return &vars_[var_index];
}

VarInfo const* VarTable::GetVarX(size_t var_index) const
{
    if (var_index >= vars_.size()) { return nullptr; }
...
size_t VarTable::FindVar(std::string const& var_name) const
{
    auto result = hash_table_.find(var_name);

    if (result == hash_table_.end()) { return INVALID_VAR_CODE; }
...
{
    if (var_index >= vars_.size()) { return false; }

    if (!vars_[var_index].value->IsArray()) { return false; }
