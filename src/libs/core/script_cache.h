...
public:
    ...
    std::enable_if_t<!std::is_enum_v<To>, To> Read()
    {
        static_assert(std::is_trivially_copyable_v<To>);

        To to;
        if (cur_pointer_ + sizeof(to) > std::size(buffer_)) { throw ReaderException(); }

        auto const from = std::data(buffer_) + cur_pointer_;
...
public:
    template <typename T>
    void WriteData(T const& buffer)
    {
        buffer_.reserve(sizeof(buffer_) + sizeof(buffer));
        std::copy_n(reinterpret_cast<char const*>(&buffer), sizeof(buffer), std::back_inserter(buffer_));
    }
