...
#endif
#endif
#elif defined(__GLIBCXX__)  // Using another compiler type with libstdc++ - we are assuming full c++11 compliance for compiler - which may
                            // not be true
...
#define PLF_IS_ALWAYS_EQUAL_SUPPORT
    ...
    && ((defined(__clang__) && ((__clang_major__ == 3 && __clang_minor__ == 9) || __clang_major__ > 3)) \
...
#define PLF_CONSTEXPR
    ...
    && ((defined(__clang__) && (__clang_major__ >= 13)) || (defined(__GNUC__) && __GNUC__ >= 10) \
...
#define PLF_NOEXCEPT_MOVE_ASSIGN(the_allocator) \
    noexcept( \
        std::allocator_traits<the_allocator>::propagate_on_container_move_assignment::value \
        || std::allocator_traits<the_allocator>::is_always_equal::value)
#define PLF_NOEXCEPT_SWAP(the_allocator) \
    noexcept( \
        std::allocator_traits<the_allocator>::propagate_on_container_swap::value \
        || std::allocator_traits<the_allocator>::is_always_equal::value)
#else
#define PLF_NOEXCEPT_MOVE_ASSIGN(the_allocator)
#define PLF_NOEXCEPT_SWAP(the_allocator)
...
#define PLF_ALLOCATE(the_allocator, allocator_instance, size, hint) \
    std::allocator_traits<the_allocator>::allocate(allocator_instance, size, hint)
...
class stack: private element_allocator_type  // Empty base class optimisation - inheriting allocator functions
{
public:
...
private:
...
#else
                                         // This is a hack around the fact that element_allocator_type::construct only supports copy
                                         // construction in C++03 and copy elision does not occur on the vast majority of compilers in this
                                         // circumstance. And to avoid running out of memory (and performance loss) from allocating the same
                                         // block twice, we're allocating in this constructor and moving data in the copy constructor.
        group(size_type const elements_per_group, group_pointer_type const previous = NULL) PLF_NOEXCEPT
            : elements(NULL)
        ...
        group(group const& source)
            ...
            , elements(PLF_ALLOCATE(
                  ...
                  *this,
                  ...
                  (source.previous_group == NULL) ? 0 : source.previous_group->elements))
            , next_group(NULL)
    ...
    group_pointer_type current_group,
        first_group;  // current group is location of top pointer, first_group is 'front' group, saves performance for ~stack etc
    element_pointer_type top_element, start_element,
        end_element;  // start_element/end_element cache current_group->end/elements for better performance
    size_type total_size, total_capacity, min_block_capacity;
    struct ebco_pair
        : group_allocator_type  // Packaging the group allocator with the least-used member variable, for empty-base-class optimization
    {
        size_type max_block_capacity;
        explicit ebco_pair(size_type const max_elements) PLF_NOEXCEPT: max_block_capacity(max_elements) {};
    } group_allocator_pair;

    inline void check_capacities_conformance(size_type const min, size_type const max) const
    {
...
#define PLF_MIN_BLOCK_CAPACITY \
    (sizeof(element_type) * 8 > (sizeof(*this) + sizeof(group)) * 2) ? 8 \
...
public:
    ...
    explicit stack(element_allocator_type const& alloc)
    ...
    stack(size_type const min, size_type const max = (std::numeric_limits<size_type>::max() / 2))
        : element_allocator_type(element_allocator_type())
        , current_group(NULL)
        , first_group(NULL)
        , top_element(NULL)
    ...
    stack(size_type const min, size_type const max, element_allocator_type const& alloc)
        : element_allocator_type(alloc)
        , current_group(NULL)
        , first_group(NULL)
        , top_element(NULL)
...
private:
    ...
    inline void progress_to_next_group()  // used by push/emplace
    {
        ...
        {
            allocate_new_group(
                (total_size < group_allocator_pair.max_block_capacity) ? total_size : group_allocator_pair.max_block_capacity,
                current_group);
        }
    ...
    void copy_from_source(stack const& source)  // Note: this function is only called on an empty un-initialize()'d stack
    {
        ...
        if (source.total_size <= group_allocator_pair.max_block_capacity)  // most common case
        {
            ...
            top_element += source.top_element
                - source.start_element;     // This should make top_element == the last "pushed" element, rather than the one past it
        ...
        } else  // uncommon edge case, so not optimising:
        {
            ...
            while (current_copy_group != end_copy_group) {
                for (element_pointer_type element_to_copy = current_copy_group->elements; element_to_copy != current_copy_group->end;
                     ++element_to_copy) {
            ...
            // Handle special case of last group:
            for (element_pointer_type element_to_copy = source.start_element; element_to_copy != source.top_element + 1;
                 ++element_to_copy) {
                push(*element_to_copy);
            }
    ...
    {
#ifdef PLF_TYPE_TRAITS_SUPPORT
        if PLF_CONSTEXPR (!std::is_trivially_destructible<element_type>::value)  // Avoid iteration for trivially-destructible types eg.
                                                                                 // POD, structs, classes with empty destructor.
#endif  // If compiler doesn't support traits, iterate regardless - trivial destructors will not be called, hopefully compiler will optimise
        // this loop out for POD types
        {
            if (total_size != 0) {
    ...
    inline void blank() PLF_NOEXCEPT
    {
        ...
        if PLF_CONSTEXPR (
            ...
            && std::is_trivial<element_pointer_type>::value)  // if all pointer types are trivial, we can just nuke it from orbit with
                                                              // memset (NULL is always 0 in C++):
        {
...
// Create element:
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    // Note: the reason for code duplication from non-move push, as opposed to using std::forward for both, was because most compilers
    // didn't actually create as-optimal code in that strategy. Also C++03 compatibility.
    ...
    inline stack& operator=(stack const& source)
    {
        assert(&source != this);
        ...
        if (source.total_size <= group_allocator_pair.max_block_capacity) {
            ...
            while (current_copy_group != end_copy_group) {
                std::uninitialized_copy(
                    std::make_move_iterator(current_copy_group->elements), std::make_move_iterator(current_copy_group->end), top_element);
                top_element += current_copy_group->end - current_copy_group->elements;
                current_copy_group = current_copy_group->next_group;
            }

            std::uninitialized_copy(
                std::make_move_iterator(source.start_element), std::make_move_iterator(source.top_element + 1), top_element);
...
public:
    void reshape(size_type const min, size_type const max)
    {
        ...
        for (group_pointer_type current = first_group; current != NULL; current = current->next_group) {
            if (static_cast<size_type>(current->end - current->elements) < min
                || static_cast<size_type>(current->end - current->elements) > max) {
#ifdef PLF_TYPE_TRAITS_SUPPORT  // If type is non-copyable/movable, cannot be consolidated, throw exception:
                if PLF_CONSTEXPR (!((std::is_copy_constructible<element_type>::value && std::is_copy_assignable<element_type>::value)
                                    || (std::is_move_constructible<element_type>::value && std::is_move_assignable<element_type>::value))) {
                    throw;
                } else
#endif
                {
    ...
    bool operator==(stack const& rh) const PLF_NOEXCEPT
    {
        assert(this != &rh);

        if (total_size != rh.total_size) { return false; }
        ...
        while (true) {
            if (*this_pointer != *rh_pointer) { return false; }

            if (this_pointer == top_element) { break; }
    ...
    inline bool operator!=(stack const& rh) const PLF_NOEXCEPT
    {
        return !(*this == rh);
    }
    ...
    void trim() PLF_NOEXCEPT
    {
        ...
        current_group->next_group =
            NULL;  // Set to NULL regardless of whether it is already NULL (avoids branching). Cuts off rest of groups from this group.
    ...
    void append(stack& source)
    {
        ...
        // with elements from the source groups, starting from the back. Then link the destination stack's groups to the source stack's
        // groups.
...
// Use the fastest method for moving elements, while preserving values if allocator provides non-trivial pointers:
...
#endif
            {
                ...
                for (element_pointer_type element_pointer = source.start_element; element_pointer != source.top_element + 1;
                     ++element_pointer) {
        ...
        if (elements_to_be_transferred != 0) {
            ...
            if PLF_CONSTEXPR (
                ...
                && std::is_trivially_destructible<element_type>::value)  // Avoid iteration for trivially-destructible iterators ie. all
                                                                         // iterators, unless allocator returns non-trivial pointers
            {
                std::memcpy(
                    static_cast<void*>(&*top_element), static_cast<void*>(&*start), elements_to_be_transferred * sizeof(element_type));
            }
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
            else if PLF_CONSTEXPR (std::is_move_constructible<element_type>::value) {
...
#endif
            {
                ...
                // The following loop is necessary because the allocator may return non-trivially-destructible pointer types, making
        ...
        if (source.min_block_capacity < min_block_capacity
            || source.group_allocator_pair.max_block_capacity > group_allocator_pair.max_block_capacity) {
            for (group_pointer_type current = source.first_group; current != NULL; current = current->next_group) {
                if (static_cast<size_type>(current->end - current->elements) < min_block_capacity
                    || static_cast<size_type>(current->end - current->elements) > group_allocator_pair.max_block_capacity) {
                    throw std::length_error(
                        "A source memory block capacity is outside of the destination's minimum or maximum memory block capacity limits - please change either the source or the destination's min/max block capacity limits using reshape() before calling append() in this case");
                }
            }
        }
    ...
    void swap(stack& source) PLF_NOEXCEPT_SWAP(element_allocator_type)
    {
        ...
        if PLF_CONSTEXPR (
            std::is_trivial<group_pointer_type>::value
            && std::is_trivial<element_pointer_type>::value)  // if all pointer types are trivial we can just copy using memcpy - faster
        {
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT  // If pointer types are not trivial, moving them is probably going to be more efficient than copying them
                                   // below
        else if PLF_CONSTEXPR (
            std::is_move_assignable<group_pointer_type>::value && std::is_move_assignable<element_pointer_type>::value
            && std::is_move_constructible<group_pointer_type>::value && std::is_move_constructible<element_pointer_type>::value) {
...
#endif
        {
            ...
            size_type const            swap_total_size = total_size, swap_total_capacity = total_capacity,
                            swap_min_block_capacity = min_block_capacity, swap_max_block_capacity = group_allocator_pair.max_block_capacity;
...
inline void swap(plf::stack<element_type, element_allocator_type>& a, plf::stack<element_type, element_allocator_type>& b)
