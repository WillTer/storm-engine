...
public:
    ...
    static constexpr Milliseconds kFPSUpdateInterval = Milliseconds {1000};  // How often to raise the event to update the FPS counter
    ...
    uint32_t Run()
    {
        ...
        if (Delta_Time > (uint32_t)kMaxDelta.count()) { Delta_Time = (uint32_t)kMaxDelta.count(); }
    ...
    void SetDelta(int32_t const DeltaTime)
    {
        if (DeltaTime <= 0) {
            FixedDelta = false;
            return;
