...
storm::script_cache::ReaderException::ReaderException() : std::runtime_error("Unable to read binary data. Scripts mismatch?") {}

storm::script_cache::BufferReader::BufferReader(std::vector<char>&& buffer) : buffer_(std::move(buffer)), cur_pointer_(0) {}

std::string_view storm::script_cache::BufferReader::ReadArray()
{
    auto const buffer_size = Read<size_t>();
    if (buffer_size == 0) { return {}; }
    if (cur_pointer_ + buffer_size > std::size(buffer_)) { throw ReaderException(); }

    std::string_view const sv {std::data(buffer_) + cur_pointer_, buffer_size};
...
void storm::script_cache::ReadScriptData(BufferReader& reader, S_TOKEN_TYPE type, DATA* data)
{
    switch (type) {
    case VAR_INTEGER: {
        auto const value = reader.Read<int32_t>();
        data->Set(value);
        break;
    }

    case VAR_FLOAT: {
        auto const value = reader.Read<float>();
        data->Set(value);
        break;
    }
    case VAR_STRING: {
        auto const value = reader.ReadArray();
...
void storm::script_cache::WriteScriptData(BufferWriter& writer, S_TOKEN_TYPE type, DATA* data)
{
    switch (type) {
    case VAR_INTEGER: {
        auto const value = data->GetInt();
        writer.WriteData(value);
        break;
    }

    case VAR_FLOAT: {
        auto const value = data->GetFloat();
        writer.WriteData(value);
        break;
    }
    case VAR_STRING: {
        auto const value = data->GetString();
