...
public:
    ...
    size_t FindFunc(std::string const& func_name) const;
    // add func to table, returns func index
    size_t AddFunc(FuncInfo const& fi);
    ...
    bool SetFuncOffset(std::string const& func_name, uint32_t offset);
    // add local var to func
    bool AddFuncVar(size_t func_index, LocalVarInfo const& lvi);
    // add arg to func (must precede all regular local vars in order not to break compiler logic)
    bool AddFuncArg(size_t func_index, LocalVarInfo const& lvi, bool is_extern = false);
    // get func's local var or arg index by name
    size_t FindVar(size_t func_index, std::string const& var_name) const;
