...
template <typename T> MessageParam convertMessageParam(T value)
{
    return MessageParam(value);
}

template <> inline MessageParam convertMessageParam(const bool value)
{
    return {value ? 1 : 0};
}

// Convert uint32_t to int32_t to prevent conversion issues
template <> inline MessageParam convertMessageParam(const uint32_t value)
{
...
class MESSAGE final
{
  public:
    ...
    template <typename... Args> void Reset(const std::string_view &format, Args... args)
    {
