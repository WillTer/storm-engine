...
{
    if (Data_num > STACK_BUFFER_LIMIT) throw std::runtime_error("stack overflaw");
    if (Data_num >= Buffer_size) {
        auto const offset = Buffer_size;
    ...
    if (pdataclass) { pStackData[Data_num]->Copy(pdataclass); }
    Data_num++;
...
{
    Assert(Data_num);
    if (Data_num == 0) throw std::runtime_error("stack 'pop' error");
    Data_num--;
...
{
    if (offset + index >= Data_num) { throw std::runtime_error("stack 'read' error"); }
...
DATA* S_STACK::Read()
{
    if (Data_num <= 0) throw std::runtime_error("stack 'read' error");
    // return &pStackData[Data_num - 1];
    return pStackData[Data_num - 1];
}
