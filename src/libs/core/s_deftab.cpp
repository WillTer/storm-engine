...
uint32_t S_DEFTAB::AddDef(DEFINFO& di)
{
    ...
    auto const hash = MakeHashValue(di.name);
    ...
    {
        if (di.name) {
            auto const len       = strlen(di.name) + 1;
...
uint32_t S_DEFTAB::MakeHashValue(char const* string)
{
    uint32_t hval = 0;
    while (*string != 0) {
        auto v = *string++;
...
uint32_t S_DEFTAB::FindDef(char const* def_name)
{
    if (def_name == nullptr) return INVALID_DEF_CODE;
    auto const hash = MakeHashValue(def_name);

    auto const hash_index = DTMAKEHASHINDEX(hash);
    for (uint32_t n = 0; n < HashLine[hash_index].nNumElements; n++) {
        auto const ni = HashLine[hash_index].pElements[n];
        if (pTable[ni].hash == hash)  // return n;
            if (storm::iEquals(pTable[ni].name, def_name)) return ni;
    }
    return INVALID_DEF_CODE;
...
{
    uint32_t const hash_index = DTMAKEHASHINDEX(hash);
