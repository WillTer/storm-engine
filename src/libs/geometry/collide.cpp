...
{
    if (!(rhead.flags & FLAGS_BSP_PRESENT)) return 2.0f;
...
rec_return:;
    ...
    loop0:
        auto const face = (static_cast<int32_t>(*(pface + 2)) << 16) | (static_cast<int32_t>(*(pface + 1)) << 8)
        ...
        auto       pvec = dirvec ^ b;
        auto const det  = a | pvec;

        auto         c = src - vrt[vindex[0]];
        double const U = c | pvec;
        double const V = dirvec | (c ^ a);
...
int32_t ClipByPlane(const GEOS::PLANE& plane, int32_t n)
{
    ...
    int32_t i;
    for (i = 0; i < n; i++)
        if (plane.nrm.x * poly[i].x + plane.nrm.y * poly[i].y + plane.nrm.z * poly[i].z - plane.d < 0.0) inside++;
    if (inside == n || inside == 0) return inside;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (sign[0] > 0.0)  // outside!
        {
            ...
            if (sign[1] > 0.0) continue;  // outside!
        ...
        ClipBPH:
            ...
            ii++;
            if (ii >= n) ii = 0;
            if (plane.nrm.x * poly[ii].x + plane.nrm.y * poly[ii].y + plane.nrm.z * poly[ii].z - plane.d <= 0) goto ClipEPH;  // inside!
            for (i4 = i3; i4 < n - 1; i4++)
                poly[i4] = poly[i4 + 1];
            n--;
            if (i3 < i) i--;
...
rec_loop:;
    ...
    double const ssrc = (src | node->norm) - node->pd;

    // trace all faces
    if (ssrc * ssrc < radius * radius) {
        auto* pface = (unsigned char*)&node->face;
        for (uint32_t f = 0; f < node->nfaces; f++) {
            int32_t const face = (static_cast<int32_t>(*(pface + 2)) << 16) | (static_cast<int32_t>(*(pface + 1)) << 8)
                | (static_cast<int32_t>(*(pface + 0)) << 0);
            // "fix" for broken models
            if (face < 0 || face >= sizeof(trgclip) / sizeof(*trgclip)) { continue; }
            ...
            if (trgclip[face] != attempt) {
                ...
                for (int32_t v = 0; v < 3; v++) {
                    int32_t const vindex =
                        (btrg[face].vindex[v][0] << 0) | (btrg[face].vindex[v][1] << 8) | (btrg[face].vindex[v][2] << 16);
                ...
                if (nverts > 0) {
                    ...
                    // if clipping
                    if (addpoly((VERTEX*)&poly[0], nverts) == false) return true;
                }
            }
        ...
        // trace to side
        if (node->left != 0) VOL[++rec_level] = node->node;
        if (node->right == 0) goto rec_continue;
        node = &sroot[node->node + node->right];
    } else {
        if (ssrc < 0.0f) {
            if (node->left == 0) goto rec_continue;
            node = &sroot[node->node];
        } else if (node->right == 0)
            goto rec_continue;
        else
...
rec_continue:;
    ...
    if (rec_level > -1) goto rec_continue;
