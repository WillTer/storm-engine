...
namespace
{
std::vector<uint32_t> getColData(GEOM_SERVICE& srv, std::string_view const& file_name)
{
    std::vector<uint32_t> result;

    auto ltfl = srv.OpenFile(file_name.data());
    if (ltfl.is_open()) {
        auto const file_size = srv.FileSize(file_name.data());
        if (file_size > 0) {
...
GEOS* CreateGeometry(char const* fname, char const* lightname, GEOM_SERVICE& srv, int32_t flags)
{
    return new GEOM(fname, lightname, srv, flags);
}

// geometry constructor does all init
GEOM::GEOM(char const* fname, char const* lightname, GEOM_SERVICE& _srv, int32_t flags) : srv(_srv)
{
    std::vector<uint32_t> colData;
    if (lightname != nullptr) { colData = getColData(srv, lightname); }

    auto file = srv.OpenFile(fname);
    // read header
    srv.ReadFile(file, &rhead, sizeof(RDF_HEAD));
    if (rhead.version != RDF_VERSION) throw std::runtime_error("invalid version");
...
bool GEOM::GetCollisionDetails(TRACE_INFO& ti) const
{
    ...
    double const  pldist = nrm | vrt[vindex[0]];
...
char const* GEOM::GetTextureName(int32_t tx) const
{
...
int32_t GEOM::FindName(char const* name) const
{
    for (int32_t n = 0; n < rhead.names; n++)
        if (storm::iEquals(&globname[names[n]], name)) return unbelievable_workaround(&globname[names[n]]);
    return -1;
}
...
int32_t GEOM::FindLabelN(int32_t start_index, int32_t name_id)
{
    for (; start_index < rhead.nlabels; start_index++)
        if (unbelievable_workaround(label[start_index].name) == name_id) return start_index;
    return -1;
}
...
{
    for (; start_index < rhead.nlabels; start_index++)
        if (unbelievable_workaround(label[start_index].group_name) == name_id) return start_index;
    return -1;
}
...
int32_t GEOM::FindObjN(int32_t start_index, int32_t name_id)
{
    for (; start_index < rhead.nobjects; start_index++)
        if (unbelievable_workaround(object[start_index].name) == name_id) return start_index;
    return -1;
}
...
{
    for (; start_index < rhead.nobjects; start_index++)
        if (unbelievable_workaround(object[start_index].group_name) == name_id) return start_index;
    return -1;
}
...
int32_t GEOM::FindMaterialN(int32_t start_index, int32_t name_id)
{
    for (; start_index < rhead.nmaterials; start_index++)
        if (unbelievable_workaround(material[start_index].name) == name_id) return start_index;
    return -1;
}
...
{
    for (; start_index < rhead.nmaterials; start_index++)
        if (unbelievable_workaround(material[start_index].group_name) == name_id) return start_index;
    return -1;
}
...
int32_t GEOM::FindLightN(int32_t start_index, int32_t name_id)
{
    for (; start_index < rhead.nlights; start_index++)
        if (unbelievable_workaround(light[start_index].name) == name_id) return start_index;
    return -1;
}
...
{
    for (; start_index < rhead.nlights; start_index++)
        if (unbelievable_workaround(light[start_index].group_name) == name_id) return start_index;
    return -1;
}
...
int32_t GEOM::FindTexture(int32_t start_index, int32_t name_id)
{
    for (; start_index < rhead.ntextures; start_index++)
        if (globname[tlookup[start_index]] == name_id) return start_index;
    return -1;
}
