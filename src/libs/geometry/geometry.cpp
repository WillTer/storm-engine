...
int32_t const SHIFT_VALUE = 999999999;
#define AVB_MAX 1024
...
GEOS* GEOMETRY::CreateGeometry(char const* file_name, char const* light_file_name, int32_t flags, char const* lmPath)
{
    ...
    if (geoLog) {
        ...
        fprintf(
            fl,
            "%.2f, trgSize: %d, vrtSize: %d, tex: %d. obj: %d, %s\n",
...
GEOS::ID GEOM_SERVICE_R::CreateTexture(char const* fname)
{
    char tex[256];
