...
public:
    GEOM(char const* fname, char const* lightname, GEOM_SERVICE& srv, int32_t flags);
    virtual ~GEOM();

    virtual int32_t FindName(char const* name) const;
    ...
    virtual char const* GetTextureName(int32_t tx) const;
