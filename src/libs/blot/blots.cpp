...
uint64_t Blots::ProcessMessage(MESSAGE& message)
{
    ...
    case MSG_BLOTS_SETMODEL:
        ...
        if (pCharAttributeRoot) {
            ...
            std::string const buf = std::to_string(BLOTS_MAX);
            blotsInfo->SetValue(buf);
            for (int32_t i = 0; i < BLOTS_MAX; i++)
                LoadBlot(i);
        }
        break;
    case MSG_BLOTS_HIT: Hit(message); break;
    }
    return 0;
}
...
void Blots::SetNodesCollision(NODE* n, bool isSet)
{
    ...
    if (isSet) {
        n->flags &= 0x00ffffff;
        n->flags |= n->flags << 24;
        auto const* const name = n->GetName();
        if (name && name[0]) {
...
void Blots::LoadBlot(int32_t i)
{
    ...
    if (blt) {
        ...
        int32_t const rnd  = blt->GetAttributeAsDword("rnd");
        auto const    x    = blt->GetAttributeAsFloat("x");
        auto const    y    = blt->GetAttributeAsFloat("y");
        auto const    z    = blt->GetAttributeAsFloat("z");
        auto const    vx   = blt->GetAttributeAsFloat("vx");
        auto const    vy   = blt->GetAttributeAsFloat("vy");
        auto const    vz   = blt->GetAttributeAsFloat("vz");
        auto const    time = blt->GetAttributeAsFloat("time");
...
void Blots::Realize(uint32_t delta_time)
{
    ...
    for (int32_t i = 0; i < BLOTS_MAX; i++) {
        ...
        if (blot[i].liveTime >= BLOTS_TIME) {
            blot[i].isUsed            = false;
            auto const    startIndex  = blot[i].startIndex;
            int32_t const numDelVerts = blot[i].numTrgs * 3;
        ...
        if (color != blot[i].lastAlpha) {
            ...
            // amount
            int32_t const numVrt = blot[i].numTrgs * 3;
            // Array
...
bool Blots::AddPolygon(const CVECTOR* v, int32_t nv)
{
    ...
    if (nv < 3) return true;
    // Normal
    auto const norm = (v[0] - v[1]) ^ (v[0] - v[2]);
