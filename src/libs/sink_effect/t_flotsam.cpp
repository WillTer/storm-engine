...
void TFlotsam::Process(uint32_t _dTime)
{
    ...
    auto const dTime = _dTime / 1000.0f;  // msec -> sec
    ...
    case FLOTSAM_RISE:
        ...
        z += dTime * vZ;
        y += dTime * vY;
        if (y > (sea->WaveXZ(x, z) + sink_effect::FLOTSAM_DY)) state = FLOTSAM_FLOAT;
        break;
    case FLOTSAM_FLOAT:
        x += dTime * vX;
        z += dTime * vZ;
        ...
        if (floatTime > maxFloatTime) state = FLOTSAM_SINK;
        break;
    case FLOTSAM_SINK:
        x += dTime * vX;
        z += dTime * vZ;
        y -= dTime * vY;
        if (y <= sea->WaveXZ(x, z) - sink_effect::FLOTSAM_START_DELTAY) enabled = false;
        break;
    }
}
