...
//--------------------------------------------------------------------
TSinkSplash::TSinkSplash() : ps(nullptr), enabled(false), sea(nullptr), time(0), distortDivider(0), center(), dir(), growK(0) {}
...
void TSinkSplash::Start(const CVECTOR& _pos, uint16_t* _indexes, SINK_VERTEX* _vertexes, int32_t vOffset)
{
    ...
    auto const midY = sea->WaveXZ(center.x, center.z) + sink_effect::SPLASH_MOVE_Y;
    distortDivider  = rand(sink_effect::SPLASH_DISTORT_DIVIDER) + 10.0f;
    //    _VSYSTEM_API->Trace("i=%X, v=%X", _indexes, _vertexes);
    auto const stepSize = sink_effect::GRID_LENGTH / sink_effect::GRID_STEPS;
    auto const halfSize = sink_effect::GRID_LENGTH / 2.0f;
...
float TSinkSplash::HeightF(uint32_t _time, float _r, float _k)
{
    ...
    auto const secondPart = ((sink_effect::GRID_LENGTH - _r) / sink_effect::GRID_LENGTH)
        * (1.0f + fabs(sin((-PI * static_cast<float>(_time) / sink_effect::SPLASH_FADE_TIME) + PIm2 * _r / sink_effect::GRID_LENGTH)));
...
void TSinkSplash::Process(uint32_t _dTime, uint16_t* _indexes, SINK_VERTEX* _vertexes)
{
    ...
    auto const     alpha = rand(PI * 2);
    auto           rho   = randUpper(sink_effect::MAX_RHO);
    auto const     r     = sink_effect::SPLASH_RADIUS;
    static CVECTOR pos;
    ...
    auto const midY     = sea->WaveXZ(center.x, center.z) + sink_effect::SPLASH_MOVE_Y;
    auto const stepSize = static_cast<float>(sink_effect::GRID_LENGTH) / static_cast<float>(sink_effect::GRID_STEPS);
    auto const halfSize = sink_effect::GRID_LENGTH / 2.0f;
    CVECTOR    curPos;
    int        x, z;
    ...
    for (z = 0; z < sink_effect::GRID_STEPS; ++z)
        for (x = 0; x < sink_effect::GRID_STEPS; ++x) {
            auto const curPosNormed = !CVECTOR(stepSize * x - halfSize, 0.0f, stepSize * z - halfSize);
            ...
            _vertexes->color =
                ((((3 * sink_effect::SPLASH_FADE_TIME - static_cast<int32_t>(time))) << 8) / (3 * sink_effect::SPLASH_FADE_TIME)) << 24;
