...
static float const farBlend     = 16.0f;
...
{
    auto const norm = !((vr[1] - vr[0]) ^ (vr[2] - vr[0]));

    auto const d  = (norm | vr[0]);
    auto const dc = (norm | camPos) - d;
    auto const dl = (norm | lightPos) - d;
    if (dc * dl < 0.0f) return true;

    auto const d0 = (norm | objPos) - d;
    ...
    for (v = 0; v < 3; v++) {
        ...
        auto const z            = (trans.m[0][2] * vr[v].x + trans.m[1][2] * vr[v].y + trans.m[2][2] * vr[v].z + trans.m[3][2]);
        auto const rhw          = perspective * 0.5f / z;
        shadvert[tot_verts].tu = rhw * (trans.m[0][0] * vr[v].x + trans.m[1][0] * vr[v].y + trans.m[2][0] * vr[v].z + trans.m[3][0]) + 0.5f;
        shadvert[tot_verts].tv =
            -rhw * (trans.m[0][1] * vr[v].x + trans.m[1][1] * vr[v].y + trans.m[2][1] * vr[v].z + trans.m[3][1]) + 0.5f;
        tot_verts++;
    }
    auto const start = tot_verts - 3;
    for (; v < nverts; v++) {
...
uint64_t Shadow::ProcessMessage(MESSAGE& message)
{
    ...
    switch (code) {
    case 0: entity = message.EntityID(); break;
