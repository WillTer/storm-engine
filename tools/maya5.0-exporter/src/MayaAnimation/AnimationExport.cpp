...
void AniExport::ExportAll(const char *fname)
{
    ...
    for (float time = startTime; time < endTime; time += 1.0f)
    {
        ...
        for (j = 0; j < ah.nJoints; j++)
        {
            ...
            CVECTOR rotAng;
            GetDoubleValue(jp.node(), "rotateX", dblv);
            rotAng.x = float(dblv);
            GetDoubleValue(jp.node(), "rotateY", dblv);
            rotAng.y = float(dblv);
            GetDoubleValue(jp.node(), "rotateZ", dblv);
            rotAng.z = float(dblv);
            CMatrix rotate;
            rotate.BuildMatrixXYZ(rotAng.x, rotAng.y, rotAng.z, 0.0f, 0.0f, 0.0f);

            CVECTOR orAng;
            GetDoubleValue(jp.node(), "jointOrientX", dblv);
            orAng.x = float(dblv);
            GetDoubleValue(jp.node(), "jointOrientY", dblv);
            orAng.y = float(dblv);
            GetDoubleValue(jp.node(), "jointOrientZ", dblv);
            orAng.z = float(dblv);
            CMatrix orient;
