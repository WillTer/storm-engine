    ...
    if (argc >= 2)
    {
        ...
        if (file.good())
        {
            ...
            for (long i = 0; i < rhead.nobjects; i++)
            {
                ...
                auto currentFaces =
                    vector<RDF_TRIANGLE>(triangles.begin() + currentObj.striangle,
                                         triangles.begin() + currentObj.striangle + currentObj.ntriangles);

                if (currentBuffer.type == 0)
                {
                    auto buf = vertices[currentObj.vertex_buff].vrt0;
                    auto currentVertices = vector<RDF_VERTEX0>(buf.begin() + currentObj.svertex,
                                                               buf.begin() + currentObj.svertex + currentObj.nvertices);
                ...
                else
                {
                    ...
                    auto currentVertices = vector<RDF_VERTEX1>(buf.begin() + currentObj.svertex,
                                                               buf.begin() + currentObj.svertex + currentObj.nvertices);
                    col.AddMesh(currentVertices, currentVertices.size(), currentFaces.data(), currentFaces.size());
                }
            }
