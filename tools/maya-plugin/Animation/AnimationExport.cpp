...
__declspec(dllexport) MStatus initializePlugin(MObject obj)
{
    ...
    status =
        plugin.registerFileTranslator("SD2AniExport", &pixmap_name[0], AniExport::creator, &options_script_name[0]);
    return status;
}
...
void AniExport::ExportAll(const char *fname)
{
    ...
    for (float time = startTime; time < endTime; time += 1.0f)
    {
        ...
        for (j = 0; j < ah.nJoints; j++)
        {
            ...
            GetDoubleValue(path, "translateX", dblv);
            jPos.x = float(CM2M_SCALE * dblv);
            GetDoubleValue(path, "translateY", dblv);
            jPos.y = float(CM2M_SCALE * dblv);
            GetDoubleValue(path, "translateZ", dblv);
            jPos.z = float(CM2M_SCALE * dblv);

            CVECTOR rotAng;
            GetDoubleValue(path, "rotateX", dblv);
            rotAng.x = float(dblv);
            GetDoubleValue(path, "rotateY", dblv);
            rotAng.y = float(dblv);
            GetDoubleValue(path, "rotateZ", dblv);
            rotAng.z = float(dblv);
            CMatrix rotate;
            rotate.BuildMatrixXYZ(rotAng.x, rotAng.y, rotAng.z, 0.0f, 0.0f, 0.0f);

            CVECTOR orAng;
            GetDoubleValue(path, "jointOrientX", dblv);
            orAng.x = float(dblv);
            GetDoubleValue(path, "jointOrientY", dblv);
            orAng.y = float(dblv);
            GetDoubleValue(path, "jointOrientZ", dblv);
            orAng.z = float(dblv);
            CMatrix orient;
            ...
            if (j == 0)
                rootPos.push_back(mtxGlob.Pos());

            // joints' begin positions
            if (ah.nFrames == 0)
                begPos.push_back(mtxGlob.Pos());
        }
        ah.nFrames++;
    }
