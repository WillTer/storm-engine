...
// init exporter
//---------------------------------------------------------------------------------
GEOM_STATIC::GEOM_STATIC(const std::string &fname, TEXTURE_FUNC _tfn, RESULT_FUNC fnr, RESULT_FUNC fne, float maxedge,
                         float msize, long _flags)
...
void GEOM_STATIC::AddObject(const OBJECT &obj)
{
    ...
    for (long m = 0; m < obj.mtl.size(); m++)
    {
        ...
        for (g = 0; g < material.size(); g++)
        {
            ...
            // D3D params
            if (obj.mtl[m].diffuse != material[g].diffuse)
                continue;
            if (obj.mtl[m].specular != material[g].specular)
                continue;
            if (obj.mtl[m].gloss != material[g].gloss)
                continue;

            // other optical params
            if (obj.mtl[m].selfIllum != material[g].selfIllum)
                continue;

            long tx;
            for (tx = 0; tx < 4; tx++)
            {
                if (obj.mtl[m].texture[tx].type != material[g].texture_type[tx])
                    break;

                if (obj.mtl[m].texture[tx].type != TEXTURE_NONE && obj.mtl[m].texture[tx].name.size() > 0)
                    if (obj.mtl[m].texture[tx].name != globname[texture[material[g].texture[tx]]])
                        break;
            }
            if (tx == 4)
                break;
        ...
        if (g == material.size())
        {
            ...
            for (long tm = 0; tm < 4; tm++)
            {
                ...
                if (mt.texture_type[tm] != TEXTURE_NONE)
                    mt.texture[tm] = AddTexture(obj.mtl[m].texture[tm]);
                else
                    mt.texture[tm] = -1;
            }
            //--------------------------------------------------------
    ...
    for (long o = 0; o < obj.mtl.size(); o++)
    {
        ...
        if (obj.flags & MERGE)
        {
            for (g = 0; g < object.size(); g++)
                if (object[g].vertex.size() + 3 <= 65536 && // vertex buffer max vertex count is 65536
                    (object[g].flags & MERGE) && mtlref[o] == object[g].mtl &&
                    (obj.flags & (VISIBLE | STATIC_LIGHT_ENABLE | DINAMIC_LIGHT_ENABLE | CAST_SHADOWS_ENABLE |
                                  VERTEX_WEIGHT /* | CULLENABLE*/)) ==
                        (object[g].flags & (VISIBLE | STATIC_LIGHT_ENABLE | DINAMIC_LIGHT_ENABLE | CAST_SHADOWS_ENABLE |
                                            VERTEX_WEIGHT /* | CULLENABLE*/)))
                    break;
        }
        else
    ...
    for (long t = 0; t < obj.trg.size(); t++)
    {
        ...
        if (obj.trg[t].material < 0 || obj.trg[t].material >= obj.mtl.size())
        {
            ...
            char er[1024];
            sprintf(er, "mtl in triangle #:%lu, %s, %s is %d", t, obj.group_name.c_str(), obj.name.c_str(),
                    obj.trg[t].material);
            fnerror(std::string(er));
            continue;
        }
        ...
        VERTEX0 *ver[3];
        long vb;
        if (material[eo->mtl].texture_type[3] != TEXTURE_NONE)
            vb = 3;
        else if (material[eo->mtl].texture_type[2] != TEXTURE_NONE)
            vb = 2;
        else if (material[eo->mtl].texture_type[1] != TEXTURE_NONE)
            vb = 1;
        else if (material[eo->mtl].texture_type[0] != TEXTURE_NONE)
            vb = 0;
        else
            continue; // if incorrect material
        if (animation)
            vb += 4;
        ...
        for (long v = 0; v < 3; v++)
        {
            ...
            {
            case 0:
                ver[v] = new VERTEX0(obj.trg[t].vrt[v]);
                break;
            case 1:
                ver[v] = new VERTEX1(obj.trg[t].vrt[v]);
                break;
            case 2:
                ver[v] = new VERTEX2(obj.trg[t].vrt[v]);
                break;
            case 3:
                ver[v] = new VERTEX3(obj.trg[t].vrt[v]);
                break;
            // animation
            case 4:
                ver[v] = new VERTEXA0(obj.trg[t].vrt[v]);
                break;
            }
        }
...
void GEOM_STATIC::AddLight(const LIGHT &l)
{
    ...
    lt.atten[0] = l.atten[0];
    lt.atten[1] = l.atten[1];
    lt.atten[2] = l.atten[2];
    lt.r = l.r;
    lt.g = l.g;
    lt.b = l.b;
...
void GEOM_STATIC::WriteFile()
{
    ...
    for (o = 0; o < object.size(); o++)
    {
        if (object[o].vertex.size() == 0)
            continue;

        long vertex_buff;
        if (material[object[o].mtl].texture_type[3] != TEXTURE_NONE)
            vertex_buff = 3;
        else if (material[object[o].mtl].texture_type[2] != TEXTURE_NONE)
            vertex_buff = 2;
        else if (material[object[o].mtl].texture_type[1] != TEXTURE_NONE)
            vertex_buff = 1;
        else
            vertex_buff = 0;
        if (animation)
